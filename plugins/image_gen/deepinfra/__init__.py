...
:func:`max_cli.models._fetch_deepinfra_models_by_tag` so newly added
models show up in ``max tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from max the next time the catalog is fetched, no patch required.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `max tools` → Image "
                    "Generation → DeepInfra to configure, or `max setup` "
                    "to add the key."
                ),
        ...
        from max_cli.models import deepinfra_base_url
