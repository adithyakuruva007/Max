...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      MAX_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      MAX_DASHBOARD_PORTAL_URL       — defaults to
                                          ``https://portal.stardustresearch.com``
...
Skip reasons:
  ...
  ("Set MAX_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via MAX_DASHBOARD_PORTAL_URL for
# staging (portal.rewbs.uk) or a custom deployment. Contract docs name
# this as the production issuer.
_DEFAULT_PORTAL_URL = "https://portal.stardustresearch.com"
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between MAX_DASHBOARD_PORTAL_URL / MAX_DASHBOARD_OAUTH_CLIENT_ID
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``MAX_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("MAX_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``MAX_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("MAX_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``MAX_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set MAX_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``max_cli.web_server.should_require_auth`` +
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"MAX_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
