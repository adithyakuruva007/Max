...
- OpenViking server running and reachable from Max

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Max can identify older servers that expose the legacy status-only health
...
connection values into Max, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.max/.env`; for a named profile use
...
OpenViking's server config is separate from Max:
...
Max-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `max` | Max peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Max lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Max sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Max also sends `User-Agent: openviking-memory-hermes/<version>` on
OpenViking requests. This standard harness identifier contains the Max
...
current user). Max caches a confirmed user only for the active connection.
If the probe fails, Max uses the configured user, or `default`, for that
...
Max built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Max action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Max
