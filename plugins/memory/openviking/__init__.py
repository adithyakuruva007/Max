...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Max peer ID in OpenViking (default: max)
...
def _load_max_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("MAX_HOME", "")).expanduser() if os.environ.get("MAX_HOME") else Path.home() / ".hermes"
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_max_openviking_config()
    ...
    def post_setup(self, max_home: str, config: dict) -> None:
        ...
        max_home_path = Path(max_home)
        env_path = max_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_max_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Max run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
