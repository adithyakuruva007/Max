...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Max peer ID in OpenViking (default: hermes)
...
_OPENVIKING_USER_AGENT = f"openviking-memory-hermes/{_MAX_VERSION}"
_AGENT_PROMPT_LABEL = "Max peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "MAX_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Max slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Max to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Max refused the connection."
        ) from exc
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("MAX_HOME", "")).expanduser() if os.environ.get("MAX_HOME") else Path.home() / ".max"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Max did not start "
    ...
    try:
        ...
        # The Max Desktop backend can include the Max venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Max venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Max venv,
        # aborting `max update` with access-denied on .pyd files.
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Max session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Max only", "write values only to Max .env"),
    ...
    _print_openviking_ready("Connection saved to Max .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See max-agent#28296 review.
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Max peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        from max_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Max will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.max/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts max (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Max will retry on a "
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Max' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Max canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See max-agent#28296.
