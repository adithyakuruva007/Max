...
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `max-{profile}` isolates memory per active Max profile. Empty placeholders collapse cleanly (e.g. `max-{user}` with no user becomes `hermes`). |
