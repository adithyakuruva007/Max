...
```bash
max memory setup honcho   # configure Honcho directly (works on a fresh install)
max memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
max config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.max/.env
```

> `max honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `max memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `max model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$MAX_HOME/honcho.json` | Profile-local (isolated Max instances) |
| 2 | `~/.max/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Max profile: `max` (default) or `hermes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `max honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Max session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`max-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`max`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-max-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening max in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Max session ID (timestamp + hex). Every `max` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Max profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`max`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Max profile: `max` (default) or `hermes_<profile>` (e.g. `max -p coder` -> host key `hermes_coder`). Older `hermes.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `MAX_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `max-agent`) |
...
|---------|-------------|
| `max memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `max honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `max memory setup`) |
| `max honcho status` | Show resolved config for active profile |
| `max honcho enable` / `disable` | Toggle Honcho for active profile |
| `max honcho mode <mode>` | Change recall or observation mode |
| `max honcho peer --user <name>` | Update user peer name |
| `max honcho peer --ai <name>` | Update AI peer name |
| `max honcho tokens --context <N>` | Set context token budget |
| `max honcho tokens --dialectic <N>` | Set dialectic max chars |
| `max honcho map <name>` | Map current directory to a session name |
| `max honcho sync` | Create host blocks for all Max profiles |
