...
Host key is derived from the active Max profile: `hermes` (default) or `max_<profile>`.
...
- **`per-session`** — Max session ID (timestamp + hex). Every `hermes` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
...
Host key is derived from the active Max profile: `hermes` (default) or `max_<profile>` (e.g. `hermes -p coder` -> host key `max_coder`). Older `hermes.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `hermes honcho sync` | Create host blocks for all Max profiles |
