...
Handles: max honcho setup | status | sessions | map | peer
"""
...
from max_cli.config import cfg_get
...
def cmd_sync(args) -> None:
    ...
    Scans all Max profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'max honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'max honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `max update` -- no output, no exceptions.
    """
    try:
        from max_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Max profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $MAX_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_max_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        max_host.get("pinUserPeer"),
        max_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in max_host:
        aliases_src = max_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in max_host:
        prefix_src = max_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(max_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom max_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    max_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        max_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(max_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = max_host.get("userPeerAliases")
    prefix = max_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(max_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(max_host)
    max_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        max_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        max_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Max persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = max_host.get("apiKey", "")
        masked = (
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(max_host)
        ...
        if use_oauth:
            ...
            try:
                ...
                print("  Re-run 'max honcho setup' to retry, or choose an API key instead.\n")
                return
            max_host["apiKey"] = cred.access_token
            max_host["oauth"] = cred.oauth_block()
            # Default the peer prompt to the name entered at consent.
            if cred.consent_peer_name:
                max_host["peerName"] = cred.consent_peer_name
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'max honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = max_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        max_host["peerName"] = new_peer

    current_ai = max_host.get("aiPeer") or cfg.get("aiPeer", "max")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        max_host["aiPeer"] = new_ai

    current_workspace = max_host.get("workspace") or cfg.get("workspace", "max")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        max_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Max GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, max_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Max gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = max_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(max_host)
            max_host["pinUserPeer"] = False
            if prior_aliases:
                max_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                max_host, current_prefix, prefix_from_root,
            ...
            if merged:
                max_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                max_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                max_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = max_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        max_host["observationMode"] = new_obs
    else:
        max_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(max_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        max_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        max_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = max_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        max_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = max_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        max_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                max_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(max_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            max_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        max_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        max_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        max_host["dialecticReasoningLevel"] = new_reasoning
    else:
        max_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = max_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        max_host["sessionStrategy"] = new_strat

    max_host["enabled"] = True
    max_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        max_config = load_config()
        max_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(max_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: max config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    max honcho status     -- show full config")
    print("    max honcho mode       -- change recall/observation mode")
    print("    max honcho tokens     -- tune context and dialectic budgets")
    print("    max honcho peer       -- update peer names")
    print("    max honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Max profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: max honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'max honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'max honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: max honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Max' identity in Honcho. Seed with 'max honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: max honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: max honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Max asks Honcho's AI peer a question")
        ...
        print("\n  Set with: max honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'max honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'max honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    max honcho identity --show        — show both peer representations")
        print("    max honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Max + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Max\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Max persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  max honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'max honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'max honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  max honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'max'")
        print("  with Honcho configured and no prior session history.")
        print("  (Max calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
            for f in user_files:
                print("    max honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'max honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Max, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'max honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    max honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  max honcho identity <path/to/SOUL.md>")
    ...
    print("    Max:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Max:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Max:   Honcho observes every message and updates representations")
    ...
    print("    Max:   per-session by default — each run gets its own session")
    print("              Map a custom name:  max honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. max honcho setup              — configure API key (required)")
        print("  2. max honcho migrate            — re-run this walkthrough")
    else:
        print("  1. max honcho status             — verify Honcho connection")
        print("  2. max                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. max honcho identity --show    — verify AI peer representation")
        print("  4. max honcho tokens             — tune context and dialectic budgets")
        print("  5. max honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'max memory setup'...\n")
        from max_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``max honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``max honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to max memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Max Honcho",
    )
