...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/max (NS-605).
    ...
    if not is_local:
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'max honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'max honcho setup' to get a new code.\n")
                return
            except AccessDenied:
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if max_cli internals move."""
    try:
        from max_cli.auth import _can_open_graphical_browser, _is_remote_session
