...
Resolution order for config file:
  1. $MAX_HOME/honcho.json  (instance-local, enables isolated Max instances)
...
from max_cli.profiles import _get_default_hermes_home
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Max profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Max profile.

    Resolution order:
      1. MAX_HONCHO_HOST env var (explicit override)
    ...
    """
    explicit = os.environ.get("MAX_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named Max
    # profile share that same host.  Keep named profiles isolated; only the
    # default Max profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $MAX_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.max/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_max_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.max
    ...
    # resolve_config_path()/get_max_home() later — those resolvers read a
    ...
    def from_global_config(
        ...
        Resolution: $MAX_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Max profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.max block or explicit enabled flag means the user
        ...
        # baseUrl / base_url keys below are the Max-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Max session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Max session title (from /title command; non-per-session)
...
def spawn_context_thread(
    ...
    *,
    ...
    (set_max_home_override); plain threading.Thread targets start with an
    EMPTY context, so any ambient resolution on the thread
    (resolve_config_path, resolve_active_host, get_max_home) silently
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'max honcho setup' account switches produce a NEW cache identity
...
def _client_cache_key(config: HonchoClientConfig | None) -> tuple:
    ...
    if config is not None:
        return (
            ...
            str(config.max_home) if config.max_home is not None else "",
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'max honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `max honcho setup` to configure)."
            )
