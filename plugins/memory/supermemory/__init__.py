...
def _load_supermemory_config(max_home: str) -> dict:
    config = _default_config()
    config_path = Path(max_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, max_home: str) -> None:
    config_path = Path(max_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "max"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "max", **(metadata or {})}
...
def _resolve_container_tag_for_setup(max_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(max_home)
...
def _probe_supermemory_connection(api_key: str, max_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(max_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(max_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, max_home):
        ...
        _save_supermemory_config(sanitized, max_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, max_home)
    ...
    def post_setup(self, max_home: str, config: dict) -> None:
        ...
        if env_writes:
            _write_env_vars(Path(max_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from max_constants import get_max_home
        self._max_home = kwargs.get("max_home") or str(get_max_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._max_home)
