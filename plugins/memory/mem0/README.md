...
Behavioral settings live in `$MAX_HOME/mem0.json` (set them via `max memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.max/.env`.
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Max session and call `mem0_search` — it connects to your server.
...
```bash
max memory setup mem0 --mode oss \
...
```bash
max memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$MAX_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.max/mem0_qdrant"}}
  }
}
```
...
```bash
max memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$MAX_HOME/mem0.json`).
