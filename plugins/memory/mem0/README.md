...
Behavioral settings live in `$MAX_HOME/mem0.json` (set them via `hermes memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.max/.env`.
...
| `user_id` | `max-user` | User identifier on Mem0 |
