...
def _save_mem0_json(max_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(max_home) / "mem0.json"
    existing = {}
...
def _setup_platform(max_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "max-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "max"},
    ...
    provider.save_config(provider_config, max_home)

    if env_writes:
        _write_env(Path(max_home) / ".env", env_writes)
...
def _setup_selfhosted(max_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "max-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "max")
...
def _setup_oss(max_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(max_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "max-user")
    ...
    if env_writes:
        _write_env(Path(max_home) / ".env", env_writes)
    _save_mem0_json(max_home, {"mode": "oss", "user_id": user_id, "agent_id": "max", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, max_home: str) -> str:
...
_PGVECTOR_CONTAINER = "max-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "max"
...
def _setup_oss_interactive(max_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], max_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], max_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'max-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "max-user")

    agent_id = input("  Agent ID [max]: ").strip()
    agent_id = agent_id or "max"
    ...
    if env_writes:
        _write_env(Path(max_home) / ".env", env_writes)
    _save_mem0_json(max_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(max_home: str, config: dict) -> None:
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(max_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(max_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(max_home, config, flags)
        return
