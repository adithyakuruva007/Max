...
Configuration
-------------
Secret (lives in $MAX_HOME/.env or the environment):
...
Behavioral settings (live in $MAX_HOME/mem0.json, set via `max memory
setup`):
  ...
  user_id            — Canonical user identifier. When set, it is applied
   ...
   agent_id           — Agent identifier (default: max)
...
def _load_config() -> dict:
    """Load config from env vars, with $MAX_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "max"),
        "oss": {},
    }
    ...
    config_path = get_max_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, max_home):
        """Write config to $MAX_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $MAX_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "max")
