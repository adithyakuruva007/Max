...
``{title, url, description, position}`` rows every other Max web
provider produces.
...
prefers Max-managed xAI Grok OAuth (via ``max auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.max/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``max tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `max auth` to sign in with "
