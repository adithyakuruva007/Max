...
two endpoints so a fresh Max install with **zero web credentials** still
...
def provider_tier(name: str) -> str:
    ...
    ``max tools`` picker's Free/Paid rows). ``free`` forces the keyless
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "backend via `max tools` for reliable service."
            ),
        }
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "via `max tools` for reliable service."
            ),
        }
