...
This plugin ships bundled with Max but is **opt-in** — it only loads when
...
max tools  # → Langfuse Observability
...
Set these in `~/.max/.env` (or via `max tools`):
...
max chat -q "hello"              # then check Langfuse for a "Max turn" trace
```

Generation observations include the Max system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `MAX_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
MAX_LANGFUSE_ENV=production       # environment tag
MAX_LANGFUSE_RELEASE=v1.0.0       # release tag
MAX_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
MAX_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
MAX_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
MAX_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`MAX_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
