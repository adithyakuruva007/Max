...
``max plugins remove`` deletes it and ``max plugins update`` git-pulls
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Max.
...
# Mirrors the plugin-name shape `max plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    :func:`max_constants.get_max_home` on every call. Don't cache the
    result across profile switches.
    """
    from max_constants import get_max_home

    root = get_max_home() / "plugin-data" / _validate_name(name)
