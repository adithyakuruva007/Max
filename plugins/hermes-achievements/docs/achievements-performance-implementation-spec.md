# Max Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.max/plugins/max-achievements/state.json` (existing unlock state)
  - `~/.max/plugins/max-achievements/scan_snapshot.json` (new)
  - `~/.max/plugins/max-achievements/scan_checkpoint.json` (new)
