# Max Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: max-achievements plugin
...
### `GET /api/plugins/max-achievements/achievements`
...
### `POST /api/plugins/max-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/max-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/max-achievements/overview`
...
- Persist plugin data under:
  - `~/.max/plugins/max-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.max/plugins/max-achievements/scan_checkpoint.json`
