...
Scope: strictly MAX_HOME and /tmp/hermes-*
...
try:
    ...
    def get_max_home() -> Path:  # type: ignore[no-redef]
        ...
        return Path(val).resolve() if val else (Path.home() / ".max").resolve()
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under MAX_HOME or ``/tmp/hermes-*``.
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under MAX_HOME (e.g. /tmp/hermes-*) — fall through.
        pass
