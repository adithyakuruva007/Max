"""disk_cleanup — ephemeral file cleanup for Max Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under MAX_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly MAX_HOME and /tmp/max-*
Never touches: ~/.max/logs/ or any system directory.
"""
...
try:
    ...
    def get_max_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("MAX_HOME") or "").strip()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$MAX_HOME/logs/``."""
    return get_max_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$MAX_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under MAX_HOME or ``/tmp/max-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    max_home = get_max_home()
    try:
        path.resolve().relative_to(max_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/max-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("max-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "max-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $MAX_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so MAX_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        max_home = get_max_home()
        for parent in ("cron", "cronjobs"):
            base = max_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside MAX_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under MAX_HOME, but never recurse into known
    # durable state trees.  Some installs place the Max checkout, venv,
    # and desktop build under MAX_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    max_home = get_max_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in max_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    max_home = get_max_home()
    try:
        rel = path.resolve().relative_to(max_home)
        ...
        # Path isn't under MAX_HOME (e.g. /tmp/max-*) — fall through.
        pass
