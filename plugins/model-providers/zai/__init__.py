...
request omits ``thinking``.  Max' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Max reasoning
...
(per Z.AI / BigModel docs).  Max' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map Max reasoning effort onto GLM-5.2's native ``high``/``max``.
