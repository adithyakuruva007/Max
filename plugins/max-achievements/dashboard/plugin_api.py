...
Mounted at /api/plugins/hermes-achievements/ by Max dashboard.
"""
...
def _data_dir() -> Path:
    ...
    ``max plugins remove``/``update``. Legacy files migrate on first read.
    """
    try:
        ...
        root = get_max_home() / "plugin-data" / "hermes-achievements"
...
def _data_file(name: str) -> Path:
    path = _data_dir() / name
    if not path.exists():
        legacy = get_max_home() / "plugins" / "hermes-achievements" / name
        if legacy.exists():
            try:
