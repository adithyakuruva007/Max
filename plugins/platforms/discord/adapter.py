...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._liveness_interval_seconds = env_float(
            "MAX_DISCORD_LIVENESS_INTERVAL_SECONDS", 60.0
        )
        self._liveness_failure_threshold = env_int(
            "MAX_DISCORD_LIVENESS_FAILURE_THRESHOLD", 3
        )
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_max_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
    ...
    async def _handle_message(self, message: DiscordMessage, role_authorized: bool = False) -> None:
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_max_auto_thread_initial_name", None)
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if lis is not None and not os.getenv("MAX_DISCORD_LIVENESS_INTERVAL_SECONDS"):
        os.environ["MAX_DISCORD_LIVENESS_INTERVAL_SECONDS"] = str(lis)
    lft = discord_cfg.get("liveness_failure_threshold")
    if lft is not None and not os.getenv("MAX_DISCORD_LIVENESS_FAILURE_THRESHOLD"):
        os.environ["MAX_DISCORD_LIVENESS_FAILURE_THRESHOLD"] = str(lft)
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'max-agent[messaging]'",
