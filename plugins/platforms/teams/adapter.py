...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        max_action = data.get("max_action", "")
        session_key = data.get("session_key", "")

        if not max_action or not session_key:
        ...
        choice = choice_map.get(max_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="max_approve",
                    data={**btn_data_base, "max_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="max_approve",
                    data={**btn_data_base, "max_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="max_approve",
                    data={**btn_data_base, "max_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="max_approve",
                    data={**btn_data_base, "max_action": "deny"},
                    style="destructive",
                ),
            ])
        )
