# A2A — Agent-to-Agent protocol for Max
...
peer (another Max, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
max gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Max serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.max/a2a_audit.jsonl`.
- Conversations persist to `~/.max/a2a_conversations/` — they survive context
