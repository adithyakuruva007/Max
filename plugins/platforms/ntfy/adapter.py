"""ntfy platform adapter (Max plugin).
...
``plugins/platforms/ntfy/``. The Max plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "max-in"              # subscribe topic (incoming)
          publish_topic: "max-out"     # optional — defaults to topic
...
_ECHO_TAG = "max-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``max cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Max dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `max gateway status` without
