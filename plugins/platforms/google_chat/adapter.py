...
# ``model_tools`` import time, so ``max status``, ``max chat``, etc.
...
# ``max_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Max is thinking…" card during the small window between
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${MAX_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _max_home = _get_max_home()
        except (ModuleNotFoundError, ImportError):
            _max_home = _Path.home() / ".hermes"
        self._thread_count_store = _ThreadCountStore(
            _max_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Max is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("MAX_HOME", str(_Path.home() / ".hermes"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${MAX_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Max is thinking…' marker message.
        ...
        "Max is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Max is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Max is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Max is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Max is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``max setup``.

    The setup wizard at ``max_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.max/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. max-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.max/.env")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Max plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'max-agent[google_chat]'",
        ...
        # the "Max is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Max is thinking…' marker message "
