"""SimpleX Chat platform adapter (Max plugin).
...
This adapter ships as a Max platform plugin under
``plugins/platforms/simplex/``. The Max plugin loader scans the
...
discoverable and ``max setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Max — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("MAX_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``max setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.max/.env`` via
    ``max_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "max_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.max/.env"
        )
        return
