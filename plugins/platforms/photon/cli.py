"""
``max photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Max
...
# Writable sidecar runtime dir (mirrors to MAX_HOME on immutable
...
# wiring imports this module for `max --help`).
...
# when called later (gateway start, max status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `max photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Max Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``max photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Max Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.max/.env,
    ...
    try:
        from max_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  max gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `max photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`max photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.max/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `max photon telemetry on` / `max photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.max/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  max gateway restart")
    return 0
...
#
# `max gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``max photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `max gateway setup` wizard."""
