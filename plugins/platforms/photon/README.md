...
This plugin connects Max Agent to iMessage (and other Spectrum
...
TypeScript-only, Max runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Max channel,
...
Run `max photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.max/.env` (the same place every other
...
Management metadata lives in `~/.max/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `max photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Max
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Max coalesces
...
- **Native polls are supported.** Max posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `max send`) cannot spawn the sidecar;
...
6. Verify end-to-end: `max photon status`, a DM and a group roundtrip,
