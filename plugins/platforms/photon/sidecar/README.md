...
Small Node helper that bridges Max Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Max is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Max-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Max can receive inbound messages
...
The Max plugin's `max photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Max is Python, so replies go through this sidecar
