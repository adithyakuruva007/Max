// Max Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Max): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Max -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Max-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `max photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Max can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Max'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Max allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Max, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Max supplied them so a known-good
