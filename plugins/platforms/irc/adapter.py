"""
IRC Platform Adapter for Max Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Max agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "max-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Max Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Max Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: max_, max_1, max_2...
...
def check_requirements() -> bool:
    ...
    # max setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `max gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Max to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    print()
    print_success("IRC configuration saved to ~/.max/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "max-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "max-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Max Agent (cron)")
