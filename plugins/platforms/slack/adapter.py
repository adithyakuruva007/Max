...
# identify MaxAgent traffic — matching other Max outbound surfaces
# that already set ``MaxAgent/<version>`` for platform-partner attribution.
try:
    from max_cli import __version__ as _MAX_VERSION
except Exception:
    _MAX_VERSION = "unknown"
_MAX_SLACK_USER_AGENT_PREFIX = f"MaxAgent/{_MAX_VERSION}"
...
def _slack_unfurl_kwargs(extra: Optional[Dict[str, Any]]) -> Dict[str, bool]:
    ...
    ``_slack_unfurl_hints``: ``max config set`` and Railway persist YAML
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from max_cli.commands import is_gateway_known_command
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `max slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `max gateway setup` "
                "or in the active profile's ~/.max/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `max gateway setup` "
                "or add it to your active profile's ~/.max/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `max gateway setup` "
                "or in the active profile's ~/.max/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `max gateway setup` "
                "or add it to your active profile's ~/.max/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_max_home() / "slack_tokens.json"
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_MAX_SLACK_USER_AGENT_PREFIX,
            )
            self._app = AsyncApp(
                token=primary_token,
            ...
            # Max has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Max can "
            ...
            # manifest (see `max slack manifest`). In Socket Mode, Slack
            ...
            from max_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'max slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Max handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Max session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Max owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message_impl(
        ...
        # the shape produced by peer Max agents in Socket Mode on some
...
# ``max_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_max_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``max_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under MAX_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from max_cli.slack_cli import _build_full_manifest
            from max_constants import get_max_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Max",
                bot_description="Your Max agent on Slack",
            )
            target = Path(get_max_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `max slack manifest --write` anytime to refresh after "
                "Max adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `max update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://max-agent.stardustresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``max_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `max setup` to install Slack support.",
        # Interactive setup wizard — replaces max_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in max_cli/gateway.py.
