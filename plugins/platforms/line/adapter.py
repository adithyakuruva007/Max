"""
LINE Messaging API platform adapter for Max Agent.
...
to Max Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``MAX_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            hermes_home = Path(get_max_home()).resolve()
        except Exception:
            hermes_home = Path.home().joinpath(".max").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``max status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``max status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``max setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.max/.env`` via ``max_cli.config``.
    """
    print()
    ...
    try:
        from max_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("max_cli.config not available; set LINE_* vars manually in ~/.max/.env")
        return
