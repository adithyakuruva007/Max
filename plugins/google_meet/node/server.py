...
Launched by ``max meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$MAX_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``max meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_max_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "max-meet-node",
    ) -> None:
        self.host = host
        self.port = port
