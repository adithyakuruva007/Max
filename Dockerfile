...
# node:26 image instead (Max pins its toolchain to Node 26 everywhere).
...
# max process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main max process, the dashboard,
...
# 'Max WebUI' catalog, NAS compose projects that preserve an old
...
#
# The [otlp] extra contains the SDK/exporter imported by Max when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Max production image.
#
...
RUN mkdir -p /opt/max/bin && \
    cp /opt/max/docker/hermes-exec-shim.sh /opt/max/bin/hermes && \
    chmod 0755 /opt/max /opt/max/bin/hermes && \
...
# host install's marker and wrongly block its ``max update``. A code-scoped
...
# `s6-setuidgid hermes` in its run script. If MAX_UID is unset, services
...
# lives outside both /opt/max (which operators sometimes bind-mount as a
# checkout) and /opt/data (the mutable MAX_HOME volume).
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `max dump` reports
...
# max_cli/build_info.py read it at runtime.  Both `max dump` and
...
ARG MAX_GIT_SHA=
RUN set -eu; \
    if [ -n "${MAX_GIT_SHA}" ]; then \
        printf '%s\n' "${MAX_GIT_SHA}" > /opt/max/.max_build_sha; \
    fi; \
    mkdir -p /etc/hermes; \
    MAX_GIT_SHA="${MAX_GIT_SHA}" python3 -c 'import json, os, pathlib, tomllib; project = tomllib.loads(pathlib.Path("/opt/max/pyproject.toml").read_text(encoding="utf-8"))["project"]; marker = pathlib.Path("/etc/hermes/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": "stardustresearch/max-agent", "version": project["version"], "revision": os.environ.get("MAX_GIT_SHA") or None}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'
...
# `docker exec <c> max ...` they default to root, and any file the
...
COPY --chmod=0755 docker/hermes-exec-shim.sh /opt/max/bin/hermes
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/max/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> max ...` and any
...
# subcommand vs. no-args), drops to the max user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/max/docker/entrypoint-dispatch.sh" ]
CMD [ ]
