...
import { SelectionSwitcher } from "@stardust-research/ui/ui/components/selection-switcher";
import { Spinner } from "@stardust-research/ui/ui/components/spinner";
import { Typography } from "@stardust-research/ui/ui/components/typography/index";
import { ConfirmDialog } from "@stardust-research/ui/ui/components/confirm-dialog";
import { cn } from "@/lib/utils";
import { SidebarFooter } from "@/components/SidebarFooter";
import { SidebarStatusStrip, gatewayLine } from "@/components/SidebarStatusStrip";
import { useBelowBreakpoint } from "@stardust-research/ui/hooks/use-below-breakpoint";
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateMax = status?.can_update_hermes === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'max update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "max update";
    return (
      t.status.updateMaxConfirmMessage ??
      `This will run 'max update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateMaxConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateMax) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateMax,
      runningLabel: t.status.updatingMax,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Max gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateMaxConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateMaxConfirmTitle ?? `${t.status.updateMax}?`}
    />
    </>
  );
}
