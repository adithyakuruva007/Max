...
export const en: Translations = {
  ...
  status: {
    ...
    updateMaxConfirmMessage:
      "This runs max update and restarts the gateway when it finishes. Active sessions keep their prompt cache until then.",
    updateMaxConfirmNow: "Update now",
    updateMaxConfirmTitle: "Update Max?",
    updatingMax: "Updating Max…",
  ...
  pluginsPage: {
    ...
    headline:
      "Discover, install, enable, and update Max plugins (`max plugins` parity).",
    ...
    removeConfirm: "Remove this plugin from ~/.max/plugins/?",
    removeHint: "Only user-installed plugins under ~/.max/plugins can be removed.",
  ...
  skills: {
    ...
    filters: "Filters",
    noSkills: "No skills found. Skills are loaded from ~/.max/skills/",
  ...
  env: {
    ...
    customHint: "Arbitrary environment variables stored in your .env that Max doesn't recognise. Use these to inject env vars for skills, MCP servers, or your own tooling.",
  ...
  achievements: {
    hero: {
      ...
      title: "Max Achievements",
      subtitle:
        "Collectible Max badges earned from real session history. Known unfinished achievements are shown as Discovered; Secret achievements stay hidden until the first matching behavior appears.",
      scan_subtitle:
        "Scanning Max session history. First scan can take 5–10 seconds on large histories.",
    },
    actions: {
      rescan: "Rescan",
    },
    stats: {
      ...
      latest: "Latest",
      latest_hint_empty: "run Max more",
      none_yet: "None yet",
    },
    state: {
      unlocked: "Unlocked",
    ...
    guide: {
      ...
      secret_body:
        "Secrets hide their exact trigger. Once Max sees a related signal, the card becomes Discovered and shows its requirement.",
      scan_status_header: "Scan status",
      scan_status_body:
        "Max is scanning local history once, then cards will appear automatically. Nothing is stuck if this takes a few seconds.",
    ...
    share: {
      ...
      tweet_text: "Just unlocked {tier_part}\"{name}\" in Max Agent ☤",
    },
  },
