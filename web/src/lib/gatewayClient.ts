...
import {
  ...
  buildMaxWebSocketUrl,
  type ConnectionState,
  type GatewayEvent,
  type GatewayEventName,
} from "@max/shared";

import { MAX_BASE_PATH, getSessionToken, buildWsAuthParam } from "@/lib/api";
...
export class GatewayClient extends JsonRpcGatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    if (!authParam[1]) {
      throw new Error(
        "Session token not available — page must be served by the Max dashboard server",
      );
    }
