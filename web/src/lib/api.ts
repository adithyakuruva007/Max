import { buildMaxWebSocketUrl } from "@hermes/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/hermes/). The Python backend
// injects ``window.__MAX_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__MAX_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export async function fetchJSON<T>(
  ...
  const token = window.__MAX_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``max update``, ``max gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__MAX_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__MAX_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__MAX_SESSION_TOKEN__`` is valid, so the next
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__MAX_AUTH_REQUIRED__) {
 ...
 * ``window.__MAX_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``max update``. */
  ...
  /** NS-656: disk-usage rollup for the MAX_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the MAX_HOME volume — no staleness dimension, so no
 * sampled_at. */
