/**
 * Max Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__MAX_PLUGIN_SDK__`` and
 * ``window.__MAX_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__MAX_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@stardust-research/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``MaxPluginSDK["sdkVersion"]`` (and the
 ...
 *     (pulls @stardust-research/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__MAX_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__MAX_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__MAX_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface MaxPluginSDK {
...
declare global {
  interface Window {
    __MAX_PLUGIN_SDK__?: MaxPluginSDK;
    __MAX_PLUGINS__?: PluginRegistry;
  }
}
