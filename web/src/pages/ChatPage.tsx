/**
 * ChatPage — embeds `max --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (max_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__MAX_SESSION_TOKEN__ &&
    !window.__MAX_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `max dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__MAX_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Max TUI has enabled xterm mouse-events
    ...
    //      payloads upload to MAX_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to MAX_HOME/images, then drive `/image`
    ...
    // WebSocket. In gated mode (``window.__MAX_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets MAX_HOME pointed at the
    ...
    // forwarding them into Max. Keyboard input, paste, and resize still
    // behave normally.
