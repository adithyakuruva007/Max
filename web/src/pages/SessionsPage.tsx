    ...
    async (id: string) => {
      try {
        const res = await fetch(api.exportSessionUrl(id), {
          ...
          headers: {
            "X-Max-Session-Token":
              (window as unknown as { __MAX_SESSION_TOKEN__?: string; __HERMES_SESSION_TOKEN__?: string })
                .__MAX_SESSION_TOKEN__ ?? (window as unknown as { __HERMES_SESSION_TOKEN__?: string }).__HERMES_SESSION_TOKEN__ ?? "",
          },
        });
