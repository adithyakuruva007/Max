"""ACP session manager — maps ACP sessions to Max AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.max/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Max itself is running in WSL.

    Windows ACP clients can launch ``max acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from max_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Max from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["max-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Max agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Max AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Max runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.max/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``MAX_HOME`` dynamically rather than relying on
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # for the rest of the session (see max_state.get_messages_as_conversation).
        try:
    ...
    def _make_agent(
        ...
        *,
        ...
        # the automatic late-refresh (see MaxACPAgent._schedule_mcp_late_refresh).
        try:
        ...
        # editor/session cwd instead of the Max daemon's process cwd.
