"""ACP auth helpers — detect and advertise Max authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "max-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Max runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Max can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Max.
    ...
    not have Max provider credentials configured yet, so Max always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Max using the currently configured "
