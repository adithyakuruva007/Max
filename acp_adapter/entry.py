"""CLI entry point for the max-agent ACP adapter.

Loads environment variables from ``~/.max/.env``, configures logging
...
try:
    ...
    # shadowing Max's own modules — ``max acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from MAX_HOME (default ``~/.max``)."""
    from max_cli.env_loader import load_max_dotenv

    max_home = get_max_home()
    loaded = load_max_dotenv(max_home=max_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", max_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="max-acp",
        description="Run Max Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Max version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Max provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.max/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import MaxACPAgent  # noqa: F401

    print("Max ACP check OK")


def _run_setup() -> None:
    from max_cli.main import main as max_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "max", "model"]
        max_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``max postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting max-agent ACP adapter")
    ...
    import acp
    from .server import MaxACPAgent
