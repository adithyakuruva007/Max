...
from tools.approval import (
    reset_max_interactive_context,
    set_max_interactive_context,
)
...
class MaxACPAgent(acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from max_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_max_session_id: str,
        previous_max_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        ...
        try:
            return session_provenance_meta(
                ...
                acp_session_id,
                current_max_session_id,
                previous_max_session_id=previous_max_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        meta = self._provenance_meta(
            session_id,
            current_max_session_id or session_id,
            previous_max_session_id,
        )
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # tools.approval (set_max_interactive_context) rather than
        ...
        def _run_agent() -> dict:
            ...
            interactive_token = set_max_interactive_context(True)
            ...
            try:
                ...
                if interactive_token is not None:
                    reset_max_interactive_context(interactive_token)
        ...
        try:
            ...
            pre_turn_max_id = getattr(state.agent, "session_id", None)
        ...
        post_turn_max_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_max_id
            and pre_turn_max_id
            and post_turn_max_id != pre_turn_max_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_max_session_id=post_turn_max_id,
                    previous_max_session_id=pre_turn_max_id,
                )
            except Exception:
                logger.debug(
