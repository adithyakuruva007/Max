"""ACP agent server — exposes Max Agent via the Agent Client Protocol."""
...
try:
    from max_cli import __version__ as MAX_VERSION
except Exception:
    MAX_VERSION = "0.0.0"
...
# already uses (`max_cli/moa_cmd.py`). This bounds each provider's row, not
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Max running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Max cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Max/OpenAI-compatible user content payload."""
...
class MaxACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Max AIAgent."""
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Max maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Max inventory is also used by ``max model``, the TUI,
        ...
        try:
            ...
            from max_cli.models import normalize_provider, provider_label
            ...
            def choice_provider(model_id: str) -> str:
                ...
                if parts[:1] == ["custom"] and len(parts) > 1:
                    from max_cli.models import _configured_custom_provider_ids
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from max_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Max estimates ``used`` from the same
    ...
    def _provenance_meta(
        ...
        """Best-effort ``_meta.max.sessionProvenance`` for an ACP session."""
        try:
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after Max changes it.

        When the internal Max head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_hermes_session_id`` so the
        attached ``_meta.max.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # max_state.py schema — only started_at/ended_at). Use "now" as
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["max-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        """
        try:
            from max_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from max_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="max-agent", version=MAX_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Max' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Max setup/model selection out-of-band.
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.max`` (ACP's extensibility
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Max
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Max on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Max' local kawaii waiting/status
        ...
        # os.environ["MAX_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue MAX_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind MAX_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Without it the prompt advertises the global Max workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.max/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global MAX_CRON_SESSION (#37968).
            ...
            previous_session_id = os.environ.get("MAX_SESSION_ID")
            ...
            try:
                ...
                # Restore MAX_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("MAX_SESSION_ID", None)
                else:
                    os.environ["MAX_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Max DB session id before the turn so we
            ...
            # stomp on each other's ContextVar writes (MAX_SESSION_KEY in
        ...
        # _meta.max.sessionProvenance so ACP clients can render the boundary
        ...
        # Max' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Max install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Max Agent v{MAX_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Max has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
