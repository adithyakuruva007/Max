...
This is an additive Max extension surfaced under ACP ``_meta.max`` so
...
compression rotates the internal Max head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    Args:
        ...
        current_max_session_id: The live internal Max DB session id
            (``state.agent.session_id``).
        previous_max_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"max": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_max_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_max_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_max_session_id}
    ...
    rotated = bool(
        ...
        and previous_max_session_id != current_max_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentMaxSessionId": current_max_session_id,
        "rootMaxSessionId": root_id,
        "parentMaxSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_max_session_id:
        provenance["previousMaxSessionId"] = previous_max_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"max": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"max": {"sessionProvenance": prov}}
