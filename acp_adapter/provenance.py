...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.max.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_hermes_session_id: The live internal Max DB session id
    ...
    provenance: Dict[str, Any] = {
        ...
        "currentMaxSessionId": current_hermes_session_id,
    ...
    if previous_hermes_session_id:
        provenance["previousMaxSessionId"] = previous_hermes_session_id
    if rotated:
