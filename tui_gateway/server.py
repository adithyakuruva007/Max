...
from tools.environments.local import max_subprocess_env
...
load_max_dotenv(
    max_home=_max_home, project_env=Path(__file__).parent.parent / ".env"
)
...
_CRASH_LOG = os.path.join(_max_home, "logs", "tui_gateway_crash.log")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("MAX_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        env = max_subprocess_env(inherit_credentials=True)
        if profile_home:
            ...
            # gateway's launch MAX_HOME (#40677).
            env["MAX_HOME"] = str(profile_home)
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from max_cli.plugins import invoke_hook
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `max --tui` keeps real _stdio.
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_max_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s MAX_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_max_home``).
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _max_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_max_home_override()
        home = override if isinstance(override, str) and override else _max_home
...
def _save_cfg(cfg: dict):
    ...
    from max_cli.config import atomic_config_write

    path = _max_home / "config.yaml"
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _MAX_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off max-cli's tool universe — can't
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from max_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("MAX_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _resolve_runtime_with_fallback(
    ...
    ``max_cli/cli_agent_setup_mixin.py``: when the primary provider
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind MAX_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `MAX_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_max_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        raw_history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_max_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_max_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds MAX_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``max --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``max gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from max_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from max_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from max_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from max_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from max_cli.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Max profile home
    # (~/.max/sessions/saved/) rather than the project/workspace CWD, and
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from max_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _max_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".max" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Max Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Max/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Max Desktop. If it serves Max/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from max_cli.model_switch import parse_model_flags
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under MAX_HOME (~/.max by default) — config/sessions/skills,
    ...
    max_home = os.path.realpath(str(get_max_home()))

    return real == home or real == max_home or real.startswith(max_home + os.sep)
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from max_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_max_home), "display": display_max_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _max_home / "config.yaml"
        try:
            return _ok(
...
def _(rid, params: dict) -> dict:
    try:
        from max_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from max_cli.runtime_provider import resolve_runtime_provider
        from max_cli.auth import has_usable_secret
        from max_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Max provider is configured.",
                },
            )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from max_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from max_cli.config import is_managed, save_env_value
        from max_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `max model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.max/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from max_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from max_cli.config import remove_env_value
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from max_cli.browser_connect import launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("MAX_API_KEY", "") or cfg.get("api_key", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_max_home / "config.yaml")],
                ],
            },
        ]
