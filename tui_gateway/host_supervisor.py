...
def _default_registry_path() -> Path:
    return get_max_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        self.expected_hermes_home = expected_hermes_home if expected_hermes_home is not None else str(get_max_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["MAX_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        if got_home and got_home != self.expected_hermes_home:
            raise RuntimeError(f"compute host MAX_HOME mismatch: {got_home} != {self.expected_hermes_home}")
