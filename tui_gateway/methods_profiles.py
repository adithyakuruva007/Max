...
handlers delegate to the same `max_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Max profiles (name, path, model, description, skill count).
    ...
    def _canonical_session_row(db, profile_path):
        ...
        describe the same session (max-agent#88200) with no client-side
        pointer involved.
    ...
    def _latest_profile_session_rows(db):
        ...
        sessions never surface in conversation lists (max-agent#90268).
...
def _(rid, params: dict) -> dict:
    ...
    ``max setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (max_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local MAX_HOME override — the same
        ...
        try:
            ...
            token = set_max_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from max_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from max_cli.config import load_config_readonly, read_user_config_raw
            from max_cli.web_routers.profiles import _write_profile_model
            from max_constants import (
                reset_max_home_override,
                set_max_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the MAX_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            # Toolsets: the same filtered universe the `max tools`
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from max_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `max tools` / Settings is where
            ...
            description = ""
            try:
                from max_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from max_cli.profiles import write_profile_meta
