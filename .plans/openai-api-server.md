# OpenAI-Compatible API Server for Max Agent
...
SSE streaming. By exposing this endpoint, max-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  max-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.max/.env`
...
4. Chat with max-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns max-agent as a model) |
...
Authorization: Bearer max-api-key-here
...
But max-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "max-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured max-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of max-agent's core prompt
...
Once implemented, max-agent works as a drop-in backend for:
