...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    try:
        ...
        probe_session_id = f"_max_fts_health_probe_{time.time_ns()}"
        try:
...
class SessionDB:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._max_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._max_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Max surface and jump
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``MAX_DISABLE_FTS_TRIGRAM`` or not yet created), so
