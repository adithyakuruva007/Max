...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Max Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Max Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Max core? Start with [Build a Max Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Max itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Max instance (throwaway MAX_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m max_cli.main
scripts/dev-sandbox.sh --persistent python -m max_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.max/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.max` — use `get_max_home()` from `max_constants` for code paths and `display_max_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/max-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) Max can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Max to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/max-agent/blob/main/LICENSE).
