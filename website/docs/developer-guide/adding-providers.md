...
description: "How to add a new inference provider to Max Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Max can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `max model` menu entries
...
1. `max_cli/auth.py` decides how credentials are found.
2. `max_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `max_cli/models.py` and `max_cli/main.py` make the provider show up in the CLI. (`max_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
:::tip
`max_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `max setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$MAX_HOME/plugins/model-providers/` both get picked up.
...
7. `max model` menu includes the provider
8. `max setup` wizard delegates to `main.py` automatically
...
User plugins at `$MAX_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `max model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `max_cli/auth.py`
- `_PROVIDER_LABELS` in `max_cli/models.py`
- `_PROVIDER_ALIASES` in both `max_cli/auth.py` and `max_cli/models.py`
- CLI `--provider` choices in `max_cli/main.py`
...
## Step 2: Add auth metadata in `max_cli/auth.py`
...
- What env vars should Max check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `max_cli/models.py`
...
## Step 4: Resolve runtime data in `max_cli/runtime_provider.py`
...
Be careful with API-key precedence. Max already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `max_cli/main.py`

A provider is not discoverable until it shows up in the interactive `max model` flow.

Update these in `max_cli/main.py`:
...
:::tip
`max_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `max model` and `max setup` automatically.
:::
...
When you add a native provider, double-check that Max is only sending fields that provider actually understands.
...
- `tests/max_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/max_cli/test_model_switch_custom_providers.py` (and adjacent `tests/max_cli/test_model_switch_*.py`)
- `tests/max_cli/test_setup_model_provider.py`
...
python -m pytest tests/max_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/max_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m max_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `max_cli/auth.py`
- [ ] aliases added in `max_cli/auth.py` and `max_cli/models.py`
- [ ] model catalog added in `max_cli/models.py`
- [ ] runtime branch added in `max_cli/runtime_provider.py`
- [ ] CLI wiring added in `max_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `max model` but not `max setup`
