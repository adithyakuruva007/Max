...
(`max-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `max-acp`, `max-api-server`, and core toolsets (gated on CDP reachability)
