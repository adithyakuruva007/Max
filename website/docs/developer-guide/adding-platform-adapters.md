...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `max config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-max-config) below).
...
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #max)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
1. **`toolsets.py`** — Add `"max-newplat"` toolset definition with `_HERMES_CORE_TOOLS`
2. **`toolsets.py`** — Add `"max-newplat"` to the `"max-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | max-newplat toolset |
