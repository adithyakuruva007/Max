...
Max runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.max/plugins/`, registering a backend the user selects
...
| `max setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `max status` / `max doctor` | `doctor_checks()` |
...
```python title="~/.max/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.max"    # where synced cache files land, or None
...
```yaml title="~/.max/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
max config set terminal.backend acmebox
```
