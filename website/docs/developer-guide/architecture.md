...
sidebar_position: 1
title: "Architecture"
description: "Max Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Max Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ max_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # MaxCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── max_state.py           # SQLite session/state database with FTS5
├── max_constants.py       # MAX_HOME, profile-aware paths
...
│
├── max_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `max` subcommands (large file)
...
│   ├── skills_config.py      # max skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # max tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # max gateway start/stop
│
...
```text
User input → MaxCLI.process_input()
...
Three discovery sources: `~/.max/plugins/` (user), `.max/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `max plugins` or `config.yaml`.
...
Exposes Max as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`max -p <name>`) gets its own MAX_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
