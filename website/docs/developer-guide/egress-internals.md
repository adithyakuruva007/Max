...
description: "How the iron-proxy egress firewall integrates with Max — module layout, lifecycle, security invariants, and extension points"
---

# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`max egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
max_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `max egress {install,setup,start,stop,
                                       status,disable,config}`.  Wires the
                                       core module into argparse.

max_cli/main.py:_dispatch_egress   Top-level subparser dispatcher.
                                       dest='egress_command' (intentionally
                                       disjoint from the inbound OAuth
                                       `max proxy` subparser, which uses
                                       dest='proxy_command').

max_cli/config.py: proxy schema    The `proxy:` block in DEFAULT_CONFIG.
...
tools/environments/docker.py
  ...
  DockerEnvironment.__init__          Docker-side merge logic: collision
                                       ...
                                       _MAX_EGRESS_NODE_OPTIONS_APPEND
...
tests/test_iron_proxy_e2e.py          Live E2E (gated on MAX_RUN_E2E=1).
...
hermes egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.max/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.max/bin/iron-proxy (atomic).
...
| Path | Mode | Test |
|---|---|---|
| `~/.max/proxy/` (dir) | `0o700` | `test_proxy_state_dir_is_0o700` |
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `max egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `max egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `max egress status` warn about it:
...
4. Sets the seven env vars (HTTPS_PROXY, NO_PROXY, REQUESTS_CA_BUNDLE, SSL_CERT_FILE, CURL_CA_BUNDLE, NODE_EXTRA_CA_CERTS, MAX_EGRESS_PROXY) and the per-mapping `MAX_PROXY_TOKEN_<NAME>` vars.
...
MAX_RUN_E2E=1 scripts/run_tests.sh tests/test_iron_proxy_e2e.py

# Live PTY smoke against `max egress`
MAX_HOME=/tmp/hermes-egress-test python3 -m max_cli.main egress --help
MAX_HOME=/tmp/hermes-egress-test python3 -m max_cli.main egress setup --help
```
...
- User-facing setup + troubleshooting: [Egress proxy](https://max-agent.stardustresearch.com/docs/user-guide/egress/iron-proxy)
- Docker backend internals: [Docker](https://max-agent.stardustresearch.com/docs/user-guide/docker)
- Bitwarden Secrets Manager integration: [`max secrets bitwarden`](https://max-agent.stardustresearch.com/docs/user-guide/secrets/bitwarden)
- CLI command reference: [`max egress`](https://max-agent.stardustresearch.com/docs/reference/cli-commands#hermes-egress)
- Sandbox-injected environment variables: [Egress proxy (sandbox-injected)](https://max-agent.stardustresearch.com/docs/reference/environment-variables#egress-proxy-sandbox-injected)
