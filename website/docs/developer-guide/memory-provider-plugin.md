...
description: "How to build a memory provider plugin for Max Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Max Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `max plugins`.
:::

## Installation Layouts

Max discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Max. Closed to new providers — see [CONTRIBUTING](https://github.com/NousResearch/max-agent/blob/main/CONTRIBUTING.md). |
| User | `$MAX_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.max/plugins/<name>/` | Opt-in via `MAX_ENABLE_PROJECT_PLUGINS=1`. |
...
Max, in `$MAX_HOME/plugins/<name>/` when installed by a user, or in
`./.max/plugins/<name>/` for a project-local one:
...
package layout. No copy under `$MAX_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Max reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `max <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          hermes_home (str): Active MAX_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `max memory setup` | **Yes** |
...
The gate binds to every compaction authority, not just the Max
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Max
...
`get_config_schema()` returns a list of field descriptors used by `max memory setup`:
...
the `messages` parameter; Max will continue calling them with the legacy
signature.
...
All storage paths **must** use the `hermes_home` kwarg from `initialize()`, not hardcoded `~/.max`:

```python
# CORRECT — profile-scoped
from max_constants import get_max_home
data_dir = get_max_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.max/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `max my-provider status`, `max my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `max <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `max --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: max my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the max my-provider argparse tree.
