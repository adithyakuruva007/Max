...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Max already has applies. The plugin never sees a
...
max-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Max provider id (e.g. "openrouter")
...
using Max built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Max provider
        # (must be one Max already knows about — same names as
        # `max model` and config.yaml model.provider).
        ...
        # plugin sends — Max does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.max/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Max' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Max subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/max-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/max-agent/blob/main/tests/agent/test_plugin_llm.py)
