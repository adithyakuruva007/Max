...
Users select your engine via `max plugins` → Provider Plugins → Context Engine, or by editing `config.yaml`:
...
When `compression.context_timeout_seconds > 0` (the default), Max runs the
