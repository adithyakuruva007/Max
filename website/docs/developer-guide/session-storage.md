...
Max Agent uses a SQLite database (`~/.max/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `max_state.py`


## Architecture Overview

```
~/.max/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `max_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `max_state.py` for the exact SQL.
...
Multiple max processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.max/state.db
...
Default path: `~/.max/state.db`

This is derived from `max_constants.get_max_home()` which resolves to
`~/.max/` by default, or the value of `MAX_HOME` environment variable.
