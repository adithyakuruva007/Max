...
Plugins can launch and supervise fresh Max child sessions without importing
...
active agent turn fails closed with `No active Max parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Max can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Max's existing unsafe-tool block remains enforced.
