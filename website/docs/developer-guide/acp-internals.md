...
The ACP adapter wraps max' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
max acp / max-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'max-agent[acp]==<version>' max-acp`, pointed at the `max-agent` PyPI release.
...
- `allow_once` -> max `once`
- `allow_always` -> max `always`
- reject options -> max `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps max tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["max-acp"])
...
Instead it reuses max' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured max provider/credentials. It also always advertises a terminal setup auth method (`max-setup`, args `--setup`) so first-run registry clients can open max' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.max/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `max-acp` toolset definition
- `hermes_cli/main.py` — `max acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `max-acp` script
