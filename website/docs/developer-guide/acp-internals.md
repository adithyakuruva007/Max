...
```text
hermes acp / max-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.max/.env
  -> configure stderr logging
  -> construct MaxACPAgent
...
- `acp_adapter/auth.py`
- `max_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Max provider/credentials. It also always advertises a terminal setup auth method (`max-setup`, args `--setup`) so first-run ACP clients can open Max' interactive model/provider configuration before starting a normal ACP session.
...
- `max_cli/main.py` — `max acp` CLI subcommand
