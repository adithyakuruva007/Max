...
description: "Run Max Agent directly on an Android phone with Termux"
---

# Max on Android with Termux
...
Max Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Max from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Max installs are marked with install method `apt`. Max therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade max-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Max runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

Max now ships a Termux-aware installer path:
...
- links `max` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `max` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/hermes" "$PREFIX/bin/hermes"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `max` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.max/.env`.
...
### `max doctor` says ripgrep or Node is missing
