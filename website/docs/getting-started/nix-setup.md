...
description: "Install and deploy Max Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
Max Agent ships a Nix flake, a NixOS module, and a Home Manager module.
...
**For non-NixOS users**, this only changes the install step. Everything after (`max setup`, `max gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage max the same way you manage any other NixOS service.
:::
...
```bash
# Run the desktop app
nix run github:NousResearch/max-agent#desktop

# Or install persistently
nix profile install github:NousResearch/max-agent#desktop

# run the tui
nix run github:NousResearch/max-agent -- setup
nix run github:NousResearch/max-agent -- --tui

# or install it in your profile
nix profile install github:NousResearch/max-agent
max setup
hermes --tui
```

After `nix profile install`, `max`, `max-agent`, and `max-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `max setup` walks you through provider selection, `max gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.max/`.

:::warning Messaging platforms (Discord, Telegram, Slack)
The default package includes ALL libraries max-agent might need. if you want a smaller variant, check the other flake outputs. 
...
:::note
This module needs NixOS. Max is an agent for one person. If you want an agent for one person and not a system service, use the [Home Manager module](#home-manager-module). That module runs on NixOS and on each other system that Home Manager supports.
:::
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    max-agent.url = "github:NousResearch/max-agent";
  };

  outputs = { nixpkgs, max-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        max-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
That's it. `nixos-rebuild switch` creates the `max` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o max /dev/stdin /var/lib/hermes/env
```

```nix
services.max-agent.environmentFiles = [ "/var/lib/hermes/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `max` CLI on your system PATH **and** sets `MAX_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `max` in your shell creates a separate `~/.max/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `max` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `max chat`, `max sessions list`, `max --version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the max codebase, set `MAX_DEV=1` to bypass container routing and run the local checkout directly

Set `container.hostUsers` to create a `~/.max` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
Users listed in `hostUsers` are automatically added to the `max` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo max chat` manually.
:::
...
systemctl status max-agent

# Watch logs (Ctrl+C to stop)
journalctl -u max-agent -f

# If addToSystemPackages is true, test the CLI
hermes --version
max config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Max defaults to OpenRouter.
:::
...
{ config, ... }: {
  services.max-agent = {
    ...
    # ── Documents ──────────────────────────────────────────────────────
    # USER.md is memory, so it goes to MAX_HOME. Workspace files use
...
```nix
services.max-agent.configFile = /etc/hermes/config.yaml;
```

This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$MAX_HOME/config.yaml` on each activation.
...
| Override the max package | `package` | `inputs.max-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/max"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$MAX_HOME/.env` at activation time (`nixos-rebuild switch`). Max reads this file on every startup, so changes take effect with a `systemctl restart max-agent` — no container recreation needed.
...
```nix
{
  ...
  services.max-agent.environmentFiles = [
...
```nix
{
  age.secrets.max-env.file = ./secrets/hermes-env.age;

  services.max-agent.environmentFiles = [
    config.age.secrets.max-env.path
  ];
}
```
...
Max reads files from two directories. Thus there are two options. Use the option for the directory that the file must go into.
...
`hermesHomeFiles` installs into **`MAX_HOME`**. Max reads the identity file and the memory files of the agent from that directory. `SOUL.md` and `memories/` work only from there. A `SOUL.md` in `documents` makes a workspace file. Max does not load that file as the identity:

```nix
{
  services.max-agent.maxHomeFiles = {
...
`hermesHomeFiles` needs no `workingDirectory`, because the module owns the `MAX_HOME` directory. Most users want `hermesHomeFiles`.
...
:::tip
Environment variables in `env` values are resolved from `$MAX_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
```nix
{
  services.max-agent.mcpServers.remote-api = {
...
Set `auth = "oauth"` for servers using OAuth 2.1. Max implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.

```nix
{
  services.max-agent.mcpServers.my-oauth-server = {
...
Tokens are stored in `$MAX_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.

<details>
<summary><strong>Initial OAuth authorization on headless servers</strong></summary>

The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Max prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u hermes` (native):

```bash
# Container mode
docker exec -it max-agent \
  max mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u max MAX_HOME=/var/lib/hermes/.max \
  max mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
max mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.max/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/hermes/.max/mcp-tokens/
...
```nix
{
  services.max-agent.mcpServers.analysis = {
    command = "npx";
...
When max runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `max setup` | Config is declarative — edit `settings` in your Nix config |
| `max config edit` | Config is generated from `settings` |
| `max config set <key> <value>` | Config is generated from `settings` |
| `max gateway install` | The systemd service is managed by NixOS |
| `max gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **The `MAX_MANAGED` environment variable.** The service sets it, and the gateway process reads it.
2. **The `.managed` marker file** in `MAX_HOME`. The activation script writes it, and an interactive shell reads it. Thus the CLI also blocks a command such as `docker exec -it max-agent max config set ...`.
...
The flake also exports `homeManagerModules.default`. Max is an agent for one person. The credentials, the memory, the sessions and the cron jobs all belong to that person. Thus a user service is the correct shape on a personal machine. It runs on each distribution that Home Manager supports, and not only on NixOS.

The option set is the same set that the NixOS module uses. It is `services.max-agent`, with the same `settings`, `environmentFiles`, `documents`, `mcpServers`, `extraPlugins` and `backend` options. Each example above works here without a change. Only the necessary parts are different:

| | NixOS module | Home Manager module |
|---|---|---|
| Runs as | a system user that you declare, with `user`, `group` and `createUser` | you |
| State directory | `stateDir` and `/.max` | `hermesHome`, set directly. The default is `~/.max`. |
| Service | `systemd.services` | `systemd.user.services` on Linux, `launchd.agents` on macOS |
| CLI on the PATH | `addToSystemPackages`, which exports `MAX_HOME` for the full system | `programs.max-agent.enable`, which exports it for your session only |
| Desktop application | not supported, because a system service cannot own a user session | `programs.max-agent.desktop.enable` |
...
`home-manager switch` makes `~/.max`, writes `config.yaml`, builds `.env` and starts the gateway as a user service.
...
`gateway.enable` runs the messaging gateway for Telegram, Discord, Slack and the other platforms. Max Desktop and the web dashboard connect to a *different* process, which is `max serve` or `max dashboard`. `backend.mode` runs that process with the gateway:
...
`serve` runs without a user interface. It gives the `/api/ws` and `/api/pty` sockets that Max Desktop connects to, and it does not build the web application. `dashboard` gives all of that, and also serves the browser admin panel. Both processes use one `MAX_HOME` with the gateway. Thus the sessions, the skills, the memory and the cron jobs are the same for all of them. `backend.mode` works in the same way on the NixOS module, but not in container mode.
...
```bash
# Linux
systemctl --user status max-agent
journalctl --user -u max-agent -f

# macOS
launchctl list | grep hermes
tail -f ~/Library/Logs/max-agent.log

hermes --version
max config     # shows the configuration that Nix wrote
```
...
When container mode is enabled, max runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-max-agent-0.1.0  ──►  /nix/store/... (ro)
~/.max -> /var/lib/hermes/.max       (symlink bridge, per hostUsers)
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the max package itself do **not** trigger recreation.
...
The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current max package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `max plugins install` needed.
...
```nix
services.max-agent.extraPlugins = [
...
Plugins are symlinked into `$MAX_HOME/plugins/` at activation time. Max discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
```nix
services.max-agent.extraPythonPackages = [
...
The package's `site-packages` is added to PYTHONPATH in the max wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in max-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.max-agent.extraDependencyGroups = [ "messaging" ];
```
...
```nix
{
  inputs.max-agent.url = "github:NousResearch/max-agent";
  outputs = { max-agent, nixpkgs, ... }: {
    nixpkgs.overlays = [ max-agent.overlays.default ];
    # Then:
...
```nix
services.max-agent.settings.plugins.enabled = [
  "hermes-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core max dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
nix build .#checks.x86_64-linux.managed-guard       # MAX_MANAGED blocks mutation
...
| Check | What it tests |
|---|---|
| `package-contents` | `max` and `max-agent` binaries exist and `max --version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `max --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `MAX_MANAGED=true max config set ...` prints the NixOS error |
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `MAX_BUNDLED_SKILLS` is set in wrapper |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the max-agent service |
| `package` | `package` | `max-agent` | The max-agent package to use |
| `user` | `str` | `"hermes"` | System user |
| `group` | `str` | `"hermes"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/hermes"` | State directory (`MAX_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `max` CLI to system PATH and set `MAX_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$MAX_HOME/.env` at activation time |
...
| `hermesHomeFiles` | `attrsOf (either str path)` | `{}` | Files that go into `MAX_HOME`. `SOUL.md` and `memories/` must be here, or Max does not load them. |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `max gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the max user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$MAX_HOME/plugins/`. Each must contain `plugin.yaml` |
...
### Backend (`max serve` / `max dashboard`)

This option runs the process that Max Desktop and the web dashboard connect to, with the gateway. You cannot use it with `container.enable`.
...
| Option | Type | Default | Description |
|---|---|---|---|
| `hermesHome` | `str` | `"${config.home.homeDirectory}/.max"` | `MAX_HOME` directly. The NixOS module builds it from `stateDir`. |
...
### `programs.max-agent` (Home Manager only)

Home Manager separates "install this application for me" from "run this
daemon". `services.max-agent` keeps the state, the configuration and the
daemons. `programs.max-agent` installs what you use, and reads
`hermesHome` and the backend address from the services.

| Option | Type | Default | Description |
|---|---|---|---|
| `enable` | `bool` | `false` | Add the `max` CLI to `home.packages`, and export `MAX_HOME` for your shells |
| `package` | `package` | `services.max-agent.package` | The package to install. The default applies `extraPythonPackages` and `extraDependencyGroups` from the services, so both are one build. |
| `desktop.enable` | `bool` | `false` | Add the Max Desktop application, with a launcher entry on Linux |
| `desktop.package` | `package` | `package.maxDesktop` | The desktop package. The default follows `package`, so the application and the services run one Max runtime. |
...
The launcher carries `MAX_HOME` itself. A desktop menu reads no shell
profile, so the value that `programs.max-agent.enable` exports with
`home.sessionVariables` reaches an interactive shell only. Without the
value in the launcher, the application opens `~/.max` while the
...
`services.max-agent.installPackage` was removed by this split. A
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.max` symlink to the service stateDir and are auto-added to the `max` group |
...
├── auth.json                        # OAuth credentials: seeded, then Max owns it
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Max binary + all Nix deps |
...
cd /etc/nixos && nix flake update max-agent
...
```bash
systemctl status max-agent
docker ps -a --filter name=max-agent
docker inspect max-agent --format='{{.State.Status}}'
docker exec -it max-agent bash
docker exec max-agent readlink /data/current-package
docker exec max-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop max-agent
docker rm -f max-agent
sudo rm /var/lib/hermes/.container-identity
sudo systemctl start max-agent
```
...
```bash
# Native mode
sudo -u max cat /var/lib/hermes/.max/.env

# Container mode
docker exec max-agent cat /data/.max/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec max-agent readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u max-agent` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `max --version` shows old version | Container not restarted | `systemctl restart max-agent` |
| Permission denied on `/var/lib/hermes` | State dir is `0750 hermes:hermes` | Use `docker exec` or `sudo -u hermes` |
| `nix-collect-garbage` removed max | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "max-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
...
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart max-agent` |
