...
To easily install the command-line and desktop applications, [download the max Desktop installer](https://hermes-agent.nousresearch.com/) from our website and run it.
...
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `max model` |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `max model` → MiniMax (OAuth) |
...
```bash
max            # classic CLI
max --tui      # modern TUI (recommended)
```
...
```bash
max --continue    # Resume the most recent session
max -c            # Short form
```
...
# ~/.max/max-agent on Linux/macOS or %LOCALAPPDATA%\max\max-agent on Windows):
cd ~/.max/max-agent
