...
Follow the [installation instructions for max Desktop](../getting-started/installation.md).
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `max desktop --cwd <path>` (or the `HERMES_DESKTOP_CWD` environment variable).
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#max-cron).
...
| `--max-root PATH` | Override the max source root the app uses (sets `HERMES_DESKTOP_HERMES_ROOT`)          |
...
max serve --host 0.0.0.0 --port 9119
```
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/max` work if you front it with a reverse proxy)
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting max Desktop to a remote backend](./features/web-dashboard.md#connecting-max-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & max Desktop](../reference/environment-variables.md#web-dashboard--max-desktop).
...
tccutil reset Microphone com.nousresearch.max
```
