...
max -s max-agent-dev,github-auth
max chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
max --continue             # Resume the most recent CLI session (-c)
max --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
max chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
max -w                         # Interactive mode in worktree
max -w -z "Fix issue #123"     # Single query in worktree
```
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `max --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches max — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while max is working, max prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::
...
```bash
max --continue                          # Resume the most recent CLI session
max -c                                  # Short form
max -c "my project"                     # Resume a named session (latest in lineage)
max --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
max --resume "refactoring auth"         # Resume by title
max -r 20260225_143052_a1b2c3           # Short form
```
