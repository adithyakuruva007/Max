...
```bash
# devtunnel (Microsoft)
devtunnel create max-bot --allow-anonymous
devtunnel port create max-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host max-bot
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create max-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs max` for tracebacks |
