...
Connect Max to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Max supports two WeCom integration modes:
...
> Run `max gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Max decrypts the message, queues it for the agent
...
(Use `max gateway start` only after `max gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Max and the token the
...
in `~/.max/.env` values around `=` will also break signature checks. After
fixing, restart `max gateway run`.
...
Check `max gateway run` logs for the bound host/port. If the adapter bound to
