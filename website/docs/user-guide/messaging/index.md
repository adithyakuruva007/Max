...
description: "Chat with max from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with max from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with max](/guides/use-voice-mode-with-max).
...
For group chats, hooks, and automation flows, max supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. max keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; max does not hide failures just because the text resembles a silence token.
...
```bash
max gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
max gateway              # Run in foreground
max gateway setup        # Configure messaging platforms interactively
max gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo max gateway install --system   # Linux only: install a boot-time system service
max gateway start        # Start the default service
max gateway stop         # Stop the default service
max gateway status       # Check default service status
max gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update max Agent to the latest version |
...
with the `session_reset` section in `~/.max/config.yaml`:
...
Configure per-platform overrides in `~/.max/gateway.json`:
...
max pairing approve telegram XKGH5N7P

# Other pairing commands:
max pairing list          # View pending + approved users
max pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, max appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.max/config.yaml`:
...
Off by default. When enabled, max prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.max/config.yaml`:
...
```bash
max gateway install               # Install as user service
max gateway start                 # Start the service
max gateway stop                  # Stop the service
max gateway status                # Check status
journalctl --user -u max-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo max gateway install --system
sudo max gateway start --system
sudo max gateway status --system
journalctl -u max-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit max installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit max-gateway` (or `sudo systemctl edit max-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `max update`. When `max update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart max-gateway` command (it never blocks on an interactive password prompt).
...
```bash
max gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `max update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo max update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/max-gateway`:

```
max ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed max-gateway*, /usr/bin/systemctl --no-ask-password start max-gateway*, /usr/bin/systemctl --no-ask-password restart max-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. max will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple max installations on the same machine (with different `HERMES_HOME` directories), each gets its own systemd service name. The default `~/.max` uses `max-gateway`; other installations use `max-gateway-<hash>`. The `max gateway` commands automatically target the correct service for your current `HERMES_HOME`.
:::

### macOS (launchd)

```bash
max gateway install               # Install as launchd agent
max gateway start                 # Start the service
max gateway stop                  # Stop the service
max gateway status                # Check status
tail -f ~/.max/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.max.gateway.plist`. It includes three environment variables:
...
- **HERMES_HOME** — scopes the gateway to your max installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `max gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `HERMES_HOME` directory gets its own launchd label. The default `~/.max` uses `ai.max.gateway`; other installations use `ai.max.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `max-cli` | Full access |
| Telegram | `max-telegram` | Full tools including terminal |
| Discord | `max-discord` | Full tools including terminal |
| WhatsApp | `max-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `max-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `max-slack` | Full tools including terminal |
| Google Chat | `max-google_chat` | Full tools including terminal |
| Signal | `max-signal` | Full tools including terminal |
| SMS | `max-sms` | Full tools including terminal |
| Email | `max-email` | Full tools including terminal |
| Home Assistant | `max-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `max-mattermost` | Full tools including terminal |
| Matrix | `max-matrix` | Full tools including terminal |
| DingTalk | `max-dingtalk` | Full tools including terminal |
| Feishu/Lark | `max-feishu` | Full tools including terminal |
| WeCom | `max-wecom` | Full tools including terminal |
| WeCom Callback | `max-wecom-callback` | Full tools including terminal |
| Weixin | `max-weixin` | Full tools including terminal |
| BlueBubbles | `max-bluebubbles` | Full tools including terminal |
| QQBot | `max-qqbot` | Full tools including terminal |
| Yuanbao | `max-yuanbao` | Full tools including terminal |
| Microsoft Teams | `max-teams` | Full tools including terminal |
| API Server | `max-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `max-webhook` | Full tools including terminal |
| Raft | `max-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.max/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
