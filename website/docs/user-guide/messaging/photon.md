...
Connect Max to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Max — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Max runs it in a
...
2. **Finds or creates** the `Max Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.max/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.max/.env`
...
(device token, dashboard project id) lives in `~/.max/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Max
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Max replies with a pairing code. Approve it with:
...
**Pre-authorize specific numbers** (in `~/.max/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.max/.env`):
...
By default Max responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Max` and
`@Max agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Max will reply.
...
assigned iMessage line Max uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run max photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `max photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `max photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `max photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Max sends images, voice
  ...
  media.
- **Native polls are supported.** Max sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Max sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `max send`, the dashboard) reuse the sidecar the
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Max is set
...
| `PHOTON_MENTION_PATTERNS` | Max wake words  | JSON list / comma / newline regex patterns for group mentions |
