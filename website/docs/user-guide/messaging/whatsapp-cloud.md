...
2. **Add** → name (e.g. `max-bot`) → role: **Admin**.
...
Message-template support (the workaround for outside-window sends) is not yet implemented in max. If you need it, please [open an issue](https://github.com/NousResearch/hermes-agent/issues) — it's planned but waiting on a clear demand signal.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `max tools list` and verify the platform is using `max-whatsapp` (the default Cloud adapter toolset, same as Baileys).
...
```bash
max config set stt.provider openai
max config set stt.use_gateway true
max gateway restart
```
