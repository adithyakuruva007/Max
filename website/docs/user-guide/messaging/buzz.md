...
> Run `max gateway setup` and pick **Buzz** for a guided walk-through.
...
You can configure Buzz two ways — the `gateway` block in `config.yaml` (canonical) or environment variables (which override it). The private key is a **secret** and always belongs in `~/.max/.env`.
...
Check status with `max gateway status` — Buzz connection state is reported there, including for env-only setups.
