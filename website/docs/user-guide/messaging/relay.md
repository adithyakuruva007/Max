...
sidebar_position: 30
title: "Max Relay"
description: "Connect Max to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---
...
Max Relay is not a chat platform itself — it is a **connector system** that
...
secret. `max gateway enroll` redeems a **single-use enrollment token**
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.max/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.max/.env`:
...
`max gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.max/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `max auth add nous` (or `max setup`) and retry.
...
startup. Restart it (`max gateway restart`).
