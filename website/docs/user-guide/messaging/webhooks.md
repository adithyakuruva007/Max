...
```python
# ~/.max/scripts/todoist-max-label.py
import json
import sys

payload = json.load(sys.stdin)
labels = payload.get("payload", {}).get("labels", [])
if "max" not in labels:
    print("[SILENT]")
    raise SystemExit(0)
...
```bash
max webhook subscribe antenna-matches \
  --deliver telegram \
...
```bash
max webhook subscribe github-issues \
  --events "issues" \
...
```bash
max webhook remove github-issues
```

### Test a subscription

```bash
max webhook test github-issues
max webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```
