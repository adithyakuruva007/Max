...
If you have local or plugin commands that should stay visible in Telegram's `/` picker, prioritize them in `~/.max/config.yaml`:
...
# named profiles
max -p research gateway start
max -p research gateway status
max -p research gateway stop
```
