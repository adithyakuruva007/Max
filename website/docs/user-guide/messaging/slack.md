...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `max-socket` (the name doesn't matter)
...
### Legacy `/max <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/max btw run the tests` — max routes it the same way as `/btw
run the tests`. Free-form questions also work: `/max what's the
...
```bash
max slack manifest --slashes-only > /tmp/slashes.json
```
