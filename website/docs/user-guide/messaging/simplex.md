...
> Run `max gateway setup` and pick **SimpleX** for a guided walk-through.
...
| `MAX_SIMPLEX_TEXT_BATCH_DELAY` | Optional | Quiet-period seconds (default: `0.8`) used to concatenate rapid-fire inbound text messages into one event |
...
2. Use **DM pairing** — send any message to the bot and it will reply with a pairing code. Enter that code via `max pairing approve simplex <CODE>`.
...
`simplex:group:12` as a cron `deliver=` target or in a `max send` call.

## Sending with `max send`
...
`max send --list` shows them by name. Before the first gateway run the
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`max send` CLI](/guides/pipe-script-output):
...
- The connection between Max and the daemon is local WebSocket (`ws://127.0.0.1:5225`) — no data leaves your machine
