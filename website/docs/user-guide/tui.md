...
max --tui -r 20260409_000000_aa11bb
max --tui --resume "my t0p session"
...
With `display.interface: tui`, a bare `max` (and `max chat`) launches the TUI. Explicit flags always win — run `max --cli` to drop back to the classic REPL for a single invocation, or `max --tui` / `HERMES_TUI=1` to force the TUI when the config default is `cli`.
...
<img alt="max TUI Session Orchestrator with one live session and a +new row" src="/img/docs/tui-session-orchestrator/session-orchestrator.png" />

<video controls muted loop playsInline src="/img/docs/tui-session-orchestrator/session-orchestrator-demo.mp4" title="max TUI Session Orchestrator demo" />
...
- **`⚠ YOLO`** — visible warning whenever YOLO mode is on (`max --yolo`, `/yolo`, or `HERMES_YOLO_MODE=1`). The same badge also appears in the startup banner so you cannot launch an auto-approving session without noticing.
...
Launching `max` (without `--tui`) stays on the classic CLI by default. To make a machine prefer the TUI, set `display.interface: tui` in `~/.max/config.yaml` (persistent) or `HERMES_TUI=1` in your shell profile (per-shell). To go back, set `interface: cli` / unset the env var, or pass `max --cli` for a one-off.
