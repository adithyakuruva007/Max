...
title: "Git Worktrees"
description: "Run multiple max agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

max Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with max so each session has a clean, isolated working directory.

## Why Use Worktrees with max?

max treats the **current working directory** as the project root:

- CLI: the directory where you run `max` or `max chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.max/config.yaml`
...
git worktree add ../repo-feature feature/max-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/max-experiment` checked out in that directory

Now you can `cd` into the new worktree and run max there:

```bash
cd ../repo-feature

# Start max in the worktree
max
```
...
git worktree add ../repo-experiment-a feature/max-a
git worktree add ../repo-experiment-b feature/max-b
```
...
- Works on its own branch (`feature/max-a` vs `feature/max-b`).
...
- max checkpoint data under `~/.max/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per max experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/max-checkpoints-docs`, `feature/max-refactor-tests`.
- **Commit frequently**
...
- **Avoid running max from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `max -w` (Automatic Worktree Mode)

max has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `max/max-<hash>`).
...
```bash
max -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `max -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each max session its own clean checkout.
