...
The location can be relocated with the `MAX_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `MAX_HOME` — set by the same administrator who owns the managed
...
export MAX_MANAGED_DIR=/opt/org/hermes-policy
```

:::warning
A user who can set `MAX_MANAGED_DIR` can repoint managed scope at a directory
