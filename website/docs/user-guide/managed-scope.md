...
`~/.max/config.yaml`, `~/.max/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/max`:

```text
/etc/max/
├── config.yaml     # managed config layer (wins over ~/.max/config.yaml)
└── .env            # managed env layer (wins over ~/.max/.env + shell)
```
...
files. It is **never persisted** to any `.env` by max.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export HERMES_MANAGED_DIR=/opt/org/max-policy
```
...
left user-settable. `max doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/max/config.yaml` (managed) | `/etc/max/.env` (managed) |
| 2 | `~/.max/config.yaml` (user) | `~/.max/.env` (user) |
...
```bash
max config        # shows a header naming the managed source + the pinned keys
max doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, max refuses and names the source:

```bash
$ max config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/max/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `max config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/max

# Pin some config values for every user on this machine
sudo tee /etc/max/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/max/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/max
sudo chmod 0644 /etc/max/config.yaml /etc/max/.env
```

Changes take effect on the next max start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`max doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs max as `root`), managed scope is advisory.
