...
```bash
coder gateway install         # creates max-gateway-coder systemd/launchd service
assistant gateway install     # creates max-gateway-assistant service
```
...
max profile export coder   # export to coder.tar.gz
max profile import coder.tar.gz   # import from archive
```
...
```bash
# Bash
eval "$(max completion bash)"

# Zsh
eval "$(max completion zsh)"
```
...
Profiles use the `HERMES_HOME` environment variable. When you run `coder chat`, the wrapper script sets `HERMES_HOME=~/.max/profiles/coder` before launching max. Since 119+ files in the codebase resolve paths via `get_hermes_home()`, max state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
...
- `HERMES_HOME` is the profile boundary. It controls max config, `.env`,
...
max also exposes `HERMES_REAL_HOME` to subprocesses so scripts can still find
