...
Run multiple independent Max agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Max home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Max state.

:::caution Give every agent its own profile
Never point two agent processes at the same profile (the same Max home). Both write memory automatically, and each loads the other's writes into its system prompt at session start — so two writers on one home compound each other's state until it stops being anything you configured. Profiles exist exactly to prevent this; agents that need shared memory should use an [external memory provider](/user-guide/features/memory-providers) instead.
:::
...
```bash
max profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Max profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `max setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::

### Blank profile

```bash
max profile create mybot
```
...
```bash
max profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `max profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
Copies your current profile's `config.yaml`, `.env`, `SOUL.md`, and skills into the new profile. Same API keys, model, and capabilities, but fresh sessions and memory. Edit `~/.max/profiles/work/.env` for different API keys, or `~/.max/profiles/work/SOUL.md` for a different personality.

### Clone everything (`--clone-all`)

```bash
max profile create backup --clone-all
```

Copies **everything** — config, API keys, personality, all memories, skills, cron jobs, plugins. A complete working snapshot. Per-profile history is excluded (session history, `state.db`, `backups/`, `state-snapshots/`, `checkpoints/`) — these belong to the source profile and can reach tens of GB. For a full backup including history, use `max profile export` or `max backup` instead.

### Clone from a specific profile

```bash
max profile create work --clone-from coder
```
...
```bash
max profile create work-backup --clone-from coder --clone-all
```
...
The alias works with every max subcommand — it's just `max -p <name>` under the hood.
...
max chat -p coder -q "hello"    # works in any position
```

### Sticky default (`max profile use`)

```bash
max profile use coder
max chat                   # now targets coder
max tools                  # configures coder's tools
max profile use default    # switch back
```

Sets a default so plain `max` commands target that profile. Like `kubectl config use-context`.
...
- **`max profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Max its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Max was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.max/profiles/coder/.env

# Edit assistant's tokens
nano ~/.max/profiles/assistant/.env
```
...
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `max profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `max -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.max/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `max profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`max update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
max profile list           # show all profiles with status
max profile show coder     # detailed info for one profile
max profile rename coder dev-bot   # rename (updates alias + service)
max profile export coder   # pack into coder.tar.gz (shareable; keys stripped)
max profile import coder.tar.gz   # install an archive as a new profile
```
...
renamed because `~/.max` is the installation root. Renaming it instead
sets a **display name**, which UI surfaces show in place of the bare ID:

```bash
max profile rename default Harumesu   # Unicode fine: 小助手
```

The display name appears in `max profile list`/`show`, the `/profile`
...
stored as `display_name` in `~/.max/profile.yaml`; remove that line to
...
```bash
max profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `max profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.max`). To remove everything, use `max uninstall`.
:::
...
Profiles use the `MAX_HOME` environment variable. When you run `coder chat`, the wrapper script sets `MAX_HOME=~/.max/profiles/coder` before launching hermes. Since 119+ files in the codebase resolve paths via `get_max_home()`, Max state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

This is separate from terminal working directory. Tool execution starts from `terminal.cwd` (or the launch directory when `cwd: "."` on the local backend), not automatically from `MAX_HOME`.
...
isolated by `MAX_HOME`, not by changing `HOME`. Container backends still use
`{MAX_HOME}/home` for persistent tool state, and host users who need strict
per-profile tool config can opt in with `terminal.home_mode: profile`.

This means two things that are easy to mix up:

- `MAX_HOME` is the profile boundary. It controls Max config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Max
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Max keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Max launches tool
subprocesses with `HOME={MAX_HOME}/home`; you then need to initialize or link
...
Max also exposes `MAX_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.max` itself. No migration needed — existing installs work identically.
...
max profile export coder
max profile import ./coder.tar.gz --name coder
```
