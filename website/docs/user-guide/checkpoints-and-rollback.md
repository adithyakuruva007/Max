...
Max Agent can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.

Enable checkpoints per-session with `--checkpoints`:

```bash
max chat --checkpoints
```

Or enable globally in `~/.max/config.yaml`:
...
This safety net is powered by an internal **Checkpoint Manager** that keeps a single shared shadow git repository under `~/.max/checkpoints/store/` — your real project `.git` is never touched. Every project the agent works in shares the same store, so git's content-addressable object DB deduplicates across projects and across turns.
...
| Command | Description |
|---------|-------------|
| `max checkpoints` | Show total size, project count, per-project breakdown |
| `max checkpoints status` | Same as bare `checkpoints` |
| `max checkpoints list` | Alias for `status` |
| `max checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `max checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `max checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- Max detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  - Resolves a reasonable project root for the file.
  - Initialises or reuses the **single shared shadow store** at `~/.max/checkpoints/store/`.
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.max/checkpoints/ at startup
  ...
  # `max checkpoints prune` command below, with a confirmation prompt.
  auto_prune: true
  retention_days: 7
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `max checkpoints prune` manually.
...
Max responds with a formatted list showing change statistics:
...
```text
Checkpoint base: /home/you/.max/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
  ...
  /home/you/code/max-agent                                        20       2h ago  live
...
Clear with: max checkpoints clear-legacy
```
...
`/rollback <N>` restores only the files Max itself changed. Every successful
...
Max last wrote (you edited it afterwards, or Max never touched it) is
...
If the ledger is empty (a store created before this feature, or Max hasn't
...
- **Directory scope** — Max skips overly broad directories (root `/`, home `$HOME`).
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `max checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.max/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.max/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:
...
- **Enable checkpoints only when you need them** — `max chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `max checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each Max session in its own worktree/branch, with checkpoints as an extra layer.
