...
- Once per conversation turn (per directory), it:
  ...
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/max/<project-hash>`).
...
```bash
max checkpoints prune --retention-days 3 --max-size-mb 200
```
...
~/.max/checkpoints/
  ...
  │   ├── refs/max/<hash> # per-project branch tip
