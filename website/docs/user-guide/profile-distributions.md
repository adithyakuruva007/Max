...
A **profile distribution** packages a complete Max agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

Max has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `max profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `max profile update` pulls new versions | Re-send the file |
...
| Made with | `max profile install` / `update` | `/export` and `/import`, or `max profile export` / `import` |
...
Before distributions, sharing a Max agent meant sending someone:
...
```bash
max profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `max profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Max in 2026, that's already true.
...
:::caution
**Max does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `max profile install` or `max profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
max profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.max/profiles/research-bot/SOUL.md
...
Create `~/.max/profiles/research-bot/distribution.yaml`:
...
Create `~/.max/profiles/research-bot/.gitignore` with at minimum:
...
Recipients who run `max profile update research-bot` will pull the latest.
...
```bash
max profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.max/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
max profile install github.com/you/research-bot

# Full HTTPS
max profile install https://github.com/you/research-bot.git

# SSH
max profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
max profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
max profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
max profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
max profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
max profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Max distribution.
...
```bash
cp ~/.max/profiles/research-bot/.env.EXAMPLE ~/.max/profiles/research-bot/.env
...
`max profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
max profile update research-bot
```
...
```bash
max profile delete research-bot
```
...
Path:    ~/.max/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `max profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
max profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `max profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.max/profiles/polymarket-trader
...
# Anyone
max profile install github.com/you/hermes-polymarket-trader --alias
```
...
You built Max-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
max profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
max profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `max profile info <name>` and hold off on updates until you're ready.
:::
...
max profile info research-bot | grep Version
...
```yaml
# ~/.max/profiles/research-bot/local/my-overrides.yaml
...
max profile delete research-bot --yes
max profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
max profile update research-bot --force-config --yes
```
...
max profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.max/profiles/forked-research-bot/
...
max profile install ~/.max/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
max profile delete research-bot-test --yes
max profile install ~/.max/profiles/research-bot --name research-bot-test
```
...
```bash
max profile export research-bot
max profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
max profile import ./research-bot.tar.gz
max profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `max -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.max`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.max`) is exported through an allow-list — only known Max artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.max/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `max -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `max profile` — not a parallel command tree.
...
- Reserved profile names (`max`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`max profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Max](../guides/use-soul-with-hermes.md) — authoring personalities
