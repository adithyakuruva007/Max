...
- **You just want to back up a profile on your own machine.** Use [`max profile export` / `import`](../reference/profile-commands.md#max-profile-export) — that's what those are for.
...
git remote add origin https://github.com/you/max-polymarket-trader.git
git push -u origin main --tags

# Anyone
max profile install github.com/you/max-polymarket-trader --alias
```
...
- [`max profile export` / `import`](../reference/profile-commands.md#max-profile-export) — local backup / restore (not distribution)
- [Using SOUL with max](../guides/use-soul-with-max.md) — authoring personalities
