...
(`max-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
`~/.local/bin/max-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: max-gateways {start|stop|restart|status|list}"
}
...
```bash
max-gateways start      # start every configured profile
max-gateways stop       # stop every configured profile
max-gateways restart    # restart all
max-gateways status     # status across all
max-gateways list       # delegates to `max gateway list`
```
...
| Linux    | `~/.config/systemd/user/max-gateway-<profile>.service`         |
...
```bash
max logs -f                  # follow default profile
max -p coder logs -f         # follow one profile
max logs --help              # filters, levels, JSON output
```
...
max-gateways status          # full status across every profile
launchctl list | grep max    # macOS — PIDs and labels
systemctl --user list-units 'max-gateway-*'   # Linux — units
```
...
systemd-inhibit --what=idle:sleep --who=max --why="gateways running" \
  sleep infinity &
...
`max-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
# Linux
systemctl --user restart max-gateway-<profile>.service
```
...
max -p <profile> doctor     # one profile
```
