...
max -c "my project"   # → resumes "my project #3"
```
...
max --resume 20250305_091523_a1b2c3d4
max -r 20250305_091523_a1b2c3d4

# Resume by title
max --resume "refactoring auth"
...
```bash
max sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
# Filter by platform
max sessions list --source telegram

# Show more sessions
max sessions list --limit 50
```
...
max sessions export backup.jsonl

# Export sessions from a specific platform
max sessions export telegram-history.jsonl --source telegram

# Export a single session
max sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
max sessions export backup.jsonl --redact
```
...
max sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
max sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
max sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
max sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `max-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
max sessions export --format trace

# One session to a local trace file
max sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
max sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
max sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
max sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
max sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
max sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
max sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
max sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
max sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
max sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
max sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
max sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
max sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
max sessions prune --newer-than 5h

# Explicit window with absolute timestamps
max sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
max sessions prune --source telegram
max sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
max sessions prune --newer-than 5h --title "smoke test"   # title substring
max sessions prune --older-than 30 --max-messages 3        # tiny sessions
max sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
max sessions prune --model gpt-5 --older-than 1w           # by model (substring)
max sessions prune --provider openrouter --older-than 60   # by billing provider
max sessions prune --branch feature/old-experiment         # by git branch
max sessions prune --user 12345678 --chat-type group       # by messaging origin
max sessions prune --max-tokens 500 --older-than 7         # by token usage
max sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
max sessions prune --newer-than 5h --dry-run

# Skip confirmation
max sessions prune --older-than 30 --yes
```
...
max sessions archive --newer-than 5h

# Archive by title substring, preview first
max sessions archive --title "dry run" --dry-run
max sessions archive --title "dry run" --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`max insights`](/reference/cli-commands#max-insights).
...
| Gateway routing index | `~/.max/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.max/sessions/sessions.json` is the **gateway routing index** — it maps
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every max process in the same `HERMES_HOME`
...
max sessions delete <session_id>
