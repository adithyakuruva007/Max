...
| | |
|---|---|
| Source | Optional — install with `max skills install official/mcp/fastmcp` |
...
| Related skills | [`max-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-max-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Max or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Max. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.max/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.max/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Max integration, either:

- configure the server in `~/.max/config.yaml` using the `native-mcp` skill, or
...
### Max cannot see the deployed server

The server-building part may be correct while the Max config is not. Load the `native-mcp` skill and configure the server in `~/.max/config.yaml`, then restart Max.
