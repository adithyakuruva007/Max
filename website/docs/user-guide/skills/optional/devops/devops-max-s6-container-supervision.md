...
│   └── dashboard/run                  ← if HERMES_DASHBOARD=1, runs `max dashboard`
│
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `max gateway start/stop/restart` and routes to s6 when running in a container. |
...
HERMES_TEST_IMAGE=max-agent-harness:latest scripts/run_tests.sh tests/docker/ -v
...
The cont-init reconciler runs as max (`s6-setuidgid max` in `02-reconcile-profiles`). If a profile dir ends up root-owned (e.g. because `docker exec <c> max profile create …` ran as root by default), the reconciler can't read SOUL.md and fails with `PermissionError`. Mitigation: `stage2-hook.sh` chowns `$HERMES_HOME/profiles` to max on **every** boot, idempotently. Don't remove that block.

### Files written by `docker exec` are root-owned

`docker exec` defaults to root. Either pass `--user max` or rely on the stage2 chown sweep next reboot. Don't write files under `$HERMES_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
