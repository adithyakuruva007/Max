...
| | |
|---|---|
| Source | Optional — install with `max skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Max Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${MAX_HOME:-~/.max}/.env`:
...
```bash
CANVAS="python $MAX_HOME/skills/productivity/canvas/scripts/canvas_api.py"
