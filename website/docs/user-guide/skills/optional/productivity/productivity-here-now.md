...
| | |
|---|---|
| Source | Optional — install with `max skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${MAX_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${MAX_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${MAX_SKILL_DIR}/scripts/publish.sh"
...
```bash
DRIVE="${MAX_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `max`)    |
