...
| | |
|---|---|
| Source | Optional — install with `max skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Max MCP catalog. The
...
Two halves, in this order: the editor side must be up before Max connects.
...
`~/.max/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Max — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Max connects
...
2. Start the Max session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Max
they appear as:
...
then happens at `max mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Max runs batched calls
...
- **Start order matters.** Editor + server up first, then the Max
...
- **Timeouts on long operations.** Max' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.max/config.yaml` for
