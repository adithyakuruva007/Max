...
| | |
|---|---|
| Source | Optional — install with `max skills install official/creative/pixel-art` |
...
```python
import sys
sys.path.insert(0, "/home/teknium/.max/skills/creative/pixel-art/scripts")
...
```bash
cd /home/teknium/.max/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — max installs package this)
