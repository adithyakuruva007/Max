...
| | |
|---|---|
| Source | Optional — install with `max skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Max core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Max native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Max native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Max native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Max usage patterns
...
- Do not silently choose Parallel when Max native tools are already sufficient.
