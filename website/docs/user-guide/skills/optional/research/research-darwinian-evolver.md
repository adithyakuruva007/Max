...
| | |
|---|---|
| Source | Optional — install with `max skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), max Agent |
| License | MIT |
...
import upstream classes into max itself.
...
```bash
mkdir -p ~/.max/cache/darwinian-evolver && cd ~/.max/cache/darwinian-evolver
...
```bash
cd ~/.max/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.max/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.max/skills/research/darwinian-evolver
DE_DIR=~/.max/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside max core.
   Custom driver scripts under `~/.max/skills/...` are user-side and fine.
