...
description: "Configure and use Honcho memory with max -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with max -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `max skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for max

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every max profile its own peer identity while sharing a unified view of the user.
...
```bash
max honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this max profile's AI peer
...
Honcho models conversations as interactions between **peers**. max creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this max instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each max run |
| `global` | Single session across all directories |

Manual override: `max honcho map my-project-name`
...
Each max profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block max.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `max.coder` host block in `honcho.json`
...
```bash
max honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for max when Honcho memory is active.
...
Run `max honcho setup`. Ensure `memory.provider: honcho` is in `~/.max/config.yaml`.

### Memory not persisting across sessions
Check `max honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `max profile create <name> --clone`. For existing profiles: `max honcho sync`.
...
| Command | Description |
|---------|-------------|
| `max honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `max honcho status` | Show resolved config, connection test, peer info for active profile |
| `max honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `max honcho disable` | Disable Honcho for the active profile |
| `max honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `max honcho peers` | Show peer identities across all profiles |
| `max honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `max honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `max honcho sessions` | List known directory-to-session-name mappings |
| `max honcho map <name>` | Map current working directory to a Honcho session name |
| `max honcho identity` | Seed AI peer identity or show both peer representations |
| `max honcho sync` | Create host blocks for all max profiles that don't have one yet |
| `max honcho migrate` | Step-by-step migration guide from OpenClaw native memory to max + Honcho |
| `max memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `max memory status` | Show active memory provider and config |
| `max memory off` | Disable external memory provider |
