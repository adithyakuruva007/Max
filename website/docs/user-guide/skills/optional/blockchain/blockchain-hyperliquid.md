...
| | |
|---|---|
| Source | Optional — install with `max skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), max Agent |
| License | MIT |
...
The script reads `${HERMES_HOME:-~/.max}/.env` for two optional defaults:
...
Helper script: `~/.max/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python3 ~/.max/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${HERMES_HOME:-~/.max}/.env`.
...
```bash
python3 ~/.max/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python3 ~/.max/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
