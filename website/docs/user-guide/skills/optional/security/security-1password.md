...
| | |
|---|---|
| Source | Optional — install with `max skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by max Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during max terminal calls (desktop app flow only)
...
### Service Account (recommended for max)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HERMES_HOME:-~/.max}/.env` (the skill will prompt for this on first load).
...
## max Execution Pattern (desktop app flow)

max terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/max-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/max-op.sock"
