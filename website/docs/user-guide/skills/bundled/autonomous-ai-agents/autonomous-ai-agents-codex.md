...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`max-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-max-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the max terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For max itself, `model.provider: openai-codex` uses max-managed Codex
OAuth from `~/.max/auth.json` after `max auth add openai-codex`. For the
...
When invoking the Codex CLI from a max gateway/service context (for example,
