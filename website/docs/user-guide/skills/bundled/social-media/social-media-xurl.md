...
> **Docker HOME pitfall:** In the official Max Docker layout, `/opt/data` is `MAX_HOME`, but Max tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Max-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
