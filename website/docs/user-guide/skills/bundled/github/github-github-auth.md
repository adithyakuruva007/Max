...
- Give it a title like "max-agent-&lt;machine-name>"
...
uv run python3 "${MAX_HOME:-$HOME/.max}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _hermes_env="${MAX_HOME:-$HOME/.max}/.env"; [ -f "$_hermes_env" ] && grep -q "^GITHUB_TOKEN=" "$_hermes_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_hermes_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python3 "${MAX_HOME:-$HOME/.max}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
