---
title: "Max Agent Skill Authoring — Author in-repo SKILL.md files: frontmatter and structure"
sidebar_label: "Max Agent Skill Authoring"
description: "Author in-repo SKILL.md files: frontmatter and structure"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Max Agent Skill Authoring
...
| Path | `skills/software-development/max-agent-skill-authoring` |
| Version | `2.0.0` |
| Author | Max Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `skills`, `authoring`, `max-agent`, `conventions`, `skill-md` |
| Related skills | `plan` (now the built-in `/plan` command), [`requesting-code-review`](/docs/user-guide/skills/bundled/software-development/software-development-requesting-code-review) |
...
# Authoring Max-Agent Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.max/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the max-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with max-agent
...
- Don't use for: personal skills in `~/.max/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `max skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), Max Agent
license: MIT
...
- Credit the **human first**, then "Max Agent" as secondary collaborator: `Ben Barclay (benbarclay), Max Agent`.
- Never `author: Max Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Max Agent`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.max/skills/`.
...
|---|---|
| Max tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.max/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Max Agent` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.max.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through Max tools
