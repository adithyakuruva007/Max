---
title: "Petdex — Install and select animated petdex mascots for max"
...
Install and select animated petdex mascots for max.
...
the max CLI, TUI, and desktop app. This skill drives the `max pets` CLI
...
Use the `terminal` tool to run `max pets <subcommand>`.

## Quick Reference

| Goal | Command |
| --- | --- |
| Browse the gallery | `max pets list` (add a substring to filter: `max pets list cat`) |
| List installed pets | `max pets list --installed` |
| Install a pet | `max pets install <slug>` (add `--select` to make it active) |
...
| Preview/animate in terminal | `max pets show [slug] [--cycle] [--state run]` |
| Disable the pet | `max pets off` |
| Remove a pet | `max pets remove <slug>` |
| Diagnose setup | `max pets doctor` |

## Procedure

1. Find a pet: `max pets list <query>` and note its `slug`.
2. Install + activate: `max pets install <slug> --select`.
3. Preview it: `max pets show` (Ctrl+C to stop).
4. Confirm setup: `max pets doctor` — shows the resolved pet, configured
...
- `scale` (float) — on-screen size of the native 192×208 frames (default 0.33,
  ...
  `max pets scale <factor>`, the `/pet scale` slash command, or the desktop
  Appearance slider.
...
- `max pets doctor` reports `✓ ready` when a pet is installed, selected,
