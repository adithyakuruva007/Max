...
| List/filter by metadata (age, source, cost, tokens, workspace) | `max sessions list` / `stats` via terminal |
| Rename | `max sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `max sessions archive <filters>` |
| Delete (destructive) | `max sessions delete` / `max sessions prune <filters>` |
| Export before deleting anything valuable | `max sessions export --session-id <id> --format md` |
...
`max sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `max sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `max sessions list`
