...
4. Store the token in `${MAX_HOME:-~/.max}/.env` (or via `max setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${MAX_HOME:-~/.max}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
