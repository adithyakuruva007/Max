...
All skills live in **`~/.max/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Max at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `max update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.max` profile):

```bash
curl -fsSL https://max-agent.stardustresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
max profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
max skills opt-out            # stop future seeding — nothing on disk is touched
max skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
max skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `max update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `max skills opt-in`) to re-enable.

:::note Safe by default
`max skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Max installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
(Plan mode works the same way but is a built-in command now: `/plan [request]` tells Max to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.max/plans/` relative to the active workspace/backend working directory.)

You can also interact with skills through natural conversation:

```bash
max chat --toolsets skills -q "What skills do you have?"
max chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Max-tool framing, no invented
commands).
...
/home/user/.max/cache/chart-q4-2025.png
...
When a missing value is encountered, Max asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `max setup` or `~/.max/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `max config migrate` prompts for unconfigured settings, and `max config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.max/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Max scans the
...
policy above), Max can run [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)
...
If you maintain skills outside of Max — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Max to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.max/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.max/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Max process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Max ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.max/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Repos can carry their own skills, active only for sessions started inside that project — the same pattern other agent harnesses use for repo-local configuration. When you launch Max inside a git checkout, it looks for skills in:

```text
<project-root>/.max/skills/    # Max-native location
...
Skills are procedure documents the agent follows, so Max does **not** auto-load them from arbitrary cloned repos. The first time you run Max in a repo with project skills, the banner shows a notice:

```text
◆ 3 project skill(s) found in /home/you/myproject but not loaded — run `max skills trust` to enable them.
```

Trust the repo once (from inside it, or by passing the path):

```bash
max skills trust             # trust the current repo
max skills trust ~/myproject # or explicitly
max skills untrust           # revoke
```

Trusted roots are stored in `skills.trusted_project_dirs` in `~/.max/config.yaml`. Set `skills.project_discovery: false` to turn the feature off entirely (no scanning, no notices).

### Precedence

Project skills are the **highest-precedence tier**: `project → local (~/.max/skills/) → external_dirs`. A project skill named `deploy` overrides a same-named profile or bundled skill for sessions inside that repo — that's the point: vendored repo skills win on their home turf, without touching your global profile. Project skills are tagged `[project]` in the agent's skill index so provenance stays visible.

Like external dirs, project skill directories are treated as repo-owned: autonomous skill maintenance (the curator) never modifies them, and new agent-created skills always go to `~/.max/skills/`.

### Scan-time quarantine

Trust is a repo-level decision, but a repo's skill content changes with every `git pull`. To close that gap, every project skill is scanned with the same security scanner used for Skills Hub installs before it enters the index. A skill whose scan verdict is **dangerous** (prompt-injection directives, credential-exfiltration commands, hidden-text tricks) is quarantined: it does not appear in the skill index, `skills_list`, slash commands, and refuses to load by name with an explanatory error. Scans are content-hash cached under `~/.max/cache/project_skill_scans/` (never inside your repo) and re-run automatically when the skill's content changes.
...
Bundles live in **`~/.max/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `max bundles list`.
...
# Re-scan ~/.max/skill-bundles/ and report changes
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.max/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.max/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.max/pending/skills/` and are
...
```bash
max skills browse                              # Browse all hub skills (official first)
max skills browse --source official            # Browse only official optional skills
max skills search kubernetes                   # Search all sources
max skills search react --source skills-sh     # Search the skills.sh directory
max skills search https://mintlify.com/docs --source well-known
max skills inspect openai/skills/k8s           # Preview before installing
max skills install openai/skills/k8s           # Install with security scan
max skills install official/security/1password
max skills install skills-sh/vercel-labs/json-render/json-render-react --force
max skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
max skills list --source hub                   # List hub-installed skills
max skills check                               # Check installed hub skills for upstream updates
max skills update                              # Reinstall hub skills with upstream changes when needed
max skills audit                               # Re-scan all hub skills for security
max skills uninstall k8s                       # Remove a hub skill
max skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
max skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
max skills publish skills/my-skill --to github --repo owner/repo
max skills snapshot export setup.json          # Export skill config
max skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Max. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `max skills search <query> --source skills-sh`. Max resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Max currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Max repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Max can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
max skills search https://mintlify.com/docs --source well-known
max skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
max skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Max can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
max skills install openai/skills/k8s
max skills tap add myorg/skills-repo
```
...
[Skills Hub](https://max-agent.stardustresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Max-side changes required.
...
- Max source id: `clawhub`

#### 6. LobeHub (`lobehub`)

Max can search and convert agent entries from LobeHub's public catalog into installable Max skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Max source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

Max integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Max' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Max source id: `browse-sh`
- Trust level: `community`

```bash
max skills search airbnb --source browse-sh
max skills inspect browse-sh/airbnb.com/search-listings-ddgioa
max skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Max also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Max source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Max does not enumerate or copy unrelated files from the host.

```bash
max skills install https://sharethis.chat/SKILL.md
max skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
max skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `max skills update` re-fetches from the same URL automatically when you want to refresh.
...
`max skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Max | Always trusted |
...
```bash
max skills check          # Report which installed hub skills changed upstream
max skills update         # Reinstall only the skills with updates available
max skills update react   # Update one specific installed hub skill
max skills update react --force   # Overwrite a skill you've edited locally
```

This uses the stored source identifier plus the current upstream bundle content hash to detect drift.

Skills you have edited locally (the on-disk content no longer matches the hash recorded at install time) are **skipped** by `max skills update` so your changes are never silently overwritten. Pass `--force` to replace them with the upstream version anyway.
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Max users add with `max skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.max.tags`, `version`, `author`, `platforms`, `metadata.max.config`).
...
Max discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Max user can subscribe and install:

```bash
max skills tap add my-org/hermes-skills
max skills search deploy
max skills install my-org/hermes-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.max/skills/.hub/taps.json`:
...
The `max skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `max skills tap list` shows the effective path per tap.
...
```bash
max skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.max/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`max skills reset`)

Max ships with a set of bundled skills in `skills/` inside the repo. On install and on every `max update`, a sync pass copies those into `~/.max/skills/` and records a manifest at `~/.max/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Max recomputes the hash of your local copy and compares it to the origin hash:
...
max skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `MAX_HOME`, so `max -p coder skills reset <name>` only affects that profile.
:::
