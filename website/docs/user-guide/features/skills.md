...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling max to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.max/plans/` relative to the active workspace/backend working directory.
...
max bundles create research

# Overwrite an existing bundle
max bundles create backend-dev --skill ... --force
...
max supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

max source id: `claude-marketplace`
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). max fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
max skills tap add my-org/max-skills
max skills search deploy
max skills install my-org/max-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.max/.hub/taps.json`:
...
Taps are stored in `~/.max/.hub/taps.json` (created on demand).
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HERMES_HOME`, so `max -p coder skills reset <name>` only affects that profile.
:::
