...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   max' own `max auth login codex` writes to `~/.max/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.
...
```bash
# Inside the work profile, you might wrap max:
...
env = { HERMES_HOME = "/your/.max", PYTHONPATH = "...", HERMES_QUIET = "1" }
...
- MCP tool calls (verified against `@modelcontextprotocol/server-filesystem` and the new `max-tools` callback)
...
- **max auth and codex auth are separate sessions.** You need both `codex login` AND `max auth login codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in max' `_import_codex_cli_tokens` — max won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
If you find a bug, [open an issue](https://github.com/NousResearch/hermes-agent/issues) with the output of `max logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
