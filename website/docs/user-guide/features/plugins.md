...
Project-local plugins under `./.max/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting max.
...
| Project | `.max/plugins/` | Project-specific plugins (requires `HERMES_ENABLE_PROJECT_PLUGINS=true`) |
...
```bash
max plugins                    # interactive toggle (space to check/uncheck)
max plugins enable <name>      # add to allow-list
max plugins disable <name>     # remove from allow-list + add to disabled
```
...
```bash
max plugins                               # unified interactive UI
max plugins list                          # table: enabled / disabled / not enabled
max plugins install user/repo             # install from Git, then prompt Enable? [y/N]
max plugins install user/repo --enable    # install AND enable (no prompt)
max plugins install user/repo --no-enable # install but leave disabled (no prompt)
max plugins update my-plugin              # pull latest
max plugins remove my-plugin              # uninstall
max plugins enable my-plugin              # add to allow-list
max plugins disable my-plugin             # remove from allow-list + add to disabled
```
