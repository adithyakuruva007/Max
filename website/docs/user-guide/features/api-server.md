...
**Streaming** (`"stream": true`): Returns Server-Sent Events (SSE) with token-by-token response chunks. For **Chat Completions**, the stream uses standard `chat.completion.chunk` events plus max' custom `max.tool.progress` event for tool-start UX. For **Responses**, the stream uses OpenAI Responses event types such as `response.created`, `response.output_text.delta`, `response.output_item.added`, `response.output_item.done`, and `response.completed`.

**Tool progress in streams**:
- **Chat Completions**: max emits `event: max.tool.progress` for tool-start visibility without polluting persisted assistant text.
...
```json
{
  "object": "max.api_server.capabilities",
  "platform": "max-agent",
  "model": "max-agent",
