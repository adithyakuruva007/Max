...
If no `MAX_SPOTIFY_CLIENT_ID` is set, Max walks you through the app registration inline:
...
Removes tokens from `~/.max/auth.json`. To also clear the app config, delete `MAX_SPOTIFY_CLIENT_ID` (and `MAX_SPOTIFY_REDIRECT_URI` if you set it) from `~/.max/.env`, or run the wizard again.
...
**`INVALID_CLIENT: Invalid redirect URI`** — the redirect URI in your Spotify app settings doesn't match what Max is using. The default is `http://127.0.0.1:43827/spotify/callback`. Either add that to your app's allowed redirect URIs, or set `MAX_SPOTIFY_REDIRECT_URI` in `~/.max/.env` to whatever you registered.
...
```
MAX_SPOTIFY_CLIENT_ID=<your_id>
MAX_SPOTIFY_REDIRECT_URI=http://localhost:3000/callback
```
...
| `~/.max/.env` | `MAX_SPOTIFY_CLIENT_ID`, optional `MAX_SPOTIFY_REDIRECT_URI` |
