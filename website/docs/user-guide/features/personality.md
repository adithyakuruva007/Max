...
More precisely, it uses the current instance's `HERMES_HOME`, so if you run max with a custom home directory, it will use:
...
- max loads `SOUL.md` only from `HERMES_HOME`
...
If max loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the max instance itself.
...
If the file is empty, whitespace-only, or cannot be read, max falls back to a built-in default identity ("You are max Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
You can also define named custom personalities in `~/.max/config.yaml` under `agent.personalities`.
...
- [SOUL.md Guide](/guides/use-soul-with-max)
