...
description: "Customize Max Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Max Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `MAX_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Max is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Max now seeds a default `SOUL.md` automatically in:

```text
~/.max/SOUL.md
```

More precisely, it uses the current instance's `MAX_HOME`, so if you run Max with a custom home directory, it will use:
...
- Max creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Max loads `SOUL.md` only from `MAX_HOME`
- Max does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Max falls back to a built-in default identity
...
If Max loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `MAX_HOME`, the personality belongs to the Max instance itself.

That also makes it easier to teach users:
- "Edit `~/.max/SOUL.md` to change Max' default personality."
...
- how Max should handle uncertainty, disagreement, or ambiguity
...
## What Max injects into the prompt
...
Max ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Max, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Max its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.max/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.max/SOUL.md`
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Max speaks
- `display.skin` and `/skin` affect how Max looks in the terminal
