...
When Max Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.max/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.max/config.yaml` (switchable per session
via `/personality`).
...
If you use Max' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.max/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Max Agent's deliverable
