...
```bash
max curator status         # last run, counts, pinned list, LRU top 5
max curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
max curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
max curator run --background  # fire-and-forget: start the run in a background thread
max curator run --dry-run  # preview only — report without any mutations
max curator backup         # take a manual snapshot of ~/.max/skills/
max curator rollback       # restore from the newest snapshot
max curator rollback --list     # list available snapshots
max curator rollback --id <ts>  # restore a specific snapshot
max curator rollback -y         # skip the confirmation prompt
max curator pause          # stop runs until resumed
max curator resume
max curator pin <skill>    # never auto-transition this skill
max curator unpin <skill>
max curator restore <skill>  # move an archived skill back to active
max curator list-archived    # list skills currently in ~/.max/skills/.archive/
max curator archive <skill>  # manually archive a single skill now
max curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
```
...
```bash
max curator rollback        # restore newest snapshot (with confirmation)
max curator rollback -y     # skip the prompt
max curator rollback --list # see all snapshots with reason + size
```
...
```bash
max curator restore <skill-name>
```
