...
description: "Overview of Max Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
Max ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, scheduled tasks, Home Assistant, and more.
...
| **X Search** | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool — gated on xAI credentials (SuperGrok OAuth or `XAI_API_KEY`); off by default, opt in via `max tools` → 🐦 X (Twitter) Search. |
...
| **Automation** | `cronjob` | Scheduled tasks with create/list/update/pause/resume/run/remove actions. Outbound delivery is handled by cron's own delivery, the `max send` CLI, and the gateway notifier — not by an agent-callable tool. |
...
Paid [Nous Portal](https://portal.stardustresearch.com) subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `max model` to enable it, or configure individual tools with `max tools`.
:::
...
max chat --toolsets "web,terminal"
...
**One persistent container, shared across the whole process.** Max starts a single long-lived container on first use (`docker run -d ... sleep infinity`) and routes every terminal, file, and `execute_code` call through `docker exec` into that same container. Working-directory changes, installed packages, environment tweaks, and files written to `/workspace` all carry over from one tool call to the next, across `/new`, `/reset`, and `delegate_task` subagents, for the lifetime of the Max process. The container is stopped and removed on shutdown.
...
# Configure
max config set terminal.backend singularity
max config set terminal.singularity_image ~/python.sif
```
...
modal setup
max config set terminal.backend modal
```
...
max config set terminal.backend vercel_sandbox
max config set terminal.vercel_runtime node24
```

Authenticate with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This access-token setup is the supported path for deployments and normal long-running Max processes on Render, Railway, Docker, and similar hosts. Supported runtimes are `node24`, `node22`, and `python3.13`; Max defaults to `/vercel/sandbox` as the remote workspace root.
...
With `container_persistent: true`, Max uses Vercel snapshots to preserve filesystem state across sandbox recreation for the same task. This can include Max-synced credentials, skills, and cache files inside the sandbox. Snapshots do not preserve live processes, PID space, or the same live sandbox identity.

Background terminal commands use Max' generic non-local process flow: spawn, poll, wait, log, and kill work through the normal process tool while the sandbox is alive, but Max does not provide native Vercel detached-process recovery after cleanup or restart.
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.max/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.max/.env`.
:::
