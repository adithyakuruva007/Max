...
| **AI peer** (`aiPeer`) | One per max profile. Host key `max` → default; `max.<profile>` for others. |
...
`--clone` creates a `max.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.max/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.max/.env
```
...
**Config:** `$HERMES_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.max/.env`.
...
| `user_id` | `max-user` | User identifier |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `max-{identity}` → `max-coder`) to isolate memories per max profile
...
| **Requires** | `pip install max-memori` + `max-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
| **Memori** | Cloud | Free/Paid | 5 | `max-memori` | Tool-aware memory + structured recall |
