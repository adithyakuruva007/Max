...
Max Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
max memory setup      # interactive picker + configuration
max memory status     # check what's active
max memory off        # disable external provider
```

You can also select the active memory provider via `max plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.max/config.yaml`:
...
When a memory provider is active, Max automatically:
...
**Setup Wizard:**
```bash
max memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `max honcho setup` command still works (it now redirects to `max memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$MAX_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$MAX_HOME/honcho.json` > `~/.max/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/max-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
:::tip Migrating from `max honcho`
If you previously used `max honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Max profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Max profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Max profile. Host key `max` → default; `hermes.<profile>` for others. |
...
```bash
max profile create coder --clone
```
...
Scans every Max profile, creates host blocks for any profile without one, inherits settings from the default `max` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `max memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/NousResearch/max-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
# Then configure Max
max memory setup    # select "openviking"
# Or manually:
max config set memory.provider openviking
```

`max memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.max/.env`, and for named profiles use
`~/.max/profiles/<profile>/.env`.
...
`OPENVIKING_AGENT` is Max' peer ID in OpenViking for peer-scoped memories.
Max sends `User-Agent: openviking-memory-hermes/<version>` on OpenViking
...
**Setup (Platform):**
```bash
max memory setup    # select "mem0" → "Platform"
# Or manually:
max config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.max/.env
```

**Setup (OSS):**
```bash
max memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
max memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
max memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
max memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
max memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.max/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.max/.env
```
...
**Config:** `$MAX_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.max/.env`.
...
**Switching modes:** Re-run `max memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
max memory setup    # select "hindsight"
# Or manually:
max config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.max/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p max ui start`

**Config:** `$MAX_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `max` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Max Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/max-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
max memory setup    # select "holographic"
# Or manually:
max config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.max-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$MAX_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
max memory setup    # select "retaindb"
# Or manually:
max config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.max/.env
```
...
# Then configure Max
max memory setup    # select "byterover"
# Or manually:
max config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$MAX_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
max memory setup    # select "supermemory"
# Or manually:
max config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.max/.env
```
...
Before running `max memory setup`, set `base_url` in
`$MAX_HOME/supermemory.json`:
...
Then run `max memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$MAX_HOME/supermemory.json`
...
| `container_tag` | `max` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hermes-{identity}` → `hermes-coder`) to isolate memories per Max profile
...
max config set memory.provider memori
max memory setup
```
...
- **Local storage providers** (Holographic, ByteRover) use `$MAX_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$MAX_HOME/` so each profile has its own credentials
