...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Max' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Max")

The wake word turns Max into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Max listens in the background for a spoken
trigger phrase. Say it, and Max starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Max backend (for example a
...
Max supports **client capture** for that case:
...
By default the phrase is **"hey hermes"** — a model for it ships with Max, so
...
```bash
cd ~/.max/max-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `max` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.max/config.yaml`,
...
default) to let Max choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Max installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Max surfaces, and `surface` picks which one
...
"Hey Max" works out of the box — the bundled openWakeWord model
...
command (`max -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.max/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
Create a "Hey Max" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey hermes"
  porcupine:
    keyword: ~/.max/wakewords/hey_hermes.ppn
```

Set your access key in `~/.max/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `max tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `max-agent[wake]`).
...
but the phrase never fires. Max detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Max
backend (it may appear as your terminal, `python`, or Max), then toggle the
...
```bash
max config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
max config set wake_word.input_device null
```
