...
The web dashboard is a browser-based UI for managing your Max Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `max setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
max dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
max dashboard --host 0.0.0.0

# Start without opening browser
max dashboard --no-open
```
...
child with the selected profile's `MAX_HOME`, so the conversation runs
...
processes (manage them via `max -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `max-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.max/max-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.max/max-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `max dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `max dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.max` is nearly full
...
- Node.js (same requirement as `max --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.max/max-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.max/.env
ExecStart=/path/to/venv/bin/python -m max_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.max/.env` containing:

```bash
MAX_DASHBOARD_BASIC_AUTH_USERNAME=admin
MAX_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
MAX_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `max config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Max instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `max profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.max/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `max skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `max mcp` reads from.
...
`max mcp catalog` / `max mcp install` use.
...
Connect Max to any messaging platform from the browser — full parity with
`max setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.max/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Max versions, CPU core count + utilization, memory, disk usage of the Max home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Max version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `max update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `max portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `max curator`.
...
This re-reads `~/.max/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`MAX_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.max`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Max Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
|-------|-----------|----------|
| `max dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `max dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `max dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `max dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Max doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `max dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.

### Default provider: Stardust Research
...
- **CLI — `max dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `max setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `MAX_DASHBOARD_OAUTH_CLIENT_ID` into `~/.max/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  max dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes MAX_DASHBOARD_OAUTH_CLIENT_ID to ~/.max/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.stardustresearch.com/local-dashboards) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `MAX_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `MAX_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `max dashboard register` |

Per the Max Agent convention (`~/.max/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: MAX_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `max dashboard register` (Nous Portal) or install a
...
#### Worked example: Stardust Research

From a logged-in Max install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `max dashboard register` uses your existing Nous login to provision an OAuth client and writes `MAX_DASHBOARD_OAUTH_CLIENT_ID` into `~/.max/.env` for you:

```bash
max setup            # if you're not already logged into Nous Portal
max dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes MAX_DASHBOARD_OAUTH_CLIENT_ID to ~/.max/.env
```
...
```bash
max dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with Stardust Research** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://hermes.example.com/auth/callback` and set `MAX_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `MAX_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `MAX_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `MAX_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `MAX_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `MAX_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.max/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.max/.env <<EOF
MAX_DASHBOARD_BASIC_AUTH_USERNAME=admin
MAX_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
MAX_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.max/.env
```
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [Stardust Research](#default-provider-stardust-research) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `MAX_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `MAX_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `MAX_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-hermes.json` — it defines a `max` realm, a **public PKCE client** (`hermes-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```bash
export MAX_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/hermes"
export MAX_DASHBOARD_OIDC_CLIENT_ID="hermes-dashboard"
export MAX_DASHBOARD_PUBLIC_URL="http://localhost:9119"
max dashboard --host 0.0.0.0 --port 9119 --no-open
```

`MAX_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `MAX_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:
...
first; without one, Max fails closed at startup. This prevents the local SPA
...
Max always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
max dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `MAX_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `MAX_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
2. Login page shows a "Continue with Stardust Research" button → `/auth/login?provider=nous`.
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://portal.stardustresearch.com/oauth/authorize?…`.
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$MAX_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.max/plugins/dashboard-auth-myidp/__init__.py
from max_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `MAX_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
MAX_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  max dashboard --host 0.0.0.0
...
Max Desktop can drive a Max backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateways → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Max Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`max dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Stardust Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.

The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Default provider: Stardust Research](#default-provider-stardust-research).

### On the backend (the remote machine)

```bash
# 1. Set the dashboard login credentials in ~/.max/.env (secrets file, 0600).
cat >> ~/.max/.env <<'EOF'
MAX_DASHBOARD_BASIC_AUTH_USERNAME=admin
MAX_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
max dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `MAX_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.max/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.max/.env`, so the credentials are in the environment at boot.
...
The session refreshes automatically and survives restarts when `MAX_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `MAX_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `MAX_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `MAX_DASHBOARD_BASIC_AUTH_USERNAME` / `MAX_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `MAX_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `max_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `max update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `max dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Max Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Max Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
