...
To point [max Desktop](#connecting-max-desktop-to-a-remote-backend) at a dashboard running on another machine instead of its own bundled backend, see the remote-backend section below.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and max versions, CPU core count + utilization, memory, disk usage of the max home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The max version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `max update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
...
| `GET /api/max/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
- **CLI — `max dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `max setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.max/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  max dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes HERMES_DASHBOARD_OAUTH_CLIENT_ID to ~/.max/.env
  ```
...
|---------|-----------|--------|----------------|
| `HERMES_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `max dashboard register` |
...
Bundled providers reported these issues:
  • nous: HERMES_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    ...
    deploys a Max Agent instance — set it to your provisioned
...
**1. Log in and register the dashboard.** `max dashboard register` uses your existing Nous login to provision an OAuth client and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.max/.env` for you:
...
dashboard:
  oauth:
    ...
    self_hosted:
      ...
      client_id: max-dashboard                              # required
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-max.json` — it defines a `max` realm, a **public PKCE client** (`max-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-max.json:/opt/keycloak/data/import/realm-max.json:ro" \
...
export HERMES_DASHBOARD_OIDC_CLIENT_ID="max-dashboard"
...
> admin console (Clients → max-dashboard → Settings). The same pattern works
...
You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`max dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/max` are supported if you front it with a reverse proxy)
