...
max has three hook systems that run custom code at key lifecycle points:
...
WEBHOOK_URL = "https://your-service.example.com/max-events"
...
```bash
max logs --follow --level INFO | grep boot-md
```
...
Declare shell-script hooks in your `cli-config.yaml` and max will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/max/.max/hooks/my-hook.py"
    }
  ]
}
```
