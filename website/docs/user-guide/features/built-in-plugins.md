...
3. **Project** — `./.max/plugins/<name>/` (requires `HERMES_ENABLE_PROJECT_PLUGINS=1`)
...
```bash
max plugins enable disk-cleanup
```
...
```bash
max plugins disable disk-cleanup
...
| `max-achievements` | dashboard tab | Steam-style collectible badges generated from your real max session history |
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HERMES_HOME` or `/tmp/max-*`, track it silently as `test` / `temp` / `cron-output`. |
...
**Safety** — cleanup only ever touches paths under `HERMES_HOME` or `/tmp/max-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.
...
max plugins enable observability/langfuse
```
...
```bash
max plugins list                 # observability/langfuse should show "enabled"
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.max/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
max plugins enable google_meet
...
**Disabling:** `max plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.max/cache/google_meet/` until you remove them.
...
- Unlock state is persisted to `$HERMES_HOME/plugins/max-achievements/state.json`
...
**API** — routes mount under `/api/plugins/max-achievements/`:
...
**State files** — live under `$HERMES_HOME/plugins/max-achievements/`:
...
**Enabling:** Nothing to enable — `max-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `max dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/max-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.max/plugins/max-achievements/` that ships no dashboard. The plugin's state files under `$HERMES_HOME/plugins/max-achievements/` survive — reinstalling preserves your unlock history.
