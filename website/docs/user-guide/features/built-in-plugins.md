...
description: "Plugins shipped with Max Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Max ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.max/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Max Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.max/plugins/<name>/`
3. **Project** — `./.max/plugins/<name>/` (requires `MAX_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `max memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `max plugins list` and the interactive `max plugins` UI), but none load until you explicitly enable them:

```bash
hermes plugins enable disk-cleanup
```

Or via `~/.max/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Max. You always opt in explicitly.
...
The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `max plugins enable <name>`.
...
| `hermes-achievements` | dashboard tab | Steam-style collectible badges generated from your real Max session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `max memory` and `max plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `MAX_HOME` or `/tmp/hermes-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under MAX_HOME | always | Never |
...
**State** — everything lives at `$MAX_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `MAX_HOME` or `/tmp/hermes-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `max plugins enable disk-cleanup` (or check the box in `max plugins`).

**Disabling again:** `max plugins disable disk-cleanup`.
...
**Enabling:** `max plugins enable security-guidance` (or check the box in `max plugins`).

**Disabling again:** `max plugins disable security-guidance`.
...
Traces Max turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Max' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `max logs`.
...
```bash
max tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Max and the next turn ships a trace.
...
Then put the credentials in `~/.max/.env`:

```bash
MAX_LANGFUSE_PUBLIC_KEY=pk-lf-...
MAX_LANGFUSE_SECRET_KEY=sk-lf-...
MAX_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Max turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `MAX_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Max session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `max chat` session lives under one Langfuse session.

**Verify:**

```bash
hermes plugins list                 # observability/langfuse should show "enabled"
max chat -q "hello"              # check the Langfuse UI for a "Max turn" trace
```
...
| `MAX_LANGFUSE_RELEASE` | — | Release/version tag |
| `MAX_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `MAX_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `MAX_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Max-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Max-prefixed wins when both are set.
...
**Disabling:** `max plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled Max plugin. Do not run `max plugins enable observability/nemo_relay`; Max core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `MAX_NEMO_RELAY_PLUGINS_TOML` to that file before starting Max. The policy is process-wide for every profile hosted by that Max process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `MAX_NEMO_RELAY_ATOF_*` and `MAX_NEMO_RELAY_ATIF_*` settings no longer activate exporters. `max doctor` reports these stale settings when no replacement `plugins.toml` is selected.
...
- Post-meeting artifacts (transcript, status) saved under `~/.max/workspace/meetings/<meeting_id>/`
...
**Disabling:** `max plugins disable google_meet`. Any saved transcripts stay in `~/.max/workspace/meetings/` until you remove them.

### hermes-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Max session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Max feature changes.

**How it works:**

- Scans your entire `~/.max/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$MAX_HOME/plugins/hermes-achievements/state.json`
...
| Secret | Hidden until Max detects the first related signal in your history |
...
**State files** — live under `$MAX_HOME/plugins/hermes-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Max updates. |
...
**Enabling:** Nothing to enable — `hermes-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `max dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/hermes-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.max/plugins/hermes-achievements/` that ships no dashboard. The plugin's state files under `$MAX_HOME/plugins/hermes-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Max plugin — see [Build a Max Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.max/plugins/<name>/`
- Manifest source is reported as `bundled` in `max plugins list`
