...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `MAX_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
- Workers spawned for a task see **only** their board's tasks — the
  dispatcher sets `MAX_KANBAN_BOARD` in the child env and every
...
2. `MAX_KANBAN_BOARD` env var (set by the dispatcher when spawning a
...
- **Storage** — files land under
  ...
  board. Set `MAX_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
Override the config flag at runtime via `MAX_KANBAN_DISPATCH_IN_GATEWAY=0`
...
kanban_show()                                     # no args — uses MAX_KANBAN_TASK
...
**Zero schema footprint on normal sessions.** A regular `max chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `MAX_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/max_cli/test_kanban_db.py -q"],
...
2. `cd $MAX_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
**Agent-side prevention:** Before the worker exits, Max injects up to two
...
guard is active only for dispatcher-spawned workers (`MAX_KANBAN_TASK` is
set) and can be disabled with `MAX_KANBAN_STOP_NUDGE=0`.
...
Kanban's per-profile configs make the planner/worker cost split natural. Decomposing a project into well-scoped cards takes frontier-level judgment; executing a card that already carries a clear goal, context, and handoff evidence usually doesn't — and the workers are where the vast majority of tokens are spent, so the worker model is where the cost lives. Run your orchestrator/dispatcher profile on a frontier model and point worker profiles at inexpensive models. Each profile has its own `config.yaml` under `~/.max/profiles/<name>/`, and the dispatcher injects the profile-scoped `MAX_HOME` when it spawns `max -p <assignee>`, so each worker reads its own profile's model settings:

```yaml
# ~/.max/config.yaml (orchestrator / dispatcher profile)
model:
  default: "your-frontier-model"

# ~/.max/profiles/coder/config.yaml (worker profile)
model:
  default: "your-inexpensive-model"

# ~/.max/profiles/researcher/config.yaml (another worker profile)
model:
  default: "your-inexpensive-model"
```

For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `max kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
hermes kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `max gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
hermes kanban stats [--json]                           # per-status + per-assignee counts
hermes kanban log <id> [--tail BYTES]                  # worker log from ~/.max/kanban/logs/
...
Every `max kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `max chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `max_cli.kanban.run_slash()` entry point that reuses the `max kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `max kanban`. You don't have to leave the chat to drive the board.
...
_metadata_: `{"changed_files": ["max_cli/retry.py", "tests/test_retry.py"], "decisions": ["capped backoff at 60s", "jitter = full"]}`
```
...
[`merge-reconciler` skill](https://github.com/NousResearch/max-agent/blob/main/skills/autonomous-ai-agents/merge-reconciler/SKILL.md)
...
```
hotspot: max_cli/kanban_db.py — third conflicting edit to the dispatch loop this wave
```
...
guide](https://github.com/NousResearch/max-agent/blob/main/docs/kanban/multi-gateway.md)),
