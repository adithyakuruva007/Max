...
description: "Durable SQLite-backed task board for coordinating multiple max profiles"
---
...
max Kanban is a durable task board, shared across all your max profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.max/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.max/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `max kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `max kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/max-kanban-v1-spec.pdf` in the repository.
...
called `default` (DB at `~/.max/kanban.db` for back-compat). Users who
only want one stream of work never need to know about boards; the feature
is opt-in.

Per-board isolation is absolute:

- Separate SQLite DB per board (`~/.max/kanban/boards/<slug>/kanban.db`).
...
max kanban boards list

# Create a new board.
max kanban boards create atm10-server \
    --name "ATM10 Server" \
...
max kanban --board atm10-server list
max kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
max kanban boards switch atm10-server
max kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
max kanban boards rename atm10-server "ATM10 (Prod)"
...
3. `~/.max/kanban/current` — the slug persisted by `max kanban
   boards switch`.
4. `default`.
...
`max dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<max-home>/kanban/attachments/<task_id>/` for the default board, or
  `<max-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
...
max kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `max kanban show t_abcd`.
...
for debugging. Standard gateway supervision applies: run `max gateway
...
until one comes up — `max kanban create` warns about this at creation
time.

Running `max kanban daemon` as a separate process is **deprecated**;
...
max kanban create "nightly ops review" \
    --assignee ops \
...
```bash
max kanban complete t_abc t_def t_hij --result "batch wrap"
max kanban archive  t_abc t_def t_hij
max kanban unblock  t_abc t_def
max kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `max kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `max kanban` CLI.
...
### Why tools instead of shelling to `max kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `max kanban complete` *inside* the container, where `max` isn't installed and `~/.max/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.max/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `max chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HERMES_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```bash
max kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

max kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `max skills list` to see what's available); there's no runtime install.
...
```bash
max kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `max kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `max kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `max profile create --description "..."`, `max profile describe <name> --text "..."`, `max profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.max/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `max profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.max/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
If you run `max dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.max/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `max -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard max dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
max kanban init                                     # create kanban.db + print daemon hint
max kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
max kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
max kanban show <id> [--json]
max kanban assign <id> <profile>                    # or 'none' to unassign
max kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
max kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
max kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
max kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
max kanban diagnostics [--json]                     # board health snapshot (alias: diag)
max kanban link <parent_id> <child_id>
max kanban unlink <parent_id> <child_id>
max kanban claim <id> [--ttl SECONDS]
max kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
max kanban complete <id>... [--result "..."]
max kanban block <id> "<reason>" [--ids <id>...]
max kanban unblock <id>...
max kanban archive <id>...

max kanban tail <id>                                # follow a single task's event stream
max kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
max kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
max kanban runs <id> [--json]                       # attempt history (one row per run)
max kanban assignees [--json]                       # profiles on disk + per-assignee task counts
max kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
max kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `max gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
max kanban stats [--json]                           # per-status + per-assignee counts
max kanban log <id> [--tail BYTES]                  # worker log from ~/.max/kanban/logs/
max kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
max kanban notify-list [<id>] [--json]
max kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
max kanban context <id>                             # what a worker sees
max kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
max kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
max kanban create "nightly backup audit" \
...
Every `max kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `max chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hermes_cli.kanban.run_slash()` entry point that reuses the `max kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `max kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.max/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`max kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `max kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/max-kanban-v1-spec.pdf`.
...
```bash
max kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
max kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
max kanban notify-list
max kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
max kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
max kanban runs t_abcd
...
**Bulk close caveat.** `max kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`max kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `max kanban heartbeat $TASK` to signal liveness during long operations. |
...
`max kanban tail <id>` shows these for a single task. `max kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.max/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/max-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
