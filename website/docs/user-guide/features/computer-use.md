...
Max Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Max integration. It
...
**Fresh installs already have the driver.** The Max installer
...
- **`max tools`** → pick `🖱️  Computer Use` — installs the driver
...
on macOS/Linux, `install.ps1` on Windows. Use `max computer-use
status` to verify the install.

Already have cua-driver? Max reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `max update`, and the first
`computer_use` call of a session, Max checks the local version and
...
selected with `MAX_CUA_DRIVER_CMD` stays
under your control, so Max reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Max autodetection is a
planned cua-driver follow-up, so currently point Max at that directory or
...
interface. The built-in toolset provides Max actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `max computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Max host identity. |
...
or add `computer_use` to your enabled toolsets in `~/.max/config.yaml`.

## Permission modes and logged-in browser profiles

Max maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Max session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Max approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Max risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.max/cua-manifest.yaml
```
...
for the format). Max launches a private runtime with
...
identity instead of resetting with every Max build), and Max verifies
...
resetting or closing the Max session, cancellation cleanup, or process exit
closes that transport session. Max also stops private runtimes that it
launched for bounded or unrestricted access. One Max
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Max host identity.
...
## `max computer-use doctor` — your first triage stop

`max computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ max computer-use doctor
...
Max run declares a public cua-driver **session name** (something like
...
without it. Max disables it automatically where it is a known failure
...
Max keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Max-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Max
autodetection is a planned cua-driver follow-up, so currently point Max at
...
These are **platform deep dives, not duplicates of the Max skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Max
...
image capture also saves a bounded, shareable copy under Max' image cache
...
Max applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.max/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Max applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Max agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Max agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Max warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Max from inside the RDP / console
...
```
MAX_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
MAX_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Max disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Max sets
...
When it's on, `max computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Max at a binary you built from source instead
of the published release. Max resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`MAX_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Max testing (see notes).
...
### Option B — point Max straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`MAX_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
MAX_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
MAX_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Max is using your build

- `max computer-use status` prints the resolved binary path and
  version.
- `max computer-use doctor` confirms the binary is reachable and
...
- **Max spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Max YOLO, Max instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Max
  change needed either way.
- **Local builds skip the version check.** Max warns when the
...
**First action when anything's off: run `max computer-use doctor`.**
...
Run `max computer-use install` to fetch the cua-driver binary, or
run `max tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `max computer-use doctor` will
...
- **Max-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Max `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Max autodetection is a
  planned follow-up; currently point Max at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Max among them)
