...
1. Run `max tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or max app). `max computer-use doctor` will tell you which permission is missing. |
...
agent is acting here." Each max run declares its own cua-driver
**session id** (something like `max-3a7b9c14d2e8`); the cursor's
...
max intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the max-side `computer_use` action vocabulary — the
...
- **max spawns its own `cua-driver mcp` child over stdio** — it does
...
- **max-side skill** — `skills/computer-use/SKILL.md` — teaches the
...
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects max (planned follow-up), this happens
