...
The CSS is injected as a single scoped `<style data-max-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**
...
A reference plugin (`example-dashboard` in [`max-example-plugins`](https://github.com/NousResearch/hermes-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
| 3 | `./.max/plugins/<name>/dashboard/` | `project` — only when `HERMES_ENABLE_PROJECT_PLUGINS` is set |
...
The [`strike-freedom-cockpit`](https://github.com/NousResearch/hermes-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `max-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
# Theme
cp max-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.max/dashboard-themes/

# Plugin
cp -r max-example-plugins/strike-freedom-cockpit ~/.max/plugins/
```
