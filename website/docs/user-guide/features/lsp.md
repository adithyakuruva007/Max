...
max runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. max
...
1. max captures a baseline of current diagnostics for the file.
...
…). max auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point max at the extracted bundle — the directory that contains
...
`max lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — max installs both packages together when you
run `max lsp install typescript` or auto-install fires on first
use.

## CLI

```
max lsp status          # service state + per-server install status
max lsp list            # registry, optionally --installed-only
max lsp install <id>    # eagerly install one server
max lsp install-all     # try every server with a known recipe
max lsp restart         # tear down running clients
max lsp which <id>      # print resolved binary path
```

`max lsp status` is the best starting point — it shows which
...
When `install_strategy: auto`, max installs binaries into
...
shared location — the staging dir is fully max-owned and is
...
Servers are kept alive for the life of the max process. There's
...
**`max lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HERMES_HOME>/lsp/bin/`. Run
`max lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `max lsp status`**
...
When `max lsp status` shows a `Backend warnings` section, install
...
Check `~/.max/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `max lsp restart` to clear the set;
