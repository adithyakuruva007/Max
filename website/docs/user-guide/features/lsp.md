...
3. Point Max at the extracted bundle — the directory that contains
   ...
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<MAX_HOME>/lsp/PowerShellEditorServices`, or
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <MAX_HOME>/lsp/bin
...
`<MAX_HOME>/lsp/bin/`. NPM packages land in
`<MAX_HOME>/lsp/node_modules/` with bin symlinks one level up.
...
The binary isn't on PATH and isn't in `<MAX_HOME>/lsp/bin/`. Run
