...
Max can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<MAX_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Max renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `max pets list [query] [--limit N]` |
| List installed pets | `max pets list --installed` |
| Install a pet | `max pets install <slug> [--select] [--force]` |
| Set the active pet | `max pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `max pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `max pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `max pets off` |
| Remove an installed pet | `max pets remove <slug>` |
| Diagnose setup | `max pets doctor` |
...
Beyond installing pre-made pets from the gallery, Max can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `max pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `max tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `MAX_PET_IMAGE_PROVIDER` env var (e.g. `MAX_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Max is
...
Run `max pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Max uses its own
  profile-scoped `<MAX_HOME>/pets/` instead — install through `max pets`.

## See also

- The [`max-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-max-agent.md)
