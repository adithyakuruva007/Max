...
Honcho is configured in `~/.honcho/config.json` (global) or `$MAX_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
hermes honcho setup           # Redirects to `max memory setup` (post-activation alias)
