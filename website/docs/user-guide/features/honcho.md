...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of max's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple max instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
max memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `max` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — max syncs them back at session init.
...
The `max honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `max memory setup honcho` (or run `max memory setup` and pick it from the list); the `max honcho` subcommand then appears on the next invocation.

```bash
max memory setup honcho    # Configure Honcho directly (works before activation)
max honcho status          # Connection status, config, and key settings
max honcho setup           # Redirects to `max memory setup` (post-activation alias)
max honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
max honcho peer            # Show or update peer names + dialectic reasoning level
max honcho mode            # Show or set recall mode (hybrid/context/tools)
max honcho tokens          # Show or set token budget for context and dialectic
max honcho identity        # Seed or show the AI peer's Honcho identity
max honcho sync            # Sync Honcho config to all existing profiles
max honcho peers           # Show peer identities across all profiles
max honcho sessions        # List known Honcho session mappings
max honcho map             # Map current directory to a Honcho session name
max honcho enable          # Enable Honcho for the active profile
max honcho disable         # Disable Honcho for the active profile
max honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `max honcho`

If you previously used the standalone `max honcho setup`:
...
No re-login or re-setup needed. Run `max memory setup` and select "honcho" — the wizard detects your existing config.
