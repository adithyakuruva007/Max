...
description: "One-command import of a Claude Code (~/.claude) or OpenAI Codex CLI (~/.codex) setup into Max — instructions, allowlists, MCP servers, skills, and memories."
---

# Import from Other Agents

`max import-agent` imports your existing **Claude Code** or **OpenAI Codex CLI** setup into Max with one command. It follows the same preview-first pattern as [`max claw migrate`](../guides/migrate-from-openclaw.md): you always see a per-item plan before anything is written, and `--dry-run` never touches disk.
...
| Claude Code | Max |
|---|---|
| `CLAUDE.md` (global instructions) | Memory entries in `~/.max/memories/MEMORY.md` |
...
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.max/skills/claude-code-imports/<name>/` |
...
| Codex CLI | Max |
|---|---|
| `AGENTS.md` (global instructions) | Memory entries in `~/.max/memories/MEMORY.md` |
| `config.toml` → `[mcp_servers.*]` | `mcp_servers` in `config.yaml` |
| `memories/*.md` | Memory entries in `~/.max/memories/MEMORY.md` |
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.max/skills/codex-imports/<name>/` |

## What is never imported

**API keys and credentials.** Credential files (`~/.claude/.credentials.json`, `~/.codex/auth.json`) are never read, and MCP server environment variables or headers with secret-looking names (`*_TOKEN`, `*_API_KEY`, `Authorization`, ...) are stripped and listed in the report so you can re-add them deliberately. Run `max setup` to configure providers, or add secrets to `~/.max/.env`.
...
- **Conflicts are skipped by default.** An MCP server or skill that already exists in Max is reported as a conflict; pass `--overwrite` to replace it.
...
- Coming from OpenClaw instead? Use [`max claw migrate`](../guides/migrate-from-openclaw.md).
