...
# Connecting Desktop to Many Max Instances

Register every Max backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Max Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Max gateway…"**) deep-links straight to
...
list of Max gateways. Its intro says it plainly: *"Manage this device and
every Max gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Max runtime managed by this app." | automatic |
| **Remote gateway** | "A Max gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Max install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Max Cloud** | "A hosted instance discovered through your Max Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Max Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Max Cloud** kind in the add-connection
...
Gateways are machines or hosted backends; profiles are isolated Max agents
...
connection is registered) dispatches `max update` to every eligible
...
- **Max Cloud** instances are skipped with a *"Managed by Max Cloud"*
...
panel, ⌘K **Update Max**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `max serve` is running on the remote host, the
...
- **"Update Max Desktop to chat with agents on other connections"** — the
