...
| `timeout` | `60` | Seconds max waits for an approval reply before timing out. |
...
| `pkill`/`killall` max/gateway | Self-termination prevention |
...
max pairing approve telegram ABC12DEF

# Revoke a user's access
max pairing revoke telegram 123456789

# Clear all pending codes
max pairing clear-pending
```
...
When loaded, max checks if these files exist in the active profile's `HERMES_HOME` and registers them for mounting:
...
max ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `hermes_cli/security_advisories.py`.
