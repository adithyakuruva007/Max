...
| **OpenAI Codex** | `max model` (ChatGPT OAuth, uses Codex models) |
...
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.max/.env` (provider: `alibaba`) |
...
max portal info        # inspect login + routing at any time
```
...
**For full details:** see the dedicated [Nous Portal integration page](/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run max Agent with Nous Portal guide](/guides/run-max-with-nous-portal).

**Client identification.** Every Portal request from max Agent carries a `client=max-client-v<version>` tag (e.g. `client=max-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish max traffic from other clients. No config required; the tag updates automatically when you `max update`.
...
If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), max marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `max auth add codex-oauth` (or `max model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::
...
max chat --provider tencent-tokenhub --model hy3-preview
...
```bash
max migrate xai          # preview replacements
max migrate xai --apply  # rewrite ~/.max/config.yaml in place
```
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. Install with `pip install 'max-agent[vertex]'` (or let max lazy-install `google-auth` on first use). See the [Google Vertex AI guide](/guides/google-vertex) for the full walkthrough, and the [Google Gemini guide](/guides/google-gemini) for the static-API-key AI Studio path instead.
...
max will automatically load a LM Studio model with 64K context length
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser max` |
...
custom_providers:
  ...
  - name: work
    ...
    api_mode: chat_completions   # set explicitly by `max model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
The `max model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.max/config.yaml` and the matching API key goes in `~/.max/.env`.
