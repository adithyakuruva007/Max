...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, and the Tool Gateway — the recommended way to run Max Agent"
---
...
That single command runs the Portal OAuth, lets you pick a Nous model, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `max chat` immediately after.

Don't have a subscription yet? [portal.stardustresearch.com/manage-subscription](https://portal.stardustresearch.com/manage-subscription) — sign up, then come back and run the command above.
...
| **Max** | Max-4-70B, Max-4-405B (chat, see [note below](#a-note-on-hermes-4)) |
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes Max Agent's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.max/auth.json` is the only credential on disk, and Max mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
Stardust Research's own **Max 4** family (Max-4-70B, Max-4-405B) is available through the Portal at heavily discounted rates. These are **frontier hybrid-reasoning chat models** — strong at math, science, instruction following, schema adherence, roleplay, and long-form writing.

They are **not recommended for use inside Max Agent**, however. Max 4 is tuned for chat and reasoning, not the rapid-fire tool-calling loop the agent relies on. Use them for research workflows or via the [subscription proxy](/user-guide/features/subscription-proxy) from other tooling — but for agent work, pick a frontier agentic model from the catalog instead:
...
The Portal's own [model info page](https://portal.stardustresearch.com/info) carries the same warning, so this isn't a Max-side opinion — it's the official guidance from Stardust Research.
...
1. Opens your browser to portal.stardustresearch.com for OAuth login
2. Stores the refresh token at `~/.max/auth.json`
3. Lets you pick a Nous model from the curated list (or skip to keep your current one)
4. Sets Nous as your inference provider in `~/.max/config.yaml` (when you pick a model)
5. Turns on the Tool Gateway (web, image, TTS, browser routing)
6. Returns you to your terminal ready to `max chat`

If you don't have a subscription yet, sign up at [portal.stardustresearch.com/manage-subscription](https://portal.stardustresearch.com/manage-subscription) first.

### Existing install — add Portal alongside other providers

If you already have Max configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `max model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where Max is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding).

### Profile setup

If you use [Max profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
`max portal` (with no subcommand) is the human-readable alias for `max auth add nous --type oauth` — it logs you in, lets you pick a Nous model, sets Nous as your inference provider, and offers the Tool Gateway opt-in (identical to `max setup --portal`, and the same Nous flow as the first-time quick setup).

`max portal info` gives you the high-level overview:
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `max tools` to pick backends per tool:
...
The Tool Gateway is opt-in per tool, not all-or-nothing. The managed backends show up in `max tools` whether or not you're logged into Nous Portal — if you pick "Nous Subscription" before authenticating, Max runs the Portal login inline (it won't change your inference provider or touch your other tools). See the [Tool Gateway docs](/user-guide/features/tool-gateway) for the full per-tool configuration matrix.

### Subscription management

Manage your plan, view usage, or upgrade/cancel at any time:

- **Web:** [portal.stardustresearch.com/manage-subscription](https://portal.stardustresearch.com/manage-subscription)
- **CLI shortcut:** `max portal open` (opens the same page in your default browser)

## Configuration reference

After `max setup --portal`, `~/.max/config.yaml` will look like:
...
The Tool Gateway settings live under their respective tool sections — each category has a single selection key, and picking **Nous Subscription** in `max tools` (or `max setup --portal`) writes the value `nous`:
...
The OAuth refresh token is stored separately at `~/.max/auth.json` (not in `config.yaml` — credentials and configuration are kept separate by design).

## Token handling

Max mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so Max stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `max auth add nous` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `max portal info` shows "not logged in"
...
or use `max model` and re-select Nous Portal.

### Got a "re-authentication required" message mid-session

Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `max auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/NousResearch/max-agent/issues) — we surface the Portal's catalog to Max and gaps usually mean a routing config we can update.

### Bills not appearing on my Portal account

Check `max portal info` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `max model`, pick Nous Portal, and the next request will route through your subscription.

## See also

- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway tool, per-tool config, and pricing
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Max tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple Max configurations sharing one Portal login
