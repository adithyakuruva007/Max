...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, the Tool Gateway, and Nous Chat — the recommended way to run Max Agent"
---
...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a Max-side opinion — it's the official guidance from Nous Research.
...
```bash
max portal            # log in to Nous Portal + set it up (one-shot onboarding)
max portal info       # login status, subscription info, model + gateway routing
max portal status     # alias for `portal info`
max portal tools      # detailed Tool Gateway catalog with per-tool routing
max portal open       # open the subscription management page in your browser
```
...
If a model is genuinely missing, [open an issue](https://github.com/NousResearch/hermes-agent/issues) — we surface the Portal's catalog to Max and gaps usually mean a routing config we can update.
