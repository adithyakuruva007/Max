...
description: "Reference for max core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `max-cli` is the default for interactive CLI sessions)
...
```bash
max chat --toolsets web,file,terminal
max chat --toolsets debugging        # composite — expands to file + terminal + web
max chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - max-cli          # default for CLI
  # - max-telegram   # override for Telegram gateway
```

### Interactive management

```bash
max tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call max tools programmatically. |
...
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `max-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `max-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `max-cli` or other messaging toolsets. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `max tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `max-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `max-cli`:

| Toolset | Differences from `max-cli` |
|---------|-------------------------------|
| `max-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal, web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, and clarify, plus the `safe` (read-only) bundle. |
| `max-acp` | Drops `clarify`, `cronjob`, `image_generate`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
| `max-api-server` | Drops `clarify` and `text_to_speech`. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `max-cron` | Same as `max-cli`. |
| `max-telegram` | Same as `max-cli`. |
| `max-discord` | Adds `discord` and `discord_admin` on top of `max-cli`. |
| `max-slack` | Same as `max-cli`. |
| `max-whatsapp` | Same as `max-cli`. |
| `max-signal` | Same as `max-cli`. |
| `max-matrix` | Same as `max-cli`. |
| `max-mattermost` | Same as `max-cli`. |
| `max-email` | Same as `max-cli`. |
| `max-sms` | Same as `max-cli`. |
| `max-bluebubbles` | Same as `max-cli`. |
| `max-dingtalk` | Same as `max-cli`. |
| `max-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `max-qqbot` | Same as `max-cli`. |
| `max-wecom` | Same as `max-cli`. |
| `max-wecom-callback` | Same as `max-cli`. |
| `max-weixin` | Same as `max-cli`. |
| `max-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `max-cli`. |
| `max-homeassistant` | Same as `max-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `max-webhook` | Same as `max-cli`. |
| `max-gateway` | Internal gateway orchestrator toolset — union of every `max-<platform>` toolset; used when the gateway needs to accept any message source. |
...
## Relationship to `max tools`

The `max tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
