...
- [Use MCP with max](../guides/use-mcp-with-max.md#wsl2-bridge-max-in-wsl-to-windows-chrome)
...
#### `max: command not found` after installation
...
# Set a valid model
max config set HERMES_MODEL anthropic/claude-opus-4.7
...
# Reattach later: tmux attach -t max
...
- [Use MCP with max](/guides/use-mcp-with-max)
...
2. On the **source machine**, create a full backup:
   ...
   max backup
   ```
   This creates a zip of your entire `~/.max/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/max-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/max-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   max import ~/max-backup-<timestamp>.zip
   ```
