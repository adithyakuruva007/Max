...
description: "Authoritative reference for Max terminal commands and command families"
---
...
| `--profile <name>`, `-p <name>` | Select which Max profile to use for this invocation. Overrides the sticky default set by `max profile use`. |
...
| `--ignore-user-config` | Ignore `~/.max/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, memory, and preloaded skills. |
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `MAX_TUI=1`. Always wins over `display.interface`. |
...
| Command | Purpose |
|---------|---------|
| `max chat` | Interactive or one-shot chat with the agent. |
| `max model` | Interactively choose the default provider and model. |
| `max moa` | Configure named Mixture of Agents presets selectable from the model picker. |
| `max fallback` | Manage fallback providers tried when the primary model errors. |
| `max gateway` | Run or manage the messaging gateway service. |
| `max proxy` | Local OpenAI-compatible proxy that attaches OAuth provider credentials. See [Subscription Proxy](../user-guide/features/subscription-proxy.md). |
| `max egress` | Outbound credential-injection firewall for remote terminal sandboxes (iron-proxy). Disabled by default. See [Egress proxy](../user-guide/egress/iron-proxy.md). |
| `max lsp` | Manage Language Server Protocol integration (semantic diagnostics for write_file/patch). |
| `max setup` | Interactive setup wizard for all or part of the configuration. |
| `max whatsapp` | Configure and pair the WhatsApp bridge. |
| `max whatsapp-cloud` | Configure the official Meta WhatsApp Business Cloud API adapter (Business account + public webhook required). Distinct from `max whatsapp` (Baileys personal-account bridge). |
| `max slack` | Slack helpers (currently: generate the app manifest with every command as a native slash). |
| `max auth` | Manage credentials — add, list, remove, reset, status, logout. Handles OAuth flows for Codex/Nous/Anthropic. |
| `max login` / `logout` | **Deprecated** — use `max auth` instead. |
| `max send` | Send a one-shot message to a configured messaging platform (Telegram, Discord, Slack, Signal, SMS, …). Useful from shell scripts, cron jobs, CI hooks, and monitoring daemons — no agent loop, no LLM. |
| `max peer` | Register peer Max gateways on other machines and DM their agents' canonical Bot Chats (`max peer dm <peer>[/<agent>] "…"`). The transport behind cross-machine bot-to-bot messaging. |
| `max secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.max/.env`. |
| `max migrate` | Diagnose and (optionally) rewrite `config.yaml` to replace references to retired models or deprecated settings (e.g. `migrate xai`). |
| `max status` | Show agent, auth, and platform status. |
| `max cron` | Inspect and tick the cron scheduler. |
| `max kanban` | Multi-profile collaboration board (tasks, links, dispatcher). |
| `max project` | Manage named, multi-folder workspaces (projects). Anchors desktop session grouping and, when bound to a kanban board, gives tasks a deterministic worktree + branch convention. State is per-profile. |
| `max webhook` | Manage dynamic webhook subscriptions for event-driven activation. |
| `max hooks` | Inspect, approve, or remove shell-script hooks declared in `config.yaml`. |
| `max doctor` | Diagnose config and dependency issues. |
| `max security audit` | On-demand supply-chain audit (OSV.dev) for the venv, plugin requirements, and pinned MCP servers. |
| `max approvals` | Approval-prompt tools — mine approval history into allowlist proposals. |
| `max dump` | Copy-pasteable setup summary for support/debugging. |
| `max prompt-size` | Show a byte breakdown of the system prompt + tool schemas (skills index, memory, profile). Runs offline. |
| `max debug` | Debug tools — upload logs and system info for support. |
| `max backup` | Back up Max home directory to a zip file. |
| `max checkpoints` | Inspect / prune / clear `~/.max/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `max import` | Restore a Max backup from a zip file. |
| `max logs` | View, tail, and filter agent/gateway/error log files. |
| `max config` | Show, edit, migrate, and query configuration files. |
| `max skin` | List, switch, and tweak display skins. |
| `max console` | Open the safe Max command console. |
| `max pairing` | Approve or revoke messaging pairing codes. |
| `max skills` | Browse, install, publish, audit, and configure skills. |
| `max bundles` | Group several skills under a single `/<name>` slash command. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
| `max curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../user-guide/features/curator.md). |
| `max journey` (aliases `learning`, `memory-graph`) | Timeline of learned skills + memories over time. |
| `max memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `max honcho`) register automatically when their provider is active. |
| `max acp` | Run Max as an ACP server for editor integration. |
| `max mcp` | Manage MCP server configurations and run Max as an MCP server. |
| `max plugins` | Manage Max Agent plugins (install, enable, disable, remove). |
| `max portal` | Nous Portal status, subscription link, and Tool Gateway routing. See [Tool Gateway](../user-guide/features/tool-gateway.md). |
| `max tools` | Configure enabled tools per platform. |
| `max computer-use` | Install or check the Computer Use (cua-driver) backend (macOS/Windows/Linux). |
| `max pets` | Browse, install, and select [petdex](../user-guide/features/pets.md) animated pets shown across the CLI, TUI, and desktop app. Subcommands: `list`, `install`, `select`, `show`, `off`, `scale`, `remove`, `doctor`. |
| `max sessions` | Browse, export, prune, rename, and delete sessions. |
| `max insights` | Show token/cost/activity analytics. |
| `max claw` | OpenClaw migration helpers. |
| `max import-agent` | Import a Claude Code (`~/.claude`) or Codex CLI (`~/.codex`) setup. |
| `max dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `max serve` | Start the Max backend server (headless; powers the desktop app and remote backends). |
| `max desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
| `max profile` | Manage profiles — multiple isolated Max instances. |
| `max completion` | Print shell completion scripts (bash/zsh/fish). |
| `max --version` | Show version information. |
| `max update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--backup` takes a pre-pull `MAX_HOME` snapshot. |
| `max uninstall` | Remove Max from the system. |
...
| `--ignore-user-config` | Ignore `~/.max/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, persistent memory, and preloaded skills. Combine with `--ignore-user-config` for a fully isolated run. |
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Max itself. |
...
```bash
hermes
max chat -q "Summarize the latest PRs"          # seeds an interactive session
max chat --oneshot -q "Summarize the latest PRs"  # answer and exit
max chat --provider openrouter --model anthropic/claude-sonnet-4.6
max chat --toolsets web,terminal,skills
max chat --quiet -q "Return only JSON"
max chat --worktree -q "Review this repo and open a PR"
max chat --ignore-user-config --ignore-rules -q "Repro without my personal setup"
max chat --safe-mode -q "Is this bug mine or Max'?"
```

### `max -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `max -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.
...
Per-run overrides (no mutation to `~/.max/config.yaml`):

| Flag | Equivalent env var | Purpose |
|---|---|---|
| `-m` / `--model <model>` | `MAX_INFERENCE_MODEL` | Override the model for this run |
...
# or:
MAX_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 max -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `max chat --oneshot -q` instead; `-z` is explicitly for "I only want the final answer".

#### `--usage-file` — JSON usage report for pipelines

`max -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, and `completed` / `failed` flags. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.
...
Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Max chat session.
...
:::warning max model vs /model — know the difference
**`max model`** (run from your terminal, outside any Max session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Max chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Max session first (`Ctrl+C` or `/quit`), then run `max model` from your terminal prompt.
:::
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `max model` from the terminal.
:::
...
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Max Relay](/user-guide/messaging/relay). |

Options:

| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `MAX_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `max update`. |
| `--no-supervise` | On `run`: inside the s6-overlay Docker image, opt out of auto-supervision and use pre-s6 foreground semantics — gateway runs as the container's main process with no auto-restart. No-op outside the s6 image. Equivalent to setting `MAX_GATEWAY_NO_SUPERVISE=1`. |
...
`max gateway enroll` accepts `--token`, `--connector-url`, `--gateway-id`, and `--wake-url`. It exchanges the enrollment token with the connector and writes the resulting `GATEWAY_RELAY_ID`, `GATEWAY_RELAY_SECRET`, `GATEWAY_RELAY_DELIVERY_KEY`, optional `GATEWAY_RELAY_URL`, and (when `--wake-url` is given) `GATEWAY_RELAY_WAKE_URL` values to the active profile's `.env`.

:::tip WSL users
Use `max gateway run` instead of `max gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s max 'max gateway run'`. See [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails) for details.
:::
...
```bash
max setup [model|tts|terminal|gateway|tools|agent] [--non-interactive] [--reset] [--quick] [--reconfigure] [--portal]
```

**Easiest path:** `max setup --portal` — OAuth into Nous Portal and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
| `--reconfigure` | Backwards-compat alias — bare `max setup` on an existing install now does this by default. |
...
| `open` | Open `portal.stardustresearch.com/manage-subscription` in your default browser. |
| `tools` | List every Tool Gateway partner (Firecrawl, FAL, OpenAI TTS, Browser Use, Modal) and which are routed via Nous. |

For configuration of the gateway itself, see [Tool Gateway](../user-guide/features/tool-gateway.md). For the one-shot setup path, see `max setup --portal` above.
...
hermes slack manifest --write      # write to ~/.max/slack-manifest.json
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$MAX_HOME/slack-manifest.json`. |
| `--name NAME` | `Max` | Bot display name in Slack. |
...
Run `max slack manifest --write` again after `max update` to pick
up any new commands.
...
echo "message" | max send --to <target>
hermes send --list [platform]
```

Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.max/.env` + `~/.max/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.

For bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp-CloudAPI) no running gateway is required — `max send` talks directly to the platform's REST endpoint. Plugin platforms that need a persistent adapter still require a live gateway.
...
If neither a positional `message` argument nor `--file` is provided, `max send` reads from stdin when it is not a TTY. Exit codes: `0` on success, `1` on delivery/backend failure, `2` on usage errors.
...
echo "RAM 92%" | max send --to telegram:-1001234567890
...
Bot-to-bot DMs across machines. Register another Max gateway (any machine
running the `api_server` platform) as a *peer*, then message its agents:
`max peer dm` resolves the remote agent's canonical **Bot Chat** session
over the peer's API server, runs one agent turn there, and prints the reply
on stdout — the cross-machine twin of the local
`max -p <bot> chat --in ~ -c "Bot Chat" …` bot-messaging command.
...
| Subcommand | Description |
|--------|-------------|
| `add <name> --url <URL> [--key <KEY>] [--note TEXT]` | Register or update a peer. The URL goes to `config.yaml` (`bot_peers`); the key is stored as `MAX_PEER_<NAME>_KEY` in `~/.max/.env`. |
...
includes the peer roster and the `max peer dm` pattern, so agents discover
...
Pull API keys from an external secret manager at process startup instead of storing them in `~/.max/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
> Not to be confused with `max claw migrate` (one-shot import of OpenClaw configuration into Max) — `max migrate` is the top-level config-rewrite command.
...
On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Max venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.max/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the Max Python venv. |
| `--skip-plugins` | off | Skip scanning plugin requirements files. |
| `--skip-mcp` | off | Skip scanning pinned MCP servers in `config.yaml`. |


## `max login` / `max logout` *(Deprecated)*

:::caution
`max login` has been removed. Use `max auth` to manage OAuth credentials, `max model` to select a provider, or `max setup` for full interactive setup.
:::
...
hermes auth spotify                                      # Authenticate Max with Spotify via PKCE
```
...
```bash
max cron <list|create|edit|pause|resume|run|remove|status|tick>
```
...
`$MAX_HOME/plugins/<name>/`. An unknown or unavailable provider falls back to
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.max/kanban/current`). |
...
Board resolution order (highest precedence first): `--board <slug>` flag → `MAX_KANBAN_BOARD` env var → `~/.max/kanban/current` file → `default`.
...
hermes egress setup && max egress start
max config set terminal.backend docker   # if not already
...
# Edit ~/.max/config.yaml proxy.extra_allowed_hosts: [api.example.com]
hermes egress setup
...
cat ~/.max/proxy/proxy.yaml           # the rendered iron-proxy config
tail -20 ~/.max/proxy/iron-proxy.log  # daemon-level diagnostics
tail -f ~/.max/proxy/iron-proxy.log | jq  # daemon + per-request log (line-delimited JSON; v0.39 combines both streams)
```
...
| `--script` | Filter/transform script under `~/.max/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload, and empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |

Subscriptions persist to `~/.max/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart.
...
Outputs a compact, plain-text summary of your entire Max setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | Max version, release date, git commit hash |
| **Environment** | OS, Python version, OpenAI SDK version |
| **Identity** | Active profile name, MAX_HOME path |
...
:::tip
`max dump` is specifically designed for sharing. For interactive diagnostics, use `max doctor`. For a visual overview, use `max status`.
:::

## `max debug`

```bash
max debug share [options]
```
...
The report includes system info (OS, Python version, Max version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.
...
```bash
max debug share              # Upload debug report, print URL
max debug share --lines 500  # Include more log lines
max debug share --expire 30  # Keep paste for 30 days
max debug share --nous       # Upload a private diagnostics bundle for Nous support
max debug share --local      # Print report to terminal (no upload)
```
...
Create a zip archive of your Max configuration, skills, sessions, and data. The backup excludes the max-agent codebase itself, and it does not nest earlier backup artifacts (`backups/`, `state-snapshots/`) — each of those already contains its own copy of `state.db`.
...
The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Max is running (WAL-mode safe).

**What's excluded from the zip:**

- `*.db-wal`, `*.db-shm`, `*.db-journal` — SQLite's WAL / shared-memory / journal sidecars. The `*.db` file already got a consistent snapshot via `sqlite3.backup()`; shipping the live sidecars alongside it would let a restore see a half-committed state.
- `checkpoints/` — per-session trajectory caches. Hash-keyed and regenerated per session; wouldn't port cleanly to another install anyway.
- The `max-agent` code itself (this is a user-data backup, not a repo snapshot).
...
Inspect and manage the shadow git store at `~/.max/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `max checkpoints` is equivalent. |
...
Restore a previously created Max backup into your Max home directory. All files in the archive overwrite existing files in your Max home; `--force` only skips the confirmation prompt that fires when the target already has a Max installation.
...
View, tail, and filter Max log files. All logs are stored in `~/.max/logs/` (or `<profile>/logs/` for non-default profiles).
...
Max uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `max logs list` subcommand shows all log files including rotated ones.
...
- **Prompt tiers** — stable / context / volatile, matching how Max layers
...
enabled. To shrink the prompt, disable unused toolsets (`max tools`) or
uninstall skills you don't need (`max skills`). Context files (AGENTS.md,
...
| `get <key> [--json]` | Print a single config value by dotted key (e.g. `max config get model.default`). `--json` emits machine-readable output. |
...
```bash
max skills browse
max skills browse --source official
max skills search react --source skills-sh
max skills search https://mintlify.com/docs --source well-known
max skills inspect official/security/1password
max skills inspect skills-sh/vercel-labs/json-render/json-render-react
max skills install official/migration/openclaw-migration
max skills install skills-sh/anthropics/skills/pdf --force
max skills install https://sharethis.chat/SKILL.md                     # Direct URL (+ referenced support files)
max skills install https://example.com/SKILL.md --name my-skill        # Override name when frontmatter has none
max skills check
max skills update
max skills config
max skills reset google-workspace
max skills reset google-workspace --restore --yes
max skills opt-out                  # stop future bundled-skill seeding (nothing deleted)
max skills opt-out --remove --yes   # also delete UNMODIFIED bundled skills
max skills opt-in --sync            # undo: remove marker and re-seed now
```
...
- `--source well-known` lets you point Max at a site exposing `/.well-known/skills/index.json`.
...
Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.max/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.max/skill-bundles/` and report added/removed bundles |
...
| `backup` | Take a manual tar.gz snapshot of `~/.max/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.max/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `max update`. Use `max curator run --dry-run` to preview before that happens.
...
`max moa configure` reuses Max' provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.
...
| `add` | Pick a provider + model (same picker as `max model`) and append to the chain |
...
Inspect shell-script hooks declared in `~/.max/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.max/shell-hooks-allowlist.json`.
...
When an external memory provider is active, it may register its own top-level `max <provider>` command for provider-specific management (e.g. `max honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `max --help` to see what's currently wired in.
:::
...
Starts Max as an ACP (Agent Client Protocol) stdio server for editor integration.
...
```bash
cd ~/.max/max-agent && uv pip install -e '.[acp]'
```
...
Manage MCP (Model Context Protocol) server configurations and run Max as an MCP server.
...
| `install <name>` | Install a catalog entry (e.g. `max mcp install n8n`). |
| `serve [-v\|--verbose]` | Run Max as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Max](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server).
...
Unified plugin management — general plugins, memory providers, and context engines in one place. Running `max plugins` with no subcommand opens a composite interactive screen with two sections:
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Max Plugin](../developer-guide/plugins/index.md).
...
`max computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`max tools` invokes when you first enable Computer Use, so it's safe
to use for re-running the install if the toolset toggle didn't trigger
it (for example, on returning-user setups).

If cua-driver is already present, Max checks its version and runtime
...
installer. Max never replaces a custom binary selected through
`MAX_CUA_DRIVER_CMD`; update that binary directly or remove the override.
`max computer-use status` reports when repair is required.

The built-in `computer_use` toolset is the recommended Max integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects Max and links Cua's
skill pack into the Max skills directory automatically.

Permission mode and capability-manifest approval
belong to runtime launch. In bounded mode Max passes Cua's canonical
...
`max update` automatically re-runs the upstream installer at the end
of the update if cua-driver is on PATH, so most users will not need to
call `--upgrade` manually. Use it when upstream ships a fix you want
right now without waiting for the next Max update.
...
[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and Max shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
Migrate your OpenClaw setup to Max. Reads from `~/.openclaw` (or a custom path) and writes to `~/.max`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Max files on conflicts (default: refuse to apply when the plan has conflicts). |
| `--migrate-secrets` | Include API keys in migration. Required even under `--preset full`. |
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.max/` (by default a single restore-point archive is written to `~/.max/backups/pre-migration-*.zip` before apply; restorable with `max import`). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Max equivalents or **archived** for manual review.
...
Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Max. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.max/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Start the Max **backend server** — the JSON-RPC/WebSocket gateway the [desktop app](/user-guide/desktop) and remote clients connect to. It is the same server `max dashboard` runs, but **headless**: it never opens a browser UI. The desktop app launches its own `max serve` backend; use this command directly when you want a headless backend on a remote host. Accepts the same `--host` / `--port` / `--insecure` / `--skip-build` / `--stop` / `--status` options as `max dashboard` below (a non-loopback bind engages the same auth gate). Requires the `[web]` extra; the embedded Chat socket additionally needs `[pty]` on a POSIX host.

**Port conflicts:** if the requested port (default `9119`) is already held by another process (e.g. a second `max serve` or the gateway), the command prints a machine-readable sentinel line `BACKEND_PORT_IN_USE port=<port>` to stdout, a human hint naming the likely holder, and exits with code **75** (`EX_TEMPFAIL`) instead of a generic error — so scripts and the desktop app can tell "port occupied" apart from "backend broken". Pass `--port 0` to bind a free ephemeral port (the successful boot announces the chosen port via `MAX_BACKEND_READY port=<port>`).

## `max dashboard`

```bash
max dashboard [options]
```

Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. (For a headless backend with no browser UI — e.g. what the desktop app spawns — use [`max serve`](#hermes-serve) above.) Requires `cd ~/.max/max-agent && uv pip install -e ".[web]"` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`cd ~/.max/max-agent && uv pip install -e ".[web,pty]"`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](/user-guide/features/web-dashboard) for full documentation.
...
| `--stop` | — | Stop running `max dashboard` processes and exit. |
| `--status` | — | List running `max dashboard` processes and exit. |

### `max dashboard register`

Register this install as a self-hosted dashboard with your Nous Portal account. Creates an OAuth client, writes `MAX_DASHBOARD_OAUTH_CLIENT_ID` into `~/.max/.env`, and prints how to engage the login gate. Requires being logged in (`max setup`).
...
| `--portal-url` | Override the Nous Portal base URL for registration (default: the portal you logged into). Also settable via `MAX_DASHBOARD_PORTAL_URL`. |
...
max dashboard --port 8080 --no-open
...
```bash
max profile <subcommand>
```

Manage profiles — multiple isolated Max instances, each with its own config, sessions, skills, and home directory.
...
```bash
max profile list
max profile create work --clone
max profile use work
max profile alias work --name h-work
max profile export work -o work-backup.tar.gz
max profile import work-backup.tar.gz --name restored
max profile install github.com/user/my-distro --alias
max profile update work
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Max commands, subcommands, and profile names.
...
```bash
max update [--gateway] [--check] [--plan] [--no-backup] [--backup] [--yes]
```

Pulls the latest `max-agent` code and reinstalls dependencies in the managed venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install. Use `--check` to see whether your checkout is behind `origin/main` without installing.

`max update` pulls the configured update branch (default: `main`). If your checkout is on another branch, Max may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
| `--backup` | Force a **full** pre-update backup for this run: the quick state snapshot plus a complete zip of `MAX_HOME` (config, auth, sessions, skills, pairing data). The default mode is `quick` — a lightweight state snapshot only. Set the permanent mode via `updates.pre_update_backup: quick | full | off` in `config.yaml`. |
| `--yes`, `-y` | Assume yes for interactive prompts such as config migration and stash restore. API-key entry is skipped; run `max config migrate` separately for those. |

Additional behavior:

- **Gateway restart.** After a successful update, Max attempts to restart all running gateway profiles automatically so they pick up the new code. Use `max gateway restart` when you want to restart a gateway without applying an update.
...
- **Update receipts + fleet version check.** Every run writes a machine-readable receipt to `~/.max/logs/update_receipts/` (pre-update fleet plan, steps, skips with reasons, restart outcome; `latest.json` points at the newest). After the restart phase the updater verifies each live gateway's running code against the updated checkout and prints a per-profile version matrix; a gateway still on pre-update code fails the update (exit 1) with the exact restart command.
...
- **npm lockfile churn.** Before stashing or switching branches, Max makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `max update`.
- **Pairing data snapshot.** Even when `--backup` is off, `max update` takes a lightweight snapshot of `~/.max/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `max backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `hermes.service` warning.** If Max detects a pre-rename `hermes.service` systemd unit (instead of the current `hermes-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| Command | Description |
|---------|-------------|
| `max --version` | Print version information. |
| `max update` | Pull latest changes and reinstall dependencies. |

| `max uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove Max, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
