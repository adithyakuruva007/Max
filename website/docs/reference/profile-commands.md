...
This page covers all commands related to [Max profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `max profile` without a subcommand shows help.
...
```bash
max profile use <name>
```

Sets `<name>` as the active profile. All subsequent `max` commands (without `-p`) will use this profile.
...
max profile use default
```

## `max profile create`

```bash
max profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `max profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `max update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.max`), use `max skills opt-out` / `max skills opt-in`. |
...
max profile create work2 --clone-from work

# Clone everything from a specific profile
max profile create work2-backup --clone-from work --clone-all
```

## `max profile describe`

```bash
max profile describe [<name>] [options]
```
...
max profile describe researcher

# Set it explicitly
max profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
max profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
max profile describe --all --auto
```

## `max profile delete`

```bash
max profile delete <name> [options]
```
...
```bash
max profile delete mybot
max profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.max`) cannot be deleted — use `max uninstall` to remove everything.
:::

## `max profile show`

```bash
max profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Max home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ max profile show work
Profile: work
Path:    ~/.max/profiles/work
...
```bash
max profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Max installation.
...
```bash
max profile alias work
# Creates/updates ~/.local/bin/work

max profile alias work --name mywork
# Creates ~/.local/bin/mywork

max profile alias work --remove
...
```bash
max profile rename <old-name> <new-name>
```
...
```bash
max profile rename mybot assistant
# ~/.max/profiles/mybot → ~/.max/profiles/assistant
...
```bash
max profile export <name> [options]
```
...
```bash
max profile export work
# Creates work.tar.gz in the current directory

max profile export work -o ./work-2026-03-29.tar.gz
```
...
```bash
max profile import <archive> [options]
```
...
```bash
max profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

max profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `max -p telemetry`). |
...
max profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
max profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
max profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
max profile install ./telemetry/
```

### `max profile update`

```bash
max profile update <name> [--force-config] [--yes]
```
...
```bash
max profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Max version, author, env var requirements, the source URL/path, and
...
`max profile list` also shows the distribution name and version in a
`Distribution` column, and `max profile show <name>` / `delete <name>`
...
max profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
max profile install https://github.com/your-org/internal-assistant.git
```
...
Max version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Max can clone from.
3. Tell recipients to run `max profile install <your-repo-url>`.
...
Global flag to run any Max command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
- `max profile <TAB>` — subcommands (list, use, create, etc.)
- `max profile use <TAB>` — profile names
- `max -p <TAB>` — profile names
