...
:::info
`max profile export` / `import` are still the right commands for
...
```bash
max -p <name> <command> [options]
max --profile <name> <command> [options]
```
...
```bash
max -p work chat -q "Check the server status"
max --profile dev gateway start
max -p personal skills list
max -p work config edit
```

## `max completion`

```bash
max completion <shell>
```
