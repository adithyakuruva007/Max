...
```bash
max webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
max webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
max webhook subscribe alert-triage \
...
```bash
max webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
max webhook subscribe stripe-payments \
