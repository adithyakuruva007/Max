---
sidebar_position: 7
title: "Use SOUL.md with max"
description: "How to use SOUL.md to shape max Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with max

`SOUL.md` is the **primary identity** for your max instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want max to feel like the same assistant every time you talk to it — or if you want to replace the max persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm max should be
- what max should avoid stylistically
- how max should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who max is and how max speaks
...
max now uses only the global SOUL file for the current instance:

```text
~/.max/SOUL.md
```

If you run max with a custom home directory, it becomes:
...
max automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, max does not overwrite it
- if the file exists but is empty, max adds nothing from it to the prompt

## How max uses it

When max starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how max feels.
...
max already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Max should sound.

# Avoid
What Max should not do.

# Defaults
How Max should behave when ambiguity appears.
```
...
Then restart max or start a new session.
...
### I edited SOUL.md but max still sounds the same

Check:
- you edited `~/.max/SOUL.md` or `$HERMES_HOME/SOUL.md`
...
### max is ignoring parts of my SOUL.md
