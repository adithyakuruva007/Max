...
```bash
max teams-pipeline token-health
max teams-pipeline token-health --force-refresh
```
...
```bash
max teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
max teams-pipeline maintain-subscriptions
max teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: max cron (recommended if you already run the max gateway)

max ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.max/scripts/`. First create the script:
...
Create `/etc/systemd/system/max-teams-pipeline-maintain.service`:
...
Type=oneshot
User=max
EnvironmentFile=/etc/max/env
ExecStart=/usr/local/bin/max teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/max-teams-pipeline-maintain.timer`:
...
sudo systemctl enable --now max-teams-pipeline-maintain.timer
systemctl list-timers max-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/max teams-pipeline maintain-subscriptions >> /var/log/max/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.max/.env` at the top of a wrapper script that crontab calls.
...
```bash
max teams-pipeline subscriptions   # should show expirationDateTime advanced
max teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
max teams-pipeline list
max teams-pipeline list --status failed
max teams-pipeline show <job-id>
```

### Replay a stored job

```bash
max teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
max teams-pipeline fetch --meeting-id <meeting-id>
max teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `max teams-pipeline maintain-subscriptions --dry-run`
- inspect `max teams-pipeline list --status failed`
...
- run `max teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `max teams-pipeline run`
...
- [ ] `max teams-pipeline validate` returns an OK snapshot
- [ ] `max teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (max cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
