...
Want max to have a stable default voice? Edit `~/.max/SOUL.md` (or `$HERMES_HOME/SOUL.md` if you use a custom max home). max now seeds a starter SOUL automatically and uses that global file as the instance-wide personality source.

For a full walkthrough, see [Use SOUL.md with max](/guides/use-soul-with-max).
...
TERMINAL_DOCKER_IMAGE=max-sandbox:latest
```
