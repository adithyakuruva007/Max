...
This tutorial walks you through setting up a Telegram bot powered by max Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **max Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.max/.env`

:::tip
A $5/month VPS is plenty for running the gateway. max itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team max Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Max Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u max-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `max gateway install` to update the plist.
:::
...
```bash
max gateway stop && max gateway start
```
...
max pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.max/.env`:
...
Control how much detail the bot shows when using tools. In `~/.max/config.yaml`:
...
Customize how the bot communicates by editing `~/.max/SOUL.md`:

For a full guide, see [Use SOUL.md with max](/guides/use-soul-with-max).
...
```bash
# From the CLI
max cron list          # View all scheduled jobs
max cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u max-gateway` (Linux) or `~/.max/logs/gateway.log` (macOS) |
| Cron job output | `~/.max/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.max/cron/jobs.json` |
| Pairing data | `~/.max/pairing/` |
| Session history | `~/.max/sessions/` |
