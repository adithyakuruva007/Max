...
- **`google-auth`** — installed automatically the first time you select Vertex (lazy install), or explicitly with `pip install 'max-agent[vertex]'`.
...
Install the extra: `pip install 'max-agent[vertex]'`. max also lazy-installs it the first time you select the Vertex provider.
