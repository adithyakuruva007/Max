...
Open [portal.stardustresearch.com/manage-subscription](https://portal.stardustresearch.com/manage-subscription), sign up, and pick a plan.
...
3. Sets `model.provider: nous` in `~/.max/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where max is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
max setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
max auth add nous --type oauth
# Then re-run `max setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Max Agent release notes" and summarize the top 3 hits.
```

You should see max call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`max setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
max config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Max-4 for agent work

Max-4-70B and Max-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.stardustresearch.com) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For max Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.stardustresearch.com/info) carries this warning too — it's the official Nous guidance, not just a max-side opinion.
...
These rows appear in `max tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, max runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and max will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
max cron create "every day at 9am" \
...
If you use [max profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.max/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `max portal info` shows "not logged in" after `max setup --portal`
...
```bash
max config set model.provider nous
```
...
Re-verify with `max portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so max doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/NousResearch/max-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`max portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple max profiles where you're using the wrong one (check `max profile list`)

### Want to revoke and start clean

```bash
max auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-max tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple max configurations
