---
sidebar_position: 8
title: "Use Voice Mode with Max"
description: "A practical guide to setting up and using Max voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Max
...
- you want Max sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Max.
...
## Step 1: make sure normal Max works first
...
Max supports both local and cloud speech stacks.
...
### If you use `max setup`

If you choose NeuTTS in the setup wizard, Max checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Max transcribes and responds
...
- asking Max to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Max loop.

## Tuning CLI behavior

### Silence threshold

If Max starts/stops too aggressively, tune:
...
Max stays a normal chat bot, but can speak replies.
...
- you want Max to function like a portable research or ops assistant
...
Max joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Max detects speech boundaries
- transcripts are posted in the associated text channel
- Max responds in text and audio
...
1. get text Max working
2. run `max setup tts` to enable voice support
