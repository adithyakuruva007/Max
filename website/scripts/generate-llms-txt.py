#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Max docs site.
...
LLM reads to learn what Max does.

Both publish at:
  https://max-agent.stardustresearch.com/docs/llms.txt
  https://max-agent.stardustresearch.com/docs/llms-full.txt
...
SITE_BASE = "https://max-agent.stardustresearch.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Max as a Python Library", None),
        ("guides/use-mcp-with-hermes", "Use MCP with Max", None),
        ("guides/use-voice-mode-with-hermes", "Use Voice Mode with Max", None),
        ("guides/use-soul-with-hermes", "Use SOUL.md with Max", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Max"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Max Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Stardust Research. A terminal-native "
    ...
    lines.append(
        ...
        "max-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/NousResearch/max-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Max Agent — Full Documentation\n",
        (
            "This file is the entire Max Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Max, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://max-agent.stardustresearch.com/docs\n",
        "Short index: https://max-agent.stardustresearch.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
