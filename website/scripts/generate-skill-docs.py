...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/StardustResearch/max-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    max_meta = (fm.get("metadata") or {}).get("max") or {}
    tags = max_meta.get("tags") or []
    related = max_meta.get("related_skills") or []
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Max Agent ships with a large built-in skill library copied into `~/.max/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Max Agent also syncs bundled skills on `max update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.max/skills/` tree, it is still shipped with Hermes; restore it with `max skills reset <name> --restore`.",
        "",
