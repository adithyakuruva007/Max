...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/max-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `max skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Max Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Max ships with a large built-in skill library copied into `~/.max/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Max also syncs bundled skills on `max update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.max/skills/` tree, it is still shipped with Max; restore it with `max skills reset <name> --restore`.",
        "",
