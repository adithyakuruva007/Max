...
معاونین (contributors) کے لیے فوری آغاز — کلون (clone) کریں اور `setup-max.sh` چلائیں:
...
cd max-agent
./setup-max.sh     # uv کو انسٹال کرتا ہے، venv بناتا ہے، .[all] کو انسٹال کرتا ہے، اور ~/.local/bin/max کا سیم لنک (symlink) بناتا ہے
./max              # خود بخود venv کی شناخت کرتا ہے، پہلے `source` کرنے کی ضرورت نہیں
```
...
- 🐛 [مسائل (Issues)](https://github.com/NousResearch/max-agent/issues)
...
- 🔌 [MaxClaw](https://github.com/AaronWong1999/maxclaw) — کمیونٹی وی چیٹ (WeChat) برج: ہرمیس ایجنٹ اور OpenClaw کو ایک ہی وی چیٹ اکاؤنٹ پر چلائیں۔
...
[نوس ریسرچ (Stardust Research)](https://stardustresearch.com) کی جانب سے تیار کردہ۔
