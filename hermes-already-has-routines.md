# Max Agent Has Had "Routines" Since March
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Max implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Max Agent |
|---|---|---|
...
## Things Max Does That Routines Can't
...
hermes cron create "every 1h" \
  ...
  --script ~/.max/scripts/watch-site.py \
...
Max has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Max Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
Full automation blueprints gallery: [max-agent.stardustresearch.com/docs/reference/automation-blueprints-catalog](https://max-agent.stardustresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [max-agent.stardustresearch.com](https://max-agent.stardustresearch.com)

GitHub: [github.com/StardustResearch/max-agent](https://github.com/StardustResearch/max-agent)

---

*Max Agent is built by [Nous Research](https://stardustresearch.com). Open source, model-agnostic, runs on your infrastructure.*
