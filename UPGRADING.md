...
2. **Backward compat is load-bearing.** Dual-read patterns (`MAX_*` /
   `HERMES_*` env vars, event names, config paths, localStorage keys, API
...
- Tool names and schemas are identical between Hermes and Max
- Usually drop-in — just copy the file and re-run tests
- Check: does the tool reference `hermes` in any string or env var? If so,
...
- Check: `plugin.yaml` author field says "Nous Research" — update to
  "Stardust Research"

### Skills (`skills/`, `optional-skills/`) — Low effort

- Text-based; review SKILL.md for Max-branded strings or `hermes` CLI refs
...
- Usually drop-in
- Check for: `hermes` binary/CLI references in Python code
- Check: `.env.example` additions for `HERMES_*` keys — add `MAX_*` dual-read
- Check: adapter name string references ("Hermes" in user-facing messages)

### Plugin System — Medium effort

- ABC-based; review ctx API compatibility
- Check: plugin hooks reference `hermes` event names — add `max.*` dual-emit
...
- **Always add a `MAX_*` variant alongside `HERMES_*`** for backward compat
...
```bash
# Check for stale "Hermes" / "Nous Research" strings
rg -n 'Hermes|Nous Research' --type ts --type py --type rs \
