#!/usr/bin/env python3
"""Live test harness for Max Agent's Tool Search feature.
...
ORIGINAL_HOME = os.environ.get("MAX_HOME")
ORIGINAL_AUTH = Path.home() / ".max" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.max/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.max/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="max_ts_live_"))
    max_home = home_dir / ".max"
    max_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, max_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".max" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, max_home / ".env")
        ...
        load_max_dotenv(max_home=str(Path.home() / ".max"))
    ...
    (max_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return max_home
...
def reset_module_state():
    """Drop cached modules so the new MAX_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "max_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original MAX_HOME
    if ORIGINAL_HOME is not None:
        os.environ["MAX_HOME"] = ORIGINAL_HOME
    else:
