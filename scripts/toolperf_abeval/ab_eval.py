...
measured in the production session DB) through `max chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    MAX_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Max home with credentials for the
...
(tracker: NousResearch/max-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "MAX_HOME": str(HOME),
                "MAX_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "max_cli.main", "chat", "--query", q,
