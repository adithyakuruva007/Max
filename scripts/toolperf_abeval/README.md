...
batch (tracker: [#77056](https://github.com/NousResearch/max-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Max home, same model, same tasks, same reps.
...
1. Create a dedicated Max home with credentials for the models under test:
   ...
   integration at it; no Max observability plugin needs to be enabled.
