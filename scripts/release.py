...
VERSION_FILE = REPO_ROOT / "max_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "kavi@local.max": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level MAX_HOME for pid/lock/status identity files; #56986)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.max/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "moonsong@stardustresearch.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "jake@stardustresearch.com": "simpolism",
    ...
    "teknium@stardustresearch.com": "teknium1",
    ...
    "274096618+max-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@stardustresearch.com": "shannonsands",
    ...
    "hermes@stardustresearch.com": "NousResearch",
    ...
    "ben@stardustresearch.com": "benbarclay",
    ...
    "emozilla@stardustresearch.com": "emozilla",
    ...
    "jonny@stardustresearch.com": "yoniebans",
    "jake@stardustresearch.com": "simpolism",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on max mcp add)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (MAX_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (MAX_HOME inject)
    ...
    "clement@stardustresearch.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Max runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level MAX_HOME state files)
    ...
    "annguyen@stardustresearch.com": "annguyenNous",  # PR #52844 co-author
    ...
    "zhchl@max-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Max
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "hermes@stardustresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/max-agent",
    ...
    lines.append(f"# Max Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Max Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Max Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Max Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Max Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Max Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Max Agent v{new_version} ({calver_date})' "
