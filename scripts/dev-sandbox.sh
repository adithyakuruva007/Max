...
unprivileged `max` user, giving the layout most people have —
$MAX_HOME/max-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  MAX_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .max-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `max setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${MAX_DEV_SANDBOX_UPSTREAM:-https://github.com/NousResearch/max-agent.git}"
...
GIT_ROOT="${MAX_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${MAX_DEV_SANDBOX_DIR:-.max-sandbox}"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/max-agent.stardustresearch.com"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/max-agent.stardustresearch.com/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/max-agent.stardustresearch.com/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://max-agent.stardustresearch.com/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/max-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `max` launcher to ~/.local/bin and the
# checkout to $MAX_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/max-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q max-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  git -C "$SNAPSHOT_REPO" config user.name 'Max sandbox'
