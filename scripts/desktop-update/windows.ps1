...
# (%MAX_HOME%\max-setup.exe). That binary has no self-update path --
...
#
# This script lives in the repo checkout, so EVERY `max update` refreshes
...
#     -InstallRoot <path>   repo checkout (MAX_HOME\max-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Max.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .max-update-in-progress in place (tests)
#
...
# .max-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim MAX_HOME\.max-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining MAX_UPDATE_STARTED_AT from the Desktop hand-off.
# max_cli/update_lock.py's ancestry rule lets our
# `max update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Max.exe pid.
try {
    Add-Type -Namespace MaxHandoff -Name Win32 -MemberDefinition @'
...
$MaxHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $MaxHome ".max-update-in-progress"
$LogDir = Join-Path $MaxHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $MaxHome ".max-update-result.json"
$script:Ui = $null
$script:UiStage = "Max will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # update marker (.max-update-in-progress) serialises hand-offs, so no
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-MaxStep and are never
...
function Show-ProgressWindow {
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [MaxHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `max debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"max debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [MaxHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Max.exe before writing the replacement,
    ...
    # DO NOT spawn Max.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [MaxHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [MaxHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [MaxHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Max process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [MaxHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `max update` deliberately runs its build steps with stdout
# inherited (max_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:MAX_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:MAX_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:MAX_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:MAX_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `max update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (max_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:MAX_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:MAX_UPDATE_PROGRESS_LOG
}
...
public static class MaxUpdateJob {
...
function Invoke-MaxStep([string]$Exe, [string[]]$MaxArgs, [string]$Tag) {
    ...
    # afterwards, where `max debug share` picks it up.
    #
    ...
    # .max-update-result.json, clearing .max-update-in-progress,
    ...
    # Max" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($MaxArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [MaxUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `max update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [MaxUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; MAX_SELFTEST_FAIL=1 exercises the
# error state, MAX_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:MAX_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:MAX_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:MAX_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-MaxStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Max install to
...
#            update log, the shape of every real `max update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:MAX_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:MAX_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:MAX_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:MAX_SELFTEST_FLOOD_KB }
...
'@
    ...
    # merely loud. `max update` is this shape -- the Electron/vite build
...
'@
    ...
    $flood = Invoke-MaxStep $powershell @(
    ...
    $stall = Invoke-MaxStep $powershell @(
    ...
    try {
        $logstall = Invoke-MaxStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-MaxStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-MaxStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:MAX_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for Max to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Max window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Max fully and try again."
    ...
    Publish-UiProgress "Preparing Max files"
    ...
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Max install open (venv\Scripts\hermes.exe locked after 20s). Nothing was changed. Close other Max windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # from a user leaving a second Max window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Max.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m max_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Max installer or `max doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "max_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Max windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m max_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed max predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-MaxStep $pythonExe $updateArgs "update"
    Write-HandoffLog "max update exit code: $($res.Code)"

    if ($res.Code -ne 0 -and $res.Code -ne 2) {
        # One retry for the update-boundary class (fresh code on disk, stale
        # code in memory). Exit 2 ("close all Max windows") is not retryable.
    ...
    # `max update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "max update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-MaxStep $pythonExe @("-m", "max_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `max desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `max debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Max is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Max was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Max."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Max"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Max to finish (it could not restart itself)."
