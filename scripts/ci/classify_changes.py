...
# tree generates llms.txt — the index every LLM (Max included, via the
# max-agent skill) reads to learn what Max can do — and
...
_MCP_CATALOG_FILES = {"max_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Max-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``max serve`` backend, the Docker image)
