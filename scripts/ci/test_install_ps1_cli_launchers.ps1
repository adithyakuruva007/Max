# Behavioral test for install.ps1's max launcher staging (PR #92092,
...
#
# The test lifts the real Install-MaxCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $MaxHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-MaxCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-MaxCommandLaunchers not found in $installPs1"
}
...
try {
    $installRoot = Join-Path $caseRoot 'max-agent'
    ...
    $missingThrew = $false
    try {
        Install-MaxCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
    ...
    $staged = Install-MaxCommandLaunchers -Root $installRoot -Destination $binDir
    ...
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'max-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'hermes.exe'), $hermesV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'max-acp.exe'), $acp)
    Install-MaxCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $hermesV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'hermes.exe'))) `
        'installer refreshes an existing Max launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'max-acp.exe'))) `
