 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.max/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.max', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (MAX_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.MAX_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.max', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.MAX_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.max', 'document_cache');
const AUDIO_CACHE_DIR = process.env.MAX_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.max', 'audio_cache');
...
// `max update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Max Agent*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Max itself created (tracked when
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Max Agent', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Max and trigger
      ...
      // Ignore Max' own reply messages in self-chat mode to avoid loops.
