...
When Max runs in TUI mode, the gateway child process communicates with
...
# ``get_max_home() / "plugins"`` (user) and ``./.max/plugins/`` (project,
# gated behind ``MAX_ENABLE_PROJECT_PLUGINS``) — see
# ``max_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``max_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import max_constants (this script
    ...
    # plugins in get_max_home()/plugins/ and project plugins in
    # ./.max/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_max_home() / "plugins"]
    if os.environ.get("MAX_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".max" / "plugins")
