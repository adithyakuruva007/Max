# ============================================================================
# Max Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$MaxHome = $(if ($env:MAX_HOME) { $env:MAX_HOME } else { "$env:LOCALAPPDATA\hermes" }),
    [string]$InstallDir = $(if ($env:MAX_HOME) { "$env:MAX_HOME\max-agent" } else { "$env:LOCALAPPDATA\hermes\max-agent" }),
    ...
    # builds apps/desktop into a launchable Max.exe.
    #
    # Why opt-in:
    #   * Max-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Max.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Max.exe
    ...
    #     `max desktop` already builds on demand.
...
# MAX_HOME and InstallDir -- in that short form:
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'MaxInstall.LongPath').Type) {
            Add-Type -Namespace 'MaxInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [MaxInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -MaxHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('MaxHome')) {
    $MaxHome = ConvertTo-LongPath $MaxHome
} else {
    $MaxHome = ConvertTo-LongPath $(
        if ($env:MAX_HOME) { $env:MAX_HOME } else { "$env:LOCALAPPDATA\hermes" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:MAX_HOME) { "$env:MAX_HOME\max-agent" } else { "$env:LOCALAPPDATA\hermes\max-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: MaxHome=$MaxHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:NousResearch/max-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/max-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Max Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Stardust Research.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $MaxHome)) {
        New-Item -ItemType Directory -Force -Path $MaxHome | Out-Null
    }
    $envFile = Join-Path $MaxHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `max update`'s npx
    ...
    $prefixDir = Join-Path $MaxHome "node"
...
function Install-Uv {
    # Max owns its own uv at $MaxHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (max_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `max update` stay in sync.
    $managedUv = Join-Path $MaxHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $MaxHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $MaxHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $MaxHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $MaxHome "bin"
        ...
        # (max_cli/managed_uv.py looks only at $MaxHome\bin\uv.exe).
...
# Put the Max-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone max-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Max-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Max-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }
...
function Resolve-UvCmd {
    ...
    # sibling process and MAX_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Max-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Max setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Max uses to run shell commands.
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Max needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Max copy of Git ourselves is predictable and
    ...
    ``MAX_GIT_BASH_PATH`` (User scope) so Max can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Max-managed PortableGit install instead..."
    }

    # Download PortableGit into $MaxHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $MaxHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Max features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Max needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``MAX_GIT_BASH_PATH`` (User env scope) so Max can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $MaxHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $MaxHome\git\usr\bin\bash.exe
    $candidates += "$MaxHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$MaxHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("MAX_GIT_BASH_PATH", $candidate, "User")
            $env:MAX_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set MAX_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Max may not find Git Bash."
    Write-Info "If needed, set MAX_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Max-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Max requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Max requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Max-managed Node.js installation instead..."

    # Prefer a Max-managed Node from a previous run over a too-old system one.
    $managedNode = "$MaxHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$MaxHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$MaxHome\node"
        Write-Success "Node.js $version found (Max-managed)"
        ...
        Update-ManagedNpm "$MaxHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Max-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $MaxHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $MaxHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$MaxHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$MaxHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$MaxHome\node") {
                    try {
                        Rename-Item "$MaxHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Max-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$MaxHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $MaxHome\node\ (portable, user-scoped)"
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. max-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``max update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Max behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running max processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/max-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/max-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/max-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\max-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\max-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `max update` (see the notes at the shared clone-path
    ...
    # next `max update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Max-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running max process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running max processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Max_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # `pythonw.exe -m max_cli.main gateway run` straight out of
            ...
            # Max children re-exec through .max-runtime, so killing only
            ...
            # user (see max_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Max " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `max update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Max (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install max-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # here, not 30 seconds later when the user runs `max`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Max processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Max processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Max and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        ...
        # `max` command while max-agent.exe / max-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m max_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `max dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\max_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `max dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `max dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: max_cli/web_server.py"
        }
    }
...
function Install-MaxCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the max launchers on PATH -- never the whole
    ...
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the max command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # max_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("hermes", "max-acp")) {
    ...
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
        ...
        throw "Cannot set up the max command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up max command..."
    
    if ($NoVenv) {
        $hermesBin = "$InstallDir"
    } else {
        # $MaxHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `max update`'s autostash
        ...
        # installer staged under max-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-MaxCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $hermesBin = "$MaxHome\bin"
        Install-MaxCommandLaunchers -Root $InstallDir -Destination $hermesBin | Out-Null
    }
    ...
    #   max-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The max-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept max via $hermesBin)"
        }
    }
    ...
    # Set MAX_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\hermes instead
    # of the Unix default ~/.max
    $currentMaxHome = [Environment]::GetEnvironmentVariable("MAX_HOME", "User")
    if (-not $currentMaxHome -or $currentMaxHome -ne $MaxHome) {
        [Environment]::SetEnvironmentVariable("MAX_HOME", $MaxHome, "User")
        Write-Success "Set MAX_HOME=$MaxHome"
    }
    $env:MAX_HOME = $MaxHome
    
    # Update current session
    $env:Path = "$hermesBin;$env:Path"
    
    Write-Success "max command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.max-bootstrap-complete which tells the Max
    ...
    $markerPath = Join-Path $InstallDir ".max-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the MAX_HOME directory structure ($MaxHome, default %LOCALAPPDATA%\hermes)
    New-Item -ItemType Directory -Force -Path "$MaxHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MaxHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MaxHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MaxHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MaxHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MaxHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MaxHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MaxHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MaxHome\skills" | Out-Null
    ...
    # Create config.yaml
    $configPath = "$MaxHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Max's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$MaxHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in max_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $MaxHome"
    
    # Seed bundled skills into $MaxHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $MaxHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $MaxHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$MaxHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $MaxHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Max-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'max setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
...
# failure is non-fatal (browser_exec can still run via uvx, and `max tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $MaxHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Max' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Max' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'max tools')"
        }
    } catch {
...
# paths still lazy-install via install_cua_driver() (max_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `max computer-use install` runs. Bounded
        ...
        # Max' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'max tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: max computer-use install"
            }
        } else {
...
# the final `electron` -> `Max` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Max.exe. Only called from
    ...
    # resolves apps/desktop/release/win-unpacked/Max.exe directly,
    ...
    # enforces the supported Node lines and prepends the Max-managed Node to
    ...
    # The --dir mode produces an unpacked Max.exe in
    ...
    # are NOT trying to work around). The Max icon + product name are
    # stamped onto Max.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Max.exe",
        "$desktopDir\release\win-arm64-unpacked\Max.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Max.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Max icon + identity are stamped onto Max.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, max-agent#38216).
    ...
    #    Max.exe. We deliberately do NOT point them at `max desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Max.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Max.lnk')
        )
        ...
        # with the Max icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.max/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'max setup'."
        return
    }
    ...
    # Run max setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m max_cli.main setup
    } else {
        python -m max_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $whatsappSession = "$MaxHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'max whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: max gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$MaxHome\logs\gateway.log"
            Start-Process -FilePath $hermesCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$MaxHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: max gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: max gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$MaxHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$MaxHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MaxHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MaxHome\max-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   max              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   max setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   max config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   max config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   max gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   max update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Max repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Max-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Max to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://max-agent.stardustresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
