...
# Python-driven tool session (e.g. Max terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/max-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/max-agent.git"
MAX_HOME="${MAX_HOME:-$HOME/.max}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${MAX_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$MAX_INSTALL_DIR"
...
#   code at /usr/local/lib/max-agent, command at /usr/local/bin/hermes,
#   data still at /root/.max (MAX_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$MAX_HOME/.no-bundled-skills so future"
            echo "                   'max update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Max.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.max/max-agent"
            echo "                   default (root, Linux): /usr/local/lib/max-agent"
            echo "  --hermes-home PATH  Data directory (default: ~/.max, or \$MAX_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Max installs the code under"
            echo "  /usr/local/lib/max-agent and links the command into"
            echo "  /usr/local/bin/hermes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$MAX_HOME"
            echo "  (default /root/.max).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$MAX_HOME/max-agent are preserved in-place."
...
print_banner() {
    ...
    echo "│             ⚕ Max Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Stardust Research.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `max update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Max-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Max Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install max command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `max` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $MAX_HOME/max-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $MAX_HOME/max-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/max-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $MAX_HOME/max-agent — then preserve it)
#
# Always no-op when the user set --dir or $MAX_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in MAX_HOME.
    if is_termux; then
        INSTALL_DIR="$MAX_HOME/max-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$MAX_HOME/max-agent/.git" ]; then
            INSTALL_DIR="$MAX_HOME/max-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/max-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/max-agent"
        ...
        log_info "  Data:    $MAX_HOME (unchanged)"
...
# Point a Max-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $MAX_HOME/node/bin — which is NOT on
...
# Max's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Max-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$MAX_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$MAX_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$MAX_HOME/node/etc/npmrc"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://max-agent.stardustresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Max owns its own uv at $MAX_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (max_cli/managed_uv.py) looks in the same
    # place, so install.sh and `max update` stay in sync.
    local _managed_uv="$MAX_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $MAX_HOME/bin ..."
    mkdir -p "$MAX_HOME/bin"
    ...
    # directly into $MAX_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$MAX_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# replaced with the Max-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Max-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Max-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Max-managed Node from a previous run over a too-old system one.
    if [ -x "$MAX_HOME/node/bin/node" ] && [ -x "$MAX_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$MAX_HOME/node/bin/node" --version)"; then
        export PATH="$MAX_HOME/node/bin:$PATH"
        log_success "Node.js $("$MAX_HOME/node/bin/node" --version) found (Max-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Max-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Max requires Node 22.22+, 24.11+, or 26+) — installing Max-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.max/node/..."
    ...
    # Place into ~/.max/node/ and symlink binaries into the same bin dir
    # the max command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$MAX_HOME/node"
    mkdir -p "$MAX_HOME"
    mv "$extracted_dir" "$MAX_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$MAX_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$MAX_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$MAX_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$MAX_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.max/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Max install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Max Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `max update` path
                # (#4735).
            ...
            # cannot succeed — mirror ``max update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Max behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up max command..."

    if [ "$USE_VENV" = true ]; then
        MAX_BIN="$INSTALL_DIR/venv/bin/python"
        MAX_ENTRYPOINT="$INSTALL_DIR/hermes"
    else
        MAX_BIN="$(which max 2>/dev/null || echo "")"
        if [ -z "$MAX_BIN" ]; then
            log_warn "max not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$MAX_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$MAX_ENTRYPOINT" ]; }; then
        log_warn "Max launcher prerequisites not found"
    ...
    # Create a user-facing shim for the max command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $MAX_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$MAX_BIN"` self-recurse. (#21454)
...
EOF
    ...
    log_success "Installed max launcher → $command_link_display_dir/hermes"

    # Also expose `max-agent`. The `max-agent` console script declared in
    ...
    rm -f "$command_link_dir/max-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/max-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MAX_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/max-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MAX_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/max-agent"
    log_success "Installed max-agent launcher → $command_link_display_dir/max-agent"

    # Also expose `max-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `max-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Max as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/max-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/max-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MAX_BIN" "$MAX_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/max-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/max-acp"
    log_success "Installed max-acp launcher → $command_link_display_dir/max-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "max command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        log_info "max not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Max Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Max Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Max Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so max works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.max directory structure (config at top level, code in subdir)
    mkdir -p "$MAX_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.max/.env (top level, easy to find)
    if [ ! -f "$MAX_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$MAX_HOME/.env"
            log_success "Created ~/.max/.env from template"
        else
            touch "$MAX_HOME/.env"
            log_success "Created ~/.max/.env"
        fi
    else
        log_info "~/.max/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$MAX_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.max/config.yaml (top level, easy to find)
    if [ ! -f "$MAX_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$MAX_HOME/config.yaml"
            log_success "Created ~/.max/config.yaml from template"
        fi
    else
        log_info "~/.max/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in max_cli/default_soul.py — the
    ...
    if [ ! -f "$MAX_HOME/SOUL.md" ]; then
        cat > "$MAX_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.max/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.max/"

    # Seed bundled skills into ~/.max/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `max update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'max update'." \
            > "$MAX_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $MAX_HOME/.no-bundled-skills"
        log_info "  Future 'max update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.max/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.max/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$MAX_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$MAX_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.max/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Max Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Max will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Max Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `max update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `max tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$MAX_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Max' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$MAX_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'max tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `max tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `max computer-use install` on their own).
    ...
    # Same upstream installer `max computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Max install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Max'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'max tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: max computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'max setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m max_cli.main setup < /dev/tty
    else
        python -m max_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$MAX_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Max to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$MAX_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'max whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                MAX_CMD="$(get_hermes_command_path)"
                $MAX_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'max whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'max gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $MAX_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $MAX_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: max gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: max gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $MAX_CMD gateway > "$MAX_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.max/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: max gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: max gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.max-bootstrap-complete, which tells the Max
    ...
    local marker_path="$INSTALL_DIR/.max-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $MAX_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $MAX_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $MAX_HOME/cron/, sessions/, logs/"
    ...
    echo -e "   ${GREEN}max setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}max config${NC}       View/edit configuration"
    echo -e "   ${GREEN}max config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}max gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}max update${NC}       Update to latest version"
    echo ""
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$MAX_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$MAX_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$MAX_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$MAX_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Max` rename
...
# _purge_electron_build_cache() used by `max desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Max-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `max desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Max" ]; then
            app="$desktop_dir/release/linux-unpacked/Max"
        ...
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Max.app" \
            "$desktop_dir/release/mac/Max.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `max desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading MAX_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if MAX_HOME="$MAX_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from max_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `max update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Max-managed Node
            # provisioned during prerequisites/node-deps (at $MAX_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $MAX_HOME.
    # $MAX_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'max update' on this host install.
