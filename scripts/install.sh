...
# Configuration
REPO_URL_SSH="git@github.com:StardustResearch/max-agent.git"
REPO_URL_HTTPS="https://github.com/StardustResearch/max-agent.git"
MAX_HOME="${MAX_HOME:-$HOME/.hermes}"
...
#   data still at /root/.hermes (MAX_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --max-home PATH  Data directory (default: ~/.max, or \$MAX_HOME)"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/max-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/max-uv-installer.$$.sh")"
...
# string clears that floor; anything below it is replaced with the Max-
...
check_node() {
    ...
    if [ -x "$MAX_HOME/node/bin/node" ] && node_satisfies_build "$("$MAX_HOME/node/bin/node" --version)"; then
    ...
    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Max-managed Node $NODE_VERSION LTS..."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                local stash_name
                stash_name="max-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        MAX_BIN="$INSTALL_DIR/venv/bin/hermes"
    else
        MAX_BIN="$(which hermes 2>/dev/null || echo "")"
        if [ -z "$MAX_BIN" ]; then
    ...
    if [ ! -x "$MAX_BIN" ]; then
        log_warn "hermes entry point not found at $MAX_BIN"
...
ensure_browser() {
    ...
    local ab_bin="$MAX_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Max for pip install"
    ...
    MAX_CMD="$(command -v hermes 2>/dev/null || echo "")"
    if [ -n "$MAX_CMD" ]; then
        log_info "Running hermes setup..."
        "$MAX_CMD" setup
    else
        log_warn "hermes command not found on PATH"
        log_info "Try: python -m max_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Max-managed Node to PATH, so
    ...
    # trips Gatekeeper's tamper check ("Max is damaged and can't be opened").
