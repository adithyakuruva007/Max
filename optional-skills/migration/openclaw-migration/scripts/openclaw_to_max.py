...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `max config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`max config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.max/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
...
# like ``openclaw`` → ``max`` (so filesystem paths like ``~/.openclaw``
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Max`` but maps ``openclaw`` → ``max`` so a
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Max memory store (memories/MEMORY.md, USER.md).
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("MAX_HOME") or str(Path.home() / ".max"), help="Max home directory")
    parser.add_argument(
        "--workspace-target",
