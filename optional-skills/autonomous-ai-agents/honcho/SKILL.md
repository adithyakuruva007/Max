---
name: honcho
description: Configure and use Honcho memory with Max -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Max Agent
license: MIT
...
Run `hermes honcho setup`. Ensure `memory.provider: honcho` is in `~/.max/config.yaml`.
...
| `hermes honcho sync` | Create host blocks for all Max profiles that don't have one yet |
| `hermes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Max + Honcho |
