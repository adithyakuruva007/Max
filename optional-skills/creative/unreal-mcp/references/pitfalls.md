...
### 1. Start order: editor first, Max second

Max probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Max session.
...
restart the editor, restart the Max session.
...
server fails to bind (Output Log) or Max' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.max/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Max

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Max' connection
lives in `~/.max/config.yaml` via `max mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Max — just don't
mistake it for the Max setup step.
...
explicitly warns against overlapping calls. Max executes same-turn tool
...
### 9. Timeouts: Max gives up before Unreal does

Max' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Max runs elsewhere (SSH backend,
