#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `max kanban list` and
...
def max_available() -> bool:
    return shutil.which("max") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed max CLI."""
    try:
        out = subprocess.run(
            ["max", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `max kanban list`
    out = subprocess.run(
        ["max", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not max_available():
        print("ERROR: 'max' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
