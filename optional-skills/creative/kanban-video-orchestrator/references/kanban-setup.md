...
> [StardustResearch/kanban-video-pipeline](https://github.com/StardustResearch/kanban-video-pipeline).
...
the Max `.env` (`${MAX_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _max_env="${MAX_HOME:-$HOME/.hermes}/.env"
    if grep -q "^${var}=" "$_max_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_max_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_max_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}
