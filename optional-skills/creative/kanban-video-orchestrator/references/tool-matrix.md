...
Maps each role archetype to the Max skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public max-agent
...
## Max skills relevant to video production

### Visual / rendering skills (`max-agent/skills/creative/`)
...
### Design / pre-production skills (`max-agent/skills/creative/`)
...
### Audio / media skills (`max-agent/skills/creative/` + `skills/media/`)
...
These are **not** Max skills but external CLIs / APIs that profiles invoke.
...
## Built-in Max tools for media review

These are native Max tools — not invoked via terminal but through their own
...
key is present in `${MAX_HOME:-~/.max}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Max profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${MAX_HOME:-~/.max}/.env` → environment variable.
...
When a new Max-public video skill ships:
