#!/usr/bin/env python3
"""Canvas LMS API CLI for Max Agent.
...
def _check_config():
    ...
    if missing:
        max_env = os.path.join(
            os.environ.get("MAX_HOME", os.path.expanduser("~/.max")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {max_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Max Agent"
    )
