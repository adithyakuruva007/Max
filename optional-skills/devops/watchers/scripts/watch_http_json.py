...
Usage (via cron with --no-agent):
    ...
    max cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$MAX_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Max-Watcher/1.0"})
