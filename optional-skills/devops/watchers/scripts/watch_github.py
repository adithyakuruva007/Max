...
Usage (via cron with --no-agent):
    ...
    max cron create hermes-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$MAX_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hermes-issues --repo NousResearch/max-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Max .env file
(``${MAX_HOME:-~/.max}/.env``) to avoid the 60 req/hr
