---
name: max-s6-container-supervision
description: Modify, debug, or extend the s6-overlay supervision tree inside the Max Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.
version: 1.0.0
author: Max Agent
license: MIT
platforms: [linux]
...
metadata:
  hermes:
    ...
    related_skills: [max-agent, max-agent-dev]
---
...
│   └── dashboard/run                  ← if MAX_DASHBOARD=1, runs `hermes dashboard`
│
...
| `max_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `hermes gateway start/stop/restart` and routes to s6 when running in a container. |
...
- `max-tool-quirks`: Specific Max-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with hermes built-in tools.
