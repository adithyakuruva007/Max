...
Usage:
  ...
  <server_name> matches the files in $MAX_HOME/mcp-tokens/<server>.json etc.
...
def _hermes_home():
    # Prefer Max' own resolver (profile-safe); fall back to env then ~/.max.
    try:
        from max_constants import get_max_home
        return str(get_max_home())
    except Exception:
        return os.environ.get("MAX_HOME") or os.path.expanduser("~/.max")
