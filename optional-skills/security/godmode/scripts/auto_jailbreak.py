...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("MAX_HOME", os.path.expanduser("~/.max")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("MAX_HOME", Path.home() / ".hermes")) / "skills" / "red-teaming" / "godmode"
...
MAX_HOME = Path(os.getenv("MAX_HOME", Path.home() / ".hermes"))
CONFIG_PATH = MAX_HOME / "config.yaml"
PREFILL_PATH = MAX_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Max models — already uncensored, just needs clean prompt
    "max": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "max" in model_lower or "nous" in model_lower:
        return "max"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Max config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.max/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Max for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Max for changes to take effect.")
