...
## 5. GODMODE FAST — Max 4 405B (Zero Refusal)

**Model:** `stardustresearch/hermes-4-405b`
**Strategy:** Max models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Max Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.max/config.yaml`:
