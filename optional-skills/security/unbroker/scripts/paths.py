...
All per-subject data lives under PDD_DATA_DIR (default: $MAX_HOME/unbroker),
which is the same trust boundary Max uses for .env and OAuth tokens.
"""
...
def max_home() -> Path:
    return Path(os.environ.get("MAX_HOME") or (Path.home() / ".hermes"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else max_home() / "unbroker"
