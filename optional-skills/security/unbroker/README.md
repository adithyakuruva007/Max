...
The design is **Max-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
max skills install official/security/unbroker
```

Then start a new Max session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Max home directory (`0600`, optionally
