"""
Max MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Max-specific extra)

Usage:
    max mcp serve
    max mcp serve --verbose
...
logger = logging.getLogger("max.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using MAX_HOME."""
    try:
        from max_constants import get_max_home
        return get_max_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("MAX_HOME", Path.home() / ".max")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from max_constants import get_max_home
        directory_file = get_max_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
os.environ.get("MAX_HOME", Path.home() / ".max")
...
class EventBridge:
    ...
    This is the Max equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from max_constants import get_max_home
            db_file = get_max_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("MAX_HOME", Path.home() / ".max")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Max MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "max",
        instructions=(
            "Max Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Max MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
