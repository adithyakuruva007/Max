...
def _real_finalize(cli_obj):
    """Call the real MaxCLI.finalize_preloaded_skills on a dummy object."""
    return _REAL_FINALIZE(cli_obj)


def _capture_real_finalize():
    import cli as cli_mod
    return cli_mod.MaxCLI.__dict__["finalize_preloaded_skills"]
...
def test_main_applies_preloaded_skills_to_system_prompt(monkeypatch):
    ...
    monkeypatch.setattr(cli_mod, "MaxCLI", fake_cli)
    monkeypatch.setattr(
        cli_mod,
        "build_preloaded_skills_prompt",
        lambda skills, task_id=None: ("skill prompt", ["max-agent-dev", "github-auth"], []),
    )

    with pytest.raises(SystemExit):
        cli_mod.main(skills="max-agent-dev,github-auth", list_tools=True)
    ...
    assert cli_obj.preloaded_skills == ["max-agent-dev", "github-auth"]
...
def test_show_banner_does_not_print_skills():
    ...
    cli_obj.preloaded_skills = ["max-agent-dev", "github-auth"]
