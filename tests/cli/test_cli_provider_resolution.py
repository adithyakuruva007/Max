...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setattr("max_cli.config.load_config", lambda: config)
    monkeypatch.setattr("max_cli.runtime_provider.load_config", lambda: config)

    shell = cli.MaxCLI(provider="gmk-lan", compact=True, max_turns=1)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "max_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("max_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="MAX_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "MAX_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from max_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
