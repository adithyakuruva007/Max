...
def _isolate(tmp_path, monkeypatch):
    """Isolate MAX_HOME so tests don't touch real config."""
    home = tmp_path / ".max"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal MaxCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = MaxCLI.__new__(MaxCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["max-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Max' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Max' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Max' minimum context length."""
