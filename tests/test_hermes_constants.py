...
from max_constants import (
    ...
    find_max_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_max_root,
    get_max_dir,
    get_max_home,
    heal_max_managed_node,
    max_managed_node_tree_present,
    iter_max_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultMaxRoot:
    """Tests for get_default_max_root() — Docker/custom deployment awareness."""

    def test_no_max_home_returns_native(self, tmp_path, monkeypatch):
        """When MAX_HOME is not set, returns ~/.max."""
        monkeypatch.delenv("MAX_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_max_root() == tmp_path / ".max"

    def test_max_home_is_native(self, tmp_path, monkeypatch):
        """When MAX_HOME = ~/.max, returns ~/.max."""
        native = tmp_path / ".max"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MAX_HOME", str(native))
        assert get_default_max_root() == native

    def test_max_home_is_profile(self, tmp_path, monkeypatch):
        """When MAX_HOME is a profile under ~/.max, returns ~/.max."""
        native = tmp_path / ".max"
    ...
    def test_max_home_is_docker(self, tmp_path, monkeypatch):
        """When MAX_HOME points outside ~/.max (Docker), returns MAX_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MAX_HOME", str(docker_home))
        assert get_default_max_root() == docker_home

    def test_max_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any MAX_HOME outside ~/.max is treated as the root."""
        custom = tmp_path / "my-max-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MAX_HOME", str(custom))
        assert get_default_max_root() == custom
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MAX_HOME", str(profile))
        assert get_default_max_root() == docker_root

    def test_no_max_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\max, not ~/.max."""
        ...
        assert get_default_max_root() == local_appdata / "max"

    def test_no_max_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        ...
        assert get_default_max_root() == home / "AppData" / "Local" / "max"
...
class TestGetMaxHome:
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When MAX_HOME is unset on Windows, use %LOCALAPPDATA%\\max."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("MAX_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        ...
        assert get_max_home() == local_appdata / "max"
...
class TestMaxManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_max_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        assert find_max_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(max_constants, "heal_max_managed_node", lambda: False)
        monkeypatch.setattr(
            max_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert max_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_max_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "max"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_max_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(max_constants, "heal_max_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_max_node_executable("npm") == str(healed_npm)
...
class TestGetMaxDir:
    """Tests for ``get_max_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_max_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_max_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_max_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_max_dir("cache/audio", "audio_cache")
