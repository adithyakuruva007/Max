...
def gateway_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("MAX_PREFILL_MESSAGES_FILE", raising=False)
    monkeypatch.delenv("MAX_EPHEMERAL_SYSTEM_PROMPT", raising=False)
    monkeypatch.delenv("MAX_GATEWAY_BUSY_INPUT_MODE", raising=False)
    monkeypatch.delenv("MAX_RESTART_DRAIN_TIMEOUT", raising=False)
    monkeypatch.delenv("MAX_BACKGROUND_NOTIFICATIONS", raising=False)
    return tmp_path
