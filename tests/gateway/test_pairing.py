...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_max_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedDiscovery:
    def test_list_approved_scopes_platform_discovery_to_profile_dir(self, tmp_path):
        ...
        # A profile's store anchors to the max ROOT, not the current
        # MAX_HOME — the current home may itself be a profile, and nesting
        # profiles inside profiles is how a `-p work` CLI and its gateway end
        # up reading different files. Patch that seam, not get_max_home.
        with patch("gateway.pairing.PAIRING_DIR", global_dir), patch(
            ...
            # consolidated layout a standalone `max -p alice` resolves —
...
class TestProfileScopedStorage:
    ...
    under each profile's own Max home so a multiplexing gateway can keep
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``max pairing`` CLI continues to work without a profile context."""
    ...
    def test_default_store_is_not_frozen_at_first_use(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("max_constants.get_max_home", lambda: first_home)
        first_store = PairingStore()
        assert first_store._dir == first_home / "platforms" / "pairing"

        monkeypatch.setattr("max_constants.get_max_home", lambda: second_home)
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        """Explicit profile stores use that profile's normal Max layout."""
        monkeypatch.setenv("MAX_HOME", str(tmp_path))
    ...
    def test_profile_store_matches_profile_cli_home(self, tmp_path, monkeypatch):
        """Gateway and ``max -p`` must resolve the same pairing store."""
