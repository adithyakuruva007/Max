...
def test_load_transcript_returns_db_messages_when_no_jsonl(tmp_path, monkeypatch):
    ...
    ~/.max/state.db. (DEFAULT_DB_PATH is a module-level constant computed
    at max_state import time, before pytest's MAX_HOME monkeypatch
    fires — the autouse fixture's MAX_HOME override doesn't help here.)
    """
    import max_state
