...
redirected ``get_max_home()`` for that turn.  The rows carried the correct
...
def multiplex_homes(tmp_path, monkeypatch):
    """A root home plus a named profile home, with MAX_HOME on the root.
    ...
    # The suite-wide fixture in conftest re-points ``max_state.DEFAULT_DB_PATH``
    ...
    # the hatch is closed and resolution goes through ``get_max_home()``,
    # which is what production does.  ``MAX_HOME`` above still keeps that
...
def test_primary_handler_rejected_route_falls_back_and_marks_sentinel(multiplex_homes):
    ...
    async def capture_scope(event):
        seen.append((Path(get_max_home()), event.source.profile_route_rejected))
