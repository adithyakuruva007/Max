...
def test_get_flush_dir_uses_get_max_home(tmp_path, monkeypatch):
    """Flush dir must use get_max_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_max_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "max_constants.get_max_home", fake_get_max_home
    )
