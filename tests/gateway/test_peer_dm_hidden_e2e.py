"""E2E: ``max peer dm`` against a REAL api_server gateway whose canonical
...
SQLite ``state.db`` (its own tmp MAX_HOME) containing a hidden
``Bot Chat`` row — exactly what Bot Mode leaves behind. The "local" side is
the stock ``max peer dm`` client code (``max_cli.subcommands.peer``),
...
def peer_gateway(tmp_path, monkeypatch):
    """A real api_server gateway (own MAX_HOME + state.db) on a real socket."""
