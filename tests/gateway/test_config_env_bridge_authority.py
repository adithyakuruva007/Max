...
def _run_gateway_import(hermes_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "MAX_TURN_LEASE_TIMEOUT",
            "MAX_SESSION_STALL_TIMEOUT",
    ...
    # otherwise intentionally clean so stale Max settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
def test_config_gateway_timeout_wins_over_stale_env(hermes_home: Path) -> None:
    ...
    assert env.get("MAX_SESSION_STALL_TIMEOUT") == "300"
...
def test_config_turn_lease_timeout_wins_over_stale_env(hermes_home: Path) -> None:
    ...
    _write_env(
        hermes_home,
        {"MAX_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(hermes_home, initial_env={})

    assert env.get("MAX_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("MAX_TURN_LEASE_TIMEOUT") == "1800"
