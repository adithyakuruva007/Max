...
class TestPayloadFilters:
    ...
    async def test_filter_accepts_nested_any_and_in_file(self, tmp_path, monkeypatch):
        """Nested any groups can match dynamic watchlists under MAX_HOME."""
                        ...
                        "any": [
                            {
                                ...
                                "in_file": "~/.max/data/watchlist.json",
                            },
                            {
    ...
    @pytest.mark.asyncio
    async def test_script_tilde_max_path_resolves_to_active_profile_home(self, tmp_path, monkeypatch):
        """~/.max/scripts paths must resolve through MAX_HOME for profiles."""
        monkeypatch.setenv("MAX_HOME", str(tmp_path))
        scripts = tmp_path / "scripts"
        scripts.mkdir()
        ...
        routes = {
            "todoist": {
                "secret": _INSECURE_NO_AUTH,
                "script": "~/.max/scripts/todoist_filter.py",
                "prompt": "Task: {body}",
            }
        }
        adapter = _make_adapter(routes=routes)
