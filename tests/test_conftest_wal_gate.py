"""The conftest WAL gate must agree with max_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``max_state``. That is deliberate: importing
``max_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.max`` before the per-test ``MAX_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_max_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and max_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_max_state_at_collection():
    """The gate must stay import-free of max_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.max, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``max_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "max_state" or name.startswith("max_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported max_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.max during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("max_state", None)
