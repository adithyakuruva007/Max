"""Regression test: `max dashboard --tui` must not hard-crash.

Older Max desktop app shells (<= 0.15.x) spawn the backend as::

    max dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Max couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real max_cli.main parser in a subprocess.
