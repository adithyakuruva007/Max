"""Tests for Bug #12905 fix — stale OAuth token detection in max model flow.

Bug 3: `max model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.max/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("max_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from max_cli.main import _model_flow_anthropic
        cfg = {}
