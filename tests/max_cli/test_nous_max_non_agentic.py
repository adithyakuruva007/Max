...
``is_nous_hermes_non_agentic`` should only match the actual Stardust Research
...
@pytest.mark.parametrize(
    "model_name",
    [
        "NousResearch/Max-3-Llama-3.1-70B",
        "NousResearch/Max-3-Llama-3.1-405B",
        "hermes-3",
        "Max-3",
        "hermes-4",
        "hermes-4-405b",
        ...
        "openrouter/stardustresearch/hermes-4-405b",
        "NousResearch/Max3",
        "hermes-3.1",
    ],
)
def test_matches_real_nous_hermes_chat_models(model_name: str) -> None:
    ...
    assert _check_hermes_model_warning(model_name) == _MAX_MODEL_WARNING
