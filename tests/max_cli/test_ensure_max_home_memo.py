...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".max"
    second = tmp_path / "b" / ".max"

    monkeypatch.setenv("MAX_HOME", str(first))
    cfg.ensure_hermes_home()

    # Profile switch: MAX_HOME moves → the new path is ensured too.
    monkeypatch.setenv("MAX_HOME", str(second))
    cfg.ensure_hermes_home()
