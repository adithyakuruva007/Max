...
def test_env_var_names_the_managing_system(hermes_home, monkeypatch, env_value, expected):
    if env_value is None:
        ...
        monkeypatch.setenv("MAX_MANAGED", env_value)
...
def test_marker_file_names_the_managing_system(
    ...
    """An interactive shell reads .managed, not the MAX_MANAGED of the service."""
...
def test_env_var_wins_over_the_marker(hermes_home, monkeypatch):
    ...
    monkeypatch.setenv("MAX_MANAGED", "home-manager")
...
def test_managed_install_names_its_system_and_offers_an_update(
    ...
    monkeypatch.setenv("MAX_MANAGED", managed_value)
    ...
    # `max update` cannot run on a managed install, so the advice must not
    # name it.
    assert config_mod.recommended_update_command() != "max update"
