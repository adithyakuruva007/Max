...
from max_state import SessionDB, _DB_PERSISTED_MARKER_KEY
...
def test_marker_constant_in_sync() -> None:
    ...
    ``max_state`` and ``agent.turn_finalizer`` import the constant from
    ``agent.context_compressor`` (single source), so the only genuine drift
    surface left is ``run_agent``'s predating copy — max_state cannot
