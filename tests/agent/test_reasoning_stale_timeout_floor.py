...
thinking phase.  Max's default cloud-stream stale detector
(``MAX_STREAM_STALE_TIMEOUT`` = 180s) and non-stream detector
(``MAX_API_CALL_STALE_TIMEOUT`` = 90s) both fire before the
...
def test_reasoning_floor_applies_to_nemotron_3_ultra(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("MAX_API_CALL_STALE_TIMEOUT", raising=False)
...
def test_reasoning_floor_loses_to_env_var_when_no_floor_match(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("MAX_HOME", str(tmp_path))
    (tmp_path / ".env").write_text("", encoding="utf-8")
    monkeypatch.setenv("MAX_API_CALL_STALE_TIMEOUT", "300")
    _write_config(tmp_path, "")
