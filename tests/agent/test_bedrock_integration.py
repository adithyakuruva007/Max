...
Note: Tests that import ``max_cli.auth`` or ``max_cli.runtime_provider``
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("max_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("max_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("max_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("max_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
    ...
    def test_bedrock_openai_models_route_to_mantle_responses(self, monkeypatch):
        ...
        for model_id in BEDROCK_OPENAI_RESPONSES_MODEL_IDS:
            with patch("max_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
                 patch("max_cli.runtime_provider._get_model_config", return_value={
                     "provider": "bedrock",
                     "default": model_id,
                 }):
                result = resolve_runtime_provider(requested="bedrock")
...
class TestProvidersModule:
    """Verify bedrock is wired into max_cli/providers.py."""

    def test_bedrock_alias_in_providers(self):
        from max_cli.providers import ALIASES
...
class TestPackaging:
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "max-agent[bedrock]" not in extras["all"]
...
class TestAuxiliaryClientBedrockResolution:
    ...
    def test_bedrock_mantle_config_region_beats_env_region(self, monkeypatch):
        ...
        with patch("max_cli.config.load_config_readonly",
