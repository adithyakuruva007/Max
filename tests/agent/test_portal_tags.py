...
def test_max_client_tag_includes_current_version():
    """The client tag must reflect max_cli.__version__ verbatim."""
    from max_cli import __version__
    from agent.portal_tags import max_client_tag

    assert max_client_tag() == f"client=max-client-v{__version__}"


def test_max_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import max_client_tag

    tag = max_client_tag()
    assert tag.startswith("client=max-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import max_client_tag, nous_portal_tags
