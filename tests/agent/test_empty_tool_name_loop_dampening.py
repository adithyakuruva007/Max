...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated MAX_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="max_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".max"))
    prev_home = os.environ.get("MAX_HOME")
    os.environ["MAX_HOME"] = os.path.join(test_home, ".max")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("max_"):
            del sys.modules[mod]
