...
path via ``max chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp MAX_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point MAX_HOME at a temp dir and turn moa.save_traces on."""
    max_home = tmp_path / ".max"
    max_home.mkdir()
    monkeypatch.setenv("MAX_HOME", str(max_home))

    # save_moa_turn reads config via max_cli.config.load_config; stub it to
