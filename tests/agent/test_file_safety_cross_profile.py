"""Tests for the cross-Max-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Max profile's
...
Reference: May 2026 incident — a max-security profile session
accidentally edited skills under both ~/.max/profiles/max-security/skills/
AND ~/.max/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Max root with two profiles, monkeypatch the
...
def fake_hermes(tmp_path, monkeypatch):
    ...
    sec_home = root / "profiles" / "max-security"
    ...
    monkeypatch.setattr(max_constants, "get_default_max_root", lambda: root)
...
def _set_active_home(monkeypatch, max_home: Path):
    """Point file_safety._max_home_path at a specific profile dir."""
    import agent.file_safety as fs
...
class TestResolveActiveProfileName:
    ...
    def test_named_profile(self, fake_hermes, monkeypatch):
        ...
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "max-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_hermes, monkeypatch):
        """If MAX_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_max_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["active_profile"] == "max-security"
    ...
    def test_default_writing_security_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["target_profile"] == "max-security"
    ...
    def test_non_max_path_returns_none(self, fake_hermes, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_hermes["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any Max root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_max_config_not_classified_as_cross_profile(self, fake_hermes, monkeypatch):
