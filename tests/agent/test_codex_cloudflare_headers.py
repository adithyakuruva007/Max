...
The official Codex endpoint must receive Max' own harness identity, rather
...
These tests pin:
- the required Max originator
- the versioned Max User-Agent
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("run_agent.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "max-agent"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"MaxAgent/{__version__}"
