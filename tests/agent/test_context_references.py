...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    assert result.expanded
    # Default container base for the docker backend is /root/.max.
    assert "/root/.max/attachments/archive.zip" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert "/root/.max/attachments/" not in result.message
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/Max/composer-images/a.png",
