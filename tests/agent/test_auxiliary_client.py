...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nous", "max-4", "https://inference-api.stardustresearch.com/v1"),
...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("max_cli.auth._read_codex_tokens", return_value={
...
class TestExpiredCodexFallback:
    ...
    def test_max_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat-max-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_try_nous_uses_portal_recommendation_for_text(self):
        ...
        fresh_base = "https://inference-api.stardustresearch.com/v1"
        with (
            ...
            patch("max_cli.models.get_nous_recommended_aux_model", return_value="minimax/minimax-m2.7") as mock_rec,
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            from agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_uses_portal_recommendation_for_vision(self):
        ...
        with (
            patch("agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("max_cli.models.get_nous_recommended_aux_model", return_value="google/gemini-3-flash-preview") as mock_rec,
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_falls_back_when_recommendation_lookup_raises(self):
        ...
        with (
            patch("agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("max_cli.models.get_nous_recommended_aux_model", side_effect=RuntimeError("portal down")),
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_nous
            client, model = _try_nous()
    ...
    def test_call_llm_refreshes_nous_after_free_tier_block_when_account_paid(self):
        ...
        with (
            ...
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", "https://inference-api.stardustresearch.com/v1")),
            patch(
                "max_cli.nous_account.get_nous_portal_account_info",
    ...
    async def test_async_call_llm_retries_nous_after_401(self):
        ...
        fresh_async_client.base_url = "https://inference-api.stardustresearch.com/v1"
...
class TestIsPaymentError:
    ...
    def test_404_free_tier_model_block_is_payment(self):
        exc = Exception(
            ...
            "Upgrade at https://portal.stardustresearch.com or pick a free model."
        )
        exc.status_code = 404
        assert _is_payment_error(exc) is True
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/StardustResearch/max-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/StardustResearch/max-agent/issues/18478
    """
...
class TestCompressionFallbackContextFilter:
    ...
    def test_main_chain_skips_too_small_candidate_for_compression(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "max_cli.fallback_config.get_fallback_chain",
            lambda cfg: chain,
        )
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("max_cli.config.load_config", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("max_cli.config.load_config", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",
