...
def test_max_meta_tolerates_non_dict():
    assert learning_graph._max_meta({"metadata": "junk"}) == {}
    assert learning_graph._max_meta({"metadata": {"max": "junk"}}) == {}
    assert learning_graph._max_meta({"metadata": {"max": {"category": "x"}}}) == {"category": "x"}
