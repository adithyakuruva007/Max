"""Regression tests for the codex_app_server → Max UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_hermes_tools_mcp_server_emits_bare_tool_name(self):
        """The hermes-tools MCP server wraps Max' own tools for codex;
        ...
        mcp.max-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "max agent docs"
        assert calls[0].args[3] == {"query": "max agent docs"}
