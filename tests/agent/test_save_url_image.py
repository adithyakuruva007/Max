...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate MAX_HOME under tmp_path."""
    monkeypatch.setenv("MAX_HOME", str(tmp_path / ".max"))
    (tmp_path / ".max").mkdir()

    # Force the constants/image cache helpers to re-read MAX_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("max_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_max_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under MAX_HOME — gateway cleanup
