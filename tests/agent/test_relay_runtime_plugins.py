...
def test_foreign_active_plugin_configuration_is_left_unchanged(
    ...
    try:
        ...
        assert relay.events == []
        assert "already active outside Max native ownership" in caplog.text
...
def test_legacy_exporter_env_without_plugins_toml_warns_and_stays_disabled(
    ...
    monkeypatch.setenv("MAX_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S", "30")
    relay = _FakeRelay()
    ...
    try:
        ...
        assert relay.events == []
        assert "no MAX_NEMO_RELAY_PLUGINS_TOML was provided" in caplog.text
        assert "MAX_NEMO_RELAY_ATOF_ENABLED" in caplog.text
        assert "MAX_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S" in caplog.text
    finally:
        host.shutdown()
...
def test_initialization_failure_is_fail_open(explicit_static_config, caplog):
    ...
    try:
        ...
        assert "Max Relay plugin initialization failed" in caplog.text
    finally:
        host.shutdown()
...
""".strip(),
    ...
    try:
        ...
        assert relay.events == []
        assert "Max [[dynamic_plugins]] records are unsupported" in caplog.text
...
""".strip(),
    ...
    try:
        for profile in ("profile-a", "profile-b"):
            ...
            monkeypatch.setenv("MAX_HOME", str(tmp_path / profile))
