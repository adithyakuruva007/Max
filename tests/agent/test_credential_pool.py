...
def test_load_pool_skips_exchange_for_suppressed_copilot(tmp_path, monkeypatch):
    ...
    removed with ``max auth remove copilot gh_cli``.  The gate must run
...
def test_load_pool_skips_resolve_when_all_copilot_sources_suppressed(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MAX_HOME", str(tmp_path / "hermes"))
    from max_cli.copilot_auth import COPILOT_ENV_VARS
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    monkeypatch.setattr("max_cli.copilot_auth.resolve_copilot_token", _boom)
