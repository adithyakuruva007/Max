"""The ACP text bridge is what makes Max' own tools reachable on an ACP provider.
...
def test_allowlist_forwards_only_the_named_tools():
    ...
    make Max re-run finished work, so those clients forward an allowlist."""
...
def test_response_level_extras_survive_the_stream_conversion():
    """Max reads provider extras off the returned object; a plain list would
    ...
    assert chunks.max_projected_messages == [{"role": "tool", "content": "x"}]
...
def test_copilot_prompt_still_carries_the_contract_and_the_tools():
    ...
    assert "Max requested model hint: gpt-5" in prompt
    assert "hi" in prompt
