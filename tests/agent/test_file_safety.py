...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_max_env(self):
        """Max' own .env inside MAX_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: hermes internal .env is in ~/.max/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.max/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.max/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Max cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        max_home = tmp_path / ".max"
        cache = max_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._max_home_path", return_value=max_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Max cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        max_home = tmp_path / ".max"
        hub = max_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_max_home(self, tmp_path):
        """The env basename guard does not depend on MAX_HOME resolution."""
        max_home = tmp_path / ".max"
        max_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        max_home = tmp_path / ".max"
        cache = max_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
