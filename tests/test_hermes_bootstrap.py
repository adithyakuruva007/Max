"""Tests for max_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Max entry point
(hermes, max-agent, max-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Max entry point imports max_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported max_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("max_bootstrap", None)
    import max_bootstrap  # noqa: WPS433
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Max CLI
...
class TestEntryPointsImportBootstrap:
    """Every Max entry point must import max_bootstrap as its
    ...
    # Entry points that invoke Max as a process.  Each one must
    # import max_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "max_cli/main.py",   # hermes CLI (console_script)
        "run_agent.py",          # max-agent (console_script)
        "acp_adapter/entry.py",  # max-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import max_bootstrap' and that
        ...
        half-finished ``max update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``max_bootstrap`` as a top-level module) leaves hermes
        ...
        """
        # Resolve relative to the max-agent repo root.  Tests live
        # at tests/test_max_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hermes start even when max_bootstrap hasn't been
        ...
        assert first_import_name == "max_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'max_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import max_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Max's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("MAX_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["MAX_PYTHON_SRC_ROOT"] = env
            elif "MAX_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["MAX_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/max")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("MAX_PYTHON_SRC_ROOT", None)
            else:
                os.environ["MAX_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/max", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Max root.  The guard must relocate Max to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/max"])
        assert result[0] == "/opt/max"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Max root.
        assert result.index("/opt/max") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/max", "/opt/max", ""])
        assert result.count("/opt/max") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["MAX_PYTHON_SRC_ROOT"] = "/env/hermes"
