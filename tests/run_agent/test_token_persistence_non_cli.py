...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    max_state = ModuleType("max_state")
    max_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "max_state", max_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Max"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Max"
