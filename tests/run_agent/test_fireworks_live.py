"""Live Fireworks smoke test — exercises the Max runtime, not a raw SDK client.

Opt-in only:
    MAX_LIVE_TESTS=1 FIREWORKS_API_KEY=fw_... \\
        pytest tests/run_agent/test_fireworks_live.py -q

Unlike a bare OpenAI() client pointed at the endpoint, this drives Max'
...
LIVE = os.environ.get("MAX_LIVE_TESTS") == "1"
FIREWORKS_KEY = os.environ.get("FIREWORKS_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only: set MAX_LIVE_TESTS=1"),
...
def _resolve_runtime_client(provider="fireworks"):
    """Build the Fireworks client the way the Max runtime does."""
    from agent.auxiliary_client import resolve_provider_client

    client, model = resolve_provider_client(provider)
    assert client is not None, "Max failed to build a Fireworks client"
    return client, model
...
def test_fireworks_basic_chat_through_runtime():
    """A single-turn completion via the Max-resolved client returns text."""
    client, model = _resolve_runtime_client()
