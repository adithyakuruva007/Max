"""The `max-agent` skill is what a running Max knows about itself.

`website/` is never packaged, so an installed Max has no local copy of the
user guide; skills ARE synced into `$MAX_HOME/skills/`. The skill therefore
...
shipped feature is invisible and the agent answers "Max can't do that."
"""
...
SKILL_DIR = REPO / "skills" / "autonomous-ai-agents" / "max-agent"
