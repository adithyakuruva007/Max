...
#
# Installs a real, earlier Max the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `max`.
#
...
# Keep sandbox state out of the default .max-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".max-sandbox-e2e-$ROUTE"
export MAX_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/hermes/.max/max-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/max-agent.git"
...
LOG_DIR="${MAX_E2E_LOG_DIR:-$(mktemp -d -t hermes-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED max accept FLAG on `max update`?
#
...
# that silently rots. `max update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "max update --help 2>&1" | grep -qF -- "$flag"
}
...
require_hermes_works() {
  local when="$1" out
  out="$(in_sandbox "max --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "max --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "max runs $when"
}

# ── install the earlier Max ─────────────────────────────────────────────
...
case "$ROUTE" in
  update)
    ...
    # max which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="max update --yes"
    else
      update_cmd="max update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "max update failed ($update_cmd)"
    fi
    require_landed_on_target 'max update'
    require_hermes_works 'after max update'
    ;;
  installer)
