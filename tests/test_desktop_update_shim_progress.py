...
# Stands in for `max update`, and reports the stage that was on screen
...
n="$(cat "$MAX_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$MAX_TEST_CALLS"
for f in "$TMPDIR"/hermes-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$MAX_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$MAX_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "max-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    max = install_root / "venv" / "bin" / "hermes"
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "MAX_TEST_CAPTURE": str(capture),
        "MAX_TEST_CALLS": str(calls),
        "MAX_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".max-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`max update` is the longest wait in the hand-off and the one the
