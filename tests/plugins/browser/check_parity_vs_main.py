...
Run from the PR worktree:
    ...
    cd ~/.max/max-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.max/max-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.max/max-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a max-agent checkout"
)
...
# Isolated MAX_HOME for the config write.
