...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(tmp_path, monkeypatch):
    ...
    OpenVikingMemoryProvider().post_setup(str(max_home), config)
...
def test_post_setup_create_remote_user_can_keep_max_only(tmp_path, monkeypatch):
    _clear_openviking_env(monkeypatch)
    max_home = tmp_path / "max"
    max_home.mkdir()
    monkeypatch.setenv("MAX_HOME", str(max_home))
    ...
    monkeypatch.setattr(
        ...
        _prompt_from_values({
            "OpenViking server URL": "https://openviking.example",
            "OpenViking user API key": "user-secret",
            "Max peer ID in OpenViking": "agent",
        }),
    )
    config = {"memory": {}}
    ...
    env_text = (max_home / ".env").read_text(encoding="utf-8")
...
def test_post_setup_remote_blank_api_key_cancels_without_saving(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(max_config, "save_config", save_config)
    choices = iter([1, 0, 1])
    monkeypatch.setattr(memory_setup, "_curses_select", lambda *args, **kwargs: next(choices))
    monkeypatch.setattr(
        memory_setup,
...
def test_post_setup_user_key_path_can_route_detected_root_key_to_root_setup(tmp_path, monkeypatch):
    ...
    assert prompt_events.count("Max peer ID in OpenViking") == 1
    env_text = (max_home / ".env").read_text(encoding="utf-8")
...
def test_start_local_openviking_server_writes_output_to_log(tmp_path, monkeypatch):
    ...
    def fake_popen(args, **kwargs):
        ...
        assert kwargs["stdout"].name == str(max_home / "logs" / "openviking-server.log")
    ...
    assert started is True
    assert str(max_home / "logs" / "openviking-server.log") in message
    assert popen_calls
...
def test_https_local_endpoint_is_not_runtime_autostart_eligible(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory disabled for this Max run. "
...
def test_runtime_openviking_waiter_warns_when_background_start_times_out(monkeypatch):
    ...
    assert warnings == [
        ...
        "within 60 seconds. OpenViking memory disabled for this Max run."
    ]
...
def test_tool_add_resource_rejects_max_credential_file_upload(tmp_path, monkeypatch):
    import agent.file_safety as fs

    max_home = tmp_path / "max_home"
    max_home.mkdir()
    auth_json = max_home / "auth.json"
    auth_json.write_text('{"OPENROUTER_API_KEY":"sk-test-secret"}', encoding="utf-8")
    monkeypatch.setattr(fs, "_max_home_path", lambda: max_home)
...
def test_tool_add_resource_directory_zip_skips_max_credential_files(tmp_path, monkeypatch):
    import agent.file_safety as fs

    max_home = tmp_path / "max_home"
    max_home.mkdir()
    (max_home / "guide.md").write_text("# Guide\n", encoding="utf-8")
    ...
    def inspect_upload(path):
        ...
        return "upload_max_home.zip"

    provider._client.upload_temp_file.side_effect = inspect_upload
    provider._client.post.return_value = {
        "status": "ok",
        "result": {"root_uri": "viking://resources/max_home"},
    }

    result = json.loads(provider._tool_add_resource({"url": str(max_home)}))
...
def test_viking_client_delete_uses_identity_headers(monkeypatch):
    ...
    assert captured["kwargs"]["headers"]["X-OpenViking-Actor-Peer"] == "max"
...
def test_viking_client_headers_include_bearer_when_api_key_set():
    ...
    assert headers["X-OpenViking-Actor-Peer"] == "max"
    assert "X-OpenViking-Agent" not in headers
    assert "X-OpenViking-Account" not in headers
    assert "X-OpenViking-User" not in headers
...
def test_viking_client_health_sends_auth_headers(monkeypatch):
    ...
    assert captured["headers"]["X-OpenViking-Actor-Peer"] == "max"
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "max"},
        ]
    }]
...
def test_sync_turn_structured_messages_include_assistant_peer_id():
    ...
    assert captured == [(
        "/api/v1/sessions/sid-structured/messages/batch",
        {
            "messages": [
                {"role": "user", "parts": [{"type": "text", "text": "u"}]},
                {"role": "assistant", "parts": [{"type": "text", "text": "Looking."}], "peer_id": "max"},
                {
                    "role": "assistant",
                    "parts": [
                        {
...
def test_on_session_switch_does_not_block_caller_on_slow_drain():
    """Regression for max-agent#28296 review (H1): on_session_switch must
...
def test_on_session_switch_defers_old_commit_to_finalizer_thread():
    ...
    -_DEFERRED_COMMIT_TIMEOUT drain or the commit POST. See max-agent#28296
