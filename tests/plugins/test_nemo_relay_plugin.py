...
""",
    ...
    monkeypatch.setenv("MAX_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
...
def test_nemo_relay_plugin_is_discoverable_as_bundled_plugin(tmp_path, monkeypatch):
    monkeypatch.setenv("MAX_HOME", str(tmp_path / "max_test"))
...
def test_nemo_relay_plugin_emits_llm_tool_and_exports_atif(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MAX_NEMO_RELAY_ATOF_ENABLED", "1")
    monkeypatch.setenv("MAX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY", str(tmp_path / "atof"))
    monkeypatch.setenv("MAX_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("MAX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif"))
    ...
    assert (tmp_path / "atif" / "max-atif-s1.json").exists()
...
def test_nemo_relay_plugin_reparents_child_session_scope_for_embedded_atif(monkeypatch):
    ...
    child_push = next(
        ...
        if event[0] == "scope.push" and event[1] == "max-session-child-session"
    )
    child_kwargs = child_push[3]
    assert child_kwargs["handle"] == ("scope", "max-session-parent-session")
...
def test_nemo_relay_plugin_skips_embedded_child_atif_file_by_default(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "atif" / "max-atif-parent-session.json").exists()
    assert not (tmp_path / "atif" / "max-atif-child-session.json").exists()
...
def test_nemo_relay_plugin_can_write_embedded_child_atif_file_in_all_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MAX_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("MAX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif"))
    monkeypatch.setenv("MAX_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE", "all")
    ...
    assert (tmp_path / "atif" / "max-atif-parent-session.json").exists()
    assert (tmp_path / "atif" / "max-atif-child-session.json").exists()
...
""",
    ...
    assert "max-session-s2" in scope_push_names
...
""",
    ...
    monkeypatch.setenv("MAX_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
    monkeypatch.setenv("MAX_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("MAX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "direct-atif"))
    ...
    assert not (tmp_path / "direct-atif" / "max-atif-s1.json").exists()
...
""",
    ...
    assert "atif.register" in event_names
    assert (tmp_path / "direct-atif" / "max-atif-s1.json").exists()
...
""",
    ...
    monkeypatch.setenv("MAX_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
    monkeypatch.setenv("MAX_NEMO_RELAY_ATOF_ENABLED", "1")
    monkeypatch.setenv("MAX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY", str(tmp_path / "direct-atof"))
...
def test_nemo_relay_downstream_unwrap_matches_real_middleware_wrapper_shape(monkeypatch):
    ...
    # ``max_cli.middleware._run_execution_chain`` hands to a middleware
    ...
    from max_cli import middleware
