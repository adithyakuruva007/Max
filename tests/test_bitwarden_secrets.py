...
binary download are exercised manually by `max secrets bitwarden
...
def max_home(tmp_path, monkeypatch):
    """Point Max at an isolated home directory."""
    home = tmp_path / ".max"
    home.mkdir()
    monkeypatch.setenv("MAX_HOME", str(home))
    # Some modules cache get_max_home; clear if needed.
    import max_constants
    if hasattr(max_constants, "_MAX_HOME_CACHE"):
        max_constants._MAX_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(max_home, monkeypatch):
...
def test_install_bws_happy_path(max_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(max_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(max_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from max_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under max_home/cache/."""
    home = tmp_path / ".max"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
