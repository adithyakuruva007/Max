...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from max_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_falls_back_to_static_models(self, monkeypatch):
        ...
        monkeypatch.setattr("max_cli.models.fetch_api_models", lambda api_key, base_url: None)
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in MAX_OVERLAYS
        overlay = MAX_OVERLAYS["gmi"]
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a MaxAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("MaxAgent/")

    def test_gmi_profile_declares_max_user_agent(self):
        """The GMI plugin sets a MaxAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("MaxAgent/"), (
            f"expected GMI profile User-Agent to start with 'MaxAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("max_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "max_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["max", "chat", "--provider", "gmi"])
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("max_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("max_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("max_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        config = yaml.safe_load((get_max_home() / "config.yaml").read_text()) or {}
