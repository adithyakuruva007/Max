...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "max-gateway.service"
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `max gateway restart`)."""
        unit_path = tmp_path / "max-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``/tmp/pytest-of-.../max_test`` MAX_HOME — silently breaking
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E MAX_HOME like
        ``/tmp/max-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 — an E2E probe ran
        ``max gateway restart`` with a /tmp MAX_HOME exported; the
        ...
        must refuse ANY temp-dir MAX_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "max-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="MAX_HOME=/tmp/max-e2e-41264"\n'
            "WorkingDirectory=/tmp/max-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="MAX_HOME=/var/tmp/max-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="MAX_HOME={tmp_path}/max-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None

    def test_detects_tmp_home_in_launchd_plist(self):
        plist = (
            "<dict>\n  <key>MAX_HOME</key>\n"
            "  <string>/tmp/max-e2e-99999</string>\n</dict>\n"
        )
        assert (
    ...
    def test_accepts_macos_real_home_plist(self):
        plist = (
            "<dict>\n  <key>MAX_HOME</key>\n"
            "  <string>/Users/alice/.max</string>\n</dict>\n"
        )
        assert gateway_cli._temp_home_in_service_definition(plist) is None

    def test_accepts_unit_without_max_home(self):
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="MAX_HOME=/tmpfs-data/.max"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_max_home_for_target_user", lambda home: "/home/alice/.max")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        ...
        monkeypatch.setenv("MAX_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("MAX_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        ...
        # MAX_HOME resolves under the (pytest tmp) test MAX_HOME.
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test MAX_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>MAX_HOME</key>"
                "<string>/Users/alice/.max</string></plist>"
            ),
        )
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_max_units", lambda: False)
...
class TestSystemUnitMaxHome:
    ...
    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: MAX_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("MAX_HOME", "/root/.max/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'MAX_HOME=/home/alice/.max/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_max_home(self, monkeypatch):
        # Custom MAX_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("MAX_HOME", "/opt/max-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'MAX_HOME=/opt/max-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's MAX_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        max_home = str(gateway_cli.get_max_home().resolve())
        assert f'MAX_HOME={max_home}' in unit


class TestMaxHomeForTargetUser:
    """Unit tests for _max_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("MAX_HOME", raising=False)

        result = gateway_cli._max_home_for_target_user("/home/alice")
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._max_home_for_target_user("/home/alice")
        assert result == "/home/alice/.max/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("MAX_HOME", "/opt/max")

        result = gateway_cli._max_home_for_target_user("/home/alice")
        assert result == "/opt/max"
...
class TestProfileArg:
    ...
    def test_default_max_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.max should not produce a --profile flag."""
        max_home = tmp_path / ".max"
        max_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MAX_HOME", str(max_home))
        result = gateway_cli._profile_arg(str(max_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.max/profiles/mybot should return '--profile mybot'."""
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".max"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile MAX_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "max"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.max/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".max" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".max" / "profiles" / "My Bot!"
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "max-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "max-gateway"])
...
class TestLegacyMaxUnitDetection:
    """Tests for _find_legacy_max_units / has_legacy_max_units.
    ...
    renamed to ``max-gateway.service``. After PR #5646 (signal recovery
    ...
    (``max-gateway-<profile>.service``) or unrelated third-party services.
    """
    ...
    def test_detects_legacy_max_service_in_user_scope(self, tmp_path, monkeypatch):
        ...
        results = gateway_cli._find_legacy_max_units()
        ...
        assert gateway_cli.has_legacy_max_units() is True

    def test_detects_legacy_max_service_in_system_scope(self, tmp_path, monkeypatch):
    ...
    def test_ignores_profile_unit_max_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``max-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "max-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "max-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "max-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_max_units() is False

    def test_ignores_unrelated_max_service(self, tmp_path, monkeypatch):
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_max_units() == []
        assert gateway_cli.has_legacy_max_units() is False
...
class TestRemoveLegacyMaxUnits:
    """Tests for remove_legacy_max_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_max_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_max_units(
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (max-gateway-coder.service)
        ...
        profile_unit = user_dir / "max-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "max-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_max_units(interactive=True)
...
class TestMigrateLegacyCommand:
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_max_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_max_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_max_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_max_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "max-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "max-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "max-gateway.service",
        )
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "max-gateway")
        ...
        assert "sudo systemctl start max-gateway" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart max-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop max-gateway" in out
...
class TestServiceWorkingDirIsStable:
    ...
    def test_stable_working_dir_uses_max_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".max"
        home.mkdir()
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # MAX_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".max"
        monkeypatch.setattr(gateway_cli, "get_max_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_max_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".max"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_max_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_max_home(self, tmp_path, monkeypatch):
        import re
