...
from max_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from max_cli.auth import _resolve_verify
    ...
    def test_missing_max_ca_bundle_env_falls_back(self, monkeypatch):
        from max_cli.auth import _resolve_verify

        monkeypatch.setenv("MAX_CA_BUNDLE", "/nonexistent/max-ca.pem")
...
def test_resolve_nous_runtime_credentials_env_override_wins_live_not_persisted(
    ...
    network_url = "https://inference-api.stardustresearch.com/v1"
    ...
    monkeypatch.setattr("max_cli.auth._refresh_access_token", _fake_refresh_access_token)
    ...
    auth_store = {
        ...
        "providers": {
            "nous": {
                ...
                "inference_base_url": "https://inference-api.stardustresearch.com/v1",
                "client_id": "max-cli",
                "token_type": "Bearer",
                "scope": auth_mod.DEFAULT_NOUS_SCOPE,
                "access_token": token,
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MAX_HOME", str(max_home))
    monkeypatch.setenv("MAX_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="max_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from max_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from max_cli.auth import get_nous_auth_status, invalidate_nous_auth_status_cache
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    max_home = tmp_path / "max"
    _setup_nous_auth(max_home, access_token="at-123")
    monkeypatch.setenv("MAX_HOME", str(max_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    max_home = tmp_path / "max"
    _setup_nous_auth(max_home, access_token="at-fresh")
    monkeypatch.setenv("MAX_HOME", str(max_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("max_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    max_home = tmp_path / "max"
    _setup_nous_auth(max_home, refresh_token="refresh-old")
    monkeypatch.setenv("MAX_HOME", str(max_home))
    from agent.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `max model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return max_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        fake_auth_state = {
            ...
            "inference_base_url": "https://inference-api.stardustresearch.com",
            "portal_base_url": "https://portal.stardustresearch.com",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from max_cli.auth import PROVIDER_REGISTRY, _login_nous

        max_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `max auth add nous`
    ...
    """
    from max_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Max is not logged into Nous Portal").
    ...
    # doesn't raise "Max is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `max auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Max must surface an
    ...
    re-authenticate" string led users to report this as a Max persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from max_cli.auth import _refresh_access_token
    ...
    assert "max auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect MAX_SHARED_AUTH_DIR to a tmp_path.
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without MAX_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.max/shared/`` across CI runs.
    """
    from max_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("MAX_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """MAX_SHARED_AUTH_DIR must redirect the path."""
    from max_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("MAX_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from max_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from max_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from max_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `max auth add nous
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from max_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "max-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different MAX_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("MAX_HOME", str(profile_a))
    ...
    # Profile B: fresh MAX_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("MAX_HOME", str(profile_b))
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from max_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL
        ...
        auth_file.write_text(json.dumps({
            ...
            "providers": {
                "nous": {
                    "portal_base_url": "https://api.stardustresearch.com",
    ...
    def test_runtime_fallback_for_invalid_portal_url(self, tmp_path, monkeypatch):
        ...
        store["providers"]["nous"]["portal_base_url"] = "https://api.stardustresearch.com"
