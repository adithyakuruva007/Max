...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    if platform == "darwin":
        ...
        exe = desktop_dir / "release" / "linux-unpacked" / "max"
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    root = _make_desktop_tree(tmp_path)
    max_root = tmp_path / "custom-hermes"
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/npm"), \
        ...
        cli_main.cmd_gui(_ns(
            ...
            max_root=str(max_root),
            cwd=str(cwd),
        ))

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["MAX_DESKTOP_BOOT_FAKE"] == "1"
    assert launch_env["MAX_DESKTOP_IGNORE_EXISTING"] == "1"
    assert launch_env["MAX_DESKTOP_MAX_ROOT"] == str(max_root)
    assert launch_env["MAX_DESKTOP_CWD"] == str(cwd)
...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with patch("max_cli.main.shutil.which", return_value=None), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_linux_configures_sandbox_before_launch(tmp_path, monkeypatch):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("max_cli.main.subprocess.run", return_value=ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_rejects_symlink_sandbox(tmp_path, monkeypatch):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("max_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_falls_back_to_no_sandbox_when_userns_is_restricted(tmp_path, monkeypatch):
    ...
    with patch("max_cli.main._desktop_linux_sandbox_fixup", return_value=False), \
         patch("max_cli.main._desktop_linux_needs_no_sandbox", return_value=True), \
         patch("max_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_exits_when_sandbox_fixup_fails_without_safe_fallback(tmp_path, monkeypatch):
    ...
    with patch("max_cli.main._desktop_linux_sandbox_fixup", return_value=False), \
         patch("max_cli.main._desktop_linux_needs_no_sandbox", return_value=False), \
         patch("max_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("max_cli.main._desktop_build_needed", return_value=True), \
         patch("max_cli.main._write_desktop_build_stamp"), \
         patch("max_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_force_build_overrides_stamp(tmp_path, monkeypatch):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("max_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("max_cli.main._desktop_build_needed", return_value=False), \
...
def test_compute_desktop_content_hash_stable(tmp_path, monkeypatch):
    ...
    (root / "package.json").write_text('{"name":"max"}', encoding="utf-8")
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    packaged_exe = root / "apps" / "desktop" / "release" / "linux-unpacked" / "max"
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("max_cli.main._electron_dist_ok", return_value=False), \
         patch("max_cli.main._redownload_electron_dist", return_value=True), \
         patch("max_cli.main.subprocess.run", side_effect=run_side_effect) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_redownloads_electron_via_mirror_then_repacks(tmp_path, monkeypatch, capsys):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("max_cli.main._electron_dist_ok", return_value=False), \
         patch("max_cli.main._redownload_electron_dist", side_effect=[False, True]) as mock_dl, \
         patch("max_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_retries_pack_under_mirror_even_when_prefetch_blocked(tmp_path, monkeypatch, capsys):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("max_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("max_cli.main._electron_dist_ok", return_value=False), \
         patch("max_cli.main._redownload_electron_dist", return_value=False), \
         patch("max_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_self_heals_electron_and_continues(tmp_path, monkeypatch, capsys):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("max_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("max_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("max_cli.main._write_desktop_build_stamp"), \
         patch("max_cli.main._electron_dist_ok", return_value=False), \
         patch("max_cli.main._try_redownload_electron_dist", return_value=True) as mock_dl, \
         patch("max_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_hard_fails_when_electron_dist_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("max_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("max_cli.main._electron_dist_ok", return_value=True), \
         patch("max_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_does_not_override_user_electron_mirror(tmp_path, monkeypatch, capsys):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("max_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("max_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("max_cli.main._purge_electron_build_cache", return_value=[]) as mock_purge, \
         patch("max_cli.main.subprocess.run", side_effect=[pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_redownload_electron_dist_noop_when_present(tmp_path, monkeypatch):
    ...
    with patch("max_cli.main.subprocess.run") as mock_run:
...
def test_redownload_electron_dist_missing_installer(tmp_path, monkeypatch):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("max_cli.main.subprocess.run") as mock_run:
...
def test_redownload_electron_dist_runs_installer_with_mirror(tmp_path, monkeypatch):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("max_cli.main.subprocess.run", side_effect=fake_run):
...
def test_redownload_electron_dist_returns_false_when_download_fails(tmp_path, monkeypatch):
    ...
    with patch("max_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("max_cli.main.subprocess.run",
...
def test_stop_desktop_build_lock_terminates_only_release_procs(tmp_path, monkeypatch):
    ...
    locker_exe = release / "Max.exe"
    locker_exe.write_text("", encoding="utf-8")
    other_exe = tmp_path / "elsewhere" / "Max.exe"
...
def test_desktop_launch_options_survives_config_error():
    with patch("max_cli.config.load_config", side_effect=RuntimeError("boom")):
        flags, gpu = cli_main._desktop_launch_options()
    assert flags == []
    assert gpu == "auto"
