...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        assert env.get("MAX_HOME") == str(get_default_max_root())

    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (MAX_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop MAX_HOME.

        Dropping it makes the child fall back to $HOME/.max
        (= /opt/data/.max), an empty auto-seeded home, so the dashboard
        ...
        """
        monkeypatch.setenv("MAX_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "max_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
