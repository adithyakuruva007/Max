...
def _make_agent(max_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = max_home / "max-agent"
...
def _make_gui_build(max_home: Path) -> None:
    """Create the source-built GUI artifacts a `max desktop` run produces."""
    desktop = max_home / "max-agent" / "apps" / "desktop"
    ...
    (max_home / "max-agent" / "node_modules").mkdir(parents=True)
    (max_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(max_home: Path) -> None:
    (max_home / "config.yaml").write_text("x: 1\n")
    (max_home / ".env").write_text("KEY=secret\n")
    (max_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    max_home = tmp_path / ".max"
    max_home.mkdir()
    assert gu.agent_is_installed(max_home) is False
    _make_agent(max_home)
    assert gu.agent_is_installed(max_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    max_home = tmp_path / ".max"
    (max_home / "max-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(max_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    max_home = tmp_path / ".max"
    _make_gui_build(max_home)
    artifacts = gu.source_built_gui_artifacts(max_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(max_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(max_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    max_home = tmp_path / ".max"
    agent_root = _make_agent(max_home)
    _make_gui_build(max_home)
    _make_user_data(max_home)
    ...
    removed = gu.uninstall_gui(max_home)
    ...
    assert not (max_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "max_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (max_home / "config.yaml").exists()
    assert (max_home / ".env").exists()
    assert (max_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    max_home = tmp_path / ".max"
    _make_agent(max_home)
    userdata = tmp_path / "Max-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(max_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(max_home)
    ...
    assert summary["max_home"] == str(max_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Max"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Max"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Max"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Max"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    MAX_HOME with all the destructive externals stubbed out.
    """
    ...
    from max_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole MAX_HOME non-interactively."""
    import max_cli.uninstall as uninstall

    max_home = tmp_path / ".max"
    (max_home / "max-agent" / "max_cli").mkdir(parents=True)
    (max_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not max_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m max_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_max_home", lambda: max_home)
    ...
    assert not (desktop / "release").exists()
    assert not (max_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "max_cli").exists()
    assert (max_home / "config.yaml").exists()
