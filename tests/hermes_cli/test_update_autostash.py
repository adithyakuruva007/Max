...
from max_cli import config as max_config
...
def test_stash_local_changes_if_needed_returns_none_when_tree_clean(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(max_main.subprocess, "run", fake_run)

    stash_ref = max_main._stash_local_changes_if_needed(["git"], tmp_path)
...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M max_cli/main.py\n?? notes.txt\n", returncode=0)
...
def test_resolve_stash_selector_returns_matching_entry(monkeypatch, tmp_path):
    ...
    assert max_main._resolve_stash_selector(["git"], tmp_path, "abc123") == "stash@{1}"
...
def test_restore_stashed_changes_prompts_before_applying(monkeypatch, tmp_path, capsys):
    ...
    restored = max_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)
...
def test_restore_stashed_changes_applies_without_prompt_when_disabled(monkeypatch, tmp_path, capsys):
    ...
    restored = max_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=False)
...
def test_print_stash_cleanup_guidance_with_selector(capsys):
    max_main._print_stash_cleanup_guidance("abc123", "stash@{2}")
...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:3] == ["diff", "--name-only"]:
            return SimpleNamespace(stdout="max_cli/main.py\n", stderr="", returncode=0)
    ...
    result = max_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)

    assert result is False
    out = capsys.readouterr().out
    assert "Conflicted files:" in out
    assert "max_cli/main.py" in out
...
def test_restore_stashed_changes_auto_resets_non_interactive(monkeypatch, tmp_path, capsys):
    ...
    result = max_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=False)
...
def test_stash_local_changes_if_needed_raises_when_stash_ref_missing(monkeypatch, tmp_path):
    ...
    with pytest.raises(CalledProcessError):
        max_main._stash_local_changes_if_needed(["git"], Path(tmp_path))
...
def test_discard_lockfile_churn_skips_lock_when_package_json_dirty(tmp_path):
    ...
    max_main._discard_lockfile_churn(["git"], tmp_path)
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(max_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(max_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(max_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(max_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(max_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(max_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(max_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(max_main, "_refresh_active_lazy_features", lambda: None)
...
def test_cmd_update_retries_optional_extras_individually_when_all_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(max_main, "_is_termux_env", lambda env=None: False)
    monkeypatch.setattr(max_main, "_load_installable_optional_extras", lambda group="all": ["matrix", "mcp"])
    ...
    max_main.cmd_update(SimpleNamespace())
...
def test_install_with_optional_fallback_honors_custom_group(monkeypatch):
    ...
    monkeypatch.setattr(max_main, "_run_install_with_heartbeat", fake_run_with_heartbeat)

    max_main._install_python_dependencies_with_optional_fallback(
        ["/usr/bin/uv", "pip"],
        group="termux-all",
    )
...
def test_install_heartbeat_prints_when_dependency_install_is_silent(monkeypatch, capsys):
    ...
    def fake_run(cmd, **kwargs):
        max_main._time.sleep(1.2)
        return SimpleNamespace(returncode=0)

    monkeypatch.setattr(max_main.subprocess, "run", fake_run)

    max_main._run_install_with_heartbeat(
        ["uv", "pip", "install", "-e", "."],
...
def test_cmd_update_falls_back_to_reset_when_ff_only_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(max_main.subprocess, "run", side_effect)
...
def test_cmd_update_restores_stash_and_branch_when_already_up_to_date(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        max_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        max_main, "_restore_stashed_changes",
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        max_main, "_discard_stashed_changes",
...
def test_non_interactive_discard_throws_changes_away(monkeypatch, tmp_path):
    ...
    max_main.cmd_update(SimpleNamespace(gateway=True))
...
def test_interactive_update_ignores_discard_setting(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(max_main.sys.stdin, "isatty", lambda: True)
    monkeypatch.setattr(max_main.sys.stdout, "isatty", lambda: True)

    max_main.cmd_update(SimpleNamespace())  # no gateway, no --yes
...
def test_non_interactive_defaults_to_stash_when_setting_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(max_config, "load_config", lambda *a, **kw: {"model": {}})
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    repo_gitignore = Path(max_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    # Exact flags used by hermes update (max_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "max-update-autostash")
