"""Tests for Codex auth — tokens stored in Max auth store (~/.max/auth.json)."""
...
def _setup_max_auth(max_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Max auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_max_auth(max_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("MAX_HOME", str(max_home))
    ...
    monkeypatch.setattr("max_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    max_home = tmp_path / "max"
    _setup_max_auth(max_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("MAX_HOME", str(max_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((max_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``max auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``max auth add openai-codex``.
    ...
    (max_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `max auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``max auth add openai-codex`` for accounts B and C
    ...
    (max_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``max auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``max auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Max auth store, not ~/.codex/."""
    max_home = tmp_path / "max"
    codex_home = tmp_path / "codex-cli"
    max_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "max-at", "refresh_token": "max-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Max store
    assert not (codex_home / "auth.json").exists()

    # Max auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "max-at"


def test_resolve_returns_max_auth_store_source(tmp_path, monkeypatch):
    max_home = tmp_path / "max"
    _setup_max_auth(max_home)
    monkeypatch.setenv("MAX_HOME", str(max_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    assert "max auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from max_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from max_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "max_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "max_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("max_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("max_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch max_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("max_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
