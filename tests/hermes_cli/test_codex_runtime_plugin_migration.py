...
class TestMigrate:
    ...
    def test_no_servers_still_writes_permissions_default(self, tmp_path):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=False, expose_max_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from max_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_max_tools=False)
        assert report.written
    ...
    def test_discover_plugins_false_skips_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, discover_plugins=False, expose_max_tools=False)
    ...
    def test_dry_run_skips_plugin_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, dry_run=True, discover_plugins=True, expose_max_tools=False)
        assert called["yes"] is False
    ...
    def test_expose_max_tools_writes_callback_mcp_entry(self, tmp_path):
        """When expose_max_tools=True (production default), an
        [mcp_servers.max-tools] entry is written so codex calls back
        into Max for browser/web/delegate_task/vision/memory tools.
        ...
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
                         expose_max_tools=True)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.max-tools]" in text
        assert "max_tools_mcp_server" in text
        ...
        assert "max-tools" in report.migrated

    def test_expose_max_tools_disabled_skips_entry(self, tmp_path):
        """expose_max_tools=False suppresses the callback registration."""
        migrate({}, codex_home=tmp_path,
                discover_plugins=False,
                default_permission_profile=None,
                expose_max_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.max-tools]" not in text
        assert "max_tools_mcp_server" not in text

    def test_dry_run_doesnt_write(self, tmp_path):
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, dry_run=True, expose_max_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(max_cfg, codex_home=tmp_path, expose_max_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_idempotent_re_run_replaces_managed_block(self, tmp_path):
        # First migration
        migrate({"mcp_servers": {"a": {"command": "x"}}}, codex_home=tmp_path, expose_max_tools=False)
        first_text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.a]" in first_text
        # Second migration with different servers
        migrate({"mcp_servers": {"b": {"command": "y"}}}, codex_home=tmp_path, expose_max_tools=False)
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Max appends
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=False, expose_max_tools=False)
        new_text = target.read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        migrate({"mcp_servers": {"max-mcp": {"command": "npx"}}},
    ...
    def test_skipped_keys_reported(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_max_tools=False)
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        report = migrate({"mcp_servers": "notadict"}, codex_home=tmp_path, expose_max_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_max_tools=False)
        new_text = target.read_text()
...
class TestMaxHomeLeakGuard:
    ...
    Previously ``_build_max_tools_mcp_entry()`` read ``MAX_HOME``
    ...
    once codex spawned the max-tools MCP subprocess.
    """

    def test_tempdir_detector_recognizes_pytest_paths(self):
        assert _looks_like_test_tempdir(
            "/private/var/folders/abc/pytest-of-kshitij/pytest-137/popen-gw2/test_X/max_test"
        )
        assert _looks_like_test_tempdir(
    ...
    def test_tempdir_detector_accepts_real_max_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.max")
        assert not _looks_like_test_tempdir("/home/bob/.max")
        assert not _looks_like_test_tempdir("/opt/max")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when MAX_HOME points at a pytest
        tempdir, _build_max_tools_mcp_entry() must NOT propagate it."""
        monkeypatch.setenv(
            "MAX_HOME",
            "/private/var/folders/xx/pytest-of-user/pytest-99/test_x/max_test",
        )
        entry = _build_max_tools_mcp_entry()
        env = entry.get("env", {})
        assert "MAX_HOME" not in env, (
            f"pytest-tempdir MAX_HOME leaked into codex MCP entry: "
            f"{env.get('MAX_HOME')!r}"
        )

    def test_real_max_home_propagates(self, monkeypatch, tmp_path):
    ...
    def test_unset_max_home_omits_env_key(self, monkeypatch):
