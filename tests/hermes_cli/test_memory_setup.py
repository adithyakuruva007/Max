...
def test_curses_select_cancel_defaults_to_selected(monkeypatch):
    ...
    monkeypatch.setattr("max_cli.curses_ui.curses_radiolist", fake_radiolist)
...
def test_cmd_setup_top_level_cancel_writes_nothing(monkeypatch):
    ...
    monkeypatch.setattr("max_cli.config.load_config", load_config)
...
def test_cmd_setup_clears_interactive_picker_before_provider_post_setup(monkeypatch):
    ...
    class PostSetupProvider:
        def post_setup(self, max_home, config):
    ...
    monkeypatch.setattr(memory_setup, "get_max_home", lambda: "/tmp/max-test")
    monkeypatch.setattr("max_cli.config.load_config", lambda: {"memory": {}})
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_max_home", lambda: tmp_path)
    monkeypatch.setattr("max_cli.config.load_config", lambda: {"memory": {}})
    monkeypatch.setattr("max_cli.config.save_config", save_config)
