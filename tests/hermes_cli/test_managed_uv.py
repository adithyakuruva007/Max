...
class TestManagedUvPath:
    def test_posix(self, tmp_path):
        with patch("max_cli.managed_uv.get_max_home", return_value=tmp_path), \
             patch("max_cli.managed_uv.platform.system", return_value="Linux"):
            from max_cli.managed_uv import managed_uv_path
            assert managed_uv_path() == tmp_path / "bin" / "uv"

    def test_windows(self, tmp_path):
        with patch("max_cli.managed_uv.get_max_home", return_value=tmp_path), \
             patch("max_cli.managed_uv.platform.system", return_value="Windows"):
            from max_cli.managed_uv import managed_uv_path
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        with pytest.raises(TypeError):
            subprocess.list2cmdline([_UvResult("C:\\max\\uv.exe"), "pip"])
...
class TestUpdateManagedUv:
    def test_no_uv_returns_none(self, tmp_path):
        with patch("max_cli.managed_uv.get_max_home", return_value=tmp_path):
            from max_cli.managed_uv import update_managed_uv
...
class TestInstallUvInternals:
    ...
    def test_windows_sets_uv_install_dir(self, tmp_path):
        ...
        with patch("max_cli.managed_uv.platform.system", return_value="Windows"), \
             patch("max_cli.managed_uv._install_uv_windows") as mock_windows:
            from max_cli.managed_uv import _install_uv
            _install_uv(target)
