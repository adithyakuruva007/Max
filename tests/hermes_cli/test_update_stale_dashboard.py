...
class TestFindStaleDashboardPids:
    ...
    def test_matches_running_dashboard(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "python3 -m max_cli.main dashboard --port 9119") + "\n",
                stderr="",
            )
    ...
    def test_multiple_matches(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(12345, "python3 -m max_cli.main dashboard --port 9119"),
                    _ps_line(12346, "max dashboard --port 9120 --no-open"),
                    _ps_line(12347, "python /home/x/max_cli/main.py dashboard"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_unrelated_process_containing_word_dashboard_not_matched(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    _ps_line(12345, "python3 -m max_cli.main dashboard --port 9119"),
                    _ps_line(22222, "python3 -m max_cli.main chat -q 'rewrite my dashboard'"),
    ...
    def test_exclude_pids_filters_specified_pids(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(11111, "max dashboard --port 9119"),
                    _ps_line(22222, "max dashboard --port 9120"),
                    _ps_line(33333, "max dashboard --port 9121"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_exclude_pids_none_is_noop(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "max dashboard --port 9119") + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_no_stale_processes_is_a_noop(self, capsys):
        with patch("max_cli.main._find_stale_dashboard_pids", return_value=[]):
            _kill_stale_dashboard_processes()
