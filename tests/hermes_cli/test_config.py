...
class TestGetMaxHome:
    ...
    def test_env_override(self):
        with patch.dict(os.environ, {"MAX_HOME": "/custom/path"}):
            home = get_max_home()
...
class TestEnsureMaxHome:
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from max_cli.default_soul import _LEGACY_TEMPLATE_SOULS
    ...
    def test_existing_named_profile_still_bootstraps_subdirs(self, tmp_path):
        profile_home = tmp_path / ".max" / "profiles" / "coder"
        profile_home.mkdir(parents=True)
        with patch.dict(os.environ, {"MAX_HOME": str(profile_home)}):
            ensure_max_home()
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"MAX_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="max_cli.config"):
                config = load_config()
            ...
            assert "max config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"MAX_HOME": str(tmp_path)}):
            ...
            assert "max config:" in after_edit, "edited file should re-warn"
...
class TestRemoveEnvValue:
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MAX_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"MAX_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``MAX_*`` runtime flag.
    ...
    def _max_home(self, tmp_path, monkeypatch):
    ...
    def test_max_integration_keys_still_writable(self, allowed_key):
        """``MAX_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``MAX_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``MAX_*``) keep working. The denylist
...
class TestCodexAppServerAutoConfig:
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"MAX_HOME": str(tmp_path)}):
            ...
            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "max"
