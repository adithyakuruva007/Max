...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `max_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "max"
    called = []

    fake_main_module = types.ModuleType("max_cli.main")

    def fake_main():
        called.append("max_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "max_cli.main", fake_main_module)
