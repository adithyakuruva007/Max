...
class TestIgnoreUserConfigEnvGate:
    ...
    When the env var is set, user config at ``<max_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._max_home at tmp_path and return a fresh load_cli_config."""
        import cli
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        ...
        import inspect
        import max_cli.main as hm
        src = inspect.getsource(hm)
        assert "MAX_IGNORE_USER_CONFIG" in src
        assert "MAX_IGNORE_RULES" in src
