...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from max_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    max_home = tmp_path / "classic"
    max_home.mkdir()
    monkeypatch.setenv("MAX_HOME", str(max_home))

    _write(max_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (MAX_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.max as the "real home" — which is NOT equal
    # to the MAX_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    max_home = tmp_path / "classic"
    ...
    _write(max_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from max_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: MAX_HOME is set to a custom path that is NOT under
    # ~/.max/profiles/ — get_default_max_root() returns MAX_HOME
