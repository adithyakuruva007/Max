"""Tests for max_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_max_home):
    """Drop the example-dashboard fixture into the per-test MAX_HOME
    ...
    ``MAX_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, max-achievements, model providers
    ...
    # plugin in exactly as a real operator would with `max plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down MAX_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Max vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Max env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints MAX_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("MAX_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("MAX_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_max_home):
        """Create a TestClient and isolate the state DB under the test MAX_HOME."""
        try:
    ...
    def test_status_active_session_count_uses_read_only_db(self, monkeypatch, tmp_path):
        ...
        fake_db_path.touch()
        monkeypatch.setattr(max_state, "DEFAULT_DB_PATH", fake_db_path)
        ...
        monkeypatch.setattr("max_state.SessionDB", _FakeDB)
    ...
    def test_status_active_session_count_fresh_install_returns_zero(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(max_state, "DEFAULT_DB_PATH", tmp_path / "absent.db")
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in max-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "max"
    ...
    def test_post_memory_provider_setup_persists_values_without_activation(self):
        from max_cli.config import load_config, load_env
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        ...
        config_path = get_max_home() / "hindsight" / "config.json"
    ...
    def test_update_max_returns_docker_guidance_without_spawning(self, monkeypatch):
        import max_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_max_action", fail_spawn)
        web_server._ACTION_PROCS.pop("max-update", None)
        web_server._ACTION_RESULTS.pop("max-update", None)
        ...
        assert data["name"] == "max-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull stardustresearch/max-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/max-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull stardustresearch/max-agent:latest" in line for line in status_data["lines"])

    def test_update_max_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import max_cli.web_server as web_server
    ...
    def test_update_max_spawns_on_non_docker_install(self, monkeypatch):
        import max_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "git")
        monkeypatch.setattr(web_server, "_spawn_max_action", fake_spawn)
        web_server._ACTION_PROCS.pop("max-update", None)
        web_server._ACTION_RESULTS.pop("max-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "max-update"}
        assert calls == [(["update"], "max-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("max-update", None)
        web_server._ACTION_RESULTS.pop("max-update", None)
        web_server._ACTION_PROCS["max-update"] = proc

        resp = self.client.get("/api/actions/max-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "max-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["max-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_action_status_tails_large_log_without_read_text(self, tmp_path, monkeypatch):
        ...
        log_path = tmp_path / web_server._ACTION_LOG_FILES["max-update"]
        log_path.write_text(
            "stale-start\n"
        ...
        resp = self.client.get("/api/actions/max-update/status?lines=3")
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from max_cli.web_server import _channel_managed_env_keys
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `max model` but
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "max_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Max provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `max import` doesn't auto-abort at the overwrite prompt."""
        import max_cli.web_server as ws
        ...
        monkeypatch.setattr(ws, "_spawn_max_action", fake_spawn)
    ...
    def test_ops_backup_defaults_to_dashboard_downloadable_archive(self, monkeypatch):
        ...
        assert archive.parent == get_max_home() / "backups"
        assert archive.name.startswith("max-backup-")
        assert archive.suffix == ".zip"

    def test_ops_backup_uses_hosted_max_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import max_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("MAX_HOME", str(hosted_home))
        captured = {}
    ...
    def test_ops_backup_download_streams_dashboard_backup(self, tmp_path):
        ...
        archive = backup_dir / "max-backup-test.zip"
    ...
    def test_ops_import_upload_rejects_invalid_zip(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_max_action", fail_spawn)
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from max_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from max_cli.config import save_env_value
        from max_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://max-agent.stardustresearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from max_cli.web_server import _build_catalog_entry

        teams = _build_catalog_entry("teams")
        assert teams["docs_url"] == (
            "https://max-agent.stardustresearch.com/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://max-agent.stardustresearch.com/docs/user-guide/messaging/google_chat"
        )
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Max Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("MaxDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "max_pair123_bot",
                "deep_link": "https://t.me/newbot/MaxSetupBot/max_pair123_bot",
                "qr_payload": "https://t.me/newbot/MaxSetupBot/max_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "max_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/MaxSetupBot/max_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/MaxSetupBot/max_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "max_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_max_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "max_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/MaxSetupBot/max_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/MaxSetupBot/max_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "max_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_max_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``max gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "max_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/MaxSetupBot/max_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/MaxSetupBot/max_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "max_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "max_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/MaxSetupBot/max_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/MaxSetupBot/max_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "max_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/MaxSetupBot/max_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/MaxSetupBot/max_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/max_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_headless_serve_disables_spa_even_with_a_dist(self, monkeypatch, tmp_path):
        """`max serve` (MAX_SERVE_HEADLESS) must NOT serve the SPA even
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.setenv("MAX_SERVE_HEADLESS", "1")
        app_ = FastAPI()
        ws.mount_spa(app_)
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import max_cli.nous_subscription as ns
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "max-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from max_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from max_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``max model`` custom flow. Regression for the desktop loop where a
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "max-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `max model`), not the first curated paid entry."""
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from max_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from max_cli.web_server import _build_schema_from_config
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from max_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_max_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_includes_default(self):
        from max_constants import get_max_home
        get_max_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        ...
        (max_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = max_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_max_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_max_for_default_profile(self):
        from max_constants import get_max_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "max setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/max/bin/hermes")
        ...
        if is_windows:
            assert lines == ["@echo off", "max -p writer %*"]
        else:
            assert lines == ["#!/bin/sh", 'exec /opt/max/bin/hermes -p writer "$@"']
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_max_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_max_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_max_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_max_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_max_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_max_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_max_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_max_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_max_home() / "profiles" / "builder"
        token = set_max_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_max_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import max_cli.skills_config as skills_config
        import max_cli.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from max_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from max_cli.web_server import _denormalize_config_from_web
...
class TestGatewayBusyReadout:
    ...
    def test_restart_drain_timeout_surfaced_and_numeric(self, monkeypatch):
        ...
        monkeypatch.setenv("MAX_RESTART_DRAIN_TIMEOUT", "90")
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from max_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.max/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``max_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in max_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_max_home):
        ...
        # Pin the SessionDB to the isolated MAX_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_max_home, _install_example_plugin):
        ...
        installs it into the per-test ``MAX_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into MAX_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, max-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (max-achievements is loaded by default).
        resp = self.client.get("/api/plugins/max-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``max --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["MAX_TUI_DASHBOARD"] == "1"
        assert env["MAX_TUI_INLINE"] == "1"
        assert env["MAX_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_max_home
    ):
        import max_cli.main as main_mod

        config_path = Path(os.environ["MAX_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/max-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/max-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"max-ws-ok" in buf:
                    break
            assert b"max-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from max_cli.pty_bridge import PtyUnavailableError
        ...
        import max_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        MAX_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from max_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("MAX_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_max_home, _install_example_plugin):
        ...
        the per-test ``MAX_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_max_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under MAX_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_max_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("MAX_DESKTOP", raising=False)
