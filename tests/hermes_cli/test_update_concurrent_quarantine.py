...
# Tests in this module either exercise the REAL _detect_concurrent_max_instances
...
# ---------------------------------------------------------------------------
# _detect_concurrent_max_instances
# ---------------------------------------------------------------------------
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "max-gateway.exe").write_bytes(b"")

    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter([]))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_max_instances(scripts_dir)
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_max_process(_winp, tmp_path):
    scripts_dir = tmp_path
    shim = scripts_dir / "hermes.exe"
    shim.write_bytes(b"")
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_max_instances(tmp_path) == []
...
# ---------------------------------------------------------------------------
# _quarantine_running_max_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    pairs = cli_main._quarantine_running_max_exe(tmp_path)
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_max_exe_shims", lambda d: [shim])
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "Max Desktop" in captured or "gateway" in captured.lower()
...
def test_resume_windows_gateways_after_update_respawns_unmapped_by_cmdline(
    ...
    scheduled_argv = ["pythonw.exe", "-m", "max_cli.main", "gateway", "run"]
    token = {
        "resume_needed": True,
        "profiles": {},
        "unmapped_pids": [7560],
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_max_instances", detect
    ), patch.object(
