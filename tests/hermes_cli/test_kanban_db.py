...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("MAX_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("MAX_HOME", str(home))
    monkeypatch.delenv("MAX_KANBAN_DB", raising=False)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("MAX_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_detect_crashed_workers_skips_freshly_claimed_tasks(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.delenv("MAX_KANBAN_CRASH_GRACE_SECONDS", raising=False)
...
def test_detect_crashed_workers_grace_period_env_override(
    ...
    """MAX_KANBAN_CRASH_GRACE_SECONDS env var adjusts the window."""
    import max_cli.kanban_db as _kb

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("MAX_KANBAN_CRASH_GRACE_SECONDS", "5")
...
def test_resolve_crash_grace_seconds_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv("MAX_KANBAN_CRASH_GRACE_SECONDS", bad_val)
...
def test_respawn_guard_rate_limit_cooldown_zero_allows_immediately(
    ...
    monkeypatch.setenv("MAX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")
    now = 6_000_000
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "MAX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Max profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="max-task", assignee="daily")
...
def test_scratch_workspace_created_under_max_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
        assert task is not None
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import max_cli.profiles as profiles
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Max-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``MAX_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("MAX_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
class TestSharedBoardPaths:
    ...
    def _set_home(self, monkeypatch, tmp_path, max_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_default_install_anchors_at_home_dot_hermes(
        ...
        # Standard install: MAX_HOME == ~/.max, no profile active.
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # MAX_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `max -p coder`).
    ...
    def test_docker_custom_max_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: MAX_HOME points outside ~/.max.
        # `get_default_max_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".max"`.
        custom_root = tmp_path / "opt" / "max"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: MAX_HOME=/opt/max/profiles/coder;
        # `get_default_max_root()` walks up to /opt/max because
    ...
    def test_explicit_override_via_max_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: MAX_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".max"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MAX_HOME", str(profile_home))
        monkeypatch.setenv("MAX_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MAX_KANBAN_HOME", "   ")
    ...
    def test_max_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # MAX_KANBAN_DB pins the file path directly and beats both
        # MAX_KANBAN_HOME and the `get_default_max_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MAX_HOME", str(default_home))
        monkeypatch.setenv("MAX_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("MAX_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows MAX_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_max_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # MAX_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".max"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("MAX_HOME", str(default_home))
        monkeypatch.setenv("MAX_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("MAX_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows MAX_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MAX_HOME", str(default_home))
        monkeypatch.setenv("MAX_KANBAN_DB", "   ")
        monkeypatch.setenv("MAX_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject MAX_KANBAN_DB
        # and MAX_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites MAX_HOME.
        default_home = tmp_path / ".max"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("max_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="max_state"):
            conn = kb.connect()
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_max_argv()
#
...
# $PATH that doesn't include the venv's bin/, so a bare `["max", ...]`
...
def test_resolve_max_argv_prefers_path_shim(monkeypatch):
    ...
    argv = kb._resolve_max_argv()
    assert argv == ["/usr/local/bin/hermes"]


def test_resolve_max_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import max_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("MAX_BIN", ".\\hermes.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_max_argv() == [os.path.abspath(".\\hermes.exe")]


def test_resolve_max_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    assert kb._resolve_max_argv() == [sys.executable, "-m", "max_cli.main"]


def test_resolve_max_argv_honors_max_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like MAX_BIN lets service managers pin the executable."""
    import shutil
    import max_cli.kanban_db as kb

    shim = tmp_path / "bin" / "max"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("MAX_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_max_argv() == [str(shim)]


def test_resolve_max_argv_max_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare MAX_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import max_cli.kanban_db as kb

    cwd_hermes = tmp_path / "max"
    cwd_hermes.write_text("wrong\n", encoding="utf-8")
    cwd_hermes.chmod(cwd_hermes.stat().st_mode | stat.S_IXUSR)
    path_hermes = tmp_path / "bin" / "max"
    ...
    monkeypatch.setenv("MAX_BIN", "max")

    assert kb._resolve_max_argv() == [str(path_hermes)]


def test_resolve_max_argv_max_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare MAX_BIN does not accept current-directory shadow executables."""
    import sys
    import max_cli.kanban_db as kb
...
def test_resolve_max_argv_max_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved MAX_BIN batch shim is not used as worker argv[0]."""
    import sys
    import max_cli.kanban_db as kb
...
def test_resolve_max_argv_max_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved MAX_BIN command names do not delegate cwd search to Popen."""
    import sys
    import max_cli.kanban_db as kb
...
def test_resolve_max_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
...
def test_resolve_max_argv_module_actually_runs():
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from max_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("max_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from max_cli.kanban_db import connect, write_txn
    import max_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from max_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("max_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("max_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("max_cli.kanban_db.os.name", "nt")
    with patch("max_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("max_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("max_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "max_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("max_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "max_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("max_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("max_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("max_cli.kanban_db._record_worker_exit"):
            with patch("max_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
