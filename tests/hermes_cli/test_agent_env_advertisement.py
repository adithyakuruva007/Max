"""Tests for the AI_AGENT / MAX_AGENT harness-attribution env vars.
...
Max-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Max' id in the public agent-harness
registry (``max-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the Max process env, and survives the
cross-session leak guard that strips ``MAX_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__MAX_CWD_testsession0__"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'MAX_AGENT="${MAX_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT HERMES=$MAX_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "MAX_AGENT")}
        out = subprocess.run(
        ...
        outer_env = dict(clean_env, AI_AGENT="pi", MAX_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
