...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_max_home):
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "max_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("max-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # max-index source advertises availability + featured search.
            idx = _Src("max-index")
        ...
        assert {"official", "github", "max-index"} <= ids
...
class TestToolsConfigEndpoints:
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_max_action", _fake_spawn)
        r = self.client.post(
