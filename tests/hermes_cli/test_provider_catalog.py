...
def test_catalog_covers_every_max_model_provider():
    """PARITY CONTRACT: the catalog == the `max model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `max model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_max_model():
    """The catalog must not invent providers `max model` doesn't show."""
