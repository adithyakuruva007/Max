"""Tests for max_cli/completion.py — shell completion script generation."""
...
from max_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_max_completion()" in out
        assert "complete -F _max_completion hermes" in out
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_max_profiles'" in out
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_max_profiles()" in out
        assert 'profiles_dir="$HOME/.max/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        ...
        out = generate_bash(_make_parser())
        # The profile case should have _max_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_max_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _max_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_max_profiles()" in out
        assert "$HOME/.max/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__max_profiles" in out
        count = out.count("(__max_profiles)")
