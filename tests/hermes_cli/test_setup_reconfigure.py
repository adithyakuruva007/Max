...
class TestExistingInstallDefault:
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`max setup --reconfigure` behaves the same as bare `max setup`."""
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["max", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["max", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["max", "setup"])
        try:
            main()
        except SystemExit:
            pass
