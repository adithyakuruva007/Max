...
@pytest.fixture
def pty_client(monkeypatch, _isolate_max_home):
...
def test_resolve_chat_argv_sets_active_session_file_env(monkeypatch):
    ...
    _argv, _cwd, env = ws._resolve_chat_argv(
        active_session_file="/tmp/max-active-session.json"
    )

    assert env["MAX_TUI_ACTIVE_SESSION_FILE"] == "/tmp/max-active-session.json"
...
def test_child_eof_closes_socket_and_bridge(pty_client, monkeypatch):
    ...
    monkeypatch.setattr(
        ws, "_resolve_chat_argv", lambda **kw: (["fake-max-tui"], None, None)
    )
