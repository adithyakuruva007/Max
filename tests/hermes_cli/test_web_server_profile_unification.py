...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_max_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_max_home", lambda: default_home)
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        assert resp.json()["max_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_max_home(self, isolated_profiles, monkeypatch):
        import max_cli.web_server as web_server
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("MAX_HOME") != str(isolated_profiles["worker_beta"])
