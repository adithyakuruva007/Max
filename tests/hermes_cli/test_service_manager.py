...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    the dispatch path inert on Fly, so `max gateway restart` spawned a
    foreground gateway that fought the supervised one. Detection must key off
    s6 being PID 1 (`_s6_running`) alone."""
    monkeypatch.setattr(
        "max_cli.service_manager._s6_running", lambda: True,
    )
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from max_cli.service_manager import _s6_running
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "max_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "max_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "max_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "max_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "max_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "max_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "max_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "max_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "max_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "max_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    assert "max -p coder gateway run" in run_text
...
def test_s6_register_staging_dir_is_dotfile_hidden_from_svscan(
    ...
    """
    import max_cli.service_manager as sm
...
def test_s6_lifecycle_persists_named_profile_desired_state(
    ...
    profile_dir = max_home / "profiles" / "coder"
...
def test_s6_lifecycle_persists_default_profile_desired_state(
    ...
    monkeypatch.setenv("MAX_HOME", str(max_home / "profiles" / "coder"))

    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.start("gateway-default")
    state = json.loads((max_home / "gateway_state.json").read_text())
...
def test_lifecycle_raises_gateway_not_registered_for_missing_slot(
    ...
    assert "'typo'" in msg
    assert "max profile create typo" in msg
...
def test_s6_log_run_chowns_gateways_parent(s6_scandir, fake_subprocess_run) -> None:
    ...
    parent_chown = 'chown hermes:hermes "$MAX_HOME/logs/gateways"'
    ...
    assert 'chown -R hermes:hermes "$MAX_HOME/logs/gateways"' not in log_text
