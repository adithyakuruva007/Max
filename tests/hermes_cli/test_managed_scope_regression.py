...
def max_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MAX_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(max_home, monkeypatch):
...
def test_env_expansion_in_user_config(max_home, monkeypatch):
    from max_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(max_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_max_dotenv(max_home=str(home))
