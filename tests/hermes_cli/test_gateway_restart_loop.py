...
class TestGatewayLifecyclePattern:
    ...
    def test_max_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "systemctl restart max-gateway",
        "systemctl stop max-gateway.service",
        "systemctl start max-gateway",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "systemctl restart max-meta.service",
        "systemctl restart max-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_max_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.max.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_max_gateway_once.sh). The script must live under
...
class TestGatewaySelfTargetingGuard:
    """Verify hermes gateway stop/restart refuse when _MAX_GATEWAY=1."""
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    Issue #37453: systemctl --user restart max-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart max-gateway", force=True
        ))
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _MAX_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart max-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart max-gateway"]
