...
def test_exec_schtasks_decodes_with_replace_errors(monkeypatch):
    ...
    code, out, err = gateway_windows._exec_schtasks(["/Query", "/TN", "Max_Gateway"])
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda max_home: "")
    monkeypatch.setattr("max_cli.config.get_max_home", lambda: str(max_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "max_cli.main"]
    assert cwd == str(max_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_max_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".max"
        home.mkdir()
...
def test_write_task_script_anchors_cmd_cd_at_max_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    max_home = tmp_path / "max-home"
    max_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(max_home.resolve()))}" in content
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda exe: (exe.replace("python.exe", "pythonw.exe"), r"C:\\Max\\max-agent\\venv", []),
    )

    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Max\\max-agent\\venv\\Scripts\\python.exe",
        r"C:\\Max\\max-agent",
        r"C:\\MaxHome\\profiles\\alice",
        "--profile alice",
    )
...
def test_install_scheduled_task_success_start_now_uses_direct_spawn_not_task_run(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_install_scheduled_task",
        lambda task_name, script_path: (True, "Created Scheduled Task 'Max_Gateway_alice'"),
    )
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated Max gateway install prompt" in out
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "max --profile alice gateway start" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated Max gateway uninstall prompt" in out
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("max_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("max_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `max gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
