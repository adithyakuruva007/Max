...
the SAME provider universe as ``max model`` (the CLI/TUI picker).
...
def test_every_max_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `max model` universe."""
    ...
    assert not missing, (
        "providers shown in `max model` but not configurable in the desktop "
