"""Regression tests for #34107 — Docker UID/GID handling in ensure_max_home.
...
The fix is a ``_chown_to_max_uid`` helper that reads the env vars and
...
class TestResolveMaxUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from max_cli.config import _resolve_max_uid_gid
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("MAX_UID", "not-a-number")
        monkeypatch.setenv("MAX_GID", "911")
        from max_cli.config import _resolve_max_uid_gid
        uid, gid = _resolve_max_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("MAX_UID", "")
        monkeypatch.setenv("MAX_GID", "")
        from max_cli.config import _resolve_max_uid_gid
        uid, gid = _resolve_max_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToMaxUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_max_uid(d)  # must not raise
