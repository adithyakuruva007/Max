...
PROVIDER_ENV_VARS = (
    ...
    "OPENAI_BASE_URL", "MAX_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("max_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("max_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        ...
        max_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: max_home / ".env")
        monkeypatch.setattr(config_module, "get_max_home", lambda: max_home)
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        max_home = tmp_path / ".max"
        max_home.mkdir()
        config_file = max_home / "config.yaml"
    ...
    def test_claude_code_creds_counted_when_max_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when Max has been explicitly configured."""
        import yaml
        from max_cli import config as config_module
        max_home = tmp_path / ".max"
        max_home.mkdir()
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "max_cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
