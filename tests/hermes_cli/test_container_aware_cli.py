...
def container_env(tmp_path, monkeypatch):
    ...
    container_mode = max_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=max-agent\n"
        "exec_user=hermes\n"
        "max_bin=/data/current-package/bin/hermes\n"
    )
    return max_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    assert info["exec_user"] == "max"
    assert info["max_bin"] == "/data/current-package/bin/hermes"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("max_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_max_dev(container_env, monkeypatch):
    """Returns None when MAX_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("MAX_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_max_dev_zero(container_env, monkeypatch):
    """MAX_DEV=0 does NOT trigger bypass — only '1' does."""
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        max_home = Path(tmpdir) / ".max"
        max_home.mkdir()
        (max_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("max_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_max_home": lambda: max_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MAX_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=max-custom\n"
        "exec_user=myuser\n"
        "max_bin=/opt/max/bin/hermes\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "max-custom"
    assert info["exec_user"] == "myuser"
    assert info["max_bin"] == "/opt/max/bin/hermes"
