"""Tests for the per-profile Projects store (max_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Max Agent", folders=["/tmp/hermes"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "max-agent"
    assert proj.name == "Max Agent"
    ...
    # Lookup by slug too.
    assert pdb.get_project(conn, "max-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Max Agent")
    pdb.create_project(conn, name="Max Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["max-agent", "max-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' MAX_HOME.
...
def test_db_path_under_max_home():
    # Resolves under MAX_HOME (set by the autouse isolation fixture).
