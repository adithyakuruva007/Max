...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["max", "--tui", "chat"], "chat"),
        (["max", "-w", "logs"], "logs"),
        (["max", "chat", "hello world"], "chat"),
        (["max", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["max", "-m", "gpt5", "chat"], "chat"),
        (["max", "--model", "gpt5", "chat", "hi"], "chat"),
        (["max", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["max", "-z", "hello world"], None),
        (["max", "-z", "hello", "chat"], "chat"),
        (["max", "--model=gpt5", "chat"], "chat"),     # inline form
        (["max", "--", "chat"], "chat"),               # -- terminator
        (["max", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["max", "some-plugin-cmd"], "some-plugin-cmd"),
        (["max", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["max"],                          # bare → chat
        ["max", "--help"],                # top-level help
        ["max", "-h"],
        ["max", "version"],               # known built-in
        ["max", "logs"],
        ["max", "gateway", "run"],
        ["max", "--tui"],
        ["max", "-w", "--tui"],
        ["max", "chat", "hi"],
        ["max", "help"],                  # accepted built-in-ish
        ["max", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["max", "meet", "join"],          # potential google_meet plugin
        ["max", "honcho", "status"],      # potential memory plugin
        ["max", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``max help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"max_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
