...
def test_loopback_protected_route_accepts_session_token(client_loopback):
    ...
    r = client_loopback.get(
        "/api/sessions",
        headers={"X-Max-Session-Token": web_server._SESSION_TOKEN},
    )
...
def test_loopback_index_injects_session_token(client_loopback):
    ...
    assert "__MAX_SESSION_TOKEN__" in r.text
...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    # bind (June 2026 max-0day hardening). Non-loopback always requires auth.
    ("0.0.0.0",   True,  True),
    ("0.0.0.0",   False, True),
...
def test_start_server_surfaces_nous_skip_reason_when_unconfigured(monkeypatch):
    ...
    'set MAX_DASHBOARD_OAUTH_CLIENT_ID', not 'install a plugin'."""
    from max_cli.dashboard_auth import clear_providers
    ...
    assert "MAX_DASHBOARD_OAUTH_CLIENT_ID" in msg
    assert "nous:" in msg
