...
($INSTALL_DIR), not into the shared $MAX_HOME.

Background (shared-$MAX_HOME bug)
------------------------------------
$MAX_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.max:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$MAX_HOME the two installs would fight over one slot — and the container,
...
``~/.max/max-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$MAX_HOME/\.install_method"', text), (
        "install.sh must not stamp $MAX_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `max update`"
    )
