...
Uses a throwaway MAX_HOME so it never touches ~/.max. No real names.
"""
...
def _fresh_home():
    """Point MAX_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_max_home() at import time)."""
...
# a different get_max_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"MAX_HOME (throwaway): {HOME}")
