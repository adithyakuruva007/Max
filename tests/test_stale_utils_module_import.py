...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.max/max-agent/utils.py)
...
2. ``max update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``max_cli/main.py`` (the ``max update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``max_constants`` after the
