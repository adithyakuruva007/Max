...
``platform="tui"`` / ``chat_id=MAX_SESSION_KEY``, but no component ever
...
class TestCollectKanbanNotifications:
    ...
    def test_profile_scoped_session_reads_the_shared_board(self, tmp_path):
        ...
        max_cli/kanban_db.py module docstring): ``kanban_home()`` anchors on
        ...
        token = set_max_home_override(str(other_profile_home))
        try:
