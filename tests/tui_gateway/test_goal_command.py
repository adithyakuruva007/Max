...
def max_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves MAX_HOME.
...
def test_moa_bare_returns_usage(server, session, max_home):
    _write_moa_config(max_home, """
moa:
...
def test_moa_arg_is_always_one_shot(server, session, max_home):
...
def test_moa_non_preset_returns_one_shot_send(server, session, max_home):
    _write_moa_config(max_home, """
moa:
  default_preset: default
  presets:
