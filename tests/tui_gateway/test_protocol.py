...
def server():
    ...
    # e.g. max_cli.active_sessions would bind the mocked get_max_home
    ...
    with patch.dict("sys.modules", {
        "max_constants": MagicMock(get_max_home=MagicMock(return_value="/tmp/hermes_test")),
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `MAX_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("MAX_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("MAX_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/max-agent-dev": {"name": "max-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "max-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient MAX_HOME
    ...
    # The gateway's own MAX_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `max config set` does)
    ...
    import max_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_max_home", lambda: tmp_path)
    ...
    # Activate midnight, as `max config set display.skin midnight` would.
