...
``MAX_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
def hermes_root(tmp_path, monkeypatch):
    """A temp MAX_HOME root with two named profiles: 'work' and 'other'.

    Pointing MAX_HOME at a dir outside ~/.max makes it the profile ROOT
...
def test_default_profile_add_when_profile_omitted(hermes_root):
    ...
    # Omitted profile → launch/default profile == MAX_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
