    ...
    with patch.dict(
        "sys.modules",
        {
            "max_constants": MagicMock(
                get_max_home=MagicMock(return_value="/tmp/max_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'max-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'max-release' patched"
    }
