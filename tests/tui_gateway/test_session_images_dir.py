...
before ``prompt.submit`` installs the session's profile MAX_HOME override, so
...
def test_two_profiles_are_isolated(tmp_path):
    ...
    home_a = tmp_path / ".max" / "profiles" / "a"
    home_b = tmp_path / ".max" / "profiles" / "b"
