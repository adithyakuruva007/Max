...
guard is covered in ``tests/max_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``max_cli.model_switch.prewarm_picker_cache_async``
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from max_cli.model_switch,
