...
def test_junk_root_never_becomes_an_auto_project():
    # A session whose git root is MAX_HOME (config/state) must not spawn a
    # phantom project; it lands in the Home bucket. A real repo alongside it
    # still groups normally.
    resolve = _resolver(
        {
            "/home/me/.max": ("/home/me/.max", "/home/me/.max"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.max", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.max"
...
def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.max")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.max"},
    )
...
def test_deleted_sibling_worktree_folds_into_parent_home_checkout():
    ...
    resolve = _resolver({"/www/max-agent": ("/www/max-agent", "/www/max-agent")})
    sessions = [
        _session("/www/max-agent", branch="main"),
        _session("/www/max-agent-session-links"),
    ]

    tree = pt.build_tree([], sessions, [], resolve, hydrate=True)
    project = tree["projects"][0]

    assert [p["id"] for p in tree["projects"]] == ["/www/max-agent"]
    assert _lane_ids(project) == ["/www/max-agent::branch::main"]
    main = project["repos"][0]["groups"][0]
    assert main["isMain"] and main["path"] == "/www/max-agent"
