...
import max_state
from max_state import SessionDB, SessionTurnLeaseLostError
    ...
    def flaky_acquire(*args, **kwargs):
        ...
        if attempts["n"] == 1:
            raise sqlite3.OperationalError(
                "database is locked (another Max process held the "
...
def test_non_expired_turn_lease_from_dead_pid_is_reclaimed(
    ...
    monkeypatch.setattr(
        max_state, "psutil", SimpleNamespace(pid_exists=pid_exists)
    )
