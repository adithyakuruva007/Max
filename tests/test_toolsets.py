...
class TestToolsetConsistency:
    ...
    def test_max_platforms_share_core_tools(self):
        """All max-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on max-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["max-cli", "max-telegram", "max-discord", "max-whatsapp", "max-slack", "max-signal", "max-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_max_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("max-whatsapp")

    def test_max_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("max-api-server")
