"""Tests for ACP Registry metadata shipped with Max."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "max-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Max Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/StardustResearch/max-agent"
    assert data["website"].startswith("https://max-agent.stardustresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"max-agent[acp]=={data['version']}"
    assert uvx["args"] == ["max-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"max-agent[acp]=={_pyproject_version()}"
    )
