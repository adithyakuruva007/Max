...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_max_session_id="old"
    )
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootMaxSessionId"] == "s0"
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_max_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousMaxSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"max"}
    assert "sessionProvenance" in meta["max"]
    assert meta["max"]["sessionProvenance"]["currentMaxSessionId"] == "root1"
