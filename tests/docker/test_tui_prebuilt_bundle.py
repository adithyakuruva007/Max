...
package-lock.json. Without MAX_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV MAX_TUI_DIR=/opt/max/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the max user, return stdout."""
    inner = (
        "source /opt/max/.venv/bin/activate && "
        "cd /opt/max && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the max user (UID 10000) so we exercise the same path the
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    """MAX_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$MAX_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/max/ui-tui", (
        f"MAX_TUI_DIR={r.stdout.strip()!r} (expected /opt/max/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from max_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/max/ui-tui')\n"
    ...
    # With MAX_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
