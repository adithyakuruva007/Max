...
Build the real image and verify the actual runtime behavior:
  ...
  1. MAX_GATEWAY_BOOTSTRAP_STATE=running on a fresh volume seeds
...
def test_seeds_running_state_on_blank_volume(
    ...
    """MAX_GATEWAY_BOOTSTRAP_STATE=running on a fresh volume must
    seed gateway_state.json with a valid running state."""
    _start_container(
        built_image, container_name,
        "MAX_GATEWAY_BOOTSTRAP_STATE=running",
    )
...
def test_does_not_clobber_existing_state(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-v", f"{volume}:/opt/data",
         "-e", "MAX_GATEWAY_BOOTSTRAP_STATE=running",
...
def test_no_seed_when_env_unset(
    ...
    """No MAX_GATEWAY_BOOTSTRAP_STATE = no seed file written."""
...
def test_non_running_value_ignored(
    ...
    for bogus in ("stopped", "Running", "1", "true", "starting"):
        ...
        _start_container(
            built_image, name,
            f"MAX_GATEWAY_BOOTSTRAP_STATE={bogus}",
        )
        r = docker_exec_sh(
            name,
            "test -f /opt/data/gateway_state.json && "
...
def _cleanup_bind_mount(built_image: str, container_name: str, host_dir: Path) -> None:
    """Remove root/max-owned files left in a bind-mounted host dir.
...
def test_does_not_seed_auth_json_through_symlink(
    ...
    try:
        ...
        _boot_with_bind_mount(
            built_image, container_name, host_data,
            'MAX_AUTH_JSON_BOOTSTRAP={"api_key":"test"}',
        )
