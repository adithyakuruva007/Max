...
The shim (docker/hermes-exec-shim.sh, installed at /opt/max/bin/hermes)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> max login` would write /opt/data/auth.json as
...
1. ``docker exec <c> max …`` (defaulting to root) gets dropped to the
   max user before the real binary runs.
2. ``docker exec --user max <c> max …`` (already non-root) short-
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    We invoke `max` with a Python-style `-c` shim equivalent — there's no
    ...
    fact that the venv's `max` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `max`
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    invokes `s6-setuidgid max max <args>` which resolves `max`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> max login`` (root) wrote
    ...
    the max UID. We use `max auth list` since that touches the
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the max user "
