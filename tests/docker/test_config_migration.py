...
def test_config_migration_runs_on_boot(
    ...
    docker_config_migrate.py, running as the max user."""
    # Start container
    ...
    # Verify config.yaml is owned by max (migration ran as hermes)
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() == "hermes", (
        f"config.yaml not owned by max (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
