...
Build the real image and verify at runtime:
  ...
  1. /opt/max is not writable by the max user (immutable install tree)
...
def test_install_tree_not_writable_by_hermes(
    ...
    """The max user must not be able to modify /opt/max.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/max as the max user
