...
Build the real image and verify at runtime:
  ...
  1. /opt/max is not writable by the hermes user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and MAX_DISABLE_LAZY_INSTALLS are set
  3. /opt/max/.install_method contains "docker" (code-scoped stamp)
  4. $MAX_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $MAX_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_hermes(
    ...
    """The hermes user must not be able to modify /opt/max.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/max as the hermes user
        "touch /opt/max/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"max user can write to /opt/max (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/max/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"max user can write to /opt/max/.venv: {r.stdout}"
    )


def test_max_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    MAX_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/max/.install_method (code-scoped), NOT in $MAX_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/max/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/max/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $MAX_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        "$MAX_HOME/.install_method is stamped 'docker' - stage2 must "
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $MAX_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $MAX_HOME was not healed on boot: "
        f"{r.stdout}"
    )
