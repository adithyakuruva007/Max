...
symlink to the pip-generated entry point at ``$MAX_BIN`` (i.e.
...
shim's ``exec "$MAX_BIN" "$@"`` then self-recursed and ``hermes`` hung on
every invocation.
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    ``MAX_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
    ...
    shim_path = command_link_dir / "max"
