...
cannot prove belongs to this MAX_HOME.
...
carries no MAX_HOME, so it can never prove home ownership; it only feeds a
...
def profile_env(tmp_path, monkeypatch):
    """Isolated MAX_HOME mirroring tests/max_cli/test_profiles.py."""
...
def _record(pid=424242, start=111222333, home=None, argv=None):
    return {
        "pid": pid,
        "kind": "hermes-gateway",
        "argv": argv or ["python", "-m", "max_cli.main", "gateway", "run"],
        "start_time": start,
        "hermes_home": home,
    }
...
class TestRecordAuthority:
    def test_valid_bound_same_home_record_allows(self, profile_env):
        ...
        with (
            patch(
                ...
                return_value=_record(home=str(profile_env / ".max")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".max" / "gateway.pid",
            ),
            patch(
        ...
        with (
            patch(
                ...
                return_value=_record(
                    home="/home/other/.max/profiles/timothy"
                ),
            ),
            patch(
                "gateway.status._get_pid_path",
            ...
            patch(
                "gateway.status._get_process_hermes_home",
                return_value=profile_env / ".max" / "profiles" / "tim",
            ),
        ):
    ...
    def test_unbound_record_wrong_pid_refuses(self, profile_env):
        ...
        with (
            patch(
                "gateway.status._read_pid_record",
                return_value=_record(pid=999999, home=str(profile_env / ".max")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".max" / "gateway.pid",
    ...
    def test_unbound_record_stale_start_time_refuses(self, profile_env):
        ...
        with (
            patch(
                "gateway.status._read_pid_record",
                return_value=_record(start=1, home=str(profile_env / ".max")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".max" / "gateway.pid",
...
class TestArgvConsistencyCheck:
    ...
    def test_prefix_collision_is_not_a_conflict(self, profile_env):
        ...
        tim_home = Path("/home/x/.max/profiles/tim")
        # Foreign target advertising timothy — NOT ours.
        assert (
            conflict("python -m max_cli.main --profile timothy gateway run", tim_home)
            is True
        )
        # Our own exact name stays consistent.
        assert (
            conflict("python -m max_cli.main --profile tim gateway run", tim_home)
            is False
        )
        assert (
            conflict("python -m max_cli.main -p tim gateway run", tim_home)
            is False
        )

    def test_explicit_home_flag_exact_compare(self, profile_env):
        """MAX_HOME= on the argv compares path-exactly, not by prefix."""
        from gateway.run import (
            _looks_like_profile_conflict_from_cmdline as conflict,
        )
        ...
        assert (
            conflict(
                "python -m max_cli.main MAX_HOME=/home/x/.max/profiles/timothy gateway run",
                tim_home,
            )
            is True
        )
        assert (
            conflict(
                "python -m max_cli.main --hermes-home /home/x/.max/profiles/tim/ gateway run",
                tim_home,
            )
    ...
    def test_consistency_contradiction_refuses_even_with_agreeing_record(
        ...
        with (
            ...
            patch(
                ...
                return_value="python -m max_cli.main --profile other-profile gateway run",
            ),
            patch(
                "gateway.status._get_process_hermes_home",
                return_value=profile_env / ".max",
