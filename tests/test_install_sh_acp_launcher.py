"""`setup_path()` must also install a `max-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `max-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report Max as not installed. `setup_path()` therefore
writes a `max-acp` launcher next to the `max` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/max-acp".*?'
    r'log_success "Installed max-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the max-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    script = (
        "set -e\n"
        f"MAX_BIN={hermes_bin}\n"
        f"MAX_ENTRYPOINT={entrypoint}\n"
    ...
    return command_link_dir / "max-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `max-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "max-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/max-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/max-acp must be replaced with a regular file"
    )


# ---------------------------------------------------------------------------
# max-agent launcher regression (#74819)
# ---------------------------------------------------------------------------

MAX_AGENT_BLOCK = re.compile(
    r'(rm -f "\$command_link_dir/max-agent".*?'
    r'log_success "Installed max-agent launcher[^\n]*\n)',
    re.S,
)


def _extract_hermes_agent_shim_block() -> str:
    match = MAX_AGENT_BLOCK.search(INSTALL_SH.read_text(encoding="utf-8"))
    assert match, (
        "could not locate the max-agent launcher block in scripts/install.sh — "
        "if it was renamed, update this test with it"
    )
    return match.group(1)
...
def _run_hermes_agent_block(tmp_path: Path, use_venv: str) -> Path | None:
    """Execute the extracted max-agent block with the env vars setup_path() sets."""
    if use_venv == "false":
        # --no-venv: max-agent is NOT installed by this block (handled
    ...
    assert result.returncode == 0, (
        f"max-agent shim block failed:\nstdout={result.stdout}\nstderr={result.stderr}"
    )
    return command_link_dir / "max-agent"


def test_venv_install_writes_executable_hermes_agent_launcher(tmp_path):
    """venv install must write a user-executable max-agent launcher."""
    ...
    assert "unset PYTHONPATH" in text
    assert "unset PYTHONHOME" in text
    assert "run_agent.py" in text, "max-agent must dispatch to run_agent.py"


def test_hermes_agent_launcher_cleanup_on_uninstall(tmp_path):
    """uninstall.remove_wrapper_script() must remove max-agent alongside
    max and max-acp."""
    from max_cli.uninstall import remove_wrapper_script

    # Simulate a max-agent wrapper in the user-local location
    local_shim = tmp_path / ".local" / "bin" / "max-agent"
    ...
    assert local_shim in removed, "local max-agent wrapper must be removed"
