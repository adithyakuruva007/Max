...
def test_session_create_rejects_at_active_session_limit(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert second["error"]["message"] == (
            "Max is at the active session limit (1/1). "
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("MAX_VOICE", "1")
    monkeypatch.delenv("MAX_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # _get_platform_tools (a non-configurable platform toolset in max-cli's
...
def test_resolve_model_strips_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/max-test "}}
    )

    assert server._resolve_model() == "nous/max-test"
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("MAX_MODEL", "nous/max-test")
    monkeypatch.setenv("MAX_TUI_PROVIDER", "nous")
    monkeypatch.delenv("MAX_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/max-test", "nous")
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    assert server._resolve_startup_runtime() == ("nous/max-test", None)
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_max_home", tmp_path)
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors MaxCLI._manual_compress's
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".max" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.max/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".max" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.max/desktop-attachments/outside.txt"
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.max/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_session_info_includes_install_warning_for_pip(monkeypatch):
    ...
    monkeypatch.setattr("max_cli.config.detect_install_method", lambda: "pip")
...
def test_session_info_omits_install_warning_for_git(monkeypatch):
    monkeypatch.setattr("max_cli.config.detect_install_method", lambda: "git")
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_max_home
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "max_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_session_save_writes_under_max_home_with_system_prompt(monkeypatch, tmp_path):
    ...
    Regression: the gateway handler wrote ``max_conversation_*.json`` to
    ...
    assert not list(work.glob("max_conversation_*.json"))
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from cli._max_home, which
    # is captured at import time — patch it directly (set_max_home_override
    # does NOT affect this snapshot).
    monkeypatch.setattr(cli, "_max_home", tmp_path)
