"""Guard: Max-owned subprocesses must not resolve managed runtimes by bare PATH.

Max installs runtimes for itself — ``uv`` at ``$MAX_HOME/bin/uv``, Node at
``$MAX_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Max's own
...
* when a system copy also exists, the one Max does not own wins — which is
...
# Runtimes Max provisions into MAX_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Max-owned subprocess code: plugins ship their own
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Max-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$MAX_HOME/bin (uv) and $MAX_HOME/node are not on an "
        ...
        "  node/npm -> max_constants.find_node_executable()\n"
        "  PATH env -> max_constants.iter_hermes_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(max_constants, helper))


def test_managed_uv_helpers_exist():
    from max_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
