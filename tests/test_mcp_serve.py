"""
Tests for mcp_serve — Max MCP server.
...
def _isolate_max_home(tmp_path, monkeypatch):
    """Redirect MAX_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking max_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <MAX_HOME>/state.db for its mtime gate; the autouse
        # fixture points MAX_HOME at tmp_path.
        db_path = tmp_path / "state.db"
