...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    fake_max_home = tmp_path / "max_test"
    fake_max_home.mkdir()
    (fake_max_home / "sessions").mkdir()
    (fake_max_home / "cron").mkdir()
    (fake_max_home / "memories").mkdir()
    (fake_max_home / "skills").mkdir()
    monkeypatch.setenv("MAX_HOME", str(fake_max_home))
...
@pytest.fixture(autouse=True)
def _isolate_max_home(_hermetic_environment):
...
# environment and finds the developer's live ``max-gateway`` process
...
#    ``check_output`` reject any ``systemctl ... <verb> max-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart max-gateway"``,
    ...
    def _matches_max_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
    ...
    def _is_blocked_systemctl(cmd) -> bool:
        ...
        if "systemctl" not in cmd_str:
            return False
        if not _matches_max_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch max-themed patterns + a
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live max-gateway systemd unit. Mock "
        ...
        if "update" in low and (
            ...
            ("max" in low and "update" in low.split())
            or
