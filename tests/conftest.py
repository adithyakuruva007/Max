"""Shared fixtures for the max-agent test suite.
...
2. **Isolated MAX_HOME.** MAX_HOME points to a per-test tempdir so
   code reading ``~/.max/*`` via ``get_max_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".max"`` instead of the canonical
   ``get_max_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No MAX_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox MAX_HOME before ANY test module is imported ──────────────────
# `max_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_max_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.max/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes MAX_HOME, but fixtures run
...
# the REAL Max root — capture it BEFORE the sandbox rewires MAX_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.max (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("MAX_KANBAN_HOME", "").strip()
_PRE_SANDBOX_MAX_HOME = os.environ.get("MAX_HOME", "")


def _hermes_home_points_at_production(value: str) -> bool:
    """True when a pre-set MAX_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    MAX_HOME=~/.max``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``max_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.max`` — the escape vector that landed pytest
    ...
    MAX_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".max").resolve()
    except Exception:
        return True
...
if _hermes_home_points_at_production(os.environ.get("MAX_HOME", "")):
    _SESSION_MAX_HOME = tempfile.mkdtemp(prefix="hermes-test-home-")
    os.environ["MAX_HOME"] = _SESSION_MAX_HOME
    atexit.register(shutil.rmtree, _SESSION_MAX_HOME, True)
...
# like a real CLI") — which used to disarm max_state's live-DB guard in
# the child at the same moment the child lost the MAX_HOME redirect.
# MAX_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# max_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export MAX_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["MAX_TEST_ISOLATION"] = os.environ.get("MAX_HOME", "") or "1"

#: MAX_HOME as it stood when conftest was imported - i.e. before any test
...
MAX_HOME_AT_CONFTEST_IMPORT = os.environ.get("MAX_HOME", "")
...
# MAX_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_MAX_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``max_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Max agent session inherits them and
    # max_constants home-resolution helpers prefer them over monkeypatched
    ...
    "MAX_MAX_ITERATIONS",
    "MAX_SESSION_PLATFORM",
    "MAX_SESSION_CHAT_ID",
    "MAX_SESSION_CHAT_NAME",
    "MAX_SESSION_CHAT_TYPE",
    "MAX_SESSION_THREAD_ID",
    "MAX_SESSION_SOURCE",
    "MAX_SESSION_KEY",
    "MAX_GATEWAY_SESSION",
    "MAX_CRON_SESSION",
    ...
    "MAX_EXEC_ASK",
    "MAX_HOME_MODE",
    "MAX_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.max/kanban.db instead of the per-test MAX_HOME.
    "MAX_KANBAN_DB",
    "MAX_KANBAN_BOARD",
    "MAX_KANBAN_HOME",
    "MAX_KANBAN_WORKSPACES_ROOT",
    "MAX_KANBAN_LOGS_ROOT",
    "MAX_KANBAN_TASK",
    "MAX_KANBAN_WORKSPACE",
    "MAX_KANBAN_RUN_ID",
    "MAX_KANBAN_CLAIM_LOCK",
    "MAX_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "MAX_DASHBOARD_OAUTH_CLIENT_ID",
    "MAX_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and MAX_HOME to per-test tempdirs so code that
    reads ``~/.max/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral MAX_* vars that could change test semantics.
    for name in _MAX_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("MAX_HONCHO_HOST", "hermes")

    # 3. Redirect MAX_HOME to a per-test tempdir. Code that reads
    #    ``~/.max/*`` via ``get_max_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.max/*`` via
    #    ``Path.home() / ".max"`` instead of ``get_max_home()``
    ...
    monkeypatch.setenv("MAX_HOME", str(fake_hermes_home))
    ...
    # max_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("MAX_TEST_ISOLATION", str(fake_hermes_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("MAX_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. max_state computes ``DEFAULT_DB_PATH = get_max_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from max_state import ...``) that
    ...
    max_state_mod = sys.modules.get("max_state")
    if max_state_mod is not None and hasattr(max_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            max_state_mod, "DEFAULT_DB_PATH", fake_hermes_home / "state.db"
        )
    ...
    #    ~/.max/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import max_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path MAX_HOME can't leak forward. Paths
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from max_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.max. This autouse
...
# allow-list because test-level fixtures legitimately move MAX_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session MAX_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``MAX_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Max root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_MAX_HOME and not _hermes_home_points_at_production(
        _PRE_SANDBOX_MAX_HOME
    ):
        # MAX_HOME was genuinely set to a CUSTOM root before the sandbox
    ...
    # No pre-existing MAX_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".max").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.max`` captured at import time. Hermetic tests that legitimately
    move MAX_HOME to sibling tempdirs are unaffected.

    Only patches when ``max_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("max_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import max_cli.kanban_db`
...
# ``max_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Max root. This fixture wires the
# test-side knobs:
...
#     installs where MAX_HOME is not ~/.max) into the guard's
...
# so subprocess children that import max_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("max_state")
    ...
    if _PRE_SANDBOX_MAX_HOME and not _hermes_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_MAX_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Max home override redirects every later
    # ``get_max_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from max_constants import get_max_home_override, set_max_home_override

        if get_max_home_override() is not None:
            set_max_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal max config dict suitable for unit tests."""
    return {
...
# correlated with a ``tests/max_cli/`` pytest run starting up.
#
...
def _wal_is_usable() -> bool:
    """True when Max will actually put a database into WAL mode here.

    Max refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Max can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Max managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``max_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_max_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_hermes_home`` fixture
    redirects ``MAX_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.max/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``max_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["MAX_VOICE_TTS"] = "1"``. The
...
#      ``max_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``max_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``max_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``max_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Max runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.max``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Max falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Max uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    def _matches_hermes_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _MAX_TOKENS)
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m max_cli.main".
                if (
                    "hermes" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        # Block any subprocess that would run `max update` (or the
        # equivalent `python -m max_cli.main update`).  These commands
        ...
        if "update" in low and (
            # max update / max update --gateway / setsid bash -c ... max update
            ("hermes" in low and "update" in low.split())
            or
            # python -m max_cli.main update --gateway
            ("max_cli" in low and "update" in low.split())
            or
            # venv/bin/max update  (absolute path variant used in tests)
            (".venv/bin/hermes" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`max update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_MAX_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import max_cli.voice as _voice
    except Exception:
