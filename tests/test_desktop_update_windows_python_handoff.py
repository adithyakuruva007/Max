...
`scripts/desktop-update/windows.ps1` drives `max update` for the in-app
...
Start Menu shortcut points at a `Max.exe` that no longer exists.

Driving the update as `python.exe -m max_cli.main update` puts the inherited
...
hand-off. The invariant it guards is that every `Invoke-MaxStep` call site
...
def _handoff_source() -> str:
    ...
    fixture runs a synthetic PowerShell step through ``Invoke-MaxStep`` to
...
def test_invoke_hermes_step_calls_drive_python_not_the_shim() -> None:
    ...
    invocations = re.findall(r"Invoke-MaxStep\s+(\$\w+)", source)
    assert invocations, (
        "Expected at least one Invoke-MaxStep call in "
    ...
    assert not offenders, (
        "Every Invoke-MaxStep call in scripts/desktop-update/windows.ps1 "
...
def test_update_invocation_uses_module_entrypoint() -> None:
    ...
    assert '@("-m", "max_cli.main", "update"' in source, (
        "The update step must invoke `python.exe -m max_cli.main update ...` "
        "so the inherited image handle lands on python.exe, which uv never has "
        "to replace."
    )
    assert (
        '@("-m", "max_cli.main", "desktop", "--force-build", "--build-only")'
        in source
    ), (
        "The desktop rebuild step must also go through "
        "`python.exe -m max_cli.main desktop ...` for the same reason."
    )
...
def test_update_no_longer_invokes_the_hermes_exe_shim() -> None:
    ...
    assert "Invoke-MaxStep $hermesExe" not in source, (
        "scripts/desktop-update/windows.ps1 still invokes the update through "
        "the hermes.exe shim (`Invoke-MaxStep $hermesExe`). That is the "
