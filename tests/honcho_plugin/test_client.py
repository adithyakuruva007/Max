...
from max_cli.profiles import _get_default_max_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "max"
        assert config.workspace_id == "max"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.max/honcho.json
        monkeypatch.setenv("MAX_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"max": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="max-agent"
        ):
            result = config.resolve_session_name("/home/user/max-agent/subdir")
        assert result == "max-agent"
...
class TestResolveConfigPath:
    def test_prefers_max_home_when_exists(self, tmp_path):
        max_home = tmp_path / "max"
        max_home.mkdir()
        local_cfg = max_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: MAX_HOME points at ~/.max/profiles/<name>, so
        # _get_default_max_home() must resolve back to ~/.max — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".max"
        ...
        assert _get_default_max_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_max_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".max".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".max"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "max_coder"
        assert profile_host_key("default") == "max"

    def test_default_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("MAX_HONCHO_HOST", None)
            os.environ.pop("MAX_HOME", None)
            assert resolve_active_host() == "max"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"MAX_HONCHO_HOST": "hermes.coder"}):
            assert resolve_active_host() == "hermes.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MAX_HONCHO_HOST", None)
            with patch("max_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "max_coder"
    ...
    def test_custom_profile_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MAX_HONCHO_HOST", None)
            with patch("max_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "max"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MAX_HONCHO_HOST", None)
            # Temporarily remove max_cli.profiles to simulate import failure
            saved = sys.modules.get("max_cli.profiles")
            sys.modules["max_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "max"
            finally:
                if saved is not None:
                    sys.modules["max_cli.profiles"] = saved
                else:
                    sys.modules.pop("max_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="max_coder")
        assert config.host == "max_coder"
        assert config.workspace_id == "max"  # shared workspace
        assert config.ai_peer == "max_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="max")
        assert config.host == "max"
        assert config.workspace_id == "max"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config = HonchoClientConfig.from_global_config(
            host="max_coder", config_path=config_file,
        )
        assert config.host == "max_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "max_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="max_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "max_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        assert config.host == "max_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "max_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"max": {"enabled": True, "aiPeer": "max"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"max": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_max_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="max",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("max_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_max_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="max",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("max_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
