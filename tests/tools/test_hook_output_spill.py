...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_user_overrides_are_respected(self):
        ...
        with patch("max_cli.config.load_config", return_value=user_cfg):
            cfg = hos.get_spill_config()
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("max_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="max-spill-test-")
    ...
    def test_session_id_with_path_separators_is_sanitised(self):
        ...
        self.assertFalse(Path("/etc/passwd-max-test").exists())
    ...
    def test_default_directory_uses_max_home(self):
        """When no directory override, spill under MAX_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="max-home-")
        try:
            with patch.dict(os.environ, {"MAX_HOME": test_home}):
                # Also patch get_max_home to the env var to mirror production.
            ...
            # ~/.max/hook_outputs depending on get_max_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.max/hook_outputs/sess")),
            ]
