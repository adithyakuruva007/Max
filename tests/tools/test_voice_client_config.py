...
Real config files in a temp MAX_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated MAX_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".max"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "MAX_LOCAL_STT_LANGUAGE",
    ):
