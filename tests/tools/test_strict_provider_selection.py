"""Strict tool-provider selection: the `max tools` choice always wins.
...
selection and pointing at `max tools`.
...
MANAGED = SimpleNamespace(
    ...
    gateway_origin="https://gateway.stardustresearch.com",
)
...
class TestImageFalStrictSelection:
    ...
    def test_nous_selection_unentitled_raises_selection_error(self):
        ...
        assert "max tools" in str(exc.value)
...
class TestSttStrictSelection:
    def test_nous_selection_beats_direct_openai_key(self):
        ...
        assert base_url.startswith("https://gateway.stardustresearch.com")
