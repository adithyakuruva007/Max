...
def test_max_provider_subclass_exists():
    """MaxMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _MAX_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _MAX_PROVIDER_CLS is not None
    assert issubclass(_MAX_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_max_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns MaxMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _MAX_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("MAX_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _MAX_PROVIDER_CLS is not None
    assert isinstance(provider, _MAX_PROVIDER_CLS)
    assert provider._max_server_name == "srv"
