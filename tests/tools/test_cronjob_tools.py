...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `max-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.max/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.max/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("MAX_INTERACTIVE", false_like_value)
        monkeypatch.delenv("MAX_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("MAX_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["MAX_INTERACTIVE", "MAX_GATEWAY_SESSION", "MAX_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("MAX_INTERACTIVE", "MAX_GATEWAY_SESSION", "MAX_EXEC_ASK"):
...
class TestResolveModelOverride:
    ...
    def test_keeps_bare_custom_when_a_named_entry_exists(self, monkeypatch):
        import max_cli.runtime_provider as rp_mod
