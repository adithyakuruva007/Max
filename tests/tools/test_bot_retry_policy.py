...
def _is_max_cli(argv) -> bool:
    """Match the delivery CLI by basename — local_delivery_command may
    resolve the venv-relative max next to the interpreter (#93590)."""
...
def _transport_calls(calls):
    ...
    return [argv for argv in calls if argv and _is_max_cli(argv)]
