...
class TestWriteDenyExactPaths:
    ...
    def test_max_env(self):
        # ``.env`` under the active MAX_HOME (profile-aware, not just
        # ``~/.max``) must be write-denied. The hermetic test conftest
        # points MAX_HOME at a tempdir — resolve via get_max_home()
        # to match the denylist.
        from max_constants import get_max_home
        path = str(get_max_home() / ".env")
        assert _is_write_denied(path) is True

    def test_max_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.max/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "max_root"
        ...
        # Sanity check: MAX_HOME does point to the profile dir, not the root.
        from max_constants import get_max_home, get_default_max_root
        assert get_max_home() == profile_home
        assert get_default_max_root() == root
...
class TestWriteAllowed:
    ...
    def test_max_control_files_requested_writable(self):
        from max_constants import get_max_home
