...
def _restore_tool_modules():
    original_max_home = os.environ.get("MAX_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_max_home is None:
            os.environ.pop("MAX_HOME", None)
        else:
            os.environ["MAX_HOME"] = original_max_home
        _reset_modules(("tools", "max_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    max_cli = types.ModuleType("max_cli")
    max_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["max_cli"] = max_cli
    max_home = tmp_path / "max-home"
    os.environ["MAX_HOME"] = str(max_home)
    sys.modules["max_cli.config"] = types.SimpleNamespace(
        get_max_home=lambda: max_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="max-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": max_home / "modal_snapshots.json",
