...
was gated on ``[ -f "$MAX_HOME/.env" ]`` while the first-boot seed that
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Max loads $MAX_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/max/.env.example -> $MAX_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $MAX_HOME/.env from it on first boot"
    )
