...
@pytest.fixture
def isolated_max_home(tmp_path, monkeypatch):
...
class TestCredentialPoolSeedsFromDotEnv:
    ...
    load_max_dotenv, the credential pool must still discover it.
    """

    def test_deepseek_key_from_dotenv_only(self, isolated_max_home):
        """Key in .env but not os.environ → _seed_from_env adds a pool entry."""
        _write_env_file(isolated_max_home, DEEPSEEK_API_KEY="sk-dotenv-only-12345")
    ...
    def test_openrouter_key_from_dotenv_only(self, isolated_max_home):
        """OpenRouter path has its own branch — verify it also reads .env."""
        _write_env_file(isolated_max_home, OPENROUTER_API_KEY="sk-or-dotenv-abc")
    ...
    def test_empty_dotenv_no_entries(self, isolated_max_home):
    ...
    def test_dotenv_wins_over_stale_os_environ(self, isolated_max_home, monkeypatch):
        ...
        """
        _write_env_file(isolated_max_home, DEEPSEEK_API_KEY="sk-dotenv-fresh")
...
class TestAuthResolvesFromDotEnv:
    ...
    def test_key_from_dotenv_only(self, isolated_max_home):
        """Key in .env but not os.environ → _resolve returns it with the env var source."""
        _write_env_file(isolated_max_home, DEEPSEEK_API_KEY="sk-dotenv-resolve-789")
        assert "DEEPSEEK_API_KEY" not in os.environ
    ...
    def test_dotenv_wins_over_stale_os_environ_on_resolve(
        self, isolated_max_home, monkeypatch
    ):
        ...
        """
        _write_env_file(isolated_max_home, DEEPSEEK_API_KEY="dotenv-fresh-deepseek")
    ...
    def test_get_anthropic_key_prefers_dotenv_over_stale_os_environ(
        ...
        """
        _write_env_file(isolated_max_home, ANTHROPIC_API_KEY="dotenv-fresh-anthropic")
...
class TestAuthCredentialPoolFallback:
    ...
    def test_credential_pool_fallback_structure(self, isolated_max_home):
    ...
    def test_credential_pool_empty_returns_empty(self, isolated_max_home):
    ...
    def test_env_var_takes_priority_over_pool(self, isolated_max_home, monkeypatch):
    ...
    def test_dotenv_takes_priority_over_pool(self, isolated_max_home):
        """Key in .env beats credential pool — pool only fires when both env sources are empty."""
        _write_env_file(isolated_max_home, DEEPSEEK_API_KEY="sk-dotenv-priority-xyz")
        assert "DEEPSEEK_API_KEY" not in os.environ
...
class TestAnthropicEnvAuthTypeClassification:
    ...
    def test_admin_key_classified_as_api_key(self, isolated_max_home):
        ...
        _write_env_file(isolated_max_home, ANTHROPIC_API_KEY="sk-ant-admin-fake-12345")
        entry = self._seed("ANTHROPIC_API_KEY", "sk-ant-admin-fake-12345")
        assert entry.auth_type == AUTH_TYPE_API_KEY

    def test_standard_api_key_classified_as_api_key(self, isolated_max_home):
        """sk-ant-api- key → AUTH_TYPE_API_KEY (unchanged behaviour)."""
        from agent.credential_pool import AUTH_TYPE_API_KEY
        _write_env_file(isolated_max_home, ANTHROPIC_API_KEY="sk-ant-api-fake-12345")
