...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, max accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, max accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    PID 1 and max would run without zombie reaping.
    """
    ...
    assert "/opt/max/docker/entrypoint-dispatch.sh" in entrypoint_line, (
    ...
    assert "exec /init /opt/max/docker/main-wrapper.sh" in dispatcher, (
