...
class TestCacheDirectoryMounts:
    ...
    def test_empty_hermes_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.max/attachments" in container_paths
        assert "/root/.max/images" in container_paths
        assert "/root/.max/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``MAX_HOME/images``, not
        ...
        assert "/root/.max/images" in by_container
        assert by_container["/root/.max/images"] == str(hermes_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/root/.max/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        assert mapped == "/root/.max/cache/images/x.png"
...
class TestToAgentVisiblePathPerBackend:
    ...
    Max cache — not just docker — and skips the ones where the host path
    ...
    def test_docker_maps_to_root_hermes(self, tmp_path, monkeypatch):
        ...
        assert to_agent_visible_cache_path(staged) == "/root/.max/attachments/drop.zip"

    def test_ssh_maps_to_tilde_hermes(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.max/attachments/drop.zip"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — MAX_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping MAX_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"MAX_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.max/google_token.json" in paths
        assert "/root/.max/.env" not in paths
