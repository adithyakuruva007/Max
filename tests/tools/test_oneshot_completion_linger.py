...
A Bot Mode agent invoked as a short-lived ``max -p <bot> chat -Q
...
``max_cli.oneshot``) a bounded linger over every tracked
...
def test_config_reader_falls_back_when_config_unreadable(monkeypatch):
    ...
    monkeypatch.setattr(
        "max_cli.config.read_raw_config", _boom, raising=False
    )
...
def test_oneshot_module_lingers_before_agent_close():
    """max_cli/oneshot.py must wait for pending completions before
    agent.close() (which kill_all()s the task's processes)."""
    src = (REPO_ROOT / "max_cli" / "oneshot.py").read_text(encoding="utf-8")
...
def _run_e2e_parent(tmp_path, *, linger: bool) -> Path:
    ...
    env = dict(os.environ)
    env.setdefault("MAX_HOME", str(tmp_path / "hermes_home"))
    proc = subprocess.run(
