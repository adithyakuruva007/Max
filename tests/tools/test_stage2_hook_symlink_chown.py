...
def _chown_max_tree_function(text: str) -> str:
...
def _run_helper(
    ...
    log_path: Path,
    *,
    max_home: Path | None = None,
) -> subprocess.CompletedProcess[str]:
    shell = shutil.which("sh")
    if shell is None:
        pytest.skip("sh not available")
    max_home = target if max_home is None else max_home
    script = (
        "set -eu\n"
        f'MAX_HOME="{max_home}"\n'
        f"{_chown_max_tree_function(text)}\n"
        f'chown() {{ printf "%s\\n" "$*" >> "{log_path}"; }}\n'
        f'chown_max_tree "{target}"\n'
    )
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "max-home"
    try:
...
def test_stage2_uses_symlink_safe_helper_for_max_home_trees(stage2_text: str) -> None:
    assert 'chown_max_tree "$MAX_HOME/$sub"' in stage2_text
    assert 'chown_max_tree "$MAX_HOME/profiles"' in stage2_text
    assert 'chown_max_tree "$MAX_HOME/cron"' in stage2_text
...
def test_stage2_skips_top_level_chown_for_symlinked_max_home(
    stage2_text: str,
) -> None:
