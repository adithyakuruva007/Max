...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad MAX_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: MAX_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_max_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "MAX_HOME": "/h", "MAX_PROFILE": "p",
        "MAX_CONFIG": "/c.yaml", "MAX_ENV": "/e",
        # other MAX_* → dropped (broad prefix removed)
        "MAX_BASE_URL": "https://x", "MAX_INTERACTIVE": "1",
        "MAX_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("MAX_HOME", "MAX_PROFILE", "MAX_CONFIG", "MAX_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "MAX_BASE_URL", "MAX_INTERACTIVE", "MAX_KANBAN_DB",
...
def test_env_scrub_logs_dropped_max_vars(caplog):
    """Dropping a non-allowlisted, non-secret MAX_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset MAX_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "MAX_HOME": "/h",          # allowlisted → kept, not logged
        "MAX_BASE_URL": "https://x",   # dropped → logged
        "MAX_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "MAX_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "MAX_HOME" in out and "PATH" in out
    assert "MAX_BASE_URL" not in out and "MAX_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "MAX_BASE_URL" in msgs and "MAX_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "MAX_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped MAX_* vars."""
    import logging
