...
class TestBrowserConsole:
    ...
    def test_restrict_evaluate_reads_browser_config(self):
        ...
        with patch("max_cli.config.read_raw_config", return_value={"browser": {"restrict_evaluate": "true"}}):
            assert _restrict_browser_evaluate() is True
        with patch("max_cli.config.read_raw_config", return_value={"browser": {"restrict_evaluate": False}}):
...
class TestBrowserVisionConfig:
    ...
    def test_browser_vision_uses_configured_temperature_and_timeout(self, tmp_path):
        ...
        with (
            patch("max_constants.get_hermes_dir", return_value=shots_dir),
