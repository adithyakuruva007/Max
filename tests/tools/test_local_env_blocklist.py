...
See: https://github.com/StardustResearch/max-agent/issues/1002
See: https://github.com/StardustResearch/max-agent/issues/1264
"""
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_max_bin_injection(self):
        """These tests assert the sane-path merge in isolation. Disable the
        max-install-dir prepend (a separate concern, covered by
        TestMaxBinDirOnPath) so a real ``hermes`` on the test runner's PATH
...
class TestMaxBinDirOnPath:
    ...
    _resolve_max_bin_dir / _prepend_max_bin_dir.
    """
    ...
    def test_resolves_via_which(self, monkeypatch):
        ...
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/max/bin/hermes" if name == "max" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/max/bin")
        assert local_mod._resolve_max_bin_dir() == "/opt/max/bin"
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "max").write_text("#!/bin/sh\n")
        ...
        assert local_mod._resolve_max_bin_dir() == str(venv_bin)
    ...
    def test_returns_none_when_unresolvable(self, monkeypatch):
        ...
        assert local_mod._resolve_max_bin_dir() is None

    def test_prepend_adds_missing_dir_at_front(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._MAX_BIN_DIR = "/opt/max/bin"
        out = local_mod._prepend_max_bin_dir("/usr/bin:/bin")
        assert out.split(os.pathsep)[0] == "/opt/max/bin"
    ...
    def test_prepend_is_idempotent(self, monkeypatch):
        ...
        self._reset_cache()
        local_mod._MAX_BIN_DIR = "/opt/max/bin"
        once = local_mod._prepend_max_bin_dir("/usr/bin:/bin")
        twice = local_mod._prepend_max_bin_dir(once)
        assert twice == once
        assert once.split(os.pathsep).count("/opt/max/bin") == 1

    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._MAX_BIN_DIR = None
        assert local_mod._prepend_max_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_max_bin_dir(self, monkeypatch):
...
class TestMaxInternalDynamicSecrets:
    ...
    ``_is_max_internal_secret`` is the single source of truth; every spawn
    path (``_sanitize_subprocess_env``, ``_make_run_env``,
    ``max_subprocess_env``, Docker forward filter, ``env_passthrough``)
    consults it. These tests exercise the terminal execute path + predicate.
    """

    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local import _is_max_internal_secret
        assert _is_max_internal_secret("AUXILIARY_VISION_API_KEY")
        assert _is_max_internal_secret("AUXILIARY_WEB_EXTRACT_API_KEY")
        assert _is_max_internal_secret("AUXILIARY_APPROVAL_API_KEY")
        # plugin-registered task names are covered by the pattern
        assert _is_max_internal_secret("AUXILIARY_MY_PLUGIN_TASK_API_KEY")

    def test_predicate_matches_auxiliary_base_url(self):
        from tools.environments.local import _is_max_internal_secret
        assert _is_max_internal_secret("AUXILIARY_VISION_BASE_URL")
        assert _is_max_internal_secret("AUXILIARY_COMPRESSION_BASE_URL")

    def test_predicate_matches_gateway_relay_auth(self):
        from tools.environments.local import _is_max_internal_secret
        assert _is_max_internal_secret("GATEWAY_RELAY_SECRET")
        assert _is_max_internal_secret("GATEWAY_RELAY_DELIVERY_KEY")
        assert _is_max_internal_secret("GATEWAY_RELAY_SESSION_TOKEN")
    ...
    def test_predicate_allows_auxiliary_non_secrets(self):
        ...
        from tools.environments.local import _is_max_internal_secret
        assert not _is_max_internal_secret("AUXILIARY_VISION_PROVIDER")
        assert not _is_max_internal_secret("AUXILIARY_VISION_MODEL")
        assert not _is_max_internal_secret("GATEWAY_RELAY_URL")
        assert not _is_max_internal_secret("GATEWAY_RELAY_PLATFORMS")
        assert not _is_max_internal_secret("GATEWAY_RELAY_ID")  # not a secret suffix
        # unrelated vars pass through
        assert not _is_max_internal_secret("PATH")
        assert not _is_max_internal_secret("MY_APP_KEY")
