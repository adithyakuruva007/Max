...
Verifies that Max-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Max secrets.

See: https://github.com/NousResearch/max-agent/issues/1002
See: https://github.com/NousResearch/max-agent/issues/1264
"""
...
from tools.environments.local import (
    LocalEnvironment,
    _MAX_PROVIDER_ENV_BLOCKLIST,
    _MAX_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.max/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Max inference secret
        ...
        models`` run inside a Max terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _MAX_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Max-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "MAX_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
    ...
    def test_bare_hermes_resolves_from_sanitized_subprocess_path(self):
        """Cron children can resolve Max even when the gateway PATH cannot."""
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _MAX_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_MAX_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_MAX_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_MAX_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_MAX_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the Max venv path instead of the project's own ``.venv`` —
    silently clobbering the Max environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Max venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.max/max-agent/venv",
        })
...
def _physical_repo_root(tmp_path: Path) -> Path:
    ...
    physical_root = tmp_path / "physical-home" / "max-agent"
    physical_root.mkdir(parents=True)
    return physical_root


class TestPythonpathSelectiveStrip:
    """PYTHONPATH Max-owned entry stripping (#74817).

    The Desktop Electron app injects the Max repo root and the Max
    ...
    entries Max itself owns (repo root, own venv site-packages),
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact Max-owned entries are removed; everything else survives
        ...
        cannot silently pass), duplicate Max entries, all-owned input
        (PYTHONPATH key removed), and mixed user/Max ordering with an
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven Max-owned is preserved byte-for-byte.
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the Max venv
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for Max-owned entries.
        ...
        Max-owned — the critical invariant is that user Windows paths
        (including site-packages paths for another Python version) are
        never destroyed.  On a real Windows host, Path splits on
        backslashes and Max venv site-packages entries are stripped
        by the same Max-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local import _strip_hermes_owned_pythonpath
        import sys
        ...
        pyver = f"python{sys.version_info[0]}.{sys.version_info[1]}"
        hermes_win = f"C:\\\\Users\\\\u\\\\.max\\\\max-agent\\\\venv\\\\lib\\\\{pyver}\\\\site-packages"
        user_win = "D:\\\\user\\\\lib"
        env = {
        ...
        # the Max-owned check cannot match a backslash path here.
        assert hermes_win in entries
        assert user_win in entries

    @pytest.mark.windows_only
    def test_windows_hermes_owned_paths_stripped(self):
        """On Windows, a Max venv site-packages entry written with
        backslashes is stripped by the same Max-owned check, while a
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic Max venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the Max runtime producer contract.
        """
        import tools.environments.local as local

        repo_root = tmp_path / "max-agent"
    ...
    def test_builders_strip_hermes_venv_pythonpath(self, builder):
        ...
        Max venv site-packages is stripped, user entries survive.
        """
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod.max_subprocess_env()
    ...
    def test_scrub_child_env_strips_hermes_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Max venv
    ...
    def test_execute_code_composition_strips_inherited_hermes_entries(self, same_env):
        ...
        Seeds a contaminated inherited PYTHONPATH (Max repo root + Max
        ...
        * inherited Max venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        assert norm_venv not in norm_parts, \
            "inherited Max venv site-packages must be stripped"
    ...
    def test_configured_home_alias_matches_launcher_output(self, tmp_path, monkeypatch):
        ...
        import tools.environments.local as local
        from max_cli.gateway_windows import _preserve_hermes_home_path
        ...
        monkeypatch.setenv("MAX_HOME", str(configured_home))
        ...
        assert launcher_entry == configured_home / "max-agent"
    ...
    def test_profile_rehome_keeps_junction_lexical_alias(self, tmp_path, monkeypatch):
        ...
        The desktop/CLI spawn children with MAX_HOME and PYTHONPATH in the
        configured (junction) spelling, but --profile / sticky active_profile
        re-home MAX_HOME through resolve_profile_env() before the
        ...
        physical_home = tmp_path / "physical-home"
        physical_root = physical_home / "max-agent"
        physical_root.mkdir(parents=True)
        ...
        monkeypatch.setenv("MAX_HOME", str(configured_home))
        lexical_root = configured_home / "max-agent"
        ...
        # The sanitizer now runs under the re-homed (profile) MAX_HOME.
        aliases = local._build_hermes_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
    ...
    def test_repo_level_junction_recovers_lexical_alias(self, tmp_path, monkeypatch):
        ...
        (e.g. D:\\hermes\\max-agent -> C:\\...\\max-agent) while the
        ...
        # repo-level link: <configured-home>/max-agent -> physical repo
        try:
            _make_directory_link(configured_home / "max-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")
    ...
    def test_same_named_non_owned_directories_preserved(self, tmp_path, monkeypatch):
        ...
        physical_root = _physical_repo_root(tmp_path)
        configured_home = tmp_path / "configured-home"
        (configured_home / "max-agent").mkdir(parents=True)
        unrelated = tmp_path / "user-tools" / "max-agent"
        ...
        for lookalike in (configured_home / "max-agent", unrelated):
    ...
    def test_profile_home_with_repo_level_junction(self, tmp_path, monkeypatch):
        ...
        is <root>/profiles/<name> while the repo is a link at <root>/max-agent.
        ...
        try:
            _make_directory_link(configured_root / "max-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")

        configured_home = configured_root / "profiles" / "coder"
        lexical_root = configured_root / "max-agent"
        aliases = local._build_hermes_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        ...
        assert any(local._same_path(a, lexical_root) for a in aliases)
        assert not any(local._same_path(a, configured_home / "max-agent") for a in aliases)
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the Max runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the Max venv and crashes
    ...
    def test_build_subprocess_env_no_scrub_preserves_pythonhome(self):
        ...
        base = {
            ...
            "HOME": "/home/user",
            "PYTHONHOME": "/opt/max-venv",
            "VIRTUAL_ENV": "/opt/max-venv",
            "SERVICE_TOKEN": "s3cr3t",
        }
        result = build_subprocess_env(base, scrub_secrets=False)
        assert result.get("PYTHONHOME") == "/opt/max-venv"
        assert result.get("VIRTUAL_ENV") == "/opt/max-venv"
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_MAX_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Max (#55878).
        """
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _MAX_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _MAX_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Max-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _MAX_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Max, and blocklisting
        ...
        leaked_block = general_chain & _MAX_PROVIDER_ENV_BLOCKLIST
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _MAX_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _MAX_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_hermes_bin_injection(self):
        ...
        TestMaxBinDirOnPath) so a real ``max`` on the test runner's PATH
        doesn't shift the asserted PATH layout."""
        from tools.environments import local as local_mod
        saved = local_mod._MAX_BIN_DIR
        local_mod._MAX_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._MAX_BIN_DIR = saved
...
class TestMaxBinDirOnPath:
    """The max install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``max`` via the terminal tool must work
    even when the gateway was launched without the max install dir on
    ...
    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._MAX_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/max/bin/hermes" if name == "hermes" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/max/bin")
        assert local_mod._resolve_hermes_bin_dir() == "/opt/max/bin"


    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._MAX_BIN_DIR = None
        assert local_mod._prepend_hermes_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_hermes_bin_dir(self):
        """A gateway env missing the max dir gets it back in the subshell PATH.
        ...
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._MAX_BIN_DIR = "/opt/max/bin"
        with patch.dict(
            os.environ,
            {"PATH": os.pathsep.join(["/usr/bin", "/bin"])},
            clear=True,
        ...
        assert entries[0] == "/opt/max/bin"
        assert "/usr/bin" in entries


class TestMaxInternalDynamicSecrets:
    """Dynamically-named Max secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_MAX_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _MAX_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _MAX_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _MAX_PROVIDER_ENV_BLOCKLIST
