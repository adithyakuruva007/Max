...
Older images made /opt/max/ui-tui and /opt/max/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV MAX_TUI_DIR=/opt/max/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R hermes:hermes /opt/max/ui-tui" not in text
    assert "chown -R hermes:hermes /opt/max/node_modules" not in text
