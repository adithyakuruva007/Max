...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._max_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(max_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(max_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_max_bin(self):
        """After _INSTALL_FAILED, manual install in MAX_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        max_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(max_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(max_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._max_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == max_bin
            assert _tirith_mod._resolved_path == max_bin
            mock_clear.assert_called_once()
...
class TestMaxHomeIsolation:
    def test_max_bin_dir_respects_max_home(self):
        """_max_bin_dir must use MAX_HOME, not hardcoded ~/.max."""
        from tools.tirith_security import _max_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"MAX_HOME": tmpdir}):
            result = _max_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_max_home(self):
        """_failure_marker_path must use MAX_HOME, not hardcoded ~/.max."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"MAX_HOME": "/custom/hermes"}):
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets MAX_HOME; verify it's active."""
        max_home = os.getenv("MAX_HOME")
        assert max_home is not None, "MAX_HOME should be set by conftest"
        assert "max_test" in max_home, "Should point to test temp dir"

    def test_get_max_home_fallback(self):
        """Without MAX_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_max_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove MAX_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".max")
            result = _get_max_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._max_bin_dir",
