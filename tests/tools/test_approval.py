...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("max_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("max_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\max-victim\file.txt"
        )
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\max-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\max-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\max-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
    ...
    def test_powershell_bare_remove_item_requires_approval(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            r"powershell Remove-Item -Recurse -Force C:\tmp\max-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows PowerShell destructive delete"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"MAX_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_max_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.max/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_max_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $MAX_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_max_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$MAX_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestMaxConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.max/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.max/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.max/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.max/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.max/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.max/config.yaml")
        assert dangerous is True
        assert "max config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.max/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_max_home_config(self):
    ...
    def test_sed_in_place_absolute_max_home_env(self):
        env_path = get_max_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_max_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $MAX_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.max/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_max_home_config(self):
        config_path = get_max_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.max/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_max_home_env(self):
        env_path = get_max_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.max/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.max/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.max/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.max/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.max/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Max config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_max_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $MAX_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.max/config.yaml/.env must require approval — the
    ...
    def test_cp_to_max_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.max/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Max-home prefixes must fold to ~/ and
    ~/.max/ in dangerous-command detection.
    ...
    SSH, and Max config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_max_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/MAX_HOME so the fold is verified on the POSIX CI runner too."""
    ...
    def test_windows_max_home_config_folds(self, monkeypatch):
        # Max home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("MAX_HOME", r"C:\Users\tester\.max")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.max\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.max/max-agent && source venv/bin/activate && python -m max_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m max_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m max_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "max_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart max-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_max_gateway_stop_detected(self):
        cmd = "max gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_max_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `hermes` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "max -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_max_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "max --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_max_gateway_multiple_flags_detected(self):
        cmd = "max -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_max_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "max -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_max_gateway_start_not_flagged(self):
        cmd = "max gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_max_detected(self):
    ...
    def test_killall_max_detected(self):
        cmd = "killall hermes"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the Max security config via IFS must be caught."""
        cmd = "sed${IFS}-i ~/.max/config.yaml"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    ...
    def test_pkill_max_still_detected(self):
    ...
    def test_kill_dollar_pidof_detected(self):
        ...
        cmd = "kill -TERM $(pidof max_cli.main)"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Max service
    label achieves the same effect as `max gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_stop_max_detected(self):
        cmd = "launchctl stop ai.max.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "max" in desc.lower()

    def test_launchctl_kickstart_max_detected(self):
        cmd = "launchctl kickstart -k system/ai.max.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_max_detected(self):
        cmd = "launchctl bootout system/ai.max.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_max_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.max.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Max label is out of scope for the
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("MAX_GATEWAY_SESSION", "MAX_CRON_SESSION",
                      "MAX_YOLO_MODE",
                      "MAX_SESSION_KEY", "MAX_INTERACTIVE")
        }
        os.environ.pop("MAX_YOLO_MODE", None)
        os.environ.pop("MAX_INTERACTIVE", None)
        # MAX_CRON_SESSION takes priority over MAX_GATEWAY_SESSION in
        ...
        os.environ.pop("MAX_CRON_SESSION", None)
        os.environ["MAX_GATEWAY_SESSION"] = "1"
        os.environ["MAX_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("max_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"MAX_INTERACTIVE": "1"}, clear=False):
