"""Contract tests for the Docker image's immutable /opt/max install tree."""
...
def test_dockerfile_makes_opt_max_readonly_for_max_user() -> None:
    ...
    assert "chown -R root:root /opt/max" not in text
    assert "chmod -R a+rX /opt/max" not in text
    assert "chmod -R a-w /opt/max" not in text


def test_dockerfile_keeps_mutable_state_under_opt_data() -> None:
    text = _dockerfile_text()

    assert "ENV MAX_HOME=/opt/data" in text
    assert "ENV MAX_WRITE_SAFE_ROOT=/opt/data" in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "ENV MAX_DISABLE_LAZY_INSTALLS=1" in text
    assert "MAX_TUI_DIR=/opt/max/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_hermes() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+hermes:hermes\s+/opt/max/\.venv",
        r"chown\s+-R\s+hermes:hermes\s+/opt/max/ui-tui",
        r"chown\s+-R\s+hermes:hermes\s+/opt/max/gateway",
        r"chown\s+-R\s+hermes:hermes\s+/opt/max/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/max must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/max/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $MAX_HOME data volume (which a host install may also use).
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/max tree.
    assert f"ENV MAX_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV MAX_LAZY_INSTALL_TARGET=/opt/max" not in text
    ...
    assert '"$MAX_HOME/lazy-packages"' in stage2, (
