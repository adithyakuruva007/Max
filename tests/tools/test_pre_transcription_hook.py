...
PROMPT = "Max, Teknium, Stardust Research, kanban"
...
def _fake_hooks(monkeypatch, results):
    ...
    monkeypatch.setattr("max_cli.plugins.invoke_hook", _invoke)
    return captured
...
def _no_hooks(monkeypatch):
    ...
    monkeypatch.setattr("max_cli.plugins.invoke_hook", _boom)
...
class TestUnsupportedBackends:
    def test_xai_logs_debug_and_proceeds_without_prompt(
        ...
        monkeypatch.setattr(
            "tools.xai_http.max_xai_user_agent", lambda: "test-ua",
        )
