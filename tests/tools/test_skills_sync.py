...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.max/)``
    ...
    def test_refuses_anything_that_is_not_a_strict_child_of_skills(self, tmp_path):
        """``/``, ``~/.max`` itself, a sibling dir, and the skills root
        ...
        sibling = max / "kanban.db"  # any non-skills path
        sibling.mkdir()
...
class TestGetBundledDir:
    def test_env_var_override_with_default_fallback(self, tmp_path, monkeypatch):
        ...
        custom_dir.mkdir()
        monkeypatch.setenv("MAX_BUNDLED_SKILLS", str(custom_dir))
        assert _get_bundled_dir() == custom_dir

        # Empty or unset falls back to the relative path from __file__.
        monkeypatch.setenv("MAX_BUNDLED_SKILLS", "")
        assert _get_bundled_dir().name == "skills"
        monkeypatch.delenv("MAX_BUNDLED_SKILLS", raising=False)
        assert _get_bundled_dir().name == "skills"
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `max profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.max) rely on so bundled
    skills are never seeded at install time NOR re-injected by `max update`.
    """
    ...
    def test_marker_skips_sync_and_removal_seeds_normally(self, tmp_path):
        ...
        def _patches():
            ...
            stack.enter_context(patch("tools.skills_sync.SKILLS_DIR", skills_dir))
            stack.enter_context(patch("tools.skills_sync.MANIFEST_FILE", manifest_file))
            stack.enter_context(patch("tools.skills_sync.MAX_HOME", hermes_home))
            return stack
...
class TestOptOutToggleAndRemove:
    """`max skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.MAX_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.MAX_HOME", home):
...
class TestCallTimeDirResolution:
    ...
    MAX_HOME at import, so a long-lived dashboard/TUI process serving a
    ...
    The accessors must follow set_max_home_override() at call time, while
    ...
    def test_explicit_module_patch_wins_over_override(self, tmp_path):
        ...
        token = set_max_home_override(str(tmp_path / "other-profile"))
        try:
