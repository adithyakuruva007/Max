...
profile's MAX_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.max/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active MAX_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever MAX_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    MAX_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import max_constants

    monkeypatch.setattr(
        max_constants, "_get_platform_default_max_home", lambda: root
    )
    monkeypatch.setenv("MAX_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile MAX_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "max_home"
    ...
    assert max_constants.get_max_home().resolve() == profile_home.resolve()
    assert max_constants.get_default_max_root().resolve() == root.resolve()

    # cron/jobs.py computes MAX_DIR from get_max_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.MAX_DIR.resolve() == profile_home.resolve()
        assert (
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_max_home", None, raising=False)
    assert scheduler._get_max_home().resolve() == profile_home.resolve()
    assert scheduler._get_max_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (MAX_HOME == root), the store is the root's cron dir
    ...
    importlib.reload(jobs)
    try:
        assert jobs.MAX_DIR.resolve() == root.resolve()
