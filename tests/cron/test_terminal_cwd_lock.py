...
def test_run_job_releases_cwd_lock_when_body_raises(tmp_path):
    ...
    with patch("cron.scheduler._hermes_home", tmp_path), \
         ...
         patch("max_state.SessionDB", return_value=MagicMock()):
...
def test_cwd_lock_timeout_derivation(monkeypatch):
    """The bound tracks MAX_CRON_TIMEOUT (+margin) with a floor, and
    ...
    monkeypatch.setenv("MAX_CRON_TIMEOUT", "1800")
    ...
    assert sched._cwd_lock_timeout_seconds() == 660.0
    monkeypatch.setenv("MAX_CRON_TIMEOUT", "bogus")
    assert sched._cwd_lock_timeout_seconds() == 660.0
