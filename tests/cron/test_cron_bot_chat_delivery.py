...
def test_named_profile_resolves_when_exists():
    with mock.patch("max_cli.profiles.profile_exists", return_value=True):
...
def test_unknown_profile_resolves_to_none():
    with mock.patch("max_cli.profiles.profile_exists", return_value=False):
...
def test_deliver_runs_canonical_bot_chat_lane():
    ...
    assert "-p" not in argv  # own profile: subprocess inherits MAX_HOME
    assert "chat" in argv
...
def test_deliver_named_profile_uses_p_flag_and_clears_home():
    ...
    with mock.patch.object(sched.subprocess, "run", side_effect=fake_run), \
         mock.patch.object(sched.shutil, "which", return_value="/usr/bin/hermes"), \
         mock.patch.dict(sched.os.environ, {"MAX_HOME": "/tmp/other-profile"}):
        err = _deliver_to_bot_chat({"id": "j1", "name": "n"}, "out", "research")

    assert err is None
    argv = calls["argv"]
    assert argv[1:3] == ["-p", "research"]
    # -p owns resolution; the scheduler's own MAX_HOME must not leak in.
    assert "MAX_HOME" not in calls["kwargs"]["env"]
...
def test_delivery_targets_include_local_profiles():
    with mock.patch("max_cli.profiles.list_profile_names",
