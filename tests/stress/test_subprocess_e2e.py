...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves max_cli.kanban_db on its own
  ...
  - worker logs are captured to MAX_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "MAX_KANBAN_TASK": task.id,
            "MAX_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="max_e2e_")
    ...
    # max_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "max")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m max_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/max_e2e_*/worker_*.log
    import glob
