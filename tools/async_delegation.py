...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from max_state import apply_durability_barriers
    ...
    for name, sql_type in (
        ...
        ("delivery_claim", "TEXT"),
        ("delivery_claimed_at", "REAL"),
        # Raw api_server session id (X-Max-Session-Id) of the ORIGINATING
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        for evt_key, env_name in (
            ("scope_id", "MAX_SESSION_SCOPE_ID"),
            ("user_id", "MAX_SESSION_USER_ID"),
            ("user_name", "MAX_SESSION_USER_NAME"),
        ):
...
def _current_origin_session_id() -> str:
    ...
    The obvious source — ``MAX_SESSION_ID`` via ``get_session_env`` — is
    ...
    The request-scoped ``MAX_SESSION_CHAT_ID`` binding survives child
    construction: ``_bind_api_server_session`` binds ``chat_id`` to the raw
    ``X-Max-Session-Id``, and its only writer is ``set_session_vars`` —
    ...
    try:
        ...
        if get_session_env("MAX_SESSION_PLATFORM", "") != "api_server":
            return ""
        return get_session_env("MAX_SESSION_CHAT_ID", "") or ""
    except Exception:
        return ""
...
def dispatch_async_delegation(
    *,
    ...
    try:
        ...
        # get_max_home() under the right profile.
