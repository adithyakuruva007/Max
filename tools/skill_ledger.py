...
``~/.max/skills/.curator_ledger.jsonl`` describing who changed what, with
before/after file manifests whose contents are stored content-addressed
(sha256-deduped) under ``~/.max/.curator_backups/blobs/``.
...
def ledger_path() -> Path:
    return get_max_home() / "skills" / ".curator_ledger.jsonl"


def blobs_dir() -> Path:
    return get_max_home() / ".curator_backups" / "blobs"
...
def _validate_entry_paths(entry: Dict[str, Any]) -> Optional[str]:
    """All paths in an entry must live under MAX_HOME. Defense in depth —
