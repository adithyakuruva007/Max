...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.max/config.yaml``. Max
...
Configuration is loaded from ~/.max/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``max_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from max_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``max auth add <provider_id>``).
...
# Base URL now resolved via max_cli.models.deepinfra_base_url (shared).
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.max/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.max/config.yaml.
    ...
    try:
        ...
        logger.debug("max_cli.config not available, using default TTS config")
        return {}
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``max tools``); otherwise the historical Edge backend remains active.
...
# custom voice-cloning scripts, etc.) into Max without any Python code
...
#           output_format: wav
#
# Max writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__MAX_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Max secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Max secrets (salvage of #56332) while still
...
# file, so retired models disappear from max the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``max_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `max setup` to configure, "
    ...
    from max_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `max model` or set XAI_API_KEY.")
    ...
    # output_format when Max actually needs a non-default format/override.
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import max_cli as _max_cli

            _hermes_version = str(_max_cli.__version__)
        except Exception:
            _hermes_version = "0.0.0"
        # Include Max client context following Gemini's partner
        # integration guidance:
        # https://ai.google.dev/gemini-api/docs/partner-integration
        headers["X-Goog-Api-Client"] = f"max-agent/{_hermes_version}"
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Max caches Piper voice models.

    Resolves to ``~/.max/cache/piper-voices/`` under the active
    MAX_HOME so voice downloads follow profile boundaries.
    """
    from max_constants import get_hermes_dir
...
def _text_to_speech_single(
    ...
    *,
    ...
    platform = get_session_env("MAX_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `max setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run max setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'max setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'max tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_max_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
