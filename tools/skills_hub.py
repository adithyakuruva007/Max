...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _max_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            ...
            if isinstance(max_meta, dict):
                tags = max_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "StardustResearch/max-agent"
...
def _max_index_cache_file() -> Path:
    return _index_cache_dir() / "max-index.json"


def _load_max_index() -> Optional[dict]:
    ...
    """
    # Check local cache
    max_index_cache_file = _max_index_cache_file()
    if max_index_cache_file.exists():
        try:
            age = time.time() - max_index_cache_file.stat().st_mtime
            if age < MAX_INDEX_TTL:
                return json.loads(max_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    # Cache locally
    try:
        max_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        max_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class MaxIndexSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "max-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "max-index"),
            identifier=entry.get("identifier", ""),
