...
* Never raises. Any I/O error (disk full, permission denied, missing
  MAX_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_max_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: MAX_HOME env var, then ~/.max
            home = os.environ.get("MAX_HOME") or os.path.expanduser("~/.max")
