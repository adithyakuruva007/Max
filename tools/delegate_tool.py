...
# Must match max_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _build_child_agent(
    ...
    # child Max/Qwen subagent onto the parent's Claude Messages wire (or the
    ...
    elif _effective_provider_norm in {"nous", "nous-portal", "stardustresearch"}:
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.max/logs/subagent-<sid>-<ts>.log``
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("MAX_UI_SESSION_ID", "") or None
            except Exception:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from max_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # MAX_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Max-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("MAX_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Max-Session-Id once the batch completes.
            ...
            # MAX_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `max -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("MAX_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("MAX_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and MAX_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'max auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Max config.

    Prefer the shared persistent loader because it follows the active
    MAX_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``MAX_IGNORE_USER_CONFIG=1`` (``max chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("MAX_IGNORE_USER_CONFIG") == "1"
