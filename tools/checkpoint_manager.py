...
CHECKPOINT_BASE = get_max_home() / "checkpoints"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Max convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("MAX_CHECKPOINT_TIMEOUT", 30)))
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Max wrote
...
def _git_env(
    ...
    The shared store is internal Max infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``max checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `max checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.name", "Max Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``MAX_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at MAX_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "MAX_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Max just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Max last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Max last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Max'
        last write or that Max never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Max has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Max never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since Max wrote it: restoring it back is
                # safe — its last content was Max-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Max changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Max changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after Max' last write — per the agent-write ledger —
        are left untouched, and only Max-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # Max-created files absent from it (delete to restore state).
            ...
            for rel in restore_paths:
                ...
                if ok_in_commit:
                    ...
                    # Max created it. Deleting it would not restore a prior
...
# ---------------------------------------------------------------------------
# Public helpers for `max checkpoints` CLI
# ---------------------------------------------------------------------------
