...
`max skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.max.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.max.blueprint`` is a mapping containing
    ...
    max_meta = meta.get("max") if isinstance(meta, dict) else None
    blueprint = max_meta.get("blueprint") if isinstance(max_meta, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.max.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.max.blueprint`` block) they can hand
    to ``max skills publish`` to share. ``body`` is the plain-language
