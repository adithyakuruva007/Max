...
Copies bundled skills from the repo's skills/ directory into ~/.max/skills/
...
The manifest lives at ~/.max/skills/.bundled_manifest.
"""
...
from max_constants import get_bundled_skills_dir, get_max_home, get_optional_skills_dir
...
# Marker file written by `max profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.max profile).
# When present in MAX_HOME, sync_skills() is a no-op so neither the
# installer, `max update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# max_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks MAX_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.max/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.max/skills/mlops/axolotl
    """
...
def _dir_hash(directory: Path) -> str:
    """Fast stat-based directory hash for change detection.

    Uses mtime_ns + size instead of reading every byte. ~100x faster than
    the old content-based hash and answers the same question correctly:
    "did anything in this directory change?"

    The one edge case — file content modified but mtime/size unchanged — is
    practically impossible for first-party skill files (git checkout, cp,
    ``hermes update`` all update mtime).
    """
    hasher = hashlib.md5()
    try:
        for entry in sorted(directory.rglob("*"), key=lambda p: p.relative_to(directory)):
            if entry.is_file():
                rel = entry.relative_to(directory)
                s = entry.stat()
                hasher.update(str(rel).encode("utf-8"))
                hasher.update(str(s.st_mtime_ns).encode())
                hasher.update(str(s.st_size).encode())
    except (OSError, IOError):
        pass
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.max/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.max) that wrote the
    ...
    if (MAX_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.max/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `max update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `max skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``MAX_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.max/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``MAX_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `max skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `max update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``max update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``max update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``max skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `max skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes MAX_HOME/.no-bundled-skills so the
    installer, ``max update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``max skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = MAX_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `max update` (or `max skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (MAX_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.max/skills/ ...")
