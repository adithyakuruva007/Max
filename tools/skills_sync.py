...
# cron, kanban workers) import this module once under the launch MAX_HOME
# and later scope requests to a different profile via
# set_max_home_override(). Frozen module constants would then resolve —
...
# re-resolve from the live profile-scoped MAX_HOME on every call.
_MAX_HOME_AT_IMPORT = MAX_HOME
_SKILLS_DIR_AT_IMPORT = SKILLS_DIR
_MANIFEST_FILE_AT_IMPORT = MANIFEST_FILE


def _hermes_home() -> Path:
    """Return the active profile's MAX_HOME at call time."""
    configured = Path(MAX_HOME)
    if configured != _MAX_HOME_AT_IMPORT:
        return configured
    return get_max_home()
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``max skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `max skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    # ``max-agent`` skill is the agent's own operating manual and the
