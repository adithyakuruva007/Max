...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            max_home = os.path.expanduser(
                os.getenv(
                    "MAX_HOME", os.path.join(os.path.expanduser("~"), ".max")
                )
            )
            candidates = [
                os.path.join(max_home, "node", "bin", resolved_command),
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'max-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'max-agent[all]'"
            )
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple max
