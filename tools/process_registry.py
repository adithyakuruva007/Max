...
CHECKPOINT_PATH = get_max_home() / "processes.json"
...
# ---------------------------------------------------------------------------
# When Max runs as a systemd gateway with MemoryHigh/MemoryMax limits,
...
def _is_supervised_gateway_process() -> bool:
    """Return whether this process is in a supervised Max gateway runtime.

    Both supervisor markers and ``_MAX_GATEWAY`` are inherited by every
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from max_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``max_cli.goals``) to support
    ...
    def wait_for_pending_completions(
        ...
        *,
        ...
        One-shot CLI runs (``max -q/-Q/-z``) exit as soon as their single
        ...
        (#90879): a recipient invoked as ``max -p <bot> chat -Q
    ...
    def _oneshot_completion_wait_seconds() -> float:
        ...
        """
        try:
            from max_cli.config import DEFAULT_CONFIG, cfg_get, read_raw_config
            cfg = read_raw_config()
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `max update` restarting the gateway) is still
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the Max desktop app.",
            }
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({
                            ...
                            # ~/.max/processes.json with the raw command
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Max'}"
    elif _reason == "lost":
