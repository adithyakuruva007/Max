...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `max model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``max auth add
    ...
    2. The environment / ``~/.max/.env``. Under a multiplexed gateway turn
       ...
       ``max_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``max auth
    ...
    when omitted, ``max_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `max auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``max auth add openai-api``) via
...
def read_selection(section: str) -> str | None:
    """Return the stored `max tools` provider string for a config section.
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        f"{section} is configured to use {selection_name} (set via max "
        f"tools), but {failure}. Run 'max tools' to change it."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.max/.env`` (via
    ``max_cli.config.get_env_value`` when available) so tool-side
