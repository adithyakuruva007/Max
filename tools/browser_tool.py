...
from max_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Max secret
...
def _build_browser_env() -> dict:
    ...
    Strips Max-managed secrets (provider keys, gateway tokens, GitHub auth,
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``max doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.max/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Max profile."""
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.max/plugins/browser/<vendor>/``) participate
...
# comparison sites (four here, plus max_cli/tools_config.py and
# max_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Max silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Max-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Max CLI where sys.stdout and sys.stderr both report
...
def _cdp_http_ready(http_cdp: str) -> bool:
    ...
    try:
        from max_cli.browser_connect import is_browser_debug_ready
...
def _real_profile_cdp() -> tuple:
    ...
    if not _use_real_profile():
        ...
        try:
            from max_cli.browser_connect import cleanup_real_profile_snapshots
    ...
    with _real_profile_cdp_lock:
        ...
        # from a previous max process; if it is driving OUR copy dir, hand it
        ...
        if err or not snap_dir:
            from max_cli.browser_connect import _PROFILE_LOCKED_PREFIX
            ...
            if err and err.startswith(_PROFILE_LOCKED_PREFIX):
                ...
                return None, (
                    ...
                    "`max browser close-profile`, then retry."
                )
        ...
        from max_cli.browser_connect import chromium_executable
...
# Hard ceiling for a daemon whose owning max process is still alive but
...
def _emergency_cleanup_all_sessions():
    ...
    crashed max processes — this way every clean max exit sweeps
    ...
    # Sweep orphans from other crashed max processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live max processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current max PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live max process
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    max restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning max process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced max PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent max instances won't reap each
         other's daemons.
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live max process and must not be touched — but "owner alive"
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Max-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Max-managed one with no recovery — every candidate
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Max-managed
    ...
    # node_modules binary, which disappears on the next ``max update`` and
    ...
    # Build an extended search PATH including Max-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_max_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_max_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_max_home() / "node")),
            ]
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``max update`` (or ``max doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Max holds, while running registry-fetched
    npm code on every ``max update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `max update`/`doctor
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this max PID as the session owner (cross-process safe
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Max
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/stardustresearch/max-agent:latest")
                else:
