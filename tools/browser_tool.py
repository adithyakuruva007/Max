...
from max_constants import agent_browser_runnable, get_max_home
...
def _build_browser_env() -> dict:
    ...
    The ``max_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = max_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    max_node_bin = str(max_home / "node" / "bin")
    max_node_root = str(max_home / "node")
    max_nm_bin = str(max_home / "node_modules" / ".bin")
    return [max_node_bin, max_node_root, max_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-max_…`` the resulting socket path exceeds the
...
def _reap_orphaned_browser_sessions():
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-max_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live max process.
            continue
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = max_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_max_dir("cache/screenshots", "browser_screenshots")
