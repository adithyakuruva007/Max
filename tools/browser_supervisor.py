...
One ``CDPSupervisor`` runs per Max ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Max logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "max-dialog-bridge.invalid"
...
(() => {
  if (window.__maxDialogBridgeInstalled) return;
  window.__maxDialogBridgeInstalled = true;
  const ENDPOINT = "http://max-dialog-bridge.invalid/";
