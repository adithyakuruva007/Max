...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``max-system`` and poison
...
def _handle_create(args: dict, **kw) -> str:
    ...
    session_id = args.get("session_id") or os.environ.get("MAX_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (MAX_KANBAN_TASK set)
    ...
    # (kanban toolset, no MAX_KANBAN_TASK) and CLI/dashboard callers
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    - **Gateway** (telegram/discord/slack/etc): ``MAX_SESSION_PLATFORM``
      and ``MAX_SESSION_CHAT_ID`` are set in ContextVars by the
