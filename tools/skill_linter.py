...
Design contract (matches the Max "no lazy-reading escape hatches / don't
    ...
    if not isinstance(hermes_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.max.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in hermes_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.max.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("hermes", "agent", "max agent") and (
        author != "Max Agent"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Max Agent' (proper caps) "
