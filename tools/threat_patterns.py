...
_PATTERNS: List[Tuple[str, str, str]] = [
    ...
    (r'\$HOME/\.(?:max|hermes)/\.env|\~/\.(?:max|hermes)/\.env', "max_env", "strict"),
    ...
    (r'(update|modify|edit|write|change|append|add\s+to)\s+[^\n]{0,2048}\.(?:max|hermes)/(config\.yaml|SOUL\.md)', "max_config_mod", "strict"),
...
def scan_for_threats(content: str, scope: str = "context") -> List[str]:
    ...
    # bypassing keyword checks (e.g. ``ｃａｔ ~/.max/.env``).  NOTE: this
