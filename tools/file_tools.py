...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``max_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from max_constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where max previously hard-rejected an oversized read
...
_max_config_resolved: str | None = None
_max_config_resolved_loaded = False


def _get_max_config_resolved() -> str | None:
    """Return the resolved absolute path of the Max config file (cached)."""
    global _max_config_resolved, _max_config_resolved_loaded
    if _max_config_resolved_loaded:
        return _max_config_resolved
    _max_config_resolved_loaded = True
    try:
        from max_cli.config import get_config_path
        _max_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _max_config_resolved = str(Path(_expand_tilde("~/.max/config.yaml")).resolve())
        except Exception:
            _max_config_resolved = None
    return _max_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Max config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    max_config = _get_max_config_resolved()
    if max_config and (resolved == max_config or normalized == max_config):
        return (
            f"Refusing to write to Max config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.max/config.yaml directly or use 'max config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Max mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Max
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Max state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.max/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Max process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.max/…`` path.

    Returns ``None`` when the write is in-scope or outside Max scope.
    ...
    # in a session that cd'd into ``~/.max/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Max internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under MAX_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == MAX_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Max-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Max-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Max profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Max profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with max_tools.write_file() for very "
            "large files."
        )
