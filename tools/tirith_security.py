...
it is automatically downloaded from GitHub releases to $MAX_HOME/bin/tirith.
...
def _get_max_home() -> str:
    """Return the Max home directory, respecting MAX_HOME env var."""
    return str(get_max_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_max_home(), ".tirith-install-failed")
...
def _max_bin_dir() -> str:
    """Return $MAX_HOME/bin, creating it if needed."""
    d = os.path.join(_get_max_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $MAX_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_max_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $MAX_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $MAX_HOME/bin/tirith
    ...
    max_bin = os.path.join(_max_bin_dir(), "tirith")
    if os.path.isfile(max_bin) and os.access(max_bin, os.X_OK):
        _resolved_path = max_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return max_bin
