...
class BaseEnvironment(ABC):
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/max-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/max-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        # caused ``C:/Users/.../max-snap-*.sh: No such file or directory``
        ...
        bootstrap = (
            ...
            f"__max_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__max_fns\" ] && declare -f $__max_fns "
