...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``MAX_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `max egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `max egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `max egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `max egress setup` to mint provider "
    ...
    env_overrides: dict[str, str] = {
        ...
        "_MAX_EGRESS_NODE_OPTIONS_APPEND": "--use-openssl-ca",
    }
    ...
    # through the proxy.  Keep the MAX_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"MAX_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        if egress_env_overrides:
            try:
                from max_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from max_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_MAX_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_MAX_EGRESS_NODE_OPTIONS_APPEND", None,
        )
    ...
    def _find_reusable_container(
        ...
        try:
            ...
            if egress_label != "off":
                ...
                # reused after the operator runs "max egress disable",
