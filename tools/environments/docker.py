...
def _load_max_env_vars() -> dict[str, str]:
    """Load ~/.max/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Max profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale max-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=max-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Max process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=max-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a max process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Max
    process leaves its container behind permanently because there's no
    subsequent Max process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=max-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=max-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``MAX_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("MAX_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using MAX_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'max'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``max-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.max/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. max-agent:latest) already use /init as PID 1
        ...
        container_name = f"max-{uuid.uuid4().hex[:8]}"
        # Labels make max-created containers identifiable to:
        #   * the orphan reaper (`max-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`max-task-id`, `max-profile`)
        #   * operators running `docker ps --filter label=max-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Max profile is captured at
        ...
        label_args = [
            "--label", "max-agent=1",
            "--label", f"max-task-id={task_label}",
            "--label", f"max-profile={profile_name}",
        ]
        ...
        self._labels = {
            "max-agent": "1",
            "max-task-id": task_label,
            "max-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Max process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Max secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Max-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_max_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_max_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _MAX_PROVIDER_ENV_BLOCKLIST)
        max_env = _load_max_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = max_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("max-task-id", "")
        profile_label = self._labels.get("max-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"max-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``max-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=max-agent=1",
                    "--filter", f"label=max-task-id={task_label}",
                    "--filter", f"label=max-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Max process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Max exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Max process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Max startup. That covers the
        ...
        # outlives Max processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"max-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``max /quit``
