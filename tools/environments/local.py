...
def _is_max_internal_secret(key: str) -> bool:
    ...
    non-terminal :func:`max_subprocess_env` helper all call it, so the
...
def _inject_context_max_home(env: dict) -> None:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_MAX_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_max_internal_secret(key):
            continue
        if key not in _MAX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_MAX_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_MAX_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_max_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_max_internal_secret(key):
            continue
        elif key not in _MAX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_max_home(sanitized)
...
# the conditional Tier-2 strip in max_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_max_internal_secret, but _ID has no secret suffix, so it must be
...
def max_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 2 (conditional):** the rest of ``_MAX_PROVIDER_ENV_BLOCKLIST``
    ...
    # legitimate use for them. See :func:`_is_max_internal_secret`.
    for key in list(env):
        if key.startswith(_MAX_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_max_internal_secret(key):
...
def _find_bash() -> str:
    ...
    #   PortableGit: %LOCALAPPDATA%\max\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\max\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _max_portable_git = os.path.join(_local_appdata, "max", "git") if _local_appdata else ""
    if _max_portable_git:
        for candidate in (
            os.path.join(_max_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_max_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
...
def _resolve_max_bin_dir() -> str | None:
    ...
    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("max")`` — normal PATH-installed shim.
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "max" or base.startswith("hermes."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "hermes.exe" if _IS_WINDOWS else "max"
            alt_shim = "max.exe" if _IS_WINDOWS else "hermes"
            if os.path.isfile(os.path.join(exe_dir, shim)) or os.path.isfile(os.path.join(exe_dir, alt_shim)):
                candidate = exe_dir
...
def _prepend_max_bin_dir(existing_path: str) -> str:
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_MAX_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if _is_max_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_max_internal_secret(k):
            continue
        elif k not in _MAX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    if path_key is not None:
        ...
        run_env[path_key] = _prepend_max_bin_dir(new_path)

    _inject_context_max_home(run_env)
...
class LocalEnvironment(BaseEnvironment):
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        if _IS_WINDOWS:
            ...
            try:
                ...
                cache_dir = Path(tempfile.gettempdir()) / "max_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._max_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_max_pgid", None)
                    if pgid is None:
                        raise
