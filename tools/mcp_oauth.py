...
URL of Max' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``MaxTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Max Agent"
...
from max_constants import secure_parent_dir
...
def _get_token_dir(hermes_home: str | Path | None = None) -> Path:
    ...
    Uses MAX_HOME so each profile gets its own OAuth tokens.
    Layout: ``MAX_HOME/mcp-tokens/``
    """
    from max_constants import get_max_home

    base = Path(hermes_home) if hermes_home is not None else Path(get_max_home())
...
def _cached_redirect_port(storage: "MaxTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Max restarts and chooses a
...
def _cached_redirect_uri(storage: "MaxTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``max mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `max mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# MaxTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class MaxTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        MAX_HOME/mcp-tokens/<server_name>.json         -- tokens
        MAX_HOME/mcp-tokens/<server_name>.client.json   -- client info
        MAX_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        MAX_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Max records an absolute wall-clock ``expires_at`` alongside the
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``max mcp login`` /
        ``max mcp remove``, so a fixed document gets another chance.
        """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Max.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://max-agent.stardustresearch.com/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"Max' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `max mcp login <server>` later to "
...
MaxOAuthClientProvider: Any = None


def _get_hermes_oauth_provider_class() -> type | None:
    global MaxOAuthClientProvider
    if MaxOAuthClientProvider is not None:
        return MaxOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _MaxOAuthClientProvider(OAuthClientProvider):
    ...
    _MaxOAuthClientProvider.__name__ = "MaxOAuthClientProvider"
    _MaxOAuthClientProvider.__qualname__ = "MaxOAuthClientProvider"
    MaxOAuthClientProvider = _MaxOAuthClientProvider
    return MaxOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = MaxTokenStorage(server_name, hermes_home=hermes_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; Max only decides whether a
...
# max-agent.stardustresearch.com/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = (
    "https://stardustresearch.github.io/max-agent/docs/oauth/client-metadata.json"
)
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port Max picks
...
def _has_cached_client_info(storage: "MaxTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "MaxTokenStorage | None") -> bool:
    ...
    Max has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "MaxTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI Max would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Max Agent" / "Max" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Max Agent")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Max is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: max mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Max auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Max is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`max mcp login {server_name}` interactively first to complete "
