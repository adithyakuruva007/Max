...
LOCAL_STT_COMMAND_ENV = "MAX_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "MAX_LOCAL_STT_LANGUAGE"
...
# DeepInfra STT base URL now resolved via max_cli.models.deepinfra_base_url (shared).
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``MAX_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
#
# The single-env-var ``MAX_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from Max's remote upload cap."""
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__MAX_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set MAX_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `max plugins list` to see "
...
def _apply_pre_transcription_hook(
    *,
    ...
    """
    try:
        from max_cli.plugins import has_hook, invoke_hook
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="hermes-local-stt-") as output_dir:
            ...
            # Scrub Max secrets from the child env (sibling path to #56332 /
...
def _transcribe_groq(
    ...
    *,
    ...
    ``stt.language`` (config.yaml) > ``MAX_LOCAL_STT_LANGUAGE`` (env).
    ...
    *,
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `max model` or set XAI_API_KEY",
        }
...
def _transcribe_deepinfra(
    ...
    *,
    ...
    ``max_cli.models`` helpers so every DeepInfra surface resolves the
...
def _dispatch_stt_provider(
    ...
    # unavailable). Surface it — with its `max tools` remediation —
