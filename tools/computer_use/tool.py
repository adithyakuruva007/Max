...
`health_report` (surfaced by `max computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Max runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Max-session cached backends. Each backend owns its own cua-driver
...
# See NousResearch/max-agent#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Max's explicit approval bypass onto Cua's immutable mode.

    Max has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown MAX_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Max YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Max sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Max process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `max computer-use install`. "
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (NousResearch/max-agent#67052).
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of NousResearch/max-agent#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    *,
    ...
    The captured base64 PNG is materialised to ``$MAX_HOME/cache/vision/``
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Save a capture in Max' media cache and return its absolute path.
...
def check_computer_use_requirements() -> bool:
    ...
    `max computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
