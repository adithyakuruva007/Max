...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (max_constants itself is stdlib-only, so no cycle).
        from max_constants import get_max_home

        return Path(get_max_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Max-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("MAX_SESSION_ID", ""),
            get_session_env("MAX_BROWSER_CONTROL_PRINCIPAL", ""),
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved MAX_HOME. A
