...
teammate agent (another Max profile on this install, or an agent on a
...
DM each other via a prompt-injected ``max -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``max -p`` / ``max peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``max -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``max peer dm <peer>[/<name>] < <tmp>``
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``max -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative max next to this gateway's interpreter
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("MAX_HOME") or os.path.expanduser("~/.max")
