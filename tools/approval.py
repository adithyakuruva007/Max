...
_max_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "max_interactive",
    default=None,
)


def set_max_interactive_context(interactive: bool) -> contextvars.Token:
    ...
    """
    return _max_interactive_ctx.set("1" if interactive else "")


def reset_max_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_max_interactive_context`."""
    _max_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    """
    ctx_val = _max_interactive_ctx.get()
...
def _is_gateway_approval_context() -> bool:
    ...
    """
    if env_var_enabled("MAX_CRON_SESSION"):
        return False
...
# active profile home path such as /home/max/.max/config.yaml. The
...
DANGEROUS_PATTERNS = [
    ...
    (r'\b(?:max|hermes)\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart max gateway (kills running agents)"),
    (r'\b(?:max|hermes)\s+update\b', "max update (restarts gateway, kills running agents)"),
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart max-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart max-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(max|hermes|gateway|cli\.py)\b', "kill max/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill max` but not
    # `kill -9 $(pgrep -f max)` because the substitution is opaque
    ...
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(max|hermes|ai\.(?:max|hermes))\b', "stop/restart max launchd service (kills running agents)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # user home (C:\Users\alice\AppData\...\.max), so folding the user home
    # first would eat the prefix the Max-home fold needs.
    command = _rewrite_resolved_max_home(command)
...
def _rewrite_resolved_max_home(command: str) -> str:
    ...
    /home/max/.max/config.yaml``) rather than ``~``, ``$HOME``, or
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Max approval prompts.
