...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("MAX_YOLO_MODE", ""))
...
# Max session id (observability identity, distinct from the gateway
...
# os.environ["MAX_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export MAX_INTERACTIVE.
...
def set_hermes_interactive_context(interactive: bool) -> contextvars.Token:
    ...
    Use this instead of mutating ``os.environ["MAX_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``MAX_INTERACTIVE`` env var for legacy callers.
    """
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``MAX_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _hermes_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("MAX_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Max session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("MAX_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("MAX_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("MAX_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("MAX_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("MAX_CRON_SESSION")


def _is_single_query_approval_context() -> bool:
    """True when the current approval decision is from a single-query (-q) session.

    ``max chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``MAX_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("MAX_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("MAX_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set MAX_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind MAX_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds MAX_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("MAX_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``MAX_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $MAX_HOME, or by the resolved absolute
# active profile home path such as /home/hermes/.max/config.yaml. The
# resolved-absolute form is folded into the ~/.max/ patterns at detection
...
# go stale when MAX_HOME is set after this module is imported, e.g. under the
...
# ~/.max/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _MAX_ENV_PATH; matches the MAX_HOME override form as
# well as ~/.max/.
_MAX_CONFIG_PATH = (
    r'(?:~\/\.max/|'
    r'(?:\$home|\$\{home\})/\.max/|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_max_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Max: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Max
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]hermes[^\n]*\.env\b', "access to Max secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `max` and `gateway` (e.g. `max -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bhermes\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart max gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "max update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `max gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `max ...
    ...
    # the `max gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.max.gateway`).
    # Catch the operations that stop, restart, or unload it.
    #
    # Order-independent (2026-08-02 incident): the previous version required
    # "hermes"/"ai.max" to appear AFTER the launchctl verb in the same
    ...
    # 'ai.max.gateway-apollo:...' ...; do label=${item%%:*}; launchctl
    ...
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:hermes|ai\.max)\b)', "stop/restart max launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Max file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.max/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Max-managed security file (~/.max/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_MAX_CONFIG_PATH}|{_MAX_ENV_PATH})', "in-place edit of Max config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_MAX_CONFIG_PATH}|{_MAX_ENV_PATH})', "in-place edit of Max config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_MAX_CONFIG_PATH}|{_MAX_ENV_PATH})', "in-place edit of Max config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.max/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / MAX_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Max home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\hermes), so folding the user home
    # first would eat the prefix the Max-home fold needs.
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / MAX_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.max``); the matched
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    """Rewrite the resolved absolute Max home prefix to ``~/.max/``.

    Resolves the active ``MAX_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.max/`` so the static ``_MAX_CONFIG_PATH`` / ``_MAX_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/hermes/.max/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$MAX_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from max_constants import get_max_home
        home = get_max_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.max")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``max send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Max ad-hoc temp script."""
    try:
...
_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = (
    "stop/restart max gateway via shell-spliced verb (kills running agents)"
)
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    ``launchctl kick"start" -k gui/501/ai.max.gateway`` auto-approved
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Max YOLO.
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["MAX_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "MAX_SPINNER_PAUSE" in os.environ:
            del os.environ["MAX_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Max approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``MAX_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            MAX_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export MAX_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set MAX_INTERACTIVE or "
                "MAX_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set MAX_INTERACTIVE or "
            "MAX_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("MAX_EXEC_ASK"):
        ...
        # can see (MAX_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``MAX_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from max_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from max_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("MAX_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # MAX_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from max_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (MAX_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # MAX_EXEC_ASK (and sometimes a session platform marker) can leak
