...
machine as Max itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* Max ships under one Python (e.g. 3.11 in a bundled venv) while the
...
def _build_probe_line() -> str:
    ...
    # Bare which() is correct here, unlike Max's own uv call sites: this
    ...
    # local.py puts the Max-managed $MAX_HOME/bin there, so a managed-only
