...
def get_max_home() -> Path:
    ...
    Reads MAX_HOME env var (with HERMES_HOME fallback for migration),
    falls back to the platform-native default.
    ...
    the platform-native default — this would brick 30+ module-level
    ...
    ``max_cli/kanban_db.py``).  See https://github.com/StardustResearch/max-agent/issues/18594.
    """
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Max is installed
...
def get_max_dir(new_subpath: str, old_name: str) -> Path:
...
def iter_max_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    # NOTE: keep this ordering in sync with maxManagedNodePathEntries() in
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    immediately throws ``MODULE_NOT_FOUND``. ``find_max_node_executable``
...
def max_managed_node_tree_present(home: Path | None = None) -> bool:
    ...
    for directory in iter_max_node_dirs(home):
        for name in names:
...
def heal_max_managed_node() -> bool:
    ...
    if _managed_node_heal_attempted:
        return False
    if not max_managed_node_tree_present():
        return False
...
def find_max_node_executable(command: str) -> str | None:
    ...
    for directory in iter_max_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_max_managed_node():
        for directory in iter_max_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_max_node_executable(command)
    if managed:
        return managed
    if max_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_max_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Max-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_max_node_dirs() if path.is_dir()]
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/StardustResearch/max-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    ...
    max_home = get_max_home_override() or (env or {}).get("MAX_HOME") or os.getenv("MAX_HOME")
    if not max_home:
        return None
    profile_home = os.path.join(max_home, "home")
...
def is_container() -> bool:
    ...
    See: StardustResearch/max-agent#47111
    """
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_max_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._max_ipv4_patched = True  # type: ignore[attr-defined]
