...
_MAX_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on MAX_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (MAX_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `max tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (MAX_KANBAN_TASK env "
    ...
    # Coding posture (base Max — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in max_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Max toolsets (CLI + messaging platforms)
    #
    ...
    # the `max send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "max-cli": {
        ...
        "tools": _MAX_CORE_TOOLS,
        "includes": []
    },

    "max-cron": {
        # Mirrors max-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `max tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as max-cli; gated by `max tools`",
        "tools": _MAX_CORE_TOOLS,
        "includes": []
    },
    ...
    "max-email": {
        "description": "Email bot toolset - interact with Max via email (IMAP/SMTP)",
        "tools": _MAX_CORE_TOOLS,
        "includes": []
    },
    ...
    "max-sms": {
        "description": "SMS bot toolset - interact with Max via SMS (Twilio)",
        "tools": _MAX_CORE_TOOLS,
        "includes": []
    },
    ...
    "max-gateway": {
        ...
        "tools": [],
        "includes": ["max-telegram", "max-discord", "max-whatsapp", "max-slack", "max-signal", "max-bluebubbles", "max-homeassistant", "max-email", "max-sms", "max-mattermost", "max-matrix", "max-dingtalk", "max-feishu", "max-wecom", "max-wecom-callback", "max-weixin", "max-qqbot", "max-webhook", "max-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``max-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_MAX_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``max-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (max-<name>).
        # Gives them _MAX_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("max-"):
            platform_name = name[len("max-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_MAX_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
