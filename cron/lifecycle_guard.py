...
or ``systemctl restart max-gateway``).  When the cron fires, the
...
command identifier (``max gateway``, ``launchctl ... max-gateway``,
``systemctl ... max-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_MAX_GATEWAY=1``, and
``max gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `max gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:(?:max|hermes)\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a max-gateway label. macOS launchd
    # labels look like `ai.max.gateway` / `max-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated max services (e.g.
    # `launchctl unload ai.max.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\b(?:max|hermes)[.\-]?gateway)"
    # Branch C: systemctl ops on a max-gateway unit.
