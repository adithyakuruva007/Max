...
# `get_max_home()` (the active profile home), NOT `get_default_max_root()`
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            if dt.tzinfo is None:
                max_tz = _max_now().tzinfo
                dt = dt.replace(tzinfo=max_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _max_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``max cron status``) never sees a
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _max_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _max_now().isoformat(),
        },
    )
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _max_now().strftime("%Y-%m-%d_%H-%M-%S")
