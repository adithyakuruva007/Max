...
Uses a file-based lock (~/.max/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `max update` reloads
# the module) fail with ModuleNotFoundError for max_time et al.
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `max fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `max cron pause {job_ref}` "
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running Max, pin it explicitly: "
                f"`max cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
    ...
    # checkout was updated underneath it (interrupted `max update`, manual
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy max_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from max_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``max tools`` config for the ``cron`` platform.
    ...
    try:
        from max_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    """
    from max_constants import parse_reasoning_effort, resolve_reasoning_config
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``MAX_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("MAX_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid MAX_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    try:
        path = _get_max_home() / "cron" / "inflight_forced_releases.jsonl"
    ...
    *,
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_max_home())
...
# (MAX_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse MAX_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("MAX_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid MAX_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_max_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_max_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.max/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``max update`` / ``max gateway stop`` / systemd restart, or an
...
def _get_max_home() -> Path:
    """Resolve Max home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active MAX_HOME at call time
    ...
    """
    return _hermes_home or get_max_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    hermes_home = _get_max_home()
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from max_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    Runs ``max [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing
    ...
    scheduler's MAX_HOME) or a validated local profile name.  Returns None
    ...
    if hermes_bin:
        ...
        try:
            ...
            if _ilu.find_spec("max_cli") is not None:
                argv = [sys.executable, "-m", "max_cli.main"]
            else:
                return "bot-chat delivery failed: max CLI not resolvable"
        except Exception:
            return "bot-chat delivery failed: max CLI not resolvable"
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution in the child; a leftover MAX_HOME
...
def _resolve_bot_chat_target(job: dict, profile_arg: str) -> Optional[dict]:
    ...
    ``profile_arg`` is ``""`` for the job's own profile (the MAX_HOME
    ...
    ``~/.max/profiles/`` tree, so same-named profiles on other gateways
    can never be targeted by accident.
    """
    if not profile_arg:
        # Own profile: chat subprocess inherits MAX_HOME, no name needed.
        return {"platform": BOT_CHAT_PLATFORM, "chat_id": "", "thread_id": None}
    try:
        from max_cli.profiles import normalize_profile_name, profile_exists
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            try:
                ...
                # MAX_CRON_MEDIA_SEND_TIMEOUT env override).
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # at boot; a standalone process (manual `max cron run` from the CLI,
    ...
    for target in targets:
        ...
        if transport is not None:
            ...
            # (`max cron run`) has no live relay adapter, so surface the
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("MAX_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid MAX_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _get_media_send_timeout() -> int:
    ...
    MAX_CRON_MEDIA_SEND_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("MAX_CRON_MEDIA_SEND_TIMEOUT", "").strip()
    if env_value:
        try:
            timeout = int(float(env_value))
            if timeout > 0:
            ...
            logger.warning(
                "Invalid MAX_CRON_MEDIA_SEND_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _get_session_db_timeout() -> float:
    ...
    Mirrors the ``script_timeout_seconds`` resolution pattern: the
    MAX_CRON_SESSION_DB_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("MAX_CRON_SESSION_DB_TIMEOUT", "").strip()
    if env_value:
        try:
            return float(env_value)
        except (ValueError, TypeError):
            logger.warning(
                "Invalid MAX_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _run_job_script(
    ...
    Scripts must reside within MAX_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Max-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against MAX_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_max_home() / "scripts"
    ...
    # escape — scripts MUST reside within MAX_HOME/scripts/.
    try:
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("MAX_MODEL") or ""
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `max setup`), or pin a "
            "working provider via `max cron edit "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`max setup` or change the job's `deliver` target."
            )
    return None
    ...
    *,
    ...
    if job.get("no_agent"):
        ...
        try:
            ...
            load_hermes_dotenv(hermes_home=_get_max_home())
        except Exception:
            logger.debug(
    ...
    # gateway message. Do not seed MAX_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads MAX_SESSION_PLATFORM
    #     and MAX_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via MAX_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     MAX_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read MAX_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the MAX_CRON_AUTO_DELIVER_* vars set
    # below, so clearing MAX_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # MAX_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ MAX_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "MAX_CRON_AUTO_DELIVER_PLATFORM",
        "MAX_CRON_AUTO_DELIVER_CHAT_ID",
        "MAX_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["MAX_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `max chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with MAX_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $MAX_KANBAN_TASK -- letting
        ...
        # recorded this MAX_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        from max_cli.env_loader import (
            load_hermes_dotenv,
        ...
        if delivery_target:
            _VAR_MAP["MAX_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["MAX_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["MAX_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > MAX_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``max cron edit --model``
        ...
        try:
            ...
            _cfg_path = str(_get_max_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"MAX_MODEL={os.getenv('MAX_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`max cron edit {job_id} --model <name>` or set a "
                "default with `max model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from max_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("MAX_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject MAX_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running Max "
                        "pin it explicitly: "
                        f"`max cron edit {job_id} --provider <provider> "
        ...
        #
        # Bounded with its own timeout (separate from MAX_CRON_TIMEOUT,
        ...
        agent = AIAgent(
            ...
            # MAX_HOME. When a workdir is configured, also inject project
        ...
        # override via MAX_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from max_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
"""
        ...
        if _session_db:
            ...
            # max_state's session_lifecycle_statuses docstring — keep the
...
def run_one_job(
    ...
    *,
    ...
    if extra_prompt is None:
        # A gateway-forwarded manual run (`max cron run --prompt` /
    ...
    profile_home = _get_max_home().resolve()
    with _running_lock:
    ...
    *,
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_max_home())
        )
        ...
        if delivery_error:
            ...
            # exact signature via `max cron incidents ack`.
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`max pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `max resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `max cron run` that claimed a job
        ...
        # Set MAX_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("MAX_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid MAX_CRON_MAX_PARALLEL value; defaulting to unbounded")
