...
from max_cli.config import load_config, _expand_env_vars
from max_cli.fallback_config import get_fallback_chain
from max_time import now as _max_now
...
def _get_max_home() -> Path:
    ...
    """
    return _max_home or get_max_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    max_home = _get_max_home()
    lock_dir = max_home / "cron"
...
def run_job(
    ...
    if job.get("no_agent"):
        ...
        now_iso = _max_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_max_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_max_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        # load_max_dotenv (not a bare load_dotenv) and reset the secret-
        ...
        # forced re-pull off the network. load_max_dotenv also handles the
        ...
        reset_secret_source_cache()
        load_max_dotenv(max_home=_get_max_home())
        ...
        # Model resolution precedence: per-job override > MAX_MODEL env >
        ...
        _raw_cron_timeout = os.getenv("MAX_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid MAX_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_max_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_max_now().strftime('%b %d %H:%M')}"
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _max_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _max_now().strftime('%H:%M:%S'), len(due_jobs))
