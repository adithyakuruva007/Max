...
class PlatformEntry:
    ...
    # status displays (``max setup``, ``max status``, the dashboard
    ...
    # Env vars this platform needs (for ``max setup`` display).
    ...
    # built-ins).  Used by ``max gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved MAX_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `max`
        # invocation -- including plain `max chat`, which never touches any
        ...
        # `max setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``max setup``/``gateway status``, channel
