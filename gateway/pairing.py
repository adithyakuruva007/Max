...
from max_constants import get_max_dir, get_max_home
...
PAIRING_DIR = get_max_dir("platforms/pairing", "pairing")
...
class PairingStore:
    ...
    ``<MAX_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<MAX_HOME>/pairing/``
    directory (backward-compat for the ``max pairing`` CLI).
    """
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        if profile:
            ...
            self._dir = get_max_home() / "profiles" / profile / "pairing"
        else:
