...
from max_constants import get_default_max_root, get_max_dir, get_max_home
...
IMAGE_CACHE_DIR = get_max_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_max_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_max_dir(new_subpath, old_name)
...
AUDIO_CACHE_DIR = get_max_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_max_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_max_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_max_dir("cache/screenshots", "browser_screenshots")
...
_MAX_ROOT = get_default_max_root()
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    for max_root in (_MAX_HOME, _MAX_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(max_root / rel)
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?(?:hermes|max)\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+(?:hermes|max)[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from max_cli.commands import should_bypass_active_session
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _max_run_generation is set on
