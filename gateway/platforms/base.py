...
# Audio file extensions Max recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Max but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Max' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Max-created Telegram private-chat
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``MAX_SESSION_PLATFORM`` contextvar — that contextvar is
...
from max_constants import get_default_hermes_root, get_hermes_dir, get_max_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.max/.env manually."
)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from max_cli.config import load_config_readonly as _load_config
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; MaxAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
_MAX_ROOT = get_default_hermes_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "MAX_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "MAX_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "MAX_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "MAX_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _MAX_HOME / "image_cache",
    _MAX_HOME / "audio_cache",
    _MAX_HOME / "video_cache",
    _MAX_HOME / "document_cache",
    _MAX_HOME / "browser_screenshots",
    ...
    _MAX_HOME / "cache" / "images",
    _MAX_HOME / "cache" / "audio",
    _MAX_HOME / "cache" / "videos",
    _MAX_HOME / "cache" / "documents",
    _MAX_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Max root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* MAX_HOME's cache, so a gateway running at
    the root (e.g. ``MAX_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when MAX_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _MAX_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("MAX_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _MAX_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Max profile and shared Max root both contain control
    ...
    # MAX_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.max
    ...
    for hermes_root in (_MAX_HOME, _MAX_ROOT):
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Max
    secrets (``~/.max/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Max cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.max/...``
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted Max cache dirs (``/root/.max/...``), the session's
    ...
    # In-process gateways (Desktop backend, `max serve`) may not have
    ...
    # are longer prefixes, so /root/.max/... still translates to the host
    # cache — this only catches stray home writes like /root/out.png.
    if not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.max/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.max"):
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    ``MAX_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Max-managed cache, under an operator-allowlisted root
    (``MAX_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Max root (~/.max/.env,
    ...
    # ``MEDIA:~/.max/google_token.json``) remain rejected.
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``MAX_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Max commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-MAX_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("MAX_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "MAX_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "MAX_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Max sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-MAX_HOME holder may be replaced only when the runner
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Max home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Max home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.max/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          MAX_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          MAX_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          MAX_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("MAX_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("MAX_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("MAX_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                    ...
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # MAX_SESSION_PLATFORM contextvar the tool would
    ...
    def build_source(
        ...
        per-profile MAX_HOME isolation.
        """
