...
There are two ways to add a platform to the Max gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.max/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `max gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `max_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Max core.
...
And add it to the `max-gateway` composite:

```python
"max-gateway": {
    "includes": [..., "max-your-platform"]
}
```
...
## 12. Status Display (`max_cli/status.py`)
...
## 13. Gateway Setup Wizard (`max_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ max_cli/ toolsets.py \
  --include="*.py" -l | sort -u
