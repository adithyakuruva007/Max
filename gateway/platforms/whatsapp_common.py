...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Max Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to MAX_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/max), this function
    mirrors the bridge source to a writable MAX_HOME location and returns that
    ...
    # Try MAX_HOME location first
    max_home = get_max_home()
    max_home_bridge = max_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to MAX_HOME if needed
    if max_home_bridge.exists():
        return max_home_bridge

    # Mirror the bridge source to MAX_HOME
    try:
        max_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            max_home_bridge,
            dirs_exist_ok=False,
