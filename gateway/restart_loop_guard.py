...
Defenses 1 and 2 (the ``_MAX_GATEWAY`` guard on ``max gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.max.gateway")``,
...
State lives in ``<MAX_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_max_home() / "gateway" / "restart_loop.json"
