"""Relay/connector support package for the Max gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``max gateway enroll`` writes them to
    ``~/.max/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``MAX_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``max gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from max_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**Max Agent:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``max gateway enroll`` CLI. Three modes, in precedence order:
    ...
    if not token_url:
        ...
        from max_cli.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.max/.env``.
    ...
    sets neither ``MAX_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``max gateway enroll``: has a PINNED
