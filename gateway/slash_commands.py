...
Module-level run.py helpers a handler needs (``_max_home``,
...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _max_home
        ...
        try:
            ...
            atomic_json_write(
                _max_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from max_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _max_home, _load_gateway_config
        import yaml
        ...
        user_provs = None
        custom_provs = None
        config_path = _max_home / "config.yaml"
        try:
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_max_home())
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _max_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _max_home, _platform_config_key
        import yaml
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> str:
        ...
        from gateway.run import _max_home, _load_gateway_config, _resolve_gateway_model
        import yaml
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _max_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _max_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _max_home, _resolve_max_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        max_cmd = _resolve_max_bin()
        if not max_cmd:
            return t("gateway.update.max_cmd_not_found")

        pending_path = _max_home / ".update_pending.json"
        output_path = _max_home / ".update_output.txt"
        exit_code_path = _max_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        try:
            if sys.platform == "win32":
                ...
                # max_cmd is a list of argv parts we can pass directly
                ...
                max_cmd_str = " ".join(shlex.quote(part) for part in max_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {max_cmd_str} update --gateway"
