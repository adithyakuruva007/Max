...
The PID file lives at ``{MAX_HOME}/gateway.pid``.  MAX_HOME defaults to
``~/.max`` but can be overridden via the environment variable.  This means
separate MAX_HOME directories naturally get separate PID files — a property
...
from max_constants import get_max_home, _get_platform_default_max_home
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_max_home() / "gateway-starts.log"
...
def _get_process_hermes_home() -> Path:
    """Return the process-level MAX_HOME, skipping context-local overrides.
    ...
    ``get_max_home()`` honors ``_MAX_HOME_OVERRIDE`` contextvar used for
...
def _canonical_hermes_home(path: Path | str) -> Path:
    """Return a stable absolute MAX_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_hermes_home(left: Path | str, right: Path | str) -> bool:
    """Compare MAX_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_hermes_home(left))) == os.path.normcase(
        str(_canonical_hermes_home(right))
    )


# Mirrors max_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (max_constants + stdlib only).
_PROFILE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")


def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a MAX_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.max/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
    ...
    try:
        ...
        pass
    try:
        if _same_hermes_home(canonical, _get_platform_default_max_home()):
            return "default"
    except Exception:
        pass
    return None
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    MAX_HOME profile than the caller.  Prefers the explicit ``profile``
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting MAX_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("MAX_GATEWAY_LOCK_DIR")
    if override:
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Max' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Max gateway lifecycle subcommand from a command line.
    ...
    test also matched ``max_cli.main gateway status`` and even unrelated
    ...
    ``--profile``/``-p`` selectors from anywhere in argv -- Max's
    ...
    joined = " ".join(tokens)
    has_gateway_entry = (
        "max_cli.main" in joined
        or "max_cli/main.py" in joined
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `max gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Max-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Max gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a MAX_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.max`` or ``$MAX_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``max_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``MAX_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a MAX_HOME= value on the argv
    ...
    # a non-matching explicit MAX_HOME= on the argv. MAX_HOME is usually
...
def _build_pid_record() -> dict:
    return {
        ...
        # MAX_HOME-local.  Persist the owning gateway's process home so an
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    ``max_cli``; the helper itself is cached per process. A gateway
    keeps serving the module versions it imported at startup, so stamping
    the identity into ``gateway_state.json`` lets `max update` (and the
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `max status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``MAX_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``max_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    MAX_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different MAX_HOME dirs).
    """
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-MAX_HOME --replace while
    ...
    target_home = record.get("target_max_home")
    ...
    # ``max gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's MAX_HOME rather than the replacer's.  Same-home callers omit
    ...
    try:
        ...
        record = {
            ...
            "target_max_home": str(marker_home),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target MAX_HOME's gateway PID record, and the live
