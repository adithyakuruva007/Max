...
def _get_process_max_home() -> Path:
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "max" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    ...
    (``"C:\\Program Files\\...\\max-gateway.exe"``) survive, and strips
    ...
    for token in tokens:
        ...
        if basename in ("max-gateway", "max-gateway.exe"):
            return "run"
    ...
    has_gateway_entry = (
        ...
        or any(t.rsplit("/", 1)[-1] in ("max", "hermes.exe") for t in tokens)
    )
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"max_home={home_lc}" in command_lc
        )
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "max_home=" in command_lc and f"max_home={home_lc}" not in command_lc:
        return False
    return True
...
# enabled in the user's systemd (e.g. ``hermes.service`` + ``max-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different MAX_HOME. When two profile gateway
    # services share the same default ~/.max (MAX_HOME not set
    ...
    # field is absent in markers written by older Max versions; treat
    ...
    replacer_home = record.get("replacer_max_home")
    if replacer_home is not None and replacer_home != str(_get_process_max_home()):
        return False
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_max_home": str(_get_process_max_home()),
