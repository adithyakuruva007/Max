...
* Stateless request/response adapters (the API server,
  ...
  ``X-Max-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-Max-Session-Id`` header — the
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-Max-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-Max-Session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "X-Max-Session-Id is rejected (403) on an unauthenticated API "
    ...
    payload = {
        "model": str(getattr(adapter, "_model_name", "") or "max-agent"),
