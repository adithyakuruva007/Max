...
MAX_HOME_DIR="${MAX_HOME:-$HOME/.hermes}"
MAX_CFG="${MAX_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Max config has twozero_td MCP entry ──
if [[ ! -f "$MAX_CFG" ]]; then
    echo -e " ${FAIL} Max config not found at ${MAX_CFG}"
    manual_steps+=("Create ${MAX_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$MAX_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Max config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Max config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${MAX_CFG} manually"); }
    manual_steps+=("Restart Max session to pick up config change")
fi
