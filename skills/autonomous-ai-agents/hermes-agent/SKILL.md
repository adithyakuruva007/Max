...
metadata:
  hermes:
    ...
    homepage: https://github.com/StardustResearch/max-agent
    related_skills: [claude-code, codex, opencode]
---

# Max Agent

Max Agent is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Max works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.
...
This skill is a concise operating guide, not the complete source of truth for every Max feature. If a Max feature, command, or setting is not mentioned here, do not treat that absence as evidence that it does not exist. Check the live repository and official docs before giving a negative answer.

Good verification targets:

- CLI commands: `hermes --help`, `hermes <command> --help`, and `max_cli/main.py`
- User documentation: https://max-agent.stardustresearch.com/docs/
- Source tree: https://github.com/StardustResearch/max-agent
...
pip install max-agent       # or: uv pip install max-agent
...
```
hermes mcp serve            Run Max as an MCP server
...
(`hermes mcp install <name>`): `skill_view(name="max-agent", file_path="references/native-mcp.md")`.
...
Max injects project-level instructions into the system prompt by reading context files from the working directory. The discovery order is **first match wins** — only one project context source is loaded per session.
...
| `AGENTS.md` / `agents.md` | **Cwd only** — subdirectory and parent copies are ignored | You want portable agent instructions that work the same in Max, Claude Code, Codex, etc. |
| `CLAUDE.md` / `claude.md` | Cwd only | Same as AGENTS.md, Claude-flavored |
| `.cursorrules` / `.cursor/rules/*.mdc` | Cwd only | Migrating from Cursor |

`SOUL.md` (in `$MAX_HOME`) is independent and always loaded when present — it sets the agent's identity, not project rules.

### Pick the right one

- **Use `.hermes.md`** when you want Max-specific behavior that lives above the cwd (root + subtree), or when you want rules to inherit from a parent directory. The parent walk stops at the git root, so a home-level `.hermes.md` won't leak into every project (a git repo's root is the boundary).
...
- **Don't put project rules in `~/.max/AGENTS.md`** (or any other home-level location). When Max runs with that directory as cwd, the file loads — but only for that one directory. For cross-project context, use `SOUL.md` (in `$MAX_HOME`, identity-only) or install a skill via `hermes skills install`.
...
`hermes --ignore-rules` skips auto-injection of all project context files (`.hermes.md`, `AGENTS.md`, `CLAUDE.md`, `.cursorrules`) **and** `SOUL.md` identity, plus user config, plugins, and MCP servers. Use it to isolate whether a problem is your setup or Max itself.
...
Max: when working in this repo, follow these rules.
...
That file at `/home/me/projects/myrepo/.hermes.md` is auto-loaded when Max runs in any subdirectory of `/home/me/projects/myrepo`, but not when it runs in `/home/me/other-project`.

## Security & Privacy Toggles

Common "why is Max doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `hermes` invocation) because they're read once at startup.
...
- **Automation Blueprints** — pick a named automation and Max asks for
...
`AF_INET` socket. Root cause is usually Max's env scrubber dropping
...
Max-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
|----------------|----------|
| Config options | `hermes config edit` or [Configuration docs](https://max-agent.stardustresearch.com/docs/user-guide/configuration) |
| Available tools | `hermes tools list` or [Tools reference](https://max-agent.stardustresearch.com/docs/reference/tools-reference) |
...
| Skills catalog | `hermes skills browse` or [Skills catalog](https://max-agent.stardustresearch.com/docs/reference/skills-catalog) |
| Provider setup | `hermes model` or [Providers guide](https://max-agent.stardustresearch.com/docs/integrations/providers) |
| Platform setup | `hermes gateway setup` or [Messaging docs](https://max-agent.stardustresearch.com/docs/user-guide/messaging/) |
| MCP servers | `hermes mcp list` or [MCP guide](https://max-agent.stardustresearch.com/docs/user-guide/features/mcp) |
| Profiles | `hermes profile list` or [Profiles docs](https://max-agent.stardustresearch.com/docs/user-guide/profiles) |
| Cron jobs | `hermes cron list` or [Cron docs](https://max-agent.stardustresearch.com/docs/user-guide/features/cron) |
| Memory | `hermes memory status` or [Memory docs](https://max-agent.stardustresearch.com/docs/user-guide/features/memory) |
| Env variables | `hermes config env-path` or [Env vars reference](https://max-agent.stardustresearch.com/docs/reference/environment-variables) |
| CLI commands | `hermes --help` or [CLI reference](https://max-agent.stardustresearch.com/docs/reference/cli-commands) |
...
`_MAX_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_max_home()` for paths,
never hardcode `~/.max`. For custom/local-only tools, write a plugin in
`~/.max/plugins/` instead of editing core — see the developer docs.
