...
1. `hermes webhook subscribe` writes to `~/.max/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status max-gateway` or `ps aux | grep gateway`
