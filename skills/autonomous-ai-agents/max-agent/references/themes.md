# Themes / Skins — Author a Max Color Theme

Author a Max **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`max_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.max/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Max itself.
...
- Write access to the Max home dir — `~/.max` by default, or `$MAX_HOME`
  / the active profile's dir. Skins live in `<hermes-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `max config set`).
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   max config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `max config set display.skin default` (or they can `/skin default`).
...
```
hermes skin set <key> <hex>      # e.g. max skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `max skin set background "#08201f"` changes
only the background; `max skin use <name>` / `max skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.max`** when a profile is active — resolve the real home
  from `$MAX_HOME` first, falling back to `~/.max`.
...
- **Never hand-edit `config.yaml` to activate.** Use `max config set
...
- **You apply it, not the user.** `max config set display.skin <name>` is
...
- Run `max config get display.skin` and confirm it reports `<name>`.
