...
## Layer 1 — Is this thing a Max plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Max process**. Its LLM calls go through Max's already-configured provider. | Already uses Portal if user's Max is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.stardustresearch.com`. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside Max. That LLM call is already authenticated
via Max's provider config. The plugin's own server auth (e.g.
...
Portal at `https://inference-api.stardustresearch.com/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.stardustresearch.com → API Keys`, or
...
What Max Agent has that *feels* like OAuth — `max login --provider nous`
opening a browser, user signs in, token lands in `~/.max/auth.json` — is a
**Max-specific browser flow**. Under the hood it produces a credential
Max uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Max's existing Portal credential out of `~/.max/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `inference-api.stardustresearch.com/v1` with that
   bearer attached.
...
rides on the credential Max already holds.

Where this could live in Max:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Max itself.
...
1. First decide: Max plugin (runs inside Max) or separate app? If plugin,
   it already uses Portal via Max's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.stardustresearch.com/v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Max agent itself") is fully supported. Max treats agent
...
pitch in the Tool Gateway announcement is specifically about Max Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Max is configured to use Portal as its provider. It is
