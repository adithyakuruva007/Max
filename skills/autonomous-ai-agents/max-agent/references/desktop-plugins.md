...
Write plugins for the Max desktop app: statusbar items, layout panes,
...
system (`~/.max/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$MAX_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$MAX_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Max desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$MAX_HOME/desktop-plugins/` (usually
  `~/.max/desktop-plugins/`).

## How to Run

1. Create `$MAX_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.max/...` by default, or `~/.max/profiles/<profile>/...` under a
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Max
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.max/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
