...
Author widget apps for the Max TUI (`max --tui`): glanceable ambient
...
- The TUI must be in use (`max --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.max/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.max/logs/tui_gateway_crash.log` if a widget never appears.
