...
Set via `max model` (picker) or `max setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://max-agent.stardustresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `max auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `max auth add openai-codex` |
| qwen-oauth | OAuth | `max auth add qwen-oauth` |
| minimax-oauth | OAuth | `max auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`max auth`).
Fallback chain when the primary fails: `max fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`max_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   max config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
