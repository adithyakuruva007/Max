...
**three** code paths in Max that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Max — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Max
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Max is
   refusing my value."
...
max config get delegation.max_concurrent_children

# 2. Did Max' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.max/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with max on path)
...
appears, **the cap is the model**, not Max.
