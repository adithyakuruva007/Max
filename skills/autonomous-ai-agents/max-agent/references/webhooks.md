...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Max agent runs by POSTing events to a URL.
...
All management is via the `max webhook` CLI command:
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.max/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.max/scripts/` (path traversal is blocked).
...
Full filter syntax: https://max-agent.stardustresearch.com/docs/user-guide/messaging/webhooks#payload-filters
...
- Subscriptions persist to `~/.max/webhook_subscriptions.json`

## How It Works

1. `max webhook subscribe` writes to `~/.max/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.max/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `max webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `max webhook test <name>` to verify the route works.
