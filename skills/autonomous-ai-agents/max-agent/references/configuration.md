...
Full reference: https://max-agent.stardustresearch.com/docs/user-guide/configuration
...
`max config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `max tools` (interactive) or `max tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_MAX_CORE_TOOLS` is the default bundle most platforms inherit).
