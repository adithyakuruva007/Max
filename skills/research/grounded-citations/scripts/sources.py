...
Ledger path resolution (first wins):
  --ledger PATH
  $MAX_CITATION_LEDGER
  $MAX_HOME/cache/citations/ledger.json
"""
...
from _hermes_home import get_max_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("MAX_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_max_home() / "cache" / "citations" / "ledger.json"
