...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _max_env="${MAX_HOME:-$HOME/.hermes}/.env"; [ -f "$_max_env" ] && grep -q "^GITHUB_TOKEN=" "$_max_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_max_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${MAX_HOME:-$HOME/.hermes}/skills/github/github-auth/scripts/gh-env.sh"
