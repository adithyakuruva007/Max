#!/usr/bin/env python3
"""Google Workspace API CLI for Max Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Max-facing JSON contract and falls back to the Python client
...
from _max_home import get_max_home
...
def _gws_binary() -> str | None:
    override = os.getenv("MAX_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Max Agent")
