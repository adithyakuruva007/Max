...
Extracted verbatim from max_state.py so the SessionDB mixin modules
(max_state_search / max_state_schema / max_state_portability) can
reference them without importing max_state (which would be a cycle).
max_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``max sessions optimize-storage``. A legacy DB sits at
...
# `max sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `max db optimize`. This is
...
#
# Several independent Max processes routinely share one state.db (gateway
# service, the Desktop app's `max serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `max_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not max_state)
# because the search/schema mixins cannot import max_state (cycle).
#
...
logger = logging.getLogger("max_state")
