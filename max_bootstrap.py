"""Windows UTF-8 bootstrap for Max entry points.
...
should be imported at the very top of every Max entry point
(``max``, ``max-agent``, ``max-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``max_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``max_bootstrap``, never ``max_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Max modules.

    Max ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Max source root
    ...
    """
    root = src_root or os.environ.get("MAX_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``MAX_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("MAX_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import max_bootstrap``
# (or ``from max_bootstrap import apply_windows_utf8_bootstrap``) at
