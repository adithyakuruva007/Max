...
or change arbitrary Max configuration.
...
small host fact requested by the RFC. Settings writes use Max'
...
`MAX_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<MAX_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
The implementation is covered with real temporary-Max-home tests for:
...
- [Issue #64227](https://github.com/NousResearch/max-agent/issues/64227)
- [RFC PR #58542](https://github.com/NousResearch/max-agent/pull/58542) by Topher Ross
