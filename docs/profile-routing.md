...
`max config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.max/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `MAX_HOME`, secret scope, and
