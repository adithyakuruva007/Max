...
This document is the formal interface between the **Max gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `max gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`max gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `max` CLI.
