...
When running Max inside Docker, the default `network_mode: host` gives the
...
The Max [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   MAX_UID=$(id -u) MAX_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=hermes,max-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://max-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Max trust model and vulnerability reporting
