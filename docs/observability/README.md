...
Max observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Max' planner, model providers, memory, tool registry,
...
Max also has a first-party NeMo Relay shared-metrics path. It uses these
...
Hook callbacks are fail-open. Max catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Max only builds
...
The native NeMo Relay SDK integration maps Max session, turn, LLM, and tool
