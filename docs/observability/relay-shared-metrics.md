...
Max includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Max and does not require a Max observability plugin. Max remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Max execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `max-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Max `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `MAX_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `MAX_NEMO_RELAY_ATOF_*` and `MAX_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Max does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Max requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Max installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Max process so configured interceptors can
...
Collection remains off unless Max policy enables it:
...
opt-in. Set `MAX_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Max does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Max
...
Relay plugin configuration is a process-level deployment choice, not a Max
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Max process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Max logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Max process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Max
process and share the same policy boundary.

Run profiles in separate Max processes when they require different trust
...
Max core owns one Relay host and one isolated Relay session scope per Max
session. Core lifecycle producers use
`max_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Max plugin registration. Shared-metrics marks must
...
```text
Max turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Max shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Max sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Max used for the logical call, such as
...
counter in any rolling 24-hour window. The metric has no dimensions; Max
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Max processes share the SQLite latch, so simultaneous
...
the owning Max session. The start counter contains only bounded execution
...
provider attempts for the same Max API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Max tool-call
...
cancellations. Active task ownership follows the task ID if Max rotates its
...
outcome, approval outcome, latency, and explicit retry-count buckets. Max
...
tool or plugin names. Max does not infer retries from repeated tool names or
...
state, and a use-count bucket. Max derives reuse and patch-generation
...
```text
$MAX_HOME/telemetry/shared_metrics/metrics.sqlite3
$MAX_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Max version,
...
schema field name, its current scope is one `MAX_HOME`, so it is more
...
Deleting `$MAX_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `MAX_HOME`. To reset it, stop Max
processes and remove `$MAX_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Max CLI turn against the deterministic local model server:
