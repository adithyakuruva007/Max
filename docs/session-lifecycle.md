...
The canonical transcript store is SQLite via `SessionDB` (from `max_state`). The
...
Counts consecutive restarts via a JSON file (`{MAX_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `max update`, `max gateway restart`,
or `/restart`.
