...
needs, and a profile is just a MAX_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.max/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — MAX_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with MAX_HOME override |
...
### Seam #1 — hub-skill install cannot use the MAX_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = MAX_HOME / "skills"` at **module
import time**. The context-local `set_max_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_max_action`
runs `python -m max_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_max_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's MAX_HOME bound
...
spawned detached (`_spawn_max_action` returns a PID immediately). So the
create flow is:

1. `create_profile()` — make the dir (synchronous)
2. write model (synchronous, MAX_HOME override)
3. write selected MCP servers (synchronous, MAX_HOME override)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, MAX_HOME override
...
page afterward. Mirror `_write_profile_model`'s MAX_HOME-override helper for
...
- Backend E2E with isolated MAX_HOME: POST a full create body
