...
has to be thrown away or summarized. Max has always done this in one batch:
...
Max folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Max's normal session flush is
...
With `micro_compact` unset or `false` Max behaves exactly as it always has:
...
Defaults to `$MAX_HOME/logs/agent.log`. It reports passes, outcome mix, net
