...
``max_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import max_state (cycle) — shared
module-level constants live in max_state_common.
"""
...
# Moved methods logged under the "max_state" logger before the split;
...
def schema_read_probe_statements() -> tuple:
    ...
    ``max_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `max update` until the user's
...
class SessionSchemaMixin:
            ...
            if escalated:
                ...
                if foreign_holders:
                    logger.error(
                        ...
                        "`max sessions optimize-storage` with the gateway stopped. "
                        "`max doctor` reports this degraded state.",
                        attempts,
                        foreign_holders,
                    )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from max_constants import get_max_home
            cache_path = get_max_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `max sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `max db optimize`) must keep its EXISTING
