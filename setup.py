...
Max Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``MAX_NIX_BUILD=1`` on the
Max package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("MAX_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for max-agent is not supported.\n"
    "Max is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://max-agent.stardustresearch.com/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Max Nix derivation sets MAX_NIX_BUILD=1. If it does, file a bug."
)
