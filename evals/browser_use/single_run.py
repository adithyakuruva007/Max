...
The run gets a throwaway MAX_HOME so no local config leaks in, and the
...
if _or_key:
    ...
    # Resolved by the orchestrator BEFORE MAX_HOME is redirected to the
    ...
    _agent_auth = dict(
        base_url=os.environ.get("BUBENCH_NOUS_BASE_URL", "https://inference-api.stardustresearch.com/v1"),
        api_key=_tok,
        provider="nous",
    )
