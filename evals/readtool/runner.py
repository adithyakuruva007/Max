"""Run the read-tool eval through the REAL Max AIAgent.

For each task: fresh temp MAX_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    hermes_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".max"
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.max/.env; set +a  — then relaunch."
        )
