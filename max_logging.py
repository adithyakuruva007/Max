...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    home = hermes_home or get_max_home()
    from max_constants import mkdir_under_hermes_home
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse max_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when max_cli.config isn't importable (bare
        # max_logging consumers).
        try:
            from max_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
