...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with max-agent)
2. User plugins: ``$MAX_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$MAX_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$MAX_HOME/plugins/model-providers/`` if it exists."""
    try:
        from max_constants import get_max_home

        d = get_max_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple MAX_HOME profiles don't alias each other.
...
def _discover_entry_point_providers() -> None:
    ...
    * **Provider targets only.** The ``hermes_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$MAX_HOME``) keep their
    ...
    try:
        from max_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$MAX_HOME/plugins/model-providers/<name>/``
    ...
    #    filesystem steps means a bundled or ``$MAX_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with max-agent.
    ...
    # 2. User plugins — under $MAX_HOME/plugins/model-providers/<name>/.
