...
React + Ink terminal UI for Max. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `MAX_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `max setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.max/.max_history` or under `MAX_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `max setup` wizard, suspends Ink while it runs
...
ui-tui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external max setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.max/tui-parent.log
...
tui_gateway/
  ...
  slash_worker.py        persistent MaxCLI subprocess for slash commands
```
