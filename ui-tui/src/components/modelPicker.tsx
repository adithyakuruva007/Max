import { Box, Text, useInput, useStdout } from '@max/ink'
...
export function ModelPicker({
  ...
  useInput((ch, key) => {
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `max model` to configure'
                      }
                    : p
                )
              )
    ...
    // filter. With Ctrl held, @max/ink reports `ch` as the key name ('g'),
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted} wrap="truncate-end">
          Paste your API key below (saved to ~/.max/.env)
        </Text>
