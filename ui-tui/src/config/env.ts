import type { MouseTrackingMode } from '@max/ink'
...
export const STARTUP_RESUME_ID = (dualEnv('TUI_RESUME') ?? '').trim()
export const STARTUP_QUERY = (dualEnv('TUI_QUERY') ?? '').trim()
export const STARTUP_IMAGE = (dualEnv('TUI_IMAGE') ?? '').trim()
...
//   behavior.
const mouseTrackingOverride = parseToggle(dualEnv('TUI_MOUSE_TRACKING'))
const mouseTrackingDisabledLegacy = truthy(dualEnv('TUI_DISABLE_MOUSE'))
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(dualEnv('TUI_NO_CONFIRM'))
...
export const DASHBOARD_TUI_MODE = truthy(dualEnv('TUI_DASHBOARD'))
...
export const DEV_CREDITS_MODE = truthy(dualEnv('DEV_CREDITS'))

const inlineOverride = parseToggle(dualEnv('TUI_INLINE'))
...
// synthetic timer).
export const SHOW_FPS = truthy(dualEnv('TUI_FPS'))
