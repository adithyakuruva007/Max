...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $MAX_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see MAX_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['MAX_TUI_BACKGROUND', process.env.MAX_TUI_BACKGROUND ?? '(unset)'],
            ['MAX_TUI_THEME', process.env.MAX_TUI_THEME ?? '(unset)'],
