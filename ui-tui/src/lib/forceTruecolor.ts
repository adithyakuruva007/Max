...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.MAX_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Max on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via MAX_TUI_TRUECOLOR=1.
