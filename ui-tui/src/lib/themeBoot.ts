...
// Profile-aware: the Python launcher exports MAX_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.max matches get_max_home()'s default.
const bootFilePath = () => join(process.env.MAX_HOME ?? join(homedir(), '.max'), 'tui-theme-boot.json')

// Never touch the user's real ~/.max from test runs (the TS suite has no
// MAX_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into MAX_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.MAX_TUI_THEME || env.MAX_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.MAX_TUI_BACKGROUND
  ) {
    env.MAX_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.MAX_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.MAX_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into MAX_TUI_THEME.
