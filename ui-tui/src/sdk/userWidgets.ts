...
/**
 * User widget apps — Max authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$MAX_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.max/plugins/`: files under MAX_HOME execute
...
const widgetsDir = () => join(process.env.MAX_HOME?.trim() || join(homedir(), '.max'), 'tui-widgets')
 ...
 *  every change, so a widget Max writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Max) ever writes must hot-load too — a 10s poll here read
