...
# 5. Symlinks the 'max' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Max Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/max-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink max into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up max command..."

MAX_BIN="$SCRIPT_DIR/venv/bin/max"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MAX_BIN" "$COMMAND_LINK_DIR/max"
echo -e "${GREEN}✓${NC} Symlinked max → $COMMAND_LINK_DISPLAY_DIR/max"
...
# ============================================================================
# Seed bundled skills into ~/.max/skills/
# ============================================================================

MAX_SKILLS_DIR="${MAX_HOME:-$HOME/.max}/skills"
mkdir -p "$MAX_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.max/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$MAX_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  max status        # Check configuration"
if is_termux; then
    echo "  max gateway       # Run gateway in foreground"
else
    echo "  max gateway install # Install gateway service (messaging + cron)"
fi
echo "  max cron list     # View scheduled jobs"
echo "  max doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m max_cli.main setup
fi
