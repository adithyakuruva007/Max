...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(MAX_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Max-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.max/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  return path.join(os.homedir(), '.max', 'desktop-ssh')
}
