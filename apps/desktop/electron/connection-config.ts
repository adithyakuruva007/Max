 ...
 *     `X-Max-Session-Token` header; WS uses `?token=`.
...
// max_cli/dashboard_auth/cookies.py.
//
...
//     reuse-detected — Portal NAS #293 / max #37247). When the AT cookie
...
// The Nous portal (NAS) does NOT use Max gateway session cookies — it is a
...
// Keep this aligned with max_cli.profiles.validate_profile_name(). `default`
...
/** Which Max profile the remote SSH dashboard should actually run as.
...
// Max Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteMaxPath = String(entry.remoteMaxPath || '').trim()

  if (remoteMaxPath) {
    out.remoteMaxPath = remoteMaxPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Max installation. Preserve an explicit mapping
  // when it is a valid Max profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `MAX_HOME` scopes it.
 *
...
/**
 * True if any cookie in `cookies` is a max session ACCESS-token cookie
 ...
 * the Max gateway session cookies, so cloud sign-in / discovery liveness
