export type ParentWatchdogEnv = {
  MAX_PARENT_PID: string
  MAX_PARENT_START_MARKER?: string
  MAX_PARENT_NONCE?: string
  /** Spawn tag consumed by max_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { MAX_PARENT_PID: String(pid) }
  env.MAX_SPAWN = spawnTag(pid, startMarker)
  ...
  env.MAX_PARENT_START_MARKER = startMarker
  env.MAX_PARENT_NONCE = nonce

  return env
}

/** Build the `MAX_SPAWN` tag mirrored by max_cli/process_identity.py.
