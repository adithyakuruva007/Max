 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `max update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `max desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Max couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>max desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>max desktop --force-build</code>, then restart the app.</p>
