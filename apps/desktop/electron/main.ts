...
import { buildDesktopBackendEnv, normalizeMaxHomeRoot } from './backend-env'
import { canImportMaxCli, verifyMaxCli } from './backend-probes'
...
const USER_DATA_OVERRIDE = process.env.MAX_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.MAX_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.MAX_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with MAX_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[max] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[max] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('max:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[max] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[max] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[max] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[max] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// MAX_HOME — the user-facing root for everything Max-related. Mirrors
// scripts/install.ps1's $MaxHome and scripts/install.sh's $MAX_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\max (matches install.ps1)
//   macOS / Linux: ~/.max (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.max directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\max yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// MAX_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// MAX_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.max / %LOCALAPPDATA%\max.
function resolveMaxHome() {
  if (process.env.MAX_HOME) {
    return normalizeMaxHomeRoot(process.env.MAX_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'max-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a MAX_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\max and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('MAX_HOME')

    if (fromRegistry) {
      return normalizeMaxHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'max')
    const legacy = path.join(app.getPath('home'), '.max')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.max setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.max')
}

const MAX_HOME = resolveMaxHome()

function maxManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_max_node_dirs() in
  // max_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(MAX_HOME, 'node')
...
function pathWithMaxManagedNode(...entries) {
  return [...maxManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_MAX_ROOT — the canonical mutable Max install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_MAX_ROOT = path.join(MAX_HOME, 'max-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_MAX_ROOT, 'venv')
...
// boots, going straight to `resolveMaxBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_MAX_ROOT (not alongside)
...
// active-profile.json records which Max profile the desktop launches its
// local backend as. When set, startMax() passes `max --profile <name>
// dashboard …`, which deterministically pins MAX_HOME (see
// _apply_profile_override in max_cli/main.py) and bypasses the sticky
// ~/.max/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors max_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// maxDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under MAX_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by max_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(MAX_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (max_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.MAX_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.MAX_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'max:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.stardustresearch.max) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.stardustresearch.max')
}

// Seed the native About panel with the live Max version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `max update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveMaxVersion(),
  copyright: 'Copyright © 2026 Stardust Research'
})
...
const MEDIA_PROTOCOL = 'max-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(max-media://…)`.
...
// (the desktop's launch profile) stays managed by maxProcess +
// connectionPromise + startMax(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.MAX_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.MAX_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startMax() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning max serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Max backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (maxLog.length > 300) {
    maxLog.splice(0, maxLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-max-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('max:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the max:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('max:bootstrap:event', ev)
}
...
// (`force_kill_other_max`, taskkill /IM max.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(MAX_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Max will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(MAX_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `max` — must not shadow `max.cmd`/`max.exe`.
...
function unwrapWindowsVenvMaxCommand(command, backendArgs) {
  ...
  if (!/^max(?:\.exe)?$/i.test(path.basename(resolved))) {
    return null
  }
  ...
  // died mid-`pip install` still has python.exe + max.exe on disk, but the
...
// spawns `max serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `max` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'max_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, MAX_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isMaxSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'max_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.MAX_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Max deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Max' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\max\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'max', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'max', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal MAX_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\max\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'max', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'max', 'git', 'bin', 'git.exe'))
  }
...
function recentMaxLog() {
  return maxLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's max root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_MAX_ROOT.
// MAX_DESKTOP_MAX_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.MAX_DESKTOP_MAX_ROOT && path.resolve(process.env.MAX_DESKTOP_MAX_ROOT),
    !IS_PACKAGED && isMaxSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isMaxSourceRoot(ACTIVE_MAX_ROOT) ? ACTIVE_MAX_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_MAX_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('max:updates:progress', merged)
  }
}
...
// MAX_HOME/max-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_max_home). That binary owns
// ALL repo mutation — running `max update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'max-setup.exe' : 'max-setup'
  const candidate = path.join(MAX_HOME, name)
...
// Path to the venv shim whose lock decides whether `max update` can write
// fresh entry points. On Windows this is the file the running backend
// `max.exe` holds open; on POSIX it's never mandatory-locked.
function venvMaxShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'max.exe')
    : path.join(updateRoot, 'venv', 'bin', 'max')
}
...
// only signals the direct child, so on Windows a backend `max.exe` that
// spawned its own grandchildren (a `max` REPL, a pty terminal session, the
...
// `maxProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `max.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a max
// REPL, a pty terminal, the gateway) keep `max.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  if (maxProcess && Number.isInteger(maxProcess.pid)) {
    pids.push(maxProcess.pid)
  }
  ...
  if (maxProcess && !maxProcess.killed) {
    try {
      maxProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvMaxShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    if (maxProcess && Number.isInteger(maxProcess.pid)) {
      stragglers.push(maxProcess.pid)
    }
...
// `max update`). Instead we spawn the staged Max-Setup binary with
// --update and quit, so it can run `max update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri max-setup. Unlike Windows
      ...
      // whole update itself: `max update` (backend) + `max desktop
    ...
    if (!updater) {
      ...
      // `max desktop`, never the Tauri installer that self-copies
      // max-setup.exe into MAX_HOME). They DO have a working `max`
      ...
      // checkout they're on — bare `max update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `max update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `max update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, maxRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Max — this window will close and the updater will open. Don’t reopen Max yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // max.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running max, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Max install open ' +
        '(a second Max window or a terminal running max?). Close it and retry.'
    ...
    // `max update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: MAX_HOME,
      env: {
        ...process.env,
        MAX_HOME,
        PATH: pathWithMaxManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(MAX_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvMax = path.join(venvBin, IS_WINDOWS ? 'max.exe' : 'max')
  ...
  // not just the `max.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvMax) || fileExists(path.join(updateRoot, '.max-bootstrap-complete'))
...
// Resolve the max CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `max` on PATH.
function resolveMaxCliBinary(updateRoot) {
  const venvMax = path.join(updateRoot, 'venv', 'bin', 'max')

  if (fileExists(venvMax)) {
    return venvMax
  }

  return findOnPath('max') || null
}
...
// macOS/Linux in-app update: backend (`max update`) + OS-aware GUI rebuild
// (`max desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const max = resolveMaxCliBinary(updateRoot)

  if (!max) {
    emitUpdateProgress({ stage: 'manual', message: 'max update', percent: null })

    return { ok: true, manual: true, command: 'max update', maxRoot: updateRoot }
  }

  // Put the Max-managed Node and the venv on PATH so `max desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\max\node, not node\bin.
  const env: Record<string, string> = {
    MAX_HOME,
    PATH: pathWithMaxManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `max update` reaps stale `max serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads MAX_DESKTOP_CHILD_PID and excludes
  ...
  if (maxProcess && Number.isInteger(maxProcess.pid)) {
    desktopChildPids.push(maxProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.MAX_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Max (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(max, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'max update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'max update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(max, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Max to retry.',
  ...
  // Linux in-app update terminal state (#45205). `max desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Max…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `max-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Max to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Max desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Max to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Max.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Max.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Max to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.max-update-new"; then
    rm -rf "$DST.max-update-old" 2>/dev/null || true
    mv "$DST" "$DST.max-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.max-update-new" "$DST"
    rm -rf "$DST.max-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Max to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_MAX_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isMaxSourceRoot(ACTIVE_MAX_ROOT) &&
    fileExists(venvPython) &&
    canImportMaxCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_MAX_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `max
...
function resolveWebDist() {
  const override = process.env.MAX_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set MAX_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: max desktop --force-build`
  )
...
function resolveMaxCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Max.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveMaxCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_MAX_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Max at ${ACTIVE_MAX_ROOT}`,
    command,
    args: ['-m', 'max_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      maxHome: MAX_HOME,
      pythonPathEntries: [ACTIVE_MAX_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_MAX_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveMaxBackend(backendArgs) {
  // 1. Explicit override -- MAX_DESKTOP_MAX_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.MAX_DESKTOP_MAX_ROOT && path.resolve(process.env.MAX_DESKTOP_MAX_ROOT)

  if (overrideRoot && isMaxSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Max source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `max` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isMaxSourceRoot.)
  if (!IS_PACKAGED && isMaxSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Max source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_MAX_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\max\max-agent (Windows) or ~/.max/max-agent.
  ...
  //    to spawning max. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `max update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `max` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    MAX_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.MAX_DESKTOP_IGNORE_EXISTING !== '1') {
    let maxCommand = null
    const maxOverride = process.env.MAX_DESKTOP_MAX

    if (maxOverride) {
      const resolvedOverride = findOnPath(maxOverride)

      if (resolvedOverride) {
        maxCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(maxOverride, { isWsl: IS_WSL })) {
        maxCommand = maxOverride
      } else {
        rememberLog(`Ignoring Windows Max override under WSL: ${maxOverride}`)
      }
    } else {
      maxCommand = findOnPath('max')
    }

    if (maxCommand) {
      if (looksLikeDesktopAppBinary(maxCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Max CLI: ${maxCommand}`)
        maxCommand = null
      }
    }

    if (maxCommand) {
      const unwrapped = unwrapWindowsVenvMaxCommand(maxCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `max` shim
      ...
      const shellForProbe = isCommandScript(maxCommand)

      if (verifyMaxCli(maxCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvMaxCommand(maxCommand, backendArgs) || {
            label: `existing Max CLI at ${maxCommand}`,
            command: maxCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
      ...
      rememberLog(
        `Ignoring existing Max CLI at ${maxCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed max_cli module via system Python.
  ...
  if (python) {
    ...
    // having max_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\max\max-agent\venv.
    if (canImportMaxCli(python)) {
      return {
        kind: 'python',
        label: `installed max_cli module via ${python}`,
        command: python,
        args: ['-m', 'max_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: max_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveMaxBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Max Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveMaxBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Max install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Max recovery was handed off to Max Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      maxHome: MAX_HOME,
      logRoot: path.join(MAX_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Max install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Max bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(MAX_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startMax() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // max:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveMaxBackend(backend.args))
  }
  ...
  if (!isMaxSourceRoot(ACTIVE_MAX_ROOT)) {
    throw new Error(
      `Max install at ${ACTIVE_MAX_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Max' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\max\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `max` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Max on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isMaxSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Max venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Max at ${ACTIVE_MAX_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Max runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Max backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Max backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Max backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Max-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('max:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveMaxCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('max:preview-file-changed', payload)
}
...
async function waitForMax(baseUrl, token) {
  ...
  throw new Error(`Max backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('max:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('max:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Max...".
function sendPowerResume() {
  ...
  webContents.send('max:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('max:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean) {
  ...
  webContents.send('max:window-state-changed', state)
}
...
function setAndPersistZoomLevel(window, zoomLevel) {
  ...
  window.webContents.send('max:zoom:changed', { level: next, percent: zoomLevelToPercent(next) })
  window.webContents
    .executeJavaScript(
...
//
// Hosted Max gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``max_session_at``),
...
//     Both are set as HttpOnly cookies (``max_session_at`` ~15 min,
//     ``max_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:max-remote-oauth'
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Max backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root MAX_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.MAX_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.MAX_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by MAX_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (hermes #37247), and the    // gateway middleware transparently rotates a fresh ~15-min access token
  ...
  if (!token) {
    throw new Error(
      'Remote Max gateway is selected, but no session token is saved. ' +
...
//   2. env override (MAX_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.MAX_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.MAX_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'MAX_DESKTOP_REMOTE_URL is set but MAX_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Max backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.MAX_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Max
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startMax())
  ...
  // connect to Max gateway". Mirror the renderer's connect here so the test
...
function resetMaxConnection() {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(maxProcess)
...
// startMax() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetMaxConnection() nulls maxProcess.
  const dying = maxProcess && !maxProcess.killed ? maxProcess : null
  resetMaxConnection()
...
// profile to startMax() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startMax() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForMax(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: maxLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited MAX_HOME env (see _apply_profile_override
  // step 3 in max_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveMaxBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const maxCwd = resolveMaxCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Max backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: maxCwd,
        MAX_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        MAX_DESKTOP: '1',
        MAX_WEB_DIST: webDist,
        ...(readyFile ? { MAX_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Max backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Max backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Max backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForMax(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Max backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_max_home() recreates the deleted profile directory.
...
async function startMax() {
  ...
  // process, every subsequent startMax() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Max backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())

    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Max backend at ${remote.baseUrl}`, 24)
      await waitForMax(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Max backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.max/active_profile file) and
    // resolves MAX_HOME the same way `max -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Max runtime', 28)
    const backend = await ensureRuntime(resolveMaxBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const maxCwd = resolveMaxCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Max backend via ${backend.label}`, 84)
    rememberLog(`Starting Max backend via ${backend.label}`)
    ...
    maxProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin MAX_HOME for the child so Python's get_max_home()
          // resolves to the SAME location our resolveMaxHome() picked. Without
          // this pin, Python falls back to ~/.max on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\max, which differs from C:\Users\<u>\.max.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets MAX_HOME via setx; the desktop
    ...
    maxProcess.stdout.on('data', rememberLog)
    maxProcess.stderr.on('data', rememberLog)
    ...
    maxProcess.once('error', error => {
      rememberLog(`Max backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Max backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    maxProcess.once('exit', (code, signal) => {
      rememberLog(`Max backend exited (${signal || code})`)
      maxProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Max backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Max backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentMaxLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Max backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(maxProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Max backend to become ready', 90)
    await Promise.race([waitForMax(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null maxProcess when the child dies.
      childAlive: () => maxProcess !== null && maxProcess.exitCode === null && !maxProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Max backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Max is
...
// pushes pet state over IPC (max:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via max:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Max app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Max icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see max:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Max in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('max:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startMax().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('max:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Max…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('max:connection:revalidate', async () => {
  if (!connectionPromise) {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetMaxConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Max backend failed liveness probe; dropping stale connection.')
    resetMaxConnection()

    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('max:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('max:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('max:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('max:window:openNewSession', async () => {
...
ipcMain.handle('max:zoom:get', event => {
...
ipcMain.on('max:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('max:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('max:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('max:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('max:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('max:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('max:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('max:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('max:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('max:pet-overlay:control', payload)
})
ipcMain.handle('max:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startMax() call restarts the
...
ipcMain.handle('max:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startMax() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('max:bootstrap:cancel', async () => {
...
ipcMain.handle('max:boot-progress:get', async () => bootProgressState)
ipcMain.handle('max:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('max:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('max:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('max:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('max:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('max:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('max:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('max:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('max:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('max:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new MAX_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('max:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('max:requestMicrophoneAccess', async () => {
...
ipcMain.handle('max:api', async (_event, request) => {
  ...
  // backend calls ensure_max_home() which recreates the profile directory,
...
ipcMain.handle('max:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('max:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('max:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('max:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('max:readFileText', async (_event, filePath) => {
...
ipcMain.handle('max:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('max:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('max:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('max:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('max:saveClipboardImage', async () => {
...
ipcMain.handle('max:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('max:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('max:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('max:titlebar-theme', (_event, payload) => {
...
ipcMain.on('max:native-theme', (_event, mode) => {
...
ipcMain.on('max:translucency', (_event, payload) => {
...
ipcMain.handle('max:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('max:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveMaxCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('max:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveMaxCwd()
}))

ipcMain.handle('max:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('max:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('max:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('max:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('max:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('max:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: maxLog.slice(-200) }))
...
function terminalShellCommand() {
  // MAX_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.MAX_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Max' TUI into
  ...
  env.TERM_PROGRAM = 'Max'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a max/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from MAX_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.MAX_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `max:terminal:${id}:${suffix}`
}
...
ipcMain.handle('max:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('max:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('max:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('max:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('max:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('max:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('max:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('max:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('max:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('max:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('max:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('max:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('max:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('max:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('max:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('max:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('max:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('max:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('max:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('max:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('max:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('max:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('max:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('max:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('max:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('max:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('max:terminal:write', (_event, id, data) => {
...
ipcMain.handle('max:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('max:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('max:updates:check', async () =>
...
ipcMain.handle('max:updates:apply', async (_event, payload) =>
...
ipcMain.handle('max:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('max:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Max version (the one `release.py` bumps in
// max_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Max version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveMaxVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'max_cli', '__init__.py')
...
// Re-resolve the live Max version and push it into the native About panel
// just before showing it, so an in-place `max update` is reflected without
...
ipcMain.handle('max:version', async () => ({
  appVersion: resolveMaxVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  maxRoot: resolveUpdateRoot()
}))
...
// the actual removal via `max uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (max_cli/uninstall.py + max_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_MAX_ROOT
  ...
  const fallback = () => ({
    max_home: MAX_HOME,
    agent_installed: isMaxSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'max_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, MAX_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Max agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import max_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_MAX_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `max-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `max-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('max:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('max:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('max:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('max:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// max:// deep links (e.g. max://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const MAX_PROTOCOL = 'max'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${MAX_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // max://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('max:deep-link', payload)
...
ipcMain.handle('max:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${MAX_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(MAX_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(MAX_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `max://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching max:// URL is in our own argv.
