...
//   MAX_WIN_SSH_HOST   ssh alias/host of the Windows box
//   MAX_WIN_SSH_USER   remote user
//   MAX_WIN_SSH_HERMES absolute path to the remote hermes.exe under test
const liveHost = process.env.MAX_WIN_SSH_HOST || ''
const liveUser = process.env.MAX_WIN_SSH_USER || ''
const configuredMax = process.env.MAX_WIN_SSH_HERMES || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Max-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredMax)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredMax.replace('hermes.exe', 'python.exe')}' -m max_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredMax.replace('hermes.exe', 'python.exe')
        const terminate = `& '${python}' -m max_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.maxPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m max_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m max_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
