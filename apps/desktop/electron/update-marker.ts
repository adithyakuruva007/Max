 ...
 * (`force_kill_other_hermes`, taskkill /IM max.exe) kills it, the launch
...
export function markerPath(maxHome) {
  return path.join(maxHome, '.max-update-in-progress')
}
 ...
 */
export function writeUpdateMarker(maxHome, pid, { now = Date.now } = {}) {
  const file = markerPath(maxHome)
