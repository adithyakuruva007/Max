...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'max serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  ...
  assert.equal(backendCommandMatches('python -m max_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/hermes --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Max Runtime\\hermes.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('max chat --query serve'), false)
