...
// Backend subcommand routing for the desktop-managed Max process.
//
// The desktop app launches its own headless backend via `max serve` — it
...
// app hasn't updated yet, or an older `max` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Max profile to pin via `--profile`.
 */
 ...
 * `-m max_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `max_cli/subcommands/dashboard.py` source registers
