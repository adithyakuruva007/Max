 ...
 * the three user-facing uninstall modes to the `max uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m max_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `max_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'max_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Max.app/Contents/MacOS/Max  → …/Max.app
 *   Windows: …\Max\Max.exe                 → …\Max  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Max.app/Contents/MacOS/Max → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Max.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Max.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Max$/i.test(dir) || /[\\/]max-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/max
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import max_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, maxHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import max_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Max install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "MAX_HOME=${String(maxHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
