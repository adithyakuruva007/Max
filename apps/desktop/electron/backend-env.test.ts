...
test('desktop backend PATH adds Max-managed bins and missing POSIX sane entries', () => {
  const result = buildDesktopBackendPath({
    hermesHome: '/Users/test/.max',
    venvRoot: '/Users/test/.max/max-agent/venv',
  ...
  assert.deepEqual(entries.slice(0, 3), [
    '/Users/test/.max/node/bin',
    '/Users/test/.max/node',
    '/Users/test/.max/max-agent/venv/bin'
  ])
...
test('managed Node dirs lead with the platform-native layout but always offer both', () => {
  const posix = hermesManagedNodePathEntries('/Users/test/.max', {
    platform: 'darwin',
    pathModule: path.posix
  })
  ...
  assert.deepEqual(posix, ['/Users/test/.max/node/bin', '/Users/test/.max/node'])
...
test('managed Node dirs are empty without a Max home', () => {
...
test('every managed Node dir outranks the inherited PATH on both platforms', () => {
  for (const [platform, pathModule, home, inherited, delimiter] of [
    ['darwin', path.posix, '/Users/test/.max', '/usr/local/bin:/usr/bin', ':'],
...
test('buildDesktopBackendEnv extends PYTHONPATH and backend PATH together', () => {
  const env = buildDesktopBackendEnv({
    hermesHome: '/Users/test/.max',
    pythonPathEntries: ['/repo/max-agent'],
    venvRoot: '/Users/test/.max/max-agent/venv',
    currentEnv: {
      PATH: '/usr/bin:/bin',
  ...
  assert.equal(env.PYTHONPATH, '/repo/max-agent:/existing/pythonpath')
  assert.ok(
    env.PATH.startsWith(
      '/Users/test/.max/node/bin:/Users/test/.max/node:/Users/test/.max/max-agent/venv/bin:'
    )
  )
...
test('normalizeMaxHomeRoot maps profile homes back to the global Max root', () => {
  assert.equal(
    normalizeMaxHomeRoot('/Users/test/.max/profiles/oracle', { pathModule: path.posix }),
    '/Users/test/.max'
  )
  assert.equal(
    normalizeMaxHomeRoot('C:\\Users\\test\\AppData\\Local\\hermes\\profiles\\oracle', { pathModule: path.win32 }),
    'C:\\Users\\test\\AppData\\Local\\hermes'
  )
  assert.equal(normalizeMaxHomeRoot('/Users/test/.max', { pathModule: path.posix }), '/Users/test/.max')
})

test('Windows PATH casing and delimiter are preserved without POSIX sane entries', () => {
  const env = buildDesktopBackendEnv({
    hermesHome: 'C:\\Users\\test\\AppData\\Local\\hermes',
    pythonPathEntries: ['C:\\repo\\max-agent'],
    venvRoot: 'C:\\Users\\test\\AppData\\Local\\hermes\\max-agent\\venv',
    currentEnv: {
