// Regression guards for Windows `max` resolution in main.ts.
//
...
//      Git-Bash `max` shim shadowed the real hermes.cmd/hermes.exe; the
...
//      --repair by checking ONLY venv\Scripts\max.exe (the console-script
...
//      installs.
//   3. unwrapWindowsVenvMaxCommand() returned the venv python with NO
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow max.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvMax\) \?/,
    'recovery regressed to gating only on the max.exe shim, which forces destructive --repair'
  )
})

test('unwrapWindowsVenvMaxCommand smoke-tests the venv python before trusting it', () => {
  const source = readMain()
  const fnStart = source.indexOf('function unwrapWindowsVenvMaxCommand(')
  assert.notEqual(fnStart, -1, 'unwrapWindowsVenvMaxCommand must exist in main.ts')
