 ...
 * console. It writes MAX_HOME/.max-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Max within 30 minutes.
...
export function handoffResultPath(hermesHome: string): string {
  return path.join(hermesHome, '.max-update-result.json')
}
