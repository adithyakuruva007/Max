...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.max-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Max', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('MAX_HOME')) {
          return '/home/alice/.max\n'
        }

        if (command.includes('.max-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.max/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.max-update-in-progress/)
})
...
test('listRemoteMaxProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/MAX_HOME/, '/Users/zillajr/.max\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteMaxProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteMaxProfiles rejects a hostile MAX_HOME', async () => {
  const ssh = fakeSsh([[/MAX_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteMaxProfiles(ssh),
    (err: any) => {
...
test('locateMax prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/hermes/, 'OK']])
  assert.equal(await locateMax(ssh, '/opt/max'), '/opt/max')
})

test('locateMax throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
...
test('locateMax falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v hermes/, '/home/u/.local/bin/hermes\n'],
    [/\[ -x .*\.local\/bin\/hermes/, 'OK']
  ])

  assert.equal(await locateMax(ssh, ''), '/home/u/.local/bin/hermes')
})

test('locateMax preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$MAX_BIN" "$MAX_ENTRYPOINT" "$@",
  // where $MAX_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.max/max-agent/venv/bin/python\n']
  ])

  assert.equal(await locateMax(ssh, ''), '/home/u/.local/bin/hermes')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateMax must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateMax returns an explicit remoteMaxPath unchanged', async () => {
  // The override half of #74411: an explicit remoteMaxPath pointing at a
  ...
  assert.equal(await locateMax(ssh, '~/.local/bin/hermes'), '~/.local/bin/hermes')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteMaxPath must never be rewritten')
})

test('locateMax falls back to ~/.local/bin/hermes when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateMax(ssh, ''), '~/.local/bin/hermes')
})

test('locateMax tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/hermes/, 'OK']])
  assert.equal(await locateMax(ssh, ''), '~/.max/max-agent/venv/bin/hermes')
})

test('locateMax throws a hermes-not-found error with an install hint', async () => {
...
test('locateMax uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.max/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.max/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.max/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/hermes', '/Users/cd9c/.max', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /max-agent.*venv.*bin.*hermes/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'max wrapper ownership '))
    ...
    const launcher = path.join(temp, 'max launcher')
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  ...
  assert.ok(!cmd.includes('MAX_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  ...
  assert.ok(cmd.includes('.max-update-in-progress.mutex'))
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('MAX_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('MAX_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'MAX_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'MAX_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.max\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'Max Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'MAX_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile hermesPath differs from the resolved path', async () => {
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteMaxPath: '/new/hermes', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'MAX_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes hermesHome + protocolVersion into the lockfile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/MAX_HOME/, '/home/alice/.max\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'MAX_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"hermesHome":"\/home\/alice\/\.max"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'MAX_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'MAX_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  ...
  assert.doesNotThrow(() => validateRemotePath('/home/user/.max/max-agent/venv/bin/hermes'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/hermes'))
  assert.doesNotThrow(() => validateRemotePath('~/.max/logs/desktop-ssh.log'))
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.max/logs/desktop-ssh.log')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('MAX_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/hermes', 'work', {
    tokenFilePath: `~/.max/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.max\/desktop-ssh\//)
})
...
test('buildSpawnCommand raises the SSH child file limit before execing Max', () => {
  const cmd = buildSpawnCommand('/x/hermes', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env MAX_DESKTOP=1/)
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.max/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.max/unrelated.log' }))
