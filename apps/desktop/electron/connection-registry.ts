 ...
 * runtime, remote gateways, Max Cloud instances, SSH hosts) that are all
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteMaxPath || '').trim() !== String(activeSsh.remoteMaxPath || '').trim() ||
...
/** Turn `ls ~/.max/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Max Cloud
 * instances are platform-managed — we never run `max update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteMaxPath: input.remoteMaxPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Max-managed names; an empty result stores nothing.
 ...
 * `remoteMaxPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Max Cloud' : 'Remote gateway'),
