 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Max Agent by spawning
...
function bootstrapCacheDir(maxHome) {
  return path.join(maxHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.max/max-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(maxHome) {
  if (!maxHome) {
    return null
  }
  const candidate = path.join(maxHome, 'max-agent', 'scripts', installScriptName())
...
function cachedScriptPath(maxHome, commit) {
  return path.join(bootstrapCacheDir(maxHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/StardustResearch/max-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(maxHome, installStamp.commit)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(maxHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, maxHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass MAX_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          MAX_HOME: maxHome || process.env.MAX_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, maxHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, maxHome }) {
  const args = ['--dir', activeRoot, '--hermes-home', maxHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, maxHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, maxHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, maxHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, maxHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(maxHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, maxHome, emit })
