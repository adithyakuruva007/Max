 ...
 *
 * The staged Tauri `max-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `max update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `max update`, swaps/relaunches
 * the app, and writes .max-update-result.json. With the app gone before
 * the update starts, the MAX_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `max update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into MAX_HOME on *every* platform
 * (`max-setup.exe` on Windows, `max-setup` elsewhere — see
 ...
 * than the hand-off protocol holds the update marker, spawns `max update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(hermesHome, 'max-setup.exe')
 ...
 * "Another Max update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `max update`, which
 ...
 * the installer is written to MAX_HOME at install/repair time, making mtime
