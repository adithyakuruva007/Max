...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'max_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'max_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'max_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Max.app/Contents/MacOS/Max', 'darwin'),
    '/Applications/Max.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Max.app/Contents/MacOS/Max', 'darwin'),
    '/Users/x/Applications/Max.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Max\\Max.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Max'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\hermes-desktop\\Max.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\hermes-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Max.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_MaxXXXX/hermes', 'linux', { APPIMAGE: '/home/x/Apps/Max.AppImage' }),
    '/home/x/Apps/Max.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/max/linux-unpacked/hermes', 'linux', {}), '/opt/max/linux-unpacked')
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Max.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Max.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.max/max-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.max/max-agent',
    uninstallArgs: ['-m', 'max_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/max/linux-unpacked',
    hermesHome: '/home/x/.max'
  })
  ...
  assert.match(script, /'-m' 'max_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/hermes\/linux-unpacked'/)
  assert.match(script, /export MAX_HOME='\/home\/x\/\.max'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.max/max-agent',
    agentRoot: '/home/x/.max/max-agent',
    uninstallArgs: ['-m', 'max_cli.uninstall', '--mode', 'full'],
    appPath: null,
    hermesHome: '/home/x/.max'
  })

  // System python + source on PYTHONPATH so import max_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.max\/max-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'max_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'max_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    hermesHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'max_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\hermes',
    uninstallArgs: ['-m', 'max_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Max',
    hermesHome: 'C:\\Users\\x\\AppData\\Local\\hermes'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import max_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\hermes;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "max_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Max" >nul 2>&1/)
