 ...
 * Background
 * ----------
 * After `max update` + `max desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything MAX_DESKTOP_* is preserved
// plus MAX_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['MAX_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['MAX_DESKTOP_']
