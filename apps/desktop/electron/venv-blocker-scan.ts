...
const SCAN_MODULE = 'max_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Max processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'max_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Max process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Max installation is free.\n' +
    '\n' +
    'Close other Max windows and terminals, then retry.  If the problem\n' +
    'persists, run `max update` in a terminal for detailed diagnostics.'
  )
}
