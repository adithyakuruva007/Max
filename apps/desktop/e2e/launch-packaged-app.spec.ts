...
/**
 * E2E smoke tests for the packaged Max desktop app.
 *
 ...
 * (credential stripping, isolated MAX_HOME + userData, unique app name).
 *
...
test('window opens with the Max title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    ...
    }).maxDesktop?.hud?.open({ sessionId: null })
  )
