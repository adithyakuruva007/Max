 ...
 * packaged binary) with a real `max serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → max serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Max title', async () => {
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { hermesDesktop?: Record<string, unknown> }).maxDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → max serve is spawned → WS connects → config
