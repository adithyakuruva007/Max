 ...
 * second gateway, "Homelab", is a REAL second `max serve` this spec spawns
 * with its own MAX_HOME, profiles and session token, registered in the v2
...
function findMaxBinary(): string {
  ...
  throw new Error('max binary not found: create the repo venv (uv sync) or put max on PATH')
}
...
/**
 * Spawn a second, fully real `max serve` as the remote gateway. Its
 * session token is pinned through MAX_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        MAX_HOME: home,
        MAX_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote max serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Max-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote max serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    seedProfiles(sandbox.maxHome, ['research'])
