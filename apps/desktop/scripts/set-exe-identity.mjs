#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Max icon + version metadata onto the
// built Max.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Max.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `max desktop`, the installer's
...
// Stamp the Max icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Max',
      FileDescription: 'Max',
      CompanyName: 'Stardust Research',
      LegalCopyright: 'Copyright (c) 2026 Stardust Research'
    }
  })

  console.log('[set-exe-identity] done — Max icon + identity stamped')
}
