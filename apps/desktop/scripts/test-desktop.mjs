...
// no longer bundles the Max Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Max.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Max'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Max.exe'),
...
// Default MAX_HOME for non-sandboxed runs -- matches main.ts's
// resolveMaxHome(). On Windows it's %LOCALAPPDATA%\hermes; elsewhere
// it's ~/.max. The fresh-install sandbox launchFresh() sets its own
// MAX_HOME and never touches this.
const DEFAULT_MAX_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'hermes')
  }
  return path.join(os.homedir(), '.max')
})()
const VENV_ROOT = path.join(DEFAULT_MAX_HOME, 'max-agent', 'venv')
...
function ensurePackagedApp() {
  if (process.env.MAX_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Max-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Max-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Max-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Max-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.MAX_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.MAX_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  env.MAX_DESKTOP_CWD = cwd
  env.MAX_DESKTOP_IGNORE_EXISTING = '1'
  env.MAX_DESKTOP_TEST_MODE = 'fresh-install'
  env.MAX_DESKTOP_USER_DATA_DIR = userDataDir
  env.MAX_HOME = hermesHome
  delete env.MAX_DESKTOP_HERMES
  delete env.MAX_DESKTOP_MAX_ROOT
  ...
  console.log(`  MAX_HOME: ${hermesHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(hermesHome, 'max-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Max Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of max_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'max-agent', 'max_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Max
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + MAX_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  MAX_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
