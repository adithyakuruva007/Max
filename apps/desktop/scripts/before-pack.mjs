 ...
 * product name (`Max`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Max'
 *
 * This is a hard failure with no obvious cause for the user — `max desktop`
 ...
 * (the mac-unpacked Max.app bundle) and Windows (win-unpacked), so we
 ...
 * Max.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `max desktop --build-only` (max_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Max.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Max'}.exe`
