 ...
 *
 * Stamps the Max icon + identity onto the packed Windows Max.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `max desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Max')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Max'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Max.exe keeps the stock Electron icon`)
  }
}
