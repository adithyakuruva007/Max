...
How to build Max Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Max surface can also
...
- A **runtime home change** (switching the underlying `MAX_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Max — do not assume
