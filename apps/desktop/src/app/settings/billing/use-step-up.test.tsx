  ...
  it('subscribes for verification, opens the verification URL, cleans up, and invalidates on completion', async () => {
    ...
    act(() => {
      gatewayMock.emit('billing.step_up.verification', {
        payload: {
          user_code: 'ABCD-1234',
          verification_url: 'https://portal.stardustresearch.com/device'
        },
    ...
    expect(result.current.verification).toEqual({
      code: 'ABCD-1234',
      url: 'https://portal.stardustresearch.com/device'
    })

    result.current.openVerification()
    expect(window.maxDesktop?.openExternal).toHaveBeenCalledWith('https://portal.stardustresearch.com/device')
