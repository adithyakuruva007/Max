 ...
 * agent sources (local runtime + any number of remote gateways / Max Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.maxDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.maxDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `max update` to every eligible source; per-connection results
