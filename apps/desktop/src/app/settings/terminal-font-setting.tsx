...
import { saveMaxConfig } from '@/hermes'
...
function fontFamilyFromConfig(config: MaxConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useMaxConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<MaxConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setMaxConfigCache, so
