...
// User-installed plugins first — mirrors `max plugins list --user`.
...
function reveal(file: string) {
  void window.maxDesktop?.revealPath?.(file)?.catch(() => undefined)
}
...
async function revealPluginsDir() {
  try {
    ...
    const dir = await window.maxDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.maxDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's max home (profile-aware), so the
// path comes from the gateway — not from the renderer's local MAX_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.maxDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Max (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
