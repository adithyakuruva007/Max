...
export function GatewaySettings() {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.maxDesktop.applyConnectionConfig(payload())
        : await window.maxDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      if (result.connected) {
        const refreshed = await window.maxDesktop.getConnectionConfig(scope)
        setState(refreshed)
  ...
  const signOut = async () => {
    ...
    try {
      await window.maxDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.maxDesktop.getConnectionConfig(scope)
      setState(refreshed)
        ...
        <ListRow
          action={
            <Input
              ...
              placeholder="https://gateway.example.com/max"
