...
import { invalidateMaxConfig, setMaxConfigCache, useMaxConfigRecord } from '../hooks/use-config-record'
...
// Max' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Max default (medium), shown as Medium.
...
// backend now surfaces the full `max model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in max_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useMaxConfigRecord()
  const setConfig = setMaxConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `max model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Max runs the flow for you.`}
          </p>
        )}
