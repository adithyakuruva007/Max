...
import { disconnectOAuthProvider, listOAuthProviders } from '@/max'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/max'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.maxDesktop?.terminal)
...
//      in max_cli/provider_catalog.py) — the SAME provider identity
//      `max model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Max can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Max can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Max never deletes creds another tool owns behind a silent API call.
