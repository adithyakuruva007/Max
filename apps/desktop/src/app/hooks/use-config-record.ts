...
import { getMaxConfigRecord, type ProfileScope, profileScopeKey } from '@/hermes'
...
export const MAX_CONFIG_KEY = ['hermes-config-record'] as const
...
export const hermesConfigKey = (profile?: ProfileScope) =>
  profile == null ? MAX_CONFIG_KEY : ([...MAX_CONFIG_KEY, profileScopeKey(profile)] as const)
...
export const useMaxConfigRecord = (profile?: ProfileScope) =>
  useQuery({
    ...
    queryFn: () => getMaxConfigRecord(profile ?? undefined),
    staleTime: 0
  })

// setMaxConfigCache writes the app-wide (base-key) record. Pass a profile to
...
export const setMaxConfigCache = writeCache<MaxConfigRecord>(MAX_CONFIG_KEY)
export const hermesConfigCacheWriter = (profile?: ProfileScope) =>
  writeCache<MaxConfigRecord>(hermesConfigKey(profile))

export const invalidateMaxConfig = (profile?: ProfileScope) =>
