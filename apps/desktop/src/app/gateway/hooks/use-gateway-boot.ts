...
export function primaryRuntimeConnectionId(connection: Pick<MaxConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.maxDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: MaxGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: MaxConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Max...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Max…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Max backend'
        )
        ...
        // as an opaque "Could not connect to Max gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshMaxConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshMaxConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startMax re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new MaxGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshMaxConfig(),
          ...
          // retry it), not brick boot behind the "Max couldn't start"
