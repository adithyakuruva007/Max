...
import type { MaxGateway } from '@/max'
...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<MaxGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.maxDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<MaxGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as MaxGateway | null
      }),
    []
  )
