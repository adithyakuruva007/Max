...
import { getMaxConfigRecord, saveMcpServers } from '@/hermes'
...
import { setMaxConfigCache } from '../hooks/use-config-record'
 ...
 * confirming lets Max spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getMaxConfigRecord())
      ...
      setMaxConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
