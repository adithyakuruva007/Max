...
import { pathFromMaxDeepLink, resolveMaxOpenPath } from '@/lib/hermes-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.maxDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.maxDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.maxDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.maxDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  useEffect(() => {
    const unsubscribe = window.maxDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        const path = resolveMaxOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.maxDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      const path = pathFromMaxDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.maxDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.maxDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.maxDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.maxDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
