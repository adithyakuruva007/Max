...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../max'
...
import { useMaxConfig } from './session/hooks/use-max-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.maxDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.maxDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshMaxConfig, sttEnabled, voiceMaxRecordingSeconds } = useMaxConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshMaxConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshMaxConfig])
