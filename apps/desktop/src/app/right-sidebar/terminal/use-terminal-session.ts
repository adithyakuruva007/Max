...
const MAX_PATHS_MIME = 'application/x-hermes-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(MAX_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(MAX_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.maxDesktop?.readClipboard?.()) ?? ''
