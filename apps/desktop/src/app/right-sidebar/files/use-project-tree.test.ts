...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { hermesDesktop: { readDir: typeof readDir } }).maxDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): MaxReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { hermesDesktop: unknown }).maxDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: MaxReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<MaxReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: MaxReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: MaxReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: MaxReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<MaxReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.maxDesktop is missing', async () => {
    delete (window as unknown as { hermesDesktop?: unknown }).maxDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: MaxReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: MaxReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: MaxReadDirResult) => void) | undefined
