...
import type { MaxReadDirEntry, MaxReadDirResult } from '@/global'
...
export type ProjectTreeEntry = MaxReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: MaxReadDirEntry) {
...
async function filterIgnored(entries: MaxReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<MaxReadDirResult> {
