        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.maxDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Max to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.maxDesktop?.watchPreviewFile ||
      !window.maxDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.maxDesktop.onPreviewFileChanged(payload => {
    ...
    void window.maxDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.maxDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.maxDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.maxDesktop?.zoom?.factor?.() || 1
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.maxDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.maxDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.maxDesktop?.openExternal(currentUrl)
                : undefined
            }
