...
import { getMaxConfig, getMaxConfigDefaults } from '@/max'
...
export function useMaxConfig({ activeSessionIdRef, refreshProjectBranch }: MaxConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshMaxConfig = useCallback(async () => {
    try {
