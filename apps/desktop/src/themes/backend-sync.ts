/**
 * Live skin sync from the Max backend.
 *
 * The backend resolves the active skin (built-in or `$MAX_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Max
 ...
 */
export function ingestBackendSkin(skin: MaxSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as MaxSkin)
