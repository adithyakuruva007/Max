...
import type {
  MaxConnection,
  MaxReadDirResult,
  MaxReadFileTextResult,
  MaxSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: MaxConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: MaxConnection | null = $connection.get()) {
...
function bridge() {
  ...
  if (!desktop) {
    throw new Error('Max Desktop bridge is unavailable')
  }
...
export async function readDesktopDir(path: string): Promise<MaxReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<MaxReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<MaxReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<MaxReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.maxDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: MaxSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
