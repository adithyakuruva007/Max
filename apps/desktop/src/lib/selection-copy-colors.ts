...
// Max is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.maxMode
 ...
 * the only carriers of Max' palette — are dropped so the paste target's
