...
/** Why a known Max command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Max profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownMaxSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Max' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownMaxSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Max and were never asked for. It is
