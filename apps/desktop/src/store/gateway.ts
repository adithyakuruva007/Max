...
import { MaxGateway, setApiRequestConnection } from '@/hermes'
...
const isOpen = (gateway: MaxGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: MaxConnection) => void
  /**
...
interface GatewayRegistryState {
  ...
  primaryGateway: MaxGateway | null
  ...
  $gateway: ReturnType<typeof atom<MaxGateway | null>>
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<MaxGateway | null>(null),
...
export function setPrimaryGateway(gateway: MaxGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<MaxConnection, 'connectionId'> | null): void {
...
export function activeGateway(): MaxGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the max module (null
...
function publishActiveConnection(connection: MaxConnection): void {
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: MaxGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Max gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.maxDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Max Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<MaxGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Max gateway is not connected" on the Sessions `+` action (#88880).
