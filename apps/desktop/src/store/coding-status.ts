...
import type { MaxGitWorktree, MaxRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: MaxGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, MaxRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, MaxGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<MaxRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<MaxGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<MaxRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<MaxGitWorktree[]>>()
const $noRepoStatus = atom<MaxRepoStatus | null>(null)
const $noWorktrees = atom<MaxGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<MaxRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<MaxGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: MaxRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: MaxGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<MaxRepoStatus | null>
  seq: number
  target: string
}
