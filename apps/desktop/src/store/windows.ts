...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await openWindow(() => window.maxDesktop.openSessionWindow(sessionId, opts), 'Could not open chat in a new window')
}

// Open a fresh compact window on the new-session draft.
export async function openNewSessionInNewWindow(): Promise<void> {
  if (!canOpenSessionWindow() || typeof window.maxDesktop.openNewSessionWindow !== 'function') {
    return
  }

  await openWindow(() => window.maxDesktop.openNewSessionWindow(), 'Could not open new session window')
}
