...
export const SESSION_DRAFTS_STORAGE_KEY = 'max:composer-drafts:v3'
const SESSION_DRAFTS_STORAGE_KEY_OLD = 'hermes:composer-drafts:v3'
...
function loadPersistedDraftTexts(): [string, SessionDraft][] {
  try {
    const raw = window.localStorage.getItem(SESSION_DRAFTS_STORAGE_KEY) ?? window.localStorage.getItem(SESSION_DRAFTS_STORAGE_KEY_OLD)
