...
/**
 * Feature store for backend (agent) plugins — the native Max plugins plus
 ...
 * primitives `max plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Max. */
  portable?: boolean
}
 ...
 *  MAX_HOME. Always refetches (it's a cheap local disk scan on the
