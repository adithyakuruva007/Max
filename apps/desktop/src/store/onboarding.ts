...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/max'
...
const CONFIGURED_CACHE_KEY = 'max-desktop-onboarded-v1'
const CONFIGURED_CACHE_KEY_OLD = 'hermes-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'max-onboarding-skipped-v1'
const SKIP_CACHE_KEY_OLD = 'hermes-onboarding-skipped-v1'
const POLL_MS = 2000
...
function readCachedConfigured(): boolean | null {
  ...
  try {
    return (window.localStorage.getItem(CONFIGURED_CACHE_KEY) ?? window.localStorage.getItem(CONFIGURED_CACHE_KEY_OLD)) === '1' ? true : null
  } catch {
    return null
  }
}
...
function readCachedSkipped(): boolean {
  ...
  try {
    return (window.localStorage.getItem(SKIP_CACHE_KEY) ?? window.localStorage.getItem(SKIP_CACHE_KEY_OLD)) === '1'
  } catch {
    return false
  }
}
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Max is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Nous Tool Gateway,
// mirroring max_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
async function fetchProviderDefaultModel(
  ...
  // `max model` does (for Nous it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Max still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Max still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Max Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Max still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Max still cannot reach ${url}.` }
    }
