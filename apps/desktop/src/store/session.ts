...
import type { SessionInfo, UsageStats } from '@/types/max'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'max.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'max.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'max.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'max.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'max.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'max.desktop.lastSessionId'
...
function workspaceCwdKey(connection: MaxConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.maxDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.maxDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<MaxConnection | null>(null)
...
export const setConnection = (next: Updater<MaxConnection | null>) => updateAtom($connection, next)
