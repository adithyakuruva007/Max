...
//
// Scope is always "uncommitted" — Max' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'max.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'max.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'max.desktop.reviewTreeMode'
const SELECTED_KEY = 'max.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<MaxReviewFile[]>([])
...
export const $reviewShipInfo = atom<MaxReviewShipInfo>({ ghReady: false, pr: null })
...
export async function selectReviewFile(file: MaxReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.maxDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.maxDesktop?.openExternal?.(url)
    }
