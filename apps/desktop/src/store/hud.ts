 ...
 * composer with the reply scrolling above it, so Max can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.maxDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.maxDesktop?.hud
...
export function resetHudLayout(): void {
  void window.maxDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.maxDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.maxDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
