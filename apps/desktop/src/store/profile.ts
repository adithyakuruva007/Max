...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/max'
...
const PROFILE_ORDER_STORAGE_KEY = 'max.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'max.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.maxDesktop.api<ActiveProfileResponse>({
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'max.desktop.showAllProfiles'
