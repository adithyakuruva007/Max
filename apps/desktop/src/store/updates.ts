...
import { checkMaxUpdate, getActionStatus, updateMax } from '@/max'
import { translateNow } from '@/i18n'
import { persistString, storedString } from '@/lib/storage'
import { dismissNotification, notify } from '@/store/notifications'
import { $connection } from '@/store/session'
import type { BackendUpdateCheckResponse } from '@/types/max'
...
const UPDATE_TOAST_SNOOZE_KEY = 'max:update-toast-snooze-until'
...
function isUpdateToastSnoozed(): boolean {
  const until = Number(storedString(UPDATE_TOAST_SNOOZE_KEY) || storedString('hermes:update-toast-snooze-until') || 0)
...
const SKEW_TOAST_SNOOZE_KEY = 'max:backend-skew-toast-snooze-until'
...
function isSkewToastSnoozed(): boolean {
  const until = Number(storedString(SKEW_TOAST_SNOOZE_KEY) || storedString('hermes:backend-skew-toast-snooze-until') || 0)
...
const INSTALL_METHOD_TOAST_SNOOZE_KEY = 'max:install-method-toast-snooze-until'
...
function isInstallMethodToastSnoozed(): boolean {
  const until = Number(storedString(INSTALL_METHOD_TOAST_SNOOZE_KEY) || storedString('hermes:install-method-toast-snooze-until') || 0)
