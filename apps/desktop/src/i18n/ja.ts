...
export const ja = defineLocale({
  ...
  settings: {
    ...
    gateway: {
      ...
      intro:
        'Max Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Max バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
      ...
      remoteUrlDesc:
        'リモートダッシュボードバックエンドのベース URL。/max などのパスプレフィックスもサポートしています。',
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ボットユーザー ID', placeholder: '@max:example.org' },
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'Max 推論ゲートウェイのステータス',
      agents: 'エージェント',
