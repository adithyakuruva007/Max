...
import { getMaxConfigRecord, type MaxConfigRecord, saveMaxConfig } from '@/hermes'
...
export interface I18nConfigClient {
  getConfig: () => Promise<MaxConfigRecord>
  saveConfig: (config: MaxConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.maxDesktop?.api) {
      return Promise.resolve({})
    }

    return getMaxConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.maxDesktop?.api) {
...
export function getConfigDisplayLanguage(config: MaxConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: MaxConfigRecord, locale: Locale): MaxConfigRecord {
