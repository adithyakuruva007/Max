...
type MaxRefType = (typeof HERMES_REF_TYPES)[number]
...
const ICON_PATHS: Record<MaxRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as MaxRefType] ?? ICON_FALLBACK
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(HERMES_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as MaxRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: MaxRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Max directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.maxDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.maxDesktop?.readFileDataUrl(id)
