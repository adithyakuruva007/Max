...
const MAX_REF_TYPES = WIRE_REFERENCE_KINDS
type MaxRefType = (typeof MAX_REF_TYPES)[number]
...
const MAX_DIRECTIVE_RE = referenceRe()
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(MAX_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...maxDirectiveFormatter.parse(text)]
  } catch {
