...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's MAX_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.maxDesktop?.logsRoot?.()
      ...
      const result = await window.maxDesktop?.openDir?.(root)
