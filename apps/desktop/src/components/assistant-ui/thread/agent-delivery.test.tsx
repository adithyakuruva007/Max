...
describe('delivery command detection', () => {
  it('matches the canonical delivery command', () => {
    const cmd = 'max -p turqoise chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 Max (@hermes): hi there"'

    expect(deliveryTargetFromCommand(cmd)).toBe('turqoise')
  })

  it('matches with a cd prefix and timeout wrapper', () => {
    const cmd = 'cd ~ && timeout 240 max -p mr-tester chat --in "~" -Q -q "Message from 🤖 Max: hello"'
  ...
  it('ignores ordinary terminal commands', () => {
    ...
    expect(deliveryTargetFromCommand('max -p turqoise chat -q "plain question"')).toBeNull()
    expect(deliveryTargetFromCommand('max sessions list')).toBeNull()
  })
})
...
describe('reply extraction', () => {
  it('strips session_id bookkeeping and keeps the reply', () => {
    ...
    expect(replyTextFromResult({ output })).toBe('Hi Max! Good to hear from you.')
  })
