...
import { getGlobalModelOptions } from '@/max'
...
import type { ModelOptionProvider, OAuthProvider } from '@/types/max'
...
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    docsUrl: 'https://github.com/StardustResearch/max-agent#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

// Build the FULL API-key provider catalog from the backend model options so the
// onboarding / Providers key form lists every `api_key` provider `max model`
...
const SHOW_ALL_KEY = 'max-onboarding-show-all-v1'
const SHOW_ALL_KEY_OLD = 'hermes-onboarding-show-all-v1'

const readShowAll = () => {
  try {
    return (window.localStorage.getItem(SHOW_ALL_KEY) ?? window.localStorage.getItem(SHOW_ALL_KEY_OLD)) === '1'
  } catch {
    return false
  }
}
