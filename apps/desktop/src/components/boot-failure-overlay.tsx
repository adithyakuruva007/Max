...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.maxDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.maxDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.maxDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }

  // Clear this gateway's stale auth first, then re-establish it through the
  // connection's owning login flow. Max Cloud must reuse its portal session
  ...
  const openLogs = () => void window.maxDesktop?.revealLogs().catch(() => undefined)
    ...
    actions = [
      {
        ...
        onClick: () => openExternalLink('https://portal.stardustresearch.com'),
        icon: <ExternalLink />
      },
      localAction,
