...
<p align="center">
  <a href="https://github.com/NousResearch/max-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Max gateway or install Max locally. Local onboarding then walks you through choosing a provider and model.
...
```bash
# throwaway MAX_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Max Agent runtime into `MAX_HOME`
(`~/.max`, or `%LOCALAPPDATA%\hermes` on Windows), using the same layout as a
CLI install.
...
- **Max Agent** runs as a headless `max serve` process and exposes the
...
1. `MAX_DESKTOP_MAX_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `MAX_DESKTOP_HERMES`, or `max` on `PATH`
5. a system Python that can import the Max runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Max
...
screen offers **Connect to existing Max** before starting the local installer.
...
terminal commands, and file operations run against the remote Max host, not
...
transport- or Max-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Max-Session-Token`.
