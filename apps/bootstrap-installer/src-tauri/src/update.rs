...
//! The only OS-specific bits here are the venv shim path (resolve_max_cli) and
...
async fn run_update(app: AppHandle) -> Result<()> {
    let max_home = crate::paths::max_home();
    let install_root = max_home.join("max-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_max`
    ...
    let max_cli = resolve_max_cli(&install_root).ok_or_else(|| {
        let msg = format!(
    ...
    // time `max update` runs there is no legitimate max.exe to protect,
    ...
    // probe only checks the max.exe shim and app.asar, so an external venv
    ...
    if let Some(target_app) = launch_target {
        ...
        crate::bootstrap::launch_max_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend max.exe (or the desktop Hermes.exe
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_max(install_root)];
...
/// Force-kill any `max.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `max.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\max.exe update`. And a
...
/// max.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named max.exe.)
fn force_kill_other_max() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
fn venv_max(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("max.exe")
    } else {
        install_root.join("venv").join("bin").join("max")
    }
}

/// Resolve the max CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `max` on PATH.
fn resolve_max_cli(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_max(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "max.exe" } else { "max" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        "MAX_HOME".to_string(),
        max_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        max_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_max_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::max_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_max_is_under_install_root() {
        let root = Path::new("/x/max-agent");
        let shim = venv_max(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_max(root)),
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let tmp = base.join("Hermes.app.max-update-new");
        let old = base.join("Hermes.app.max-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Hermes.app");
        let tmp = base.join("Hermes.app.max-update-new"); // intentionally absent
        let old = base.join("Hermes.app.max-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Hermes.app");
        let tmp = base.join("Hermes.app.max-update-new"); // absent
        let old = base.join("Hermes.app.max-update-old");
        write_marker(&target, "OLD");
