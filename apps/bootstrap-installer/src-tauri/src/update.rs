...
//!
//! Driven when the installer is launched as `Max-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Max desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `max update`
//!      or repair bootstrap can race locked files),
//!   2. run `max update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in max_cli/main.py),
//!   3. run `max desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `max update` already handles macOS/Linux (git/pip).
...
/// `max update` exit code meaning "another max process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// max_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `max update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `max update` claims
/// the same file (see `max_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `max update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in max_cli/update_lock.py — all three read
...
/// a dashboard-spawned `max update`) still blocks.
...
/// the marker name our PID — because a `max update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let install_root = hermes_home.join("max-agent");
    ...
    //
    // The same marker is the cross-process update lock (max_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `max update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Max update is already running (PID {}, started {} ago). \
    ...
    let max = resolve_hermes(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the max CLI under {}. Is Max installed? \
    ...
    // async on Windows. If it still holds the venv shim, `max update`
    ...
    // ---- stage 2: max update -----------------------------------------
    // Pass --branch so `max update` targets the branch this installer was
    ...
    // without --branch, `max update` switches the checkout to `main` (a
    ...
    // --force skips `max update`'s Windows running-exe guard (which would
    ...
    // time `max update` runs there is no legitimate hermes.exe to protect,
    // and the guard would only produce a false "Max is still running" stop.
    //
    ...
    // below surfaces the correct "close all Max windows" message.
    ...
    // Retry-once for the update-boundary crash. `max update` lazily imports
    ...
    // second `max update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Max" state
    // a retry can't fix.
    ...
    // checkout predating the MAX_UPDATE_HANDOFF_PID env fix (8c76fe19f)
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Max is still running. Close all Max windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "max update failed (exit {:?}). See {} for details.",
                other,
    ...
    // ---- stage 3: max desktop --build-only ----------------------------
    // `max update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `max update` already does above, and `max update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `max desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Max manually."),
            );
        }
    } else if let Err(err) =
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Max to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // Max.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] Max still holding install files ({}); locating backend shims…",
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Max.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Max.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// also Max.exe, so matching by image name alone is unsafe.
#[cfg(windows)]
...
/// Spawn `max <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `max update` is precisely the shape that
...
/// Path to the venv max shim under an install root, regardless of existence.
...
/// Resolve the max CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `max` on PATH.
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    // `max update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `max update` child claims that SAME lock (max_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Max is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in max_cli/update_lock.py.
    envs.push((
        "MAX_UPDATE_HANDOFF_PID".to_string(),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_hermes_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Max.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.max-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.max-update-old", target_app.display()));
...
mod tests {
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/max-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/max-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "MAX_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the max update child claims the same marker we hold; without our pid \
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/max-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.max\max-agent\venv\scripts\HERMES.EXE"),
            Path::new(r"c:\users\tester\.max\max-agent\venv\Scripts\hermes.exe"),
        ));
    }
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.max\max-agent\apps\desktop\Max.exe"),
            Path::new(r"C:\Users\tester\.max\max-agent\venv\Scripts\hermes.exe"),
        ));
    }
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".max-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `max update` and install-mode bootstrap mutate one tree
    ...
    // checkout's `max update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".max-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running Max, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Max.app"]),
            Some(PathBuf::from("/Applications/Max.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Max.app");
        let tmp = base.join("Max.app.max-update-new");
        let old = base.join("Max.app.max-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Max.app");
        let tmp = base.join("Max.app.max-update-new"); // intentionally absent
        let old = base.join("Max.app.max-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Max.app");
        let tmp = base.join("Max.app.max-update-new"); // absent
        let old = base.join("Max.app.max-update-old");
        write_marker(&target, "OLD");
