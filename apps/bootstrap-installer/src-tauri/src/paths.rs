...
//! Electron desktop's resolveHermesHome() ALL use ~/.max on macOS — there
...
/// Returns the canonical Hermes home directory, respecting $MAX_HOME if set.
pub fn max_home() -> PathBuf {
    ...
    {
        // %LOCALAPPDATA%\max — matches scripts/install.ps1's $HermesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("max");
        }
    }
...
/// max-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\max\max-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_max`,
...
pub fn update_in_progress_marker() -> PathBuf {
    max_home().join(".max-update-in-progress")
}
...
pub fn copy_self_to_max_home() -> std::io::Result<()> {
...
///   const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_HERMES_ROOT, '.max-bootstrap-complete')
...
pub fn get_max_home() -> String {
    max_home().to_string_lossy().into_owned()
}
