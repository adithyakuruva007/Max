...
//! MAX_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for MAX_HOME. Tests use this; production
...
/// Spawn the locally-built Max desktop binary, then close the installer
...
pub async fn launch_hermes_desktop(
    ...
    let exe_path = resolve_hermes_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Max desktop at {}. The desktop build step \
             may have been skipped or failed. Run `max desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Max desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Max
...
pub(crate) fn resolve_hermes_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Max.exe"),
            ("win-arm64-unpacked", "Max.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Max.app/Contents/MacOS", "Max"),
            ("mac-arm64/Max.app/Contents/MacOS", "Max"),
        ]
    } else {
...
pub(crate) fn resolve_hermes_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        // .../Max.app/Contents/MacOS/Max -> .../Max.app
...
/// path so a bare re-open just opens Max instead of re-running setup.
pub(crate) fn hermes_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".max-bootstrap-complete").exists()
...
fn write_bootstrap_complete_marker(install_root: &Path, pin: &Pin) -> Result<serde_json::Value> {
    ...
    let tmp_path = install_root.join(".max-bootstrap-complete.tmp");
    {
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_hermes_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Max desktop app")
    })?;
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $MaxHome which Stage-Repository clones
    // the repo INTO at $MaxHome\max-agent. Mirrors max_constants.
    let hermes_home = args
        .max_home
        .clone()
        .unwrap_or_else(|| crate::paths::hermes_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&hermes_home).join("max-agent");
    ...
    // Copy ourselves to MAX_HOME/max-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_hermes_home() {
        tracing::warn!(?err, "failed to copy installer into MAX_HOME (non-fatal)");
        emit_log(&format!(
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Max"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Max.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Max.exe");
    ...
    // what the updater ditto's over /Applications/Max.app). A regression in
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        let marker_path = root.join(".max-bootstrap-complete");
    ...
    fn bootstrap_complete_marker_is_published_atomically() {
        ...
        let marker_path = root.join(".max-bootstrap-complete");
        let tmp_path = root.join(".max-bootstrap-complete.tmp");
        assert!(
            marker_path.is_file(),
    ...
    fn hermes_is_installed_treats_marker_existence_as_sufficient() {
        ...
        make_release_tree(&root);
        std::fs::write(root.join(".max-bootstrap-complete"), b"").unwrap();
    ...
    fn marker_write_failure_leaves_no_final_marker() {
        ...
        assert!(
            !not_a_dir.join(".max-bootstrap-complete").exists(),
            "failed write must not leave a final marker that enables the fast path"
        );
        assert!(
            !not_a_dir.join(".max-bootstrap-complete.tmp").exists(),
