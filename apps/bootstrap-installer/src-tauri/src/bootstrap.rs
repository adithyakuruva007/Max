...
#[tauri::command]
pub async fn launch_max_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_max_desktop_exe(&install_root).ok_or_else(|| {
        format!(
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_max_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
...
pub(crate) fn resolve_max_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_max_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
...
pub(crate) fn max_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".max-bootstrap-complete").exists()
        && resolve_max_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_max_desktop_exe(install_root).ok_or_else(|| {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_max_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::max_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // the repo INTO at $HermesHome\max-agent. Mirrors max_constants.
    let max_home = args
        .max_home
        .clone()
        .unwrap_or_else(|| crate::paths::max_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&max_home).join("max-agent");
    ...
    if let Err(err) = crate::paths::copy_self_to_max_home() {
...
async fn run_install_script(
    ...
    powershell::run_script(script_path, args, sink, max_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    #[test]
    fn resolve_max_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_max_desktop_app(&root)
    ...
    #[test]
    fn resolve_max_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_max_desktop_app(&root).is_none(),
