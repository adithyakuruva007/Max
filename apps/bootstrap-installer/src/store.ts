...
export const $maxHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.max/logs/bootstrap-installer.log')
    $maxHome.set('~/.max')
  ...
  try {
    const [logPath, maxHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_max_home'),
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Max" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Max-Setup.exe --update);
...
export async function launchMaxDesktop(): Promise<void> {
  ...
  await invoke('launch_max_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Max repository' },
