...
# Max WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: MAX_TINI_SHIM_TARGET overrides the `/init` path (and
# MAX_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${MAX_TINI_SHIM_TARGET:-/init}"
WRAPPER="${MAX_TINI_SHIM_WRAPPER:-/opt/max/docker/main-wrapper.sh}"
