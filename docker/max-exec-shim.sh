#!/bin/sh
# shellcheck shell=sh
# /opt/max/bin/max — `docker exec` privilege-drop shim.
#
# Background
# ----------
...
# Portal" on every incoming message — even though `docker exec <c> max
...
# Fix
# ---
# This shim sits at /opt/max/bin/max and is placed earliest on PATH.
...
# `docker exec --user max`, kanban subagents, etc. — it short-circuits
...
# (/opt/max/.venv/bin/max), so the second hop cannot re-enter this
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "max-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "max-shim: re-run with --user max or set MAX_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi
