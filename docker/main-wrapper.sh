...
# with-contenv before touching MAX_HOME / PATH. On the non-PID-1
...
if [ -z "${MAX_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${MAX_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export MAX_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset MAX_MAIN_WRAPPER_ENV_READY
...
_hermes_orig_cwd="${MAX_ORIG_CWD:-$PWD}"
