...
as_max() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid max "$@"; }
...
# `id -u max` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# max UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u max)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-max UID).
...
# root can create (e.g. `MAX_HOME=/home/max/.max` in a Compose
...
if [ -n "${MAX_UID:-}" ] && validate_uid_gid "$MAX_UID" && [ "$MAX_UID" != "$(id -u max)" ]; then
    echo "[stage2] Changing max UID to $MAX_UID"
    usermod -u "$MAX_UID" max
fi
if [ -n "${MAX_GID:-}" ] && validate_uid_gid "$MAX_GID" && [ "$MAX_GID" != "$(id -g max)" ]; then
...
# matching entry that includes max. Idempotent across container
...
#
# The canonical list of max-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_max_uid=$(id -u max)
...
chown_max_tree() {
    ...
    chown -R max:max "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$MAX_HOME" 2>/dev/null)" != "$actual_max_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $MAX_HOME (targeted) to max ($actual_max_uid)"
    ...
    if refuse_symlinked_path "chown" "$MAX_HOME"; then
        ...
        chown max:max "$MAX_HOME" 2>/dev/null || \
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$MAX_HOME/$sub" ]; then
            chown_max_tree "$MAX_HOME/$sub"
        fi
    done
fi
...
# even though installs are re-enabled. The dir is seeded + chowned to max
...
# chown would fail.
if [ -d "$MAX_HOME/profiles" ]; then
    chown_max_tree "$MAX_HOME/profiles"
fi
...
if [ -d "$MAX_HOME/cron" ]; then
    chown_max_tree "$MAX_HOME/cron"
fi
...
# is negligible.
if [ -d "$MAX_HOME/platforms/pairing" ]; then
    chown_max_tree "$MAX_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$MAX_HOME/pairing" ]; then
    chown_max_tree "$MAX_HOME/pairing"
fi

# Reset ownership of max-owned top-level state files on every boot.
# The targeted data-volume chown above only covers max-owned
...
for f in \
    ...
    if [ -e "$MAX_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$MAX_HOME/$f"; then
            ...
            chown max:max "$MAX_HOME/$f" 2>/dev/null || true
        fi
    fi
done
...
if [ -f "$MAX_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$MAX_HOME/config.yaml"; then
        ...
        chown max:max "$MAX_HOME/config.yaml" 2>/dev/null || true
...
seed_one() {
    ...
    if [ ! -f "$MAX_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$MAX_HOME/$dest"; then
            ...
            as_max cp "$INSTALL_DIR/$src" "$MAX_HOME/$dest"
        fi
    fi
}
...
if [ -f "$MAX_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$MAX_HOME/.env"; then
        ...
        chown max:max "$MAX_HOME/.env" 2>/dev/null || true
...
if [ ! -f "$MAX_HOME/auth.json" ] && [ -n "${MAX_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$MAX_HOME/auth.json"; then
        ...
        chown max:max "$MAX_HOME/auth.json" 2>/dev/null || true
...
if [ ! -f "$MAX_HOME/gateway_state.json" ] && \
    ...
    if refuse_symlinked_path "seed" "$MAX_HOME/gateway_state.json"; then
        ...
        chown max:max "$MAX_HOME/gateway_state.json" 2>/dev/null || true
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_max "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-max, dashboard, etc.).
