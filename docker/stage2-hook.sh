...
#
# Wired into the image as /etc/cont-init.d/01-max-setup by the
...
MAX_HOME="${MAX_HOME:-/opt/data}"
INSTALL_DIR="/opt/max"

# Drop to max via s6-setuidgid, but skip it when already non-root.
as_hermes() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid max "$@"; }
...
# starts non-root. The baked install tree under /opt/max is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $MAX_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass MAX_UID/MAX_GID — or the PUID/PGID aliases — which the
...
and the baked /opt/max install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e MAX_UID=\$(id -u) -e MAX_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the max user to that UID/GID at boot and chowns the data
...
# --- Bootstrap MAX_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid max mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom MAX_HOME paths whose parents only
# root can create (e.g. `MAX_HOME=/home/hermes/.max` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for MAX_UID/MAX_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  MAX_UID/
# MAX_GID still win when both are set.  See #15290, salvages #25872.
MAX_UID="${MAX_UID:-${PUID:-}}"
MAX_GID="${MAX_GID:-${PGID:-}}"

if [ -n "${MAX_UID:-}" ] && validate_uid_gid "$MAX_UID" && [ "$MAX_UID" != "$(id -u hermes)" ]; then
    echo "[stage2] Changing max UID to $MAX_UID"
    usermod -u "$MAX_UID" hermes
fi
if [ -n "${MAX_GID:-}" ] && validate_uid_gid "$MAX_GID" && [ "$MAX_GID" != "$(id -g hermes)" ]; then
    echo "[stage2] Changing max GID to $MAX_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$MAX_GID" max 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised max user (UID 10000)
...
# max process with `Groups: 10000` (998 gone); after this hook adds
...
#   - max is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G max 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] max already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" max 2>/dev/null; then
        echo "[stage2] Added max to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group max failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When MAX_UID is remapped or the top-level $MAX_HOME isn't owned by
# the runtime max UID, restore ownership to max — but ONLY for the
# directories max actually writes to. The full $MAX_HOME may be a
...
tree_has_non_hermes_owner() {
    target="$1"
    find "$target" \( ! -user max -o ! -group max \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$MAX_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $MAX_HOME (targeted) to max ($actual_hermes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $MAX_HOME: chown the directory itself (not its contents)
    # so max can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$MAX_HOME"; then
        :
    else
        chown hermes:hermes "$MAX_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $MAX_HOME failed (rootless container?) — continuing"
    fi
    # Max-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by max (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$MAX_HOME/$sub" ] && tree_has_non_hermes_owner "$MAX_HOME/$sub"; then
            chown_hermes_tree "$MAX_HOME/$sub"
        fi
    done
fi
...
# max user. Hosted/container instances keep mutable state under
# $MAX_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# MAX_DISABLE_LAZY_INSTALLS=1. Keeping /opt/max root-owned and
...
# $MAX_HOME/lazy-packages dir on the data volume (Dockerfile sets
# MAX_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# Always reset ownership of $MAX_HOME/profiles to max on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> max …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as max and walks
...
if [ -d "$MAX_HOME/profiles" ] && tree_has_non_hermes_owner "$MAX_HOME/profiles"; then
    chown_hermes_tree "$MAX_HOME/profiles"
fi

# Always reset ownership of $MAX_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged max runtime even
...
# gate as profiles/).
if [ -d "$MAX_HOME/cron" ] && tree_has_non_hermes_owner "$MAX_HOME/cron"; then
    chown_hermes_tree "$MAX_HOME/cron"
fi
...
# top-level $MAX_HOME is mis-owned, so a warm volume with hermes-owned
# MAX_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid max mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as hermes.
if [ -d "$MAX_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$MAX_HOME/logs/gateways"; then
        :
    else
        chown hermes:hermes "$MAX_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# max pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged max gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $MAX_HOME is
...
# and cron/.
if [ -d "$MAX_HOME/platforms/pairing" ] && tree_has_non_hermes_owner "$MAX_HOME/platforms/pairing"; then
    chown_hermes_tree "$MAX_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$MAX_HOME/pairing" ] && tree_has_non_hermes_owner "$MAX_HOME/pairing"; then
    chown_hermes_tree "$MAX_HOME/pairing"
fi
...
# *subdirectories*; loose state files living directly under $MAX_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> max …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged max runtime then hits
...
# sweep so host-owned files in a bind-mounted $MAX_HOME are never
...
# entries of max_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$MAX_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$MAX_HOME/$f"; then
            :
        else
            chown hermes:hermes "$MAX_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the max runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$MAX_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$MAX_HOME/config.yaml"; then
        :
    else
        chown hermes:hermes "$MAX_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$MAX_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as max user ---
# Run as max via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $MAX_HOME values
...
as_hermes mkdir -p \
    ...
    "$MAX_HOME/home" \
    "$MAX_HOME/pairing" \
    "$MAX_HOME/platforms/pairing" \
    "$MAX_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/max/.install_method (see Dockerfile), NOT written here into
# $MAX_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $MAX_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.max:/opt/data) and sometimes shared with a
...
# 'max update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $MAX_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/max stamp, so deleting the data-dir copy is safe).
if [ -f "$MAX_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$MAX_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$MAX_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$MAX_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$MAX_HOME/$dest"; then
            :
        else
            as_hermes cp "$INSTALL_DIR/$src" "$MAX_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/max/.env.example being present
...
# not generate one. Max loads $MAX_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$MAX_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$MAX_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $MAX_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$MAX_HOME/.env" ] && ! refuse_symlinked_path "clean" "$MAX_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$MAX_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$MAX_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$MAX_HOME/.env"; then
        :
    else
        if [ ! -f "$MAX_HOME/.env" ]; then
            ...
            (umask 077 && as_hermes touch "$MAX_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$MAX_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$MAX_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$MAX_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $MAX_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $MAX_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$MAX_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$MAX_HOME/.env"; then
        :
    else
        chown hermes:hermes "$MAX_HOME/.env" 2>/dev/null || true
        chmod 600 "$MAX_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `max` user — without this it gets EACCES on every
...
# max group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp max /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted max group access to the Fly Machines API socket"
    else
...
# $MAX_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `max update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set MAX_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$MAX_HOME/config.yaml" ]; then
    s6-setuidgid max "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$MAX_HOME/auth.json" ] && [ -n "${MAX_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$MAX_HOME/auth.json"; then
        :
    else
        printf '%s' "$MAX_AUTH_JSON_BOOTSTRAP" > "$MAX_HOME/auth.json"
        chown hermes:hermes "$MAX_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$MAX_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via MAX_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$MAX_HOME/auth.json" ] && [ -n "${MAX_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$MAX_HOME/auth.json"; then
        :
    else
        s6-setuidgid max "$INSTALL_DIR/.venv/bin/python" \
...
# can set MAX_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# MAX_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$MAX_HOME/gateway_state.json" ] && \
        [ "${MAX_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$MAX_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$MAX_HOME/gateway_state.json"
        chown hermes:hermes "$MAX_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$MAX_HOME/gateway_state.json"
    fi
fi
...
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/max/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Max spawns for the
...
# main-wrapper.sh propagates it into the supervised ``max`` process
