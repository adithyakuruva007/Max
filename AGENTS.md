# Max Agent - Development Guide

Instructions for AI coding assistants and developers working on the max-agent codebase.

**Never give up on the right solution.**

## What Max Is

Max is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Max ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`max tools`, `max setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `MAX_HOME`. Mocks hide integration bugs.
...
- **New `MAX_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `max tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.max/plugins/` (or via a
  pip entry point), and promote them in the Stardust Research Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `max <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `max webhook`, `max cron`, `max tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.max/plugins/` or a pip package, discovered at runtime.
...
one behind a plain URL + token, or Max Cloud. Only the first two are spawned
by us and carry `MAX_DESKTOP=1`. Every env-keyed GUI gate is therefore a
...
- **The toolset is the surface gate.** Keep the tools off `_MAX_CORE_TOOLS`
...
- **Ask which identity you actually mean.** `MAX_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`max --tui` against that same backend) is
...
`$HOME/.max/max-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _MAX_CORE_TOOLS list
├── cli.py                # MaxCLI class — interactive CLI orchestrator (~11k LOC)
├── max_state.py       # SessionDB — SQLite session store (FTS5 search)
├── max_constants.py   # get_max_home(), display_max_home() — profile-aware paths
├── max_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── max_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
├── ui-tui/               # Ink (React) terminal UI — `max --tui`
...
**User config:** `~/.max/config.yaml` (settings), `~/.max/.env` (API keys only).
**Logs:** `~/.max/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_max_home()`.
Browse with `max logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Max: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`max_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `MaxCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.max/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`max_cli/commands.py`)
...
1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `max_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `MaxCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `max --tui` or `MAX_TUI=1`.
...
### TUI in the Dashboard (`max dashboard` → `/chat`)

The dashboard embeds the real `max --tui` — **not** a rewrite.  See `max_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `max_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `max --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `max --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@hermes/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `max serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `MAX_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `max` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `max --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Max built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Max core.
Use the plugin route instead: create `~/.max/plugins/<name>/plugin.yaml`
and `~/.max/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Max tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_MAX_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_MAX_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_max_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `MAX_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_max_home()` for the base directory — never `Path.home() / ".max"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `max_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `max_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `max tools`, `max setup`, most CLI subcommands | `max_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`max_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
max_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.max/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Max gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `max_cli/skin_engine.py`:
...
Users create `~/.max/skins/<name>.yaml`:
...
Max has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.max/plugins/` and pip-installed entry points.

### General plugins (`max_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.max/plugins/`, `./.max/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `max` at startup so
  `max <pluginname> <subcmd>` works with no change to `main.py`
...
bundled, `$MAX_HOME/plugins/`, `./.max/plugins/` (opt-in via
`MAX_ENABLE_PROJECT_PLUGINS`), and `hermes_agent.memory_providers` entry
...
it at argparse setup time and wires it into `max <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `max --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `max_cli/main.py`, etc.).
...
into `~/.max/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `max memory setup` / `post_setup()` without
...
`~/.max/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the Stardust Research Discord
...
2. User: `$MAX_HOME/plugins/model-providers/<name>/`
...
The desktop "Bots" experience ships bundled in-tree. Each bot is a Max
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `max skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.max.tags`, `metadata.max.category`,
`metadata.max.related_skills`, `metadata.max.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.max.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Max tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Max Agent" is the secondary collaborator. If the
   contributor's commit shows "Max Agent" as author (because they
   used Max to draft the skill), replace it with their actual name
...
lives in the `max-agent-dev` skill at
...
`"messaging"`); `_MAX_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `max tools` (the curses UI) or the
...
Max returns a delegation id immediately and the result re-enters the
...
go to `~/.max/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `max_cli/curator.py` wires `max curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.max/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `max cron <verb>`
...
- File lock at `~/.max/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `max kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `max_cli/kanban.py` wires `max kanban` with verbs
...
- **Board** is the hard boundary — workers are spawned with
  `MAX_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
- **Plan** (`max_cli/update_inventory.py`, `max update --plan`):
...
- **Snapshot** (`max_cli/backup.py`): pre-update quick snapshot for
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE
  (every `hermes-gateway*` unit / `ai.max.gateway*` LaunchAgent),
...
- **Report**: every run writes a machine-readable receipt to
  `~/.max/logs/update_receipts/` (`latest.json` pointer; steps,
...
`max serve` (control plane, desktop-spawned child) dies with the app
...
Max-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `MAX_BACKGROUND_NOTIFICATIONS` env var):
...
Max supports **profiles** — multiple fully isolated instances, each with its own
`MAX_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `max_cli/main.py` sets
`MAX_HOME` before any module imports. All `get_max_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_max_home()` for all MAX_HOME paths.** Import from `max_constants`.
   NEVER hardcode `~/.max` or `Path.home() / ".max"` in code that reads/writes state.
   ```python
   # GOOD
   from max_constants import get_max_home
   config_path = get_max_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".max" / "config.yaml"
   ```

2. **Use `display_max_home()` for user-facing messages.** Import from `max_constants`.
   This returns `~/.max` for default or `~/.max/profiles/<name>` for profiles.
   ```python
   # GOOD
   from max_constants import display_max_home
   print(f"Config saved to {display_max_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.max/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_max_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_max_home()`,
   not `Path.home() / ".max"`.

4. **Tests that mock `Path.home()` must also set `MAX_HOME`** — since code now uses
   `get_max_home()` (reads env var), not `Path.home() / ".max"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"MAX_HOME": str(tmp_path / ".max")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not MAX_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".max" / "profiles"`, NOT `get_max_home() / "profiles"`.
   This is intentional — it lets `max -p coder profile list` see all profiles regardless
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  (gateway run), `max_cli.update_cmd._hermes_holder_subcommand`
  (top-level subcommand of any Max argv). Never hand-roll token scans.
...
### DO NOT hardcode `~/.max` paths
Use `get_max_home()` from `max_constants` for code paths. Use `display_max_home()`
for user-facing print/log messages. Hardcoding `~/.max` breaks profiles — each profile
has its own `MAX_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### All CLI menu-pickers MUST use curses.
Interactive menus must use `max_cli/curses_ui.py`. See `max_cli/tools_config.py` for an example.
...
with actual imports (not mocks) against a temp `MAX_HOME`.

### Tests must not write to `~/.max/`
The `_isolate_hermes_home` autouse fixture in `tests/conftest.py` redirects `MAX_HOME` to a temp dir. Never hardcode `~/.max/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_hermes_home()` resolve within the temp dir.
Use the pattern from `tests/max_cli/test_profiles.py`:
```python
@pytest.fixture
...
subprocess (`--file-retries`, default 1; `MAX_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.max/` | Your real config+auth.json                  | Temp dir per test                         |
...
Max supports Linux, macOS and native Windows, and plenty of its behaviour
...
`tests/max_cli/test_venv_holder_windows_live.py` on a real
