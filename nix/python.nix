...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Max derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            max-agent = prev.max-agent.overrideAttrs (_old: {
              MAX_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $MAX_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$MAX_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        max-agent = prev.max-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "max-agent-env" {
    max-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "max-agent-editable-env" {
    max-agent = dependency-groups;
  };
}
