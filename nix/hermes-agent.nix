...
let
  ...
  maxVenv = (mkMaxVenv extraDependencyGroups).venv;

  maxNpmLib = callPackage ./lib.nix {
    inherit npm-lockfile-fix nodejs;
  };

  maxTui = callPackage ./tui.nix {
    inherit maxNpmLib;
  };

  maxWeb = callPackage ./web.nix {
    inherit maxNpmLib;
  };
  ...
  checkPackageCollisions = ''
    ...
    core = set()
    venv_sp = pathlib.Path('${maxVenv}/${sitePackagesPath}')
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    cp -r ${bundledSkills} $out/share/max-agent/skills
    cp -r ${bundledPlugins} $out/share/max-agent/plugins
    cp -r ${bundledLocales} $out/share/max-agent/locales
    cp -r ${maxWeb} $out/share/max-agent/web_dist

    mkdir -p $out/ui-tui
    cp -r ${maxTui}/lib/max-tui/* $out/ui-tui/

    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${maxVenv}/bin/${name} $out/bin/${name} \
          ...
          --set MAX_PYTHON ${maxVenv}/bin/python3 \
          --set MAX_NODE ${lib.getExe nodejs} \
          ${lib.optionalString (rev != null) ''--set MAX_REVISION ${rev} \''}
    ...
    ${lib.optionalString (extraPythonPackages != [ ]) ''
      ...
      ${maxVenv}/bin/python3 -c "${checkPackageCollisions}"
      echo "=== No collisions ==="
    ''}
  ...
  passthru =
    ...
    in
    {
      ...
      # `maxDesktop` references `finalAttrs.finalPackage` (this whole
      ...
      maxDesktop = callPackage ./desktop.nix {
        inherit maxNpmLib electron;
        maxAgent = finalAttrs.finalPackage;
      };
