...
#
# `maxAgent` is the fully-built `.#default` package — it ships the
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "max-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "max-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "max-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/max-desktop $out/bin
    cp -r ${renderer}/* $out/share/max-desktop/
    ...
    substituteInPlace $out/share/max-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/max-desktop'"
    ...
    makeWrapper ${lib.getExe electron} $out/bin/max-desktop \
      --add-flags "$out/share/max-desktop" \
      --set MAX_DESKTOP_HERMES "${lib.getExe maxAgent}" \
