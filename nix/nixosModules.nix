# nix/nixosModules.nix — NixOS module for max-agent
#
# Two modes:
...
{ inputs, ... }: {
  ...
  let
    ...
    # Deep-merge config type (from 0xrsydn/nix-max-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "max-config-attrs";
    ...
    generatedConfigFile = pkgs.writeText "max-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "max-documents" { } (
      ''
        mkdir -p $out
    ...
    containerEntrypoint = pkgs.writeShellScript "max-container-entrypoint" ''
      set -eu
      ...
      if [ ! -f /var/lib/max-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/max-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/max ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/max
    ...
    # Default: /var/lib/max/workspace → /data/workspace.
  ...
  in {
    options.services.max-agent = with lib; {
      enable = mkEnableOption "Max Agent gateway service";
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/max";
        description = "State directory. Contains .hermes/ subdir (MAX_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Max config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $MAX_HOME/.env at activation time.
          Max reads this file on every startup via load_max_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $MAX_HOME/.env
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the max user's per-user profile
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          max_agent.plugins entry-point group. Each package must be built
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.max-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      (lib.mkIf cfg.addToSystemPackages {
        ...
        environment.variables.MAX_HOME = "${cfg.stateDir}/.hermes";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.max-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            ...
            is set, but addToSystemPackages is false. Without a host-installed max
      ...
      {
        system.activationScripts."max-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          ...
          # max-group users can save settings via the CLI/TUI, else 0640).
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.hermes/.container-mode <<'MAX_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    max_bin=${containerDataDir}/current-package/bin/max
    MAX_CONTAINER_MODE_EOF
          ...
          # Create ~/.max -> stateDir/.hermes for each hostUser so the
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.max-agent = {
          ...
          serviceConfig = {
            ...
            # $MAX_HOME/.env by the activation script. load_max_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/max"
              "gateway"
            ] ++ cfg.extraArgs);
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env MAX_UID="$MAX_UID" \
                --env MAX_GID="$MAX_GID" \
                --env MAX_HOME=${containerDataDir}/.hermes \
