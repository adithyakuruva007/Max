# nix/packages.nix — Max Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./max-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.maxTui;
        web = full.maxWeb;
        desktop = full.maxDesktop;

        update-npm-lockfile = full.maxNpmLib.updateNpmLockfile;
      };
    };
}
