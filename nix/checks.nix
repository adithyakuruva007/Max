...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      ...
      maxVenv = max-agent.maxVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "max-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
            ${maxVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "max-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "max-build-package" { } ''
        ...
        build-devshell = pkgs.runCommand "max-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "max-package-contents" { } ''
          ...
          test -x ${max-agent}/bin/hermes || (echo "FAIL: hermes binary missing"; exit 1)
          test -x ${max-agent}/bin/max-agent || (echo "FAIL: max-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${max-agent}/bin/hermes version 2>&1 | grep -qi "max" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "max-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in hermes max-agent max-acp; do
        ...
        cli-commands = pkgs.runCommand "max-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking hermes --help (binary) ==="
        ...
        bundled-skills = pkgs.runCommand "max-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${max-agent}/share/max-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${max-agent}/share/max-agent/skills -name "SKILL.md" | wc -l)
        ...
        bundled-plugins = pkgs.runCommand "max-bundled-plugins" { } ''
          set -e
        ...
        bundled-locales = pkgs.runCommand "max-bundled-locales" { } ''
          set -e
          ...
          LOC_COUNT=$(find ${max-agent}/share/max-agent/locales -name "*.yaml" | wc -l)
          ...
          echo "=== Rendering via the wrapper override (MAX_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          ...
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without MAX_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "max-bundled-tui" { } ''
          set -e
        ...
        # Verify MAX_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        max-node = pkgs.runCommand "max-node-version" { } ''
          set -e
        ...
        managed-guard = pkgs.runCommand "max-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked "config set" ${max-agent}/bin/hermes config set model foo
          check_blocked "config edit" ${max-agent}/bin/hermes config edit
        ...
        extra-python-packages = let
          ...
          maxWithExtra = max-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "max-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${maxWithExtra}/bin/hermes || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${maxWithExtra}/bin/hermes || \
        ...
        extra-dependency-groups = let
          maxWithGroups = max-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "max-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${maxWithGroups}"
          echo "venv: ${maxWithGroups.maxVenv}"
        ...
        messaging-variant = pkgs.runCommand "max-messaging-variant" { } ''
          set -e
        ...
        in pkgs.runCommand "max-config-roundtrip" {
          ...
          merge_and_load() {
            local max_home="$1"
            export MAX_HOME="$max_home"
            ${configMergeScript} ${nixSettings} "$max_home/config.yaml"
${maxVenv}/bin/python3 -c '
import json, sys
