"""
Top-level argparse construction for the max CLI.
...
# argparse runs (it sets ``MAX_HOME`` and strips itself from ``sys.argv``),
...
# tests/max_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``max --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``max_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    max                        Start interactive chat
    max chat -q "Hello"        Single query mode
    max --tui                  Launch the modern TUI (or set display.interface: tui)
    max --cli                  Force the classic REPL (overrides display.interface: tui)
    max -c                     Resume the most recent session
    max -c "my project"        Resume a session by name (latest in lineage)
    max --resume <session_id>  Resume a specific session by ID
    max --resume latest        Resume the most recent session (same as -c)
    max --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    max setup                  Run setup wizard
    max logout                 Clear stored authentication
    max auth add <provider>    Add a pooled credential
    max auth list              List pooled credentials
    max auth remove <p> <t>    Remove pooled credential by index, id, or label
    max auth reset <provider>  Clear exhaustion status for a provider
    max model                  Select default model
    max fallback [list]        Show fallback provider chain
    max fallback add           Add a fallback provider (same picker as `max model`)
    max fallback remove        Remove a fallback provider from the chain
    max config                 View configuration
    max config edit            Edit config in $EDITOR
    max config set model gpt-4 Set a config value
    max gateway                Run messaging gateway
    max -s max-agent-dev,github-auth
    max -w                     Start in isolated git worktree
    max gateway install        Install gateway background service
    max sessions list          List past sessions
    max sessions browse        Interactive session picker
    max sessions rename ID T   Rename/title a session
    max logs                   View agent.log (last 50 lines)
    max logs -f                Follow agent.log in real time
    max logs errors            View errors.log
    max logs --since 1h        Lines from the last hour
    max debug share             Upload debug report for support
    max console                Open the safe Max command console
    max update                 Update to latest version
    max dashboard              Start web UI dashboard (port 9119)
    max dashboard --stop       Stop running dashboard processes
    max dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="hermes",
        description="Max Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `max chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via MAX_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `max setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to MAX_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.max/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Max Agent",
    )
    ...
    # top-level parser: when the user writes `max -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/max_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also MAX_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.max/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Max itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
