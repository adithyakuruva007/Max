...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``python -m max_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``max gateway run``.
    ...
    lines.append(f'set "MAX_HOME={hermes_home}"')
    ...
    prog_args = [python_exe_path, "-m", "max_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('MAX_HOME')}) = {_quote_vbs_string(hermes_home)}",
...
def _write_task_script() -> Path:
    ...
    hermes_home = str(Path(get_max_home()))
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (MAX_HOME, VIRTUAL_ENV,
    ...
    try:
        hermes_home = str(Path(get_max_home()).resolve())
    except Exception:
        hermes_home = ""
    ...
    if hermes_home:
        env_overlay["MAX_HOME"] = hermes_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m max_cli.main gateway run`` directly — NOT
