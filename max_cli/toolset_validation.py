...
the tool registry or launching Max (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``max-cli`` to the non-existent ``max``. ``resolve_toolset('max')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``max-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``max`` instead of ``max-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `max tools` to reconfigure."
        )
    return warnings
