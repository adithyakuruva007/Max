"""Default configuration data for Max Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from max_cli/config.py. Must not import from max_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["hermes-cli"],
    # SQLite journal mode used by every Max database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Max server processes.
    ...
    "session": {
        # Per-terminal `max -c`: each CLI session drops a breadcrumb file
        # under $MAX_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by max_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `max chat --run-budget N`.
        ...
        # an interactive `max gateway restart` must never block for hours
        ...
        # `max status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``MAX_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # Root directory for Max' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # Max uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        # storage at MAX_HOME/cache/terminal (auto-pruned after 72h) — NOT
        ...
        # bot_relay from a short-lived `max -p <bot> chat -Q` recipient
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `max tools` picker's
    ...
    "browser": {
        ...
        # browser tools) runs on a Max-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Max' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.max/browser-profile/) so copied
        ...
        # approval does it run `max browser close-profile` (terminates the
    ...
    "checkpoints": {
        ...
        # ``max checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .max.md, or .cursorrules before Max applies
    ...
    # Single-query (``max -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    "compression": {
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Max' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Max does.
    ...
    "display": {
        ...
        # model — see max_cli/focus_view.py.
        "focus_view": False,
    ...
    "dashboard": {
        ...
        # protocol ping (see max_cli/web_server.py — an event-loop stall
        ...
        # MAX_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (MAX_TUI_SESSION_TTL_S, default 6h) are closed with
    ...
    # "Hey Max" hands-free wake word. Always-on, on-device hotword
    ...
    "skills": {
        ...
        # checkout, ``<root>/.max/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``max skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``max skills trust`` / ``max skills untrust``.
        ...
        # (`max skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.max/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.max/.curator_backups/blobs/. Enables `max curator ledger`
        # and single-mutation `max curator rollback <entry-id>`.
    ...
    "curator": {
        ...
        # skills are kept forever. When > 0, `max curator purge` deletes
    ...
    # dangerous command. -q runs export MAX_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; MAX_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # ``max plugins enable|disable`` and intentionally omitted here so an
    ...
    "security": {
        ...
        # .cursorrules, project-local .max config) always require human
    ...
    "cron": {
        ...
        # MAX_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # MAX_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via MAX_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # #8731).
    #
    # Provider keys accept the Max provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # `max update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``MAX_GATEWAY_MAX_STARTS`` / ``MAX_GATEWAY_START_WINDOW_S``
    ...
    "sessions": {
        ...
        # the default of ``max sessions prune``.
        "retention_days": 90,
        ...
        # user runs `max sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `max update` prints a one-line notice with
        ...
        # ~/.max/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to MAX_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # MAX_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `max sessions export`. Checked
    ...
    # ``max doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `max doctor --live`
    ...
    "updates": {
        ...
        #     DBs) into <MAX_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``max backup``-style zip
        #     of MAX_HOME into <MAX_HOME>/backups/, restorable with
        #     ``max import``. Can add minutes on large homes. This is the
        ...
        # When `max update` finds the source checkout parked on a feature
        ...
        #                         merge. `max update --switch-branch`
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # material under ~/.max/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Max may use this encrypted
    ...
    "computer_use": {
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Max also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Max computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Max passes the
    ...
    #
    # Configure with `max egress setup`.  Disabled by default — the rest of
    # Max works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.max/bin/ on
    ...
    "desktop": {
        ...
        # Ignored on macOS/Windows. Bridged to the MAX_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Max' wake word engine (optional; openWakeWord is the free default)",
    ...
    # MAX_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to MAX_TOOL_PROGRESS_MODE for backward
    ...
    # MAX_TOOL_PROGRESS is fully unsupported since the v12 config support
