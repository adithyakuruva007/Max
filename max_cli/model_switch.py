...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Max wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Max.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Max versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``max model`` wizard — not a
    ...
    while ``max model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Max itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Max itself discovered (``models_discovered: true``
...
_MAX_MODEL_WARNING = (
    "Stardust Research Max 3 & 4 models are NOT agentic and are not designed "
    "for use with Max Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Stardust Research Max 3 / Max 4 chat families.
...
#   NousResearch/Max-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
...
def resolve_display_context_length(
    ...
    if config_context_length is not None and (
        ...
        try:
            from max_cli.route_identity import should_clear_context_pin
...
async def resolve_display_context_length_async(
    ...
    ``max_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if provider_changed or explicit_provider:
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from max_cli.models import should_use_ollama_native_catalog
    ...
    if target_provider in {"nous", "nous-portal", "stardustresearch"}:
    ...
    # kimi) the same way.
    from max_cli.models import opencode_provider_family as _oc_family_fn
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Max provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from max_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from max_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from max_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from max_cli.providers import MAX_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from max_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Max-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from max_cli.auth import is_runtime_provider_routable
    ...
    # --- Section 2: Max-only providers (MAX_OVERLAYS) ---
...
def list_authenticated_providers(
    ...
    *,
    ...
    from max_cli.config import coerce_provider_id, stringify_provider_map
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Max may not route yet.
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            if group_key not in ep_groups:
                ...
                # Max's own writer uses (mirrors section-4 grouping).
            ...
            # ``max model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``max model``. Pin with
            ...
            if _probe_live:
                ...
                try:
                    from max_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for grp in groups.values():
            ...
            #   ``max model``. Pin a dict catalog with
