...
to the resolved absolute path of the client binary.  Max' interpreter is
...
* Dynamically-linked builds look up ``libpython`` via
  ...
  no dylib — every max command, including update/doctor, died in dyld
  (#95425).
...
def _install_anchor(venv_dir: Path, source_file: Path) -> None:
    ...
    try:
        ...
        try:
            from max_cli.managed_uv import _macos_sign_managed_python
...
def tcc_anchor_state(project_root: Path | None = None) -> tuple[str, str]:
    """Report the anchor state for ``max doctor``.
