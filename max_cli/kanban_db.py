...
coding workloads.  See ``docs/max-kanban-v1-spec.pdf`` for the full
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "max_kanban_current_board_override",
    default=None,
)
...
def kanban_home() -> Path:
    ...
    2. ``get_default_max_root()``, which already returns ``<root>``
...
def _absolute_max_path(path: str) -> str:
...
def _max_path_argv(path: str) -> list[str]:
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_max_argv()
    return [_absolute_max_path(path)]


def _resolve_max_argv() -> list[str]:
    ...
    2. ``shutil.which("max")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
        ``.\\max.CMD`` when the current directory is on ``PATH``; directly
    ...
    Mirrors ``gateway.run._resolve_max_bin`` for the same reason. Kept
    ...
    if env_bin:
        if _looks_like_path(env_bin):
            return _max_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _max_path_argv(resolved_env_bin)
        return _module_max_argv()

    max_bin = _safe_which_no_cwd("max") if _IS_WINDOWS else shutil.which("max")
    if max_bin:
        return _max_path_argv(max_bin)
    return _module_max_argv()
...
def _resolve_worker_cli_toolsets(max_home: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        token = set_max_home_override(max_home)
        try:
            cfg = load_config()
...
def _default_spawn(
    ...
    *,
    ...
    # dispatcher's. Belt-and-braces with the `get_default_max_root()`
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled MAX_PROFILE values like "max-system"
