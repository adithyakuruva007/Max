"""`max checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.max/checkpoints/``.  Actions:

    max checkpoints               # same as `status`
    max checkpoints status        # total size, project count, breakdown
    max checkpoints list          # per-project checkpoint counts + workdir
    max checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    max checkpoints clear [-f]    # nuke the entire base (asks first)
    max checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    max checkpoints
    max checkpoints prune --retention-days 3 --max-size-mb 200
    max checkpoints clear -f
...
from max_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: max checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``max checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `max checkpoints` → status
