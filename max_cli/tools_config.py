...
TOOL_CATEGORIES = {
    ...
    "browser": {
        ...
        # _plugin_browser_providers() in _visible_providers().
        # "Local Browser" is the default — no API key needed.
        "providers": [
            {
                "name": "Local Browser",
                "badge": "★ recommended · free",
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_max_home()}/max-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in max-agent directory)")
            return
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``max-discord``, which contains those
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``max-discord``) is an opt-in to the platform's native default-off
    ...
    # "max-cli" (which include all _MAX_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[max-cli, spotify]`` after enabling Spotify via ``max
        ...
        # (e.g. "max-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
    ...
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"max-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("max-")}
    ...
    # `max-cli`), resolve_toolset() returns [] for each and the platform ends
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (max-cli, max-telegram, etc.)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Changes take effect on next 'max' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, max mcp
