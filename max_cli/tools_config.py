"""
Unified tool configuration for Max Agent.

`max tools` and `max setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.max/config.yaml under
...
from max_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from max_cli.nous_account import format_nous_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``max tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`max_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from max_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _MAX_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `max tools` → Video Generation, which walks
...
# `max tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `max tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from max_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `max tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``max tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from max_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from max_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``max tools`` checklist actually offers
    for ``platform``.
    ...
    so ``max tools`` never claims to add or remove a toolset the user was
...
from max_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Max routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set MAX_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "MAX_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "MAX_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "MAX_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "MAX_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "MAX_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `max model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("MAX_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``MAX_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Max telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Max' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $MAX_HOME/bin is never on PATH, so a bare which()
    # misses the uv Max installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``max update`` and
      by ``max computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``max update`` sets
    ...
    a further ~600s wait on Windows). ``max computer-use install
    ...
    ``max update`` already prints a contextual line before its update
    ...
    ``max update`` and must not inherit the explicit install command's
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `max update` calls this
    ...
    override = os.environ.get("MAX_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    MAX_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Max' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Max' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `max update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Max cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("MAX_CUA_DRIVER_CMD", "").strip():
            _print_info(
        ...
        if is_windows and require_confirmed_update:
            ...
            _print_info(
                ...
                "max computer-use install --upgrade"
            )
            return False
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `max computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `max update` (require_confirmed_update=True) keeps the
    ...
    # `max computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: max computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: max computer-use doctor")
            return False
    if ok and before:
        try:
...
# Optional refreshes launched by ``max update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `max update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``max update`` hangs past its
    ...
    try:
        # When not verbose (e.g. `max update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `max update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.max/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Max process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Max-managed ``$MAX_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Max installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # (PATH -> Homebrew/Max-managed node -> npx) rather than a bare
        # shutil.which — Max-managed-Node-only setups resolve agent-browser
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/stardustresearch/max-agent:latest"
            )
            return

        # browser_cmd was already resolved above (same PATH -> Homebrew ->
        # Max-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Max-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.max/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.max/config.yaml")
        ...
        # Run the full `max auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.max/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from max_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: max auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `max auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: max auth spotify")
        except Exception as exc:
        ...
        try:
            from max_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: max plugins enable observability/langfuse")
        _print_info("    Restart Max for tracing to take effect.")
        _print_info("    Verify: max plugins list")
        ...
        try:
            ...
            _print_info("    Run later: max auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `max auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: max auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: max auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``max tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``max tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: max tools post-setup <key>")
        return 2
...
#:
#: Saving ``max tools`` (or one toggle in the desktop Toolsets UI) replaces
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``max tools``
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (max-acp, hermes-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # "hermes-cli" (which include all _MAX_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # through ``max tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `max tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `max tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `max tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by max tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `max config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `max` instead of
    ...
    # at runtime, not only during the next `max update`/`max doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `max tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `max tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `max tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``max tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``max tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from max_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``max tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from max_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (max_cli/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `max model`).
    if managed_feature:
        from max_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``max model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``max model`` aux-task
    ...
    if not providers:
        _print_warning(
            ...
            "with `max model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `max model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `max tools` and `max setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Max Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://max-agent.stardustresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color(f"  Tool configuration saved to {display_max_home()}/config.yaml", Colors.DIM))
