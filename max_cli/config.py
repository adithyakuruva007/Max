...
- max config get      - Print a resolved configuration value
- max config set      - Set a specific value
- max config unset    - Remove a user configuration value
...
from max_cli.route_identity import normalize_route_base_url
...
def _warn_config_parse_failure(
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (max config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "MAX_COPILOT_ACP_COMMAND", "MAX_COPILOT_ACP_ARGS",
    # Max security policy / approval-routing context. These remain available
    ...
    "MAX_YOLO_MODE", "MAX_ACCEPT_HOOKS", "MAX_REDACT_SECRETS",
    "MAX_INTERACTIVE", "MAX_EXEC_ASK", "MAX_GATEWAY_SESSION",
    "MAX_CRON_SESSION", "MAX_SINGLE_QUERY_SESSION",
    "MAX_SESSION_KEY", "MAX_SESSION_PLATFORM",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "and security policy (MAX_HOME, MAX_YOLO_MODE, ...) "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # MAX_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
...
# from `nix run` / `nix profile install` (which don't set MAX_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    if raw:
        ...
        # MAX_MANAGED variable of the service. A marker with content
...
_NIX_UPDATE_MSG = (
    "Update Max through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/max`` inside the published image. It is a property of
    the running interpreter, NOT of ``$MAX_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Max was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    3. MAX_MANAGED env / .managed marker (NixOS managed mode)
    ...
    # detect Nix installs that don't set MAX_MANAGED (e.g. ``nix run``,
...
from max_constants import get_max_home, get_process_hermes_home  # noqa: F811,E402
...
def ensure_hermes_home():
    ...
    After the first successful pass for a given ``MAX_HOME`` we only re-run
    ...
    ``get_max_home()`` and therefore re-run for the new path.
    """
    home = get_max_home()
    key = str(home)
    ...
    from max_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _MAX_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
...
from max_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `max config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.max.config`` entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    # Max (``_save_discovered_models_to_config``), not hand-curated.
    # Older Max versions wrote an in-mapping ``__discovered_model_catalog__``
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (max_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (max_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (max_cli/setup.py)
    "known_plugin_toolsets", # written/read by max_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (max_cli/nous_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    # scalars are bridged into os.environ (gateway/run.py, max send) so
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from max_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ max config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in max_cli.config_migrations as a
    ...
    # metadata.max.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.max/.env); "
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``MaxCLI`` construction: the model becomes a plain
...
#   - survives the ``str() → int()`` round-trip through the MAX_MAX_ITERATIONS
...
# Back-compat alias — canonical set lives in max_cli.personality.
from max_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`max_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from max_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from max_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``MAX_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`max_cli.personality` (single owner).
    """
    from max_cli.personality import resolve_ephemeral_system_prompt
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Max host rewrites it to the host or container
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``MAX_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"MAX_CUSTOM_{slug}_API_KEY" if slug else "MAX_CUSTOM_API_KEY"
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.max/.env``, scope-aware.
...
def show_config():
    ...
    try:
        from max_cli.personality import active_personality_name
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``MAX_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("MAX_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `max cron list`, then pin the "
        "intended values with `max cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (max_cli/plugins_cmd.py, max_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `max config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``max config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``max config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``max config set --force``.
    """
    if is_managed():
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   max config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   max config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``max config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``max config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    max config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    max config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_max_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `max config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: max config get <key> [--json]")
            print()
            print("Examples:")
            print("  max config get model")
            print("  max config get terminal.backend")
            print("  max config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: max config set [--force] <key> <value>")
            print()
            print("Examples:")
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: max config unset <key>")
            print()
            print("Examples:")
            print("  max config unset model")
            print("  max config unset terminal.backend")
            print("  max config unset OPENROUTER_API_KEY")
            sys.exit(1)
        try:
        ...
        print("  max config get <key>          Print a resolved config value")
        print("  max config set <key> <value>   Set a config value")
        print("  max config unset <key>        Remove a config value")
