"""
Configuration management for Max Agent.

Config files are stored in ~/.max/ for easy access:
- ~/.max/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.max/.env         - API keys and secrets

This module provides:
- max config          - Show current configuration
- max config edit     - Open config in editor
- max config set      - Set a specific value
- max config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``max config set`` (which rewrites
    ...
    ``config.yaml`` in place: max never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.max/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``max logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  max config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Max makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Max itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``max update``, the TUI build.
...
#   on every plugin install / ``max update``.
...
# * ``MAX_HOME`` / ``MAX_PROFILE`` / ``MAX_CONFIG`` /
#   ``MAX_ENV`` — Max runtime location flags. Writing these into
...
#
# IMPORTANT: ``MAX_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (MAX_LANGFUSE_PUBLIC_KEY,
# MAX_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Max runtime location — never via dashboard env writer.
    # NOT a MAX_* blanket: integration credentials (MAX_GEMINI_*,
    # MAX_LANGFUSE_*, MAX_SPOTIFY_*, ...) ARE allowed.
    "MAX_HOME", "MAX_PROFILE", "MAX_CONFIG", "MAX_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Max runtime location "
            "(MAX_HOME, MAX_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "MAX_TOOL_PROGRESS", "MAX_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `max plugins enable
    # observability/langfuse` or `max tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "MAX_LANGFUSE_ENV",
    "MAX_LANGFUSE_RELEASE",
    "MAX_LANGFUSE_SAMPLE_RATE",
    "MAX_LANGFUSE_MAX_CHARS",
    "MAX_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from max_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("MAX_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_max_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Max is running in package-manager-managed mode.

    Two signals: the MAX_MANAGED env var (set by the systemd service),
    or a .managed marker file in MAX_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``max_cli/`` — i.e. the git checkout for source
    installs, ``/opt/max`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$MAX_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Max was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$MAX_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. MAX_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.max``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$MAX_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.max:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$MAX_HOME``, a
    ...
    ``docker`` and ``max update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``stardustresearch/max-agent`` image bakes a ``docker``
        stamp into ``/opt/max`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $MAX_HOME.
    try:
    ...
    #    host install whose shared $MAX_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``max update``.
    try:
        method = (
            (get_max_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``max_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$MAX_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/max`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Max lives in a ``uv tool`` layout.

    ``uv tool install max-agent`` places the install at
    ``.../uv/tools/max-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``max-agent`` happens to be uv-tool-installed on the machine while
    the *active* Max is a regular pip/venv install, causing
    ``max update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/max-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade max-agent"
    if method == "docker":
        return "docker pull stardustresearch/max-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade max-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade max-agent"
        return "pip install --upgrade max-agent"
    return "max update"
...
# banner, the TUI/desktop session info panel, and ``max update``. NixOS
# stays fully supported (Tier 2) and must never hit this path.

PLATFORM_SUPPORT_DOCS_URL = "https://max-agent.stardustresearch.com/docs/getting-started/platform-support"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``max update`` / ``max update --check`` so the wording — and the
...
# Long-form text for ``max update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# max_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Max, it doesn't update
...
✗ ``max update`` doesn't apply inside the Docker container.

Max Agent runs as a published image (stardustresearch/max-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull stardustresearch/max-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate max-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm stardustresearch/max-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/stardustresearch/max-agent/tags
  • Your config and session history live under ``$MAX_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``max update`` inside Docker.
...
def format_managed_message(action: str = "modify this Max installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("MAX_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Max installation is managed by NixOS "
            f"(MAX_MANAGED={env_hint}).\n"
            "Edit services.max-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Max installation is managed by Homebrew "
            f"(MAX_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade max-agent"
        )

    return (
        f"Cannot {action}: this Max installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Max."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from MAX_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, max_bin
    or None if container mode is not active, we're already inside the
    container, or MAX_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("MAX_DEV") == "1":
        return None
    ...
    container_mode_file = get_max_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "max-agent")
    exec_user = info.get("exec_user", "max")
    max_bin = info.get("max_bin", "/data/current-package/bin/max")
...
# Re-export from max_constants — canonical definition lives there.
from max_constants import get_max_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_max_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_max_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_max_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the MAX_UID / MAX_GID env vars set by Docker deployments.

    Docker containers running Max commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level MAX_HOME once, but
    subdirectories created at runtime by ``ensure_max_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("MAX_UID", "").strip()
    gid_str = os.environ.get("MAX_GID", "").strip()
    try:
...
def _chown_to_max_uid(path) -> None:
    """Chown ``path`` to ``MAX_UID:MAX_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_max_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_max_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the max group can
    share state with the gateway service.

    The mode can be overridden via the MAX_HOME_MODE environment variable
    (e.g. MAX_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse MAX_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``MAX_UID``/``MAX_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("MAX_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_max_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Max runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("MAX_CONTAINER") or os.environ.get("MAX_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set MAX_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into MAX_HOME, upgrading legacy empty templates.
...
def ensure_max_home():
    """Ensure ~/.max directory structure exists with secure permissions.
    ...
    """
    home = get_max_home()
    # Named profiles must be created explicitly (e.g. ``max profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old MAX_HOME would resurrect an empty skeleton
...
def _ensure_max_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"MAX_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Max-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Max
        ...
        # identity slot (SOUL.md). Empty by default. The MAX_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Max adds a coding operating brief
    ...
    "terminal": {
        ...
        #             MAX_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force MAX_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Max auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Max sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Max
        ...
        # bundled Max image, which drops to the `max` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Max sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Max should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``max chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.max/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: max sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .hermes.md, or .cursorrules before Max applies
    ...
    # single read_file page exceeds these limits, Max truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Max' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); max =
                                      # Max' compression threshold triggers
    ...
    # See tools/kanban_tools.py and max_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``max dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``max kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``max kanban decompose`` and the kanban
        ...
        # ``max profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `max model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `max` (and `max chat`) launches by default:
        ...
        # REPL and `--tui` (or MAX_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `max --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `max -c` muscle memory.  Default off so existing
        # users aren't surprised.  MAX_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `max --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to MAX_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/max`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `max pets`. Disabled until
        ...
        "pet": {
            ...
            # get_max_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``MAX_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``MAX_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.stardustresearch.com).
        "oauth": {
        ...
        # (``MAX_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``MAX_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``MAX_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/max``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Max appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.max/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.max/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.max/cache/delegation/
    ...
    # response. If not, Max feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Max auto-pauses the goal and
    ...
    "moa": {
        ...
        # <max_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.max/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${MAX_SKILL_DIR} and ${MAX_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `max curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `max curator run --consolidate` overrides
        ...
        # normally restored on every `max update`, so pruning them only
        ...
        # skipped), snapshot ~/.max/skills/ into
        # ~/.max/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `max curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for max-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Max Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (MAX_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Max' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.max/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or MAX_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `max doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``max_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Max to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $MAX_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        "chronos": {
            ...
            "portal_url": "https://portal.stardustresearch.com",
        ...
        # Also overridable via MAX_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `max -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `max` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `max kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     max-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Max tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.max/logs/.
    ...
    # update model picker lists without shipping a max-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://max-agent.stardustresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `max model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal MAX_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.max/sessions/sessions.json entirely.
        ...
        # the MAX_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.max.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.max/.env,
        ...
        # behavior: files must live under the Max cache, under
        ...
        # user. Bridged to MAX_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Max
        # cache (~/.max/cache/{documents,images,audio,video,screenshots})
        ...
        # to MAX_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # MAX_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to MAX_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.max/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.max/sessions/session_{sid}.json`` on every turn
    ...
    # ``max update`` behaviour.
    "updates": {
        # Run a full ``max backup``-style zip of MAX_HOME before every
        # ``max update``.  Backups land in ``<MAX_HOME>/backups/`` and
        # can be restored with ``max import <path>``.  Off by default:
        # zipping a large MAX_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``max update
        ...
        "backup_keep": 5,
        # What `max update` does with uncommitted local changes to the
    ...
    # `max tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.max/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.max/.env (or your shell) and never in config.yaml.
            ...
            # ~/.max/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.max/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Max disables it for our users unless they
        # explicitly opt in here. When false (default), Max sets
    ...
    # Max Desktop (Electron app) launch options. These only affect
    # `max desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the MAX_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Max mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'max model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. stardustresearch.com -> firecrawl-gateway.stardustresearch.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Max auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Max, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @max:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. MAX_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #max)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: max-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'max-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Max API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Max API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Max API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # MAX_TOOL_PROGRESS and MAX_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `max update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Max' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Max versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``max_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'max setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Max won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'max doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("MAX_HOME", "~/.max")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "max update / max -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("MAX_TOOL_PROGRESS")
            old_mode = get_env_value("MAX_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from MAX_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from MAX_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("MAX_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from MAX_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$MAX_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_max_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `max plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `max curator status` has no
    ...
    #   3. Creates `~/.max/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_max_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_max_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `max config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `max config set`"
                    )
    ...
    # behavior set it to true explicitly via `max config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from max_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: max config set <key> <value>")
    ...
    # `max update`, but only the version bump is persisted.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_max_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``max config set``
    ...
    backends) — while display paths (``max status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _max_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.max/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.max/config.yaml.
    ...
    (which change ``MAX_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/max/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_max_dotenv()
...
#   openai-codex (OAuth — max auth) — OpenAI Codex
#   nous         (OAuth — max auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.max/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Max writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.max/.env.
    ...
    interactive menu render (`max tools`, `max setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `max tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Max env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.max/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.max/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.max/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.max/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Max — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Max vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.max/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.max/.env`` over ``os.environ``.

    Used for Max-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Max Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Warn on stale MAX_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("MAX_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale MAX_MAX_ITERATIONS={_env_ghost} "
                f"(run 'max doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  max config edit     # Edit config file", Colors.DIM))
    print(color("  max config set <key> <value>", Colors.DIM))
    print(color("  max setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `max config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `max config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: max config set <key> <value>")
            print()
            print("Examples:")
            print("  max config set model anthropic/claude-sonnet-4")
            print("  max config set terminal.backend docker")
            print("  max config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'max config migrate' to add them")
        ...
        print("  max config           Show current configuration")
        print("  max config edit      Open config in editor")
        print("  max config set <key> <value>   Set a config value")
        print("  max config check     Check for missing/outdated config")
        print("  max config migrate   Update config with new options")
        print("  max config path      Show config file path")
        print("  max config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``max config`` UI so users
