"""
Max Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``max desktop`` / ``max gui``)
     Built inside the agent checkout under ``$MAX_HOME/max-agent/``:
       ...
       - ``$MAX_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Max.app`` or ``~/Applications/Max.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Max`` (NSIS per-user)
...
the app name ("Max"), separate from ``$MAX_HOME``:
  - macOS:   ``~/Library/Application Support/Max``
  - Windows: ``%APPDATA%\\Max``
  - Linux:   ``$XDG_CONFIG_HOME/Max`` (default ``~/.config/Max``)
...
``max uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Max"
...
def source_built_gui_artifacts(max_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``max desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``max-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(max_home)
    ...
    return [
        ...
        max_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Max" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        ...
        paths += [
            data_base / "applications" / "max.desktop",
            data_base / "applications" / "Max.desktop",
        ]
    return paths


def agent_is_installed(max_home: Path) -> bool:
    """Return True when a usable Python agent install exists under MAX_HOME.
...
def gui_is_installed(max_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(max_home):
        if p.exists():
            return True
...
def gui_install_summary(max_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = max_home if max_home is not None else get_max_home()
...
def uninstall_gui(max_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``max-agent/max_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$MAX_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove max' or "
            "'sudo dnf remove max'). AppImage builds are a single file you "
