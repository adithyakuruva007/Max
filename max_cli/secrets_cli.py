...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        "  Status:  [cyan]max secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]max secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]max secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]max secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]max[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]max secrets bitwarden setup[/cyan] and pick EU or "
