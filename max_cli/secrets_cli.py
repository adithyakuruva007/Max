"""CLI handlers for ``max secrets bitwarden ...``.
...
# that process is ``max update``, the self-lock preflight detects the
# loaded native module and defers (#86781).  Keep the backend import lazy:
# this module is registered parse-time from ``max_cli.main`` and must not
...
def __getattr__(name: str):
    ...
    ``max_cli.secrets_cli.bw`` directly.  Resolving that attribute at
    ...
    the first actual attribute access, so ``import max_cli.secrets_cli``
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Max process."
    )
    console.print(
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Max will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `max secrets bitwarden setup` to pick a "
    ...
    console.print(
        ...
        "Takes effect on the next Max invocation."
    )
    if not bw_cfg.get("enabled"):
        console.print(
            "[yellow]Note: the Bitwarden integration is currently disabled — "
            "run `max secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`max secrets bitwarden setup` first.[/yellow]"
        )
        return 1
