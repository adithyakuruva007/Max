"""Shell completion script generation for max CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_max_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Max Agent bash completion
# Add to ~/.bashrc:
#   eval "$(max completion bash)"

_max_profiles() {{
    local profiles_dir="$HOME/.max/profiles"
...
_max_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_max_profiles)" -- "$cur"))
        return
    fi
...
complete -F _max_completion max
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef max
# Max Agent zsh completion
# Add to ~/.zshrc:
...
_max_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.max/profiles" ]]; then
        profiles+=($HOME/.max/profiles/*(N/:t))
    fi
...
_max_complete() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_max_profiles' \
        '1:command:->commands' \
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Max Agent fish completion",
        "# Add to your config:",
        "#   max completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __max_profiles",
        "    echo default",
        "    if test -d $HOME/.max/profiles",
        "        for d in $HOME/.max/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c max -f -s p -l profile"
        " -d 'Profile name' -xa '(__max_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__max_profiles)' -d 'Profile name'"
                )
