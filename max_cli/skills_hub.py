#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Max Skills Hub.

Powers both:
  - `max skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # skill: `max skills install impeccable` should resolve to the
        ...
        # so users can copy it for `max skills install`.
...
def _resolve_source_meta_and_bundle(identifier: str, sources):
    ...
    skill is how ``max skills inspect owner/repo/skills/foo`` showed
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.max/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.max/skills/<name>/)[/]"
        )
    from max_cli.cli_output import line_input
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    c.print(table)
    c.print("[dim]Use: max skills inspect <identifier> to preview, "
            "max skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} official optional skill(s) from Stardust Research[/]")
    c.print()
    ...
    # The identifier is what you pass to `max skills install`. Browse used
    ...
    c.print("[dim]Tip: 'max skills inspect <identifier>' to preview, "
            "'max skills install <identifier>' to install, "
            "'max skills search <query>' to search deeper[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]max skills install {url} --name <your-name>[/]\n\n"
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                "[bold bright_cyan]This is an official optional skill maintained by Stardust Research.[/]\n\n"
                "It ships with max-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_max_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    # metadata.max.blueprint block, it is a runnable automation. Register it as
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="max skills install <id> to install"))
...
def do_list(source_filter: str = "all",
    ...
    config — ``max -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``MAX_HOME`` at process
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    already get from ``max update`` (ported from
    ...
    if skipped_local:
        ...
        c.print("[dim]Overwrite with: max skills update <name> --force[/]\n")
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `max update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `max update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   max skills diff <name>[/]")
    c.print("[dim]Resume updates: max skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: max skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: max skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: max skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: max skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: max skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_max_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Max Skills Hub.\n\n"
                        f"This skill was scanned by the Max Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `max skills <subcommand>` — called from max_cli/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: max skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: max skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: max skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'max skills <command> --help' for details.\n")
