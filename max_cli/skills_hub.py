...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "max-index".
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "max-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"max-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying max-index AND the external APIs at once
