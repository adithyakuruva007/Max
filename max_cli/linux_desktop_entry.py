...
``max desktop`` builds and launches the Electron app. On Linux, a
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``max desktop``.

    Prefer the real ``max`` executable (argv[0] or PATH). When Max
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from max_cli.relaunch import resolve_hermes_bin
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `max` script with
            ...
            # sys.executable is the interpreter actually running Max (the
            # venv one), so prefix it explicitly.
            argv = [str(Path(sys.executable).resolve()), str(resolved), "desktop"]
        else:
            argv = [str(resolved), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "max_cli.main", "desktop"]
    return " ".join(_quote_exec_arg(a) for a in argv)


def _needs_interpreter(bin_path: Path) -> bool:
    """Whether ``bin_path`` is a Python script that must run under
    ``sys.executable`` to see Max' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Max\n"
        "GenericName=Max Desktop\n"
        "Comment=Launch Max Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Max desktop entry. Return its path.
