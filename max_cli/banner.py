...
Pure display functions with no MaxCLI state dependency.
"""
import json
import logging
import os
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `max ... > file`) raises
...
from max_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built max — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/max-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/stardustresearch/max-agent"
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/stardustresearch/max-agent/"
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `max update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a Max update is available.

    Two paths: if ``MAX_REVISION`` is set (nix builds embed it), compare
    ...
    embedded_rev = os.environ.get("MAX_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # MAX_REVISION (that's nix-only). Returning None makes both the Rich
    ...
    try:
        from max_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $MAX_HOME/max-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hermes_home / "max-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Max git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$MAX_HOME/max-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``max_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from max_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/max-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Max checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/max-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Max Agent v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from max_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
...
def _banner_snapshot_path() -> Path:
    return get_max_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_max_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else MAX_CADUCEUS
    except Exception:
        _bskin = None
        _hero = MAX_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Stardust Research[/]")
    else:
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or max setup[/]"
            )
        else:
            ...
            ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
            left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Stardust Research[/]")

    if os.getenv("MAX_YOLO_MODE"):
    ...
    if not _has_native_mcp:
        try:
            from max_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from max_cli.codex_runtime_switch import get_current_runtime
        from max_cli.config import load_config as _load_cfg
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else MAX_AGENT_LOGO
        console.print(_logo)
        console.print()
