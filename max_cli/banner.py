...
_UPSTREAM_REPO_URL = "https://github.com/StardustResearch/max-agent.git"
...
def _fetch_pypi_latest(package: str = "max-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    cache_file = max_home / ".update_check"
    embedded_rev = os.environ.get("MAX_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # MAX_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `max update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/max/update/check`
...
_RELEASE_URL_BASE = "https://github.com/StardustResearch/max-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical StardustResearch/max-agent repo (forks don't get a link).
    """
    ...
    try:
        ...
        if behind is not None and behind != 0:
            ...
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built max; we know an update
