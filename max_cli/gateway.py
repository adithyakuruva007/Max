    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"max_home={current_home_lc}" in command_lc
            )
        ...
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "max_home=" in command_lc
            and f"max_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def _sync_max_home_from_systemd_unit(system: bool) -> None:
    ...
    unit_home = env.get("MAX_HOME", "").strip()
    if not unit_home:
        return
...
def _profile_suffix() -> str:
    ...
    default = get_default_max_root().resolve()
    if home == default:
        return ""
...
def _profile_arg(max_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        max_home: Optional explicit MAX_HOME path. Defaults to the current
        ...
        default_root: Optional Max root to compare against. Used when
            ...
            process, where ``Path.home()`` and ``get_default_max_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from max_constants import get_default_max_root

    home = Path(max_home or str(get_max_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_max_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(max_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".max"
    try:
        Path(max_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(max_home, default_root=target_root)
    except ValueError:
        return _profile_arg(max_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this MAX_HOME.

    Default ``~/.max`` returns ``max-gateway`` (backward compatible).
    Profile ``~/.max/profiles/coder`` returns ``max-gateway-coder``.
...
# max-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (max-gateway-*.service) and unrelated third-party
# "max" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("max.service",)
...
def _find_legacy_max_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``max.service`` before the rename to
    ``max-gateway.service``). When both a legacy unit and the current
    ``max-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``max-gateway-coder.service`` and unrelated third-party
      ``max-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``max.service`` running an unrelated
...
def has_legacy_max_units() -> bool:
    """Return True when any legacy Max gateway unit files exist."""
    return bool(_find_legacy_max_units())
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("  These run alongside the current max-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    max gateway migrate-legacy")


def remove_legacy_max_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Max gateway unit files.

    Iterates over whatever ``_find_legacy_max_units()`` returns — which is
...
def _max_home_for_target_user(target_home_dir: str) -> str:
    ...
    root's home.  This translates it to the target user's equivalent path:
      ...
      /opt/custom-max               → /opt/custom-max  (kept as-is)
    """
    current_max_home = get_max_home().resolve()
    current_default = (Path.home() / ".max").resolve()
    target_default = Path(target_home_dir) / ".max"

    # Default ~/.max → remap to target user's default
    if current_max_home == current_default:
        return str(target_default)

    # Profile or subdir of ~/.max → preserve the relative structure
    try:
        relative = current_max_home.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.max) — keep as-is
        return str(current_max_home)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    max_node = max_home / "node" / "bin"
    if _is_dir(max_node):
        candidates.append(str(max_node))
    max_nm = max_home / "node_modules" / ".bin"
    if _is_dir(max_nm):
        candidates.append(str(max_nm))
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        max_home = _max_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(max_home, home_dir)
        ...
        working_dir = str(max_home) if max_home else _remap_path_for_user(working_dir, home_dir)
...
"""
    ...
    max_home = str(get_max_home().resolve())
    profile_arg = _profile_arg(max_home)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    with ``MAX_HOME=/tmp/max-e2e-<pr>`` exported; the restart path's
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # generated unit bakes that ``/tmp/pytest-of-.../max_test`` path into
    ...
    if not system and (
        ...
        or '/max_test"' in new_unit
        or "/max_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # MAX_HOME into the unit (manual E2E homes like /tmp/max-e2e-NNN
...
def systemd_install(
    ...
    # Offer to remove legacy units (max.service from pre-rename installs)
    # before installing the new max-gateway.service. If both remain, they
...
def systemd_stop(system: bool = False):
    ...
    _sync_max_home_from_systemd_unit(system=system)
    try:
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # can tail to detect a persistent orphan. See max-restart
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    try:
        from max_constants import get_default_max_root
        default_root = get_default_max_root()
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        "  The image entrypoint normally drops privileges to the 'max' user. "
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "            2. Give it a username (e.g. max) and copy the bot token",
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    #
    # Prefer `sleep infinity` (matches the static main-max service's
    # pattern in docker/s6-rc.d/main-max/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        # pre-rename installs (e.g. max.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_max_units(interactive=not yes, dry_run=dry_run)
