"""
Gateway subcommand for max CLI.

Handles: max gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_max_home is imported lazily at call sites to avoid ImportError
# when max_constants is cached from a pre-update version during `max update`.
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``ai.max.gateway*`` LaunchAgent — the update path needs the whole
    ...
    if is_macos():
        ...
        if all_profiles:
            ...
            # ``launchctl list`` prefix scan also catches ai.max.gateway*
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "max CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `max update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``max gateway status`` from falsely counting the ``max`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``max gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_max_home().resolve())
    # Forward slashes on both sides of the MAX_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. MAX_HOME
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``max update`` hung exactly there
            ...
            # watchdog probe.
            from max_cli._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``max update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Max profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    *,
    ...
    """Return running gateway PIDs mapped to Max profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def find_windows_gateway_services(
    *,
    ...
    validated profile gateway PID comes from Max's own PID file, and its
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "max_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m max_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``max update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Max's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose MAX_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``max update`` users
    # with no running gateway until they re-invoke ``max gateway``
    ...
    from max_cli._subprocess_compat import (
    ...
    # interpreter and captures the stable cwd + env overlay (MAX_HOME,
    ...
    if sys.platform == "win32":
        try:
            from max_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / MAX_HOME) the windowless
        # base interpreter needs to import max_cli.  Empty on POSIX, where
...
def _hermes_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``MAX_HOME`` from the on-disk unit file (not ``systemctl show``).
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``MAX_HOME``.

    Under ``sudo``, ``MAX_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_max_home` falls back to ``/root/.max`` — the wrong
    profile. The unit file pins ``MAX_HOME`` for the actual gateway
    ...
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("MAX_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("MAX_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["MAX_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}max gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}max gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from max_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: max gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `max gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``max gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from max_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (max_cli/gateway.py restart
    ...
    if is_windows():
        try:
            # The install-time task name is profile-aware (Max_Gateway /
            # Max_Gateway_<profile>) — never hardcode it, or the guard is
            # dormant on every standard `max gateway install` deployment.
            from max_cli.gateway_windows import get_task_name

            _task_name = get_task_name()
        except Exception:
            _task_name = "Max_Gateway"
    ...
    try:
        ...
        # whole ai.max.gateway* fleet — not just the current profile's
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (MAX_HOME-scoped).
...
from max_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Max Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``max gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``MAX_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("MAX_GATEWAY_DETACHED", "").strip().lower()
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from max_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
SERVICE_DESCRIPTION = "Max Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current MAX_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (MAX_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from max_constants import get_default_hermes_root

    home = get_max_home().resolve()
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary MAX_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when MAX_HOME is a named profile.

    For ``~/.max/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit MAX_HOME path. Defaults to the current
            ``get_max_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Max root to compare against. Used when
    ...
    home = Path(hermes_home or str(get_max_home())).resolve()
...
def _profile_arg_for_target_user(hermes_home: str, target_home_dir: str) -> str:
    ...
    target_root = Path(target_home_dir) / ".max"
    try:
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this MAX_HOME.

    Default ``~/.max`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.max/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other MAX_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Max installs that predate the
...
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "max_cli.main gateway",
    "max_cli/main.py gateway",
    "gateway/run.py",
    " max gateway ",
    "/max gateway ",
)
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Max gateway units.

    Detects unit files installed by older Max versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Max gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Max gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Max gateway unit(s) detected from an older install:")
    ...
    print_info("    max gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    """Stop, disable, and remove legacy Max gateway unit files.
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Max gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Max gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: max gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo max gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    max gateway uninstall")
    print_info("    sudo max gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: max gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``max setup`` (end of wizard)
    and ``max import``: if this host supports a service manager and no
    ...
    ``max gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   max gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: max gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: max gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Max often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.max`` → ``ai.max.gateway.plist`` (backward compatible).
    Profile ``~/.max/profiles/coder`` → ``ai.max.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.max.gateway-{suffix}" if suffix else "ai.max.gateway"
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    ``MAX_HOME`` (tests, capture sandboxes, side-by-side installs) must
    ...
    for profile in list_profiles():
        if profile.is_default:
            root_label.append("ai.max.gateway")
        elif _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", profile.name):
            profile_labels.append(f"ai.max.gateway-{profile.name}")
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # max_constants cached from before this symbol existed. See
            # _reload_max_constants() in max_cli/managed_uv.py.
            from max_cli.managed_uv import _reload_max_constants

            venv_python_path = _reload_max_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.max/max-agent  -> /home/alice/.max/max-agent
      /opt/max                 -> /opt/max  (kept as-is)
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current MAX_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_max_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.max                    → /home/alice/.max
      /root/.max/profiles/coder     → /home/alice/.max/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes_raw = os.environ.get("MAX_HOME", "").strip()
    current_hermes = (
    ...
    # different MAX_HOME into the generated service unit.
    current_default = Path.home() / ".max"
    target_default = Path(target_home_dir) / ".max"

    # Default ~/.max → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.max → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.max) — keep as-is
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m max_cli.main``, so module
    ...
    dir, or a clone that ``max update`` later relocates/removes — the path
    ...
    ``MAX_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if MAX_HOME
...
def _systemd_watchdog_seconds(hermes_home: str | Path | None = None) -> int:
    ...
    if hermes_home is not None:
        ...
        override_token = set_max_home_override(hermes_home)
        reset_home_override = reset_max_home_override
    try:
...
def _append_node_dir_for_service(
    ...
    The Max-managed Node under ``$MAX_HOME/node`` goes first when it
    ...
    *hermes_root* is the Max home the unit will run against. System units
    ...
    # target Max home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Max home is known — probing here would stat the calling
    ...
    if system:
        ...
        # Anchor cwd to the target user's MAX_HOME (stable, always exists)
...
Group={group_name}
ExecStart={python_path} -m max_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="MAX_HOME={hermes_home}"
Restart=always
RestartSec=5
...
"""
    ...
    hermes_home = str(get_max_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        hermes_home
    )
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m max_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="MAX_HOME={hermes_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned MAX_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo max gateway … --system``, MAX_HOME is often stripped
    # and falls back to ``/root/.max``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / MAX_HOME comparisons
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir MAX_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved MAX_HOME
    ...
    exported a throwaway ``MAX_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``max gateway restart``
    with ``MAX_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="MAX_HOME=..."`` lines and launchd
    ``<key>MAX_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'MAX_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>MAX_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp MAX_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: MAX_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported MAX_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single MAX_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only MAX_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp MAX_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="MAX_HOME=..."``. Writing that to the developer's
    ...
    # MAX_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Max install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo max gateway uninstall --system")
    print_info("         max gateway install")
    print_info("         max gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("MAX_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_cron_drain_timeout() -> float:
    ...
    env_raw = os.getenv("MAX_CRON_DRAIN_TIMEOUT")
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("MAX_RESTART_AFTER_TURN_TIMEOUT")
    if env_raw is not None and str(env_raw).strip() != "":
...
def systemd_install(
    ...
    # Existing system units already pin MAX_HOME; adopt it before any
    ...
    # ``sudo max gateway install --system --force`` would bake /root/.max
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}max gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}max gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}max gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # MAX_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `max gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `max gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}max gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}max gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}max gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}max gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.max.gateway-{suffix}" if suffix else "ai.max.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``max gateway start/stop/status`` call).
...
#      background process (the `nohup max gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_max_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_max_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m max_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active MAX_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "max_cli.main"]
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``max update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup max gateway run --replace`
    ...
    working.
    """
    from max_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_max_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup max gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `max gateway start`)
    ...
    nofile_block = ""
    try:
        from max_cli.resource_limits import configured_nofile_soft_limit
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.max/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See hermes-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_max_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Max install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  max gateway status             # Check status")
    from max_constants import display_max_home as _dhh
...
def launchd_stop():
    ...
    # `max gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `max update`
...
def launchd_status(deep: bool = False):
    ...
    # Max PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Max PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Max install")
    else:
        print("⚠ Service definition is stale relative to the current Max install")
        print("  Run: max gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: max gateway stop")
            else:
    ...
    if deep:
        log_file = get_max_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``MAX_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``max gateway run`` is always a misconfiguration, so we
    ...
    try:
        ...
        from max_cli.profiles import normalize_profile_name
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``max gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``max gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other MAX_HOME roots.
    """
    ...
    print("  Use 'max gateway restart' to replace it,")
    print("  or 'max gateway stop' first.")
    print("  Or use 'max gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("MAX_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Max gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/max/docker/entrypoint.sh before the Max command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$MAX_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set MAX_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `max gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set MAX_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = (
        os.getenv("MAX_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # /restart) rather than through `max gateway restart` which already
    ...
    # `max gateway start/restart` — leaving the gateway vulnerable to
    ...
    # MAX_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("MAX_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``MAX_GATEWAY_MAX_STARTS`` /
    # ``MAX_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("MAX_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("MAX_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``max gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Max delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Max connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: max pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Max will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``max setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run max under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.max/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_max_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    from max_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from max_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'max pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: max pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'max gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Max will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Max will store the returned account_id/token in ~/.max/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `max gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_max_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `max pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `max pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "MaxAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``max_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from max_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.max/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.max/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: max gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: max gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: max gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo max gateway install --system"
                        )
                    print_info("  Or run in foreground:  max gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: max gateway run")
                print_info(
                    "  For persistence:   tmux new -s max 'max gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: max gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup max gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'max gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`max_cli.service_manager` are caught and surfaced as
    ...
    if profile is None:
        ...
        # MAX_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``max gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``max gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``max gateway run`` are unaffected.
      2. ``MAX_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``max gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``MAX_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("MAX_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("MAX_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${MAX_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if MAX_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or MAX_GATEWAY_NO_SUPERVISE=1) to opt out\n"
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: max gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: max gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s max 'max gateway run'"
                )
                print()
            ...
            print()
            print(
                "  max gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s max 'max gateway run'         # persistent via tmux"
            )
            print(
                "  nohup max gateway run > ~/.max/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from max_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  max profile create <name>     # creates the s6 service slot")
                print("  max -p <name> gateway start   # bring it up via s6")
                print("  max status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: max gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: max gateway run")
            sys.exit(1)
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "Use `max gateway uninstall` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: max gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  max profile delete <name>     # tears down the s6 service slot")
                print("  max -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `max -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: max gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        # The supervised probe also PASSES a plain foreground `max gateway run`
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "Use `max gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `max gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: max gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  max gateway install")
                else:
                    print("To install as a service:")
                    print("  max gateway install")
                    print("  sudo max gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  max gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup max gateway run > ~/.max/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s max 'max gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  max gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  max gateway install  # Install as user service")
                    print(
                        "  sudo max gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Max gateway unit files from
