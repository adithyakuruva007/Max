"""Windows ConPTY bridge for the `max dashboard` chat tab.

Drop-in counterpart to ``max_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``max_cli.web_server`` consumes: ``spawn``, ``read``,
