"""
Skills configuration for Max Agent.
`max skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.max/config.yaml under:
...
from max_cli.config import cfg_get, load_config, save_config
from max_cli.colors import Colors, color
from max_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``max-agent``) are silently dropped from the
