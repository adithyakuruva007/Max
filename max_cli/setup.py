...
def _print_setup_summary(config: dict, max_home):
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {max_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('max', Colors.GREEN)}              Start chatting")
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        from tools.tool_backend_helpers import managed_nous_tools_enabled, normalize_modal_mode
...
def _setup_telegram_auto_result():
    ...
    try:
        profile_name = _profile_name_from_max_home(Path(get_max_home()))
    except Exception:
        pass
...
def setup_gateway(config: dict):
    ...
    if any_messaging:
        ...
        if supports_systemd and has_legacy_max_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: max gateway install --system")
                print_info("  Or run in foreground:  max gateway")
        else:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   max gateway              # Run in foreground")
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_max migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_max", _OPENCLAW_SCRIPT
    )
...
def _offer_openclaw_migration(max_home: Path) -> bool:
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, max_home)
            return
        ...
        migration_ran = _offer_openclaw_migration(max_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 1:
            _run_blank_slate_setup(config, max_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {max_home}")
    ...
    _print_setup_summary(config, max_home)


def _run_first_time_quick_setup(config: dict, max_home, is_existing: bool):
...
def _run_blank_slate_setup(config: dict, max_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `max update` runs don't re-inject them.
        try:
    ...
    _blank_slate_walkthrough(config, max_home)


def _blank_slate_walkthrough(config: dict, max_home):
...
def _run_quick_setup(config: dict, max_home):
