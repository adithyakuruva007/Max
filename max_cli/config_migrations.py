...
``max_cli.config.migrate_config``. Each step is a function
...
``max_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``max_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``max_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``max_cli.config`` (e.g.
``patch("max_cli.config.read_raw_config", ...)``) keep working, because
...
#: MAX_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_max_home()}/config.yaml and run `max setup` to "
...
def _cfg():
    """Return the live ``max_cli.config`` module (lazy, cycle-free)."""
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from max_cli.relay_plugin_cutover import legacy_relay_plugin_keys
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # provider surface (`video_generate`, `max tools` → Video Generation).
    ...
    if changed:
        ...
        if not quiet:
            print(
                ...
                "lists — video generation now lives under `max tools` → "
