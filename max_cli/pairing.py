...
Usage:
    max pairing list              # Show all pending + approved users
    max pairing approve <platform> <request-id|code>  # Approve a pairing request
    max pairing revoke <platform> <user_id> # Revoke user access
    max pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle max pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: max pairing {list|approve|revoke|clear-pending}")
        print("Run 'max pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: max pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.max/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'max pairing list' to see pending requests.\n")
