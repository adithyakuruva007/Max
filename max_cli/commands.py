...
from max_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``max_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``max_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("plan", "Write a markdown implementation plan to .max/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Max' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Max is working", "Configuration",
...
#     /max update on Slack. Demoted to free the native slot /approvals now
...
_SLACK_VIA_MAX_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform", "insights"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("hermes", "Talk to Max or run a subcommand", "[subcommand] [args]"))
    seen.add("hermes")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_VIA_MAX_ONLY:
            # Intentionally Slack-via-/hermes only (see _SLACK_VIA_MAX_ONLY).
            return
...
class SlashCommandCompleter(Completer):
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via max_cli.personality."""
        try:
