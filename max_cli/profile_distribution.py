"""Profile distributions — shareable, packaged Max profiles via git.

A distribution is a Max profile published as a git repository (or
...
* ``max profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``max skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``max profile``, not a parallel tree):

    max profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "max-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            max_requires=str(data.get("max_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.max_requires:
            out["max_requires"] = self.max_requires
        if self.author:
...
def check_max_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Max {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``max profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Max profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Max distribution."
        )

    # Version check up-front so we fail fast
    check_max_requires(manifest.max_requires, max_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.max).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="max_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `max profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`max profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="max_dist_update_") as tmp:
        plan = plan_install(
