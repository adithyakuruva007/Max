"""
Status command for max CLI.

Shows the status of all Max Agent components.
"""
...
from max_cli.auth import AuthError, resolve_provider
from max_cli.colors import Colors, color
from max_cli.config import get_env_path, get_env_value, get_max_home, load_config
from max_cli.models import provider_label
from max_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from max_cli.nous_subscription import get_nous_subscription_features
from max_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``max config``'s
...
def _estop_status_line():
    """One-line pause banner for `max status`, or None when not paused.

    Cheap: a single stat on $MAX_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `max resume` to lift)"


def show_status(args):
    """Show status of all Max Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Max Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `max status`
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: max model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: max auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: max auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: max model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from max_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'max-agent[vercel]')"
        ...
        # provider's doctor rows (fail-soft — never break `max status`).
        try:
    ...
    try:
        from max_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_max_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_max_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'max doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'max setup' to configure", Colors.DIM))
    print()
