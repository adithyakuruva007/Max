"""``max peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Max gateway (any machine: homelab, Spark, Max
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    max peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>
    max peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
    max peer dm spark/researcher "..."      # named profile (multiplexed peer)
...
``max -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.max/.env`` as
  ``MAX_PEER_<NAME>_KEY``.
...
def _peer_key_env(name: str) -> str:
    return f"MAX_PEER_{name.upper().replace('-', '_')}_KEY"
    ...
    try:
        ...
        if exc.code == 400 and "title" in detail.lower():
            ...
            raise RuntimeError(
                ...
                f"(HTTP 400: {detail}). Update the peer's max-agent, or unhide the "
...
def cmd_peer(args) -> int:
    ...
    if action in ("add", "set"):
        ...
        if key:
            ...
            print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.max/.env")
        else:
            print(
                f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
                f"  max peer add {name} --url {url} --key <key>\n"
                f"  (or add {_peer_key_env(name)}=<key> to ~/.max/.env)"
            )
        return 0
    ...
    if action in ("list", "ls", None):
        peers = _load_peers()
        if not peers:
            print("No peers registered. Add one: max peer add <name> --url http://host:port --key <API_SERVER_KEY>")
            return 0
    ...
    if action == "dm":
        ...
        if not isinstance(peer, dict) or not peer.get("url"):
            print(f"No peer named '{peer_name}'. Run: max peer list", file=sys.stderr)
            return 1
        key = _peer_secret(peer_name)
        if not key:
            print(
                f"No API key for peer '{peer_name}'. Set it: max peer add {peer_name} "
                f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.max/.env)",
                file=sys.stderr,
            )
            return 1
    ...
    print("Unknown peer action. See: max peer --help", file=sys.stderr)
    return 2
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Max gateways)",
        description=(
            "Register other Max gateways as peers and message their agents. "
            "'max peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'max -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.max/.env."
        ),
        epilog=(
            "Examples:\n"
            "  max peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  max peer list\n"
            '  max peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  max peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  max peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.max/.env)")
