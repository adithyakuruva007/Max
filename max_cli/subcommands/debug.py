...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Max Agent. Use 'max debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    max debug share              Upload debug report (asks for confirmation)
    max debug share --yes        Skip confirmation (for scripts/CI)
    max debug share --lines 500  Include more log lines
    max debug share --expire 30  Keep paste for 30 days
    max debug share --local      Print report locally (no upload)
    max debug share --no-redact  Disable upload-time secret redaction
    max debug share --nous       Upload to Nous-internal storage (private)
    max debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'max debug share'",
    )
