"""``max gateway`` and ``max proxy`` subcommand parsers.
...
from max_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if MAX_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy max.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Max gateway unit files "
            "(e.g. max.service) left over from older installs. Profile "
            "units (max-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.max/.env. "
             "Requires being logged in (max setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.max/.env and forwarded at provision. "
    ...
    proxy_start.add_argument(
        "--provider",
        default="xai",
        help="Upstream provider: xai (default: xai).",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
