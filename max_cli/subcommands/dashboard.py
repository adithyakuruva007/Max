"""``max dashboard`` / ``max serve`` subcommand parsers.
...
``max_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `max dashboard` / `max serve` cmdlines and SIGTERM them directly —
    # the same path `max update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Max web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Max web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Max Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Max desktop app shells (<= 0.15.x) spawn the
    # backend as `max dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Max couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Max Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Max backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Max backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports MAX_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `max dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.max/.env.
    # Nested subparser so bare `max dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "MAX_DASHBOARD_OAUTH_CLIENT_ID into ~/.max/.env, and prints "
            "how to engage the login gate. Requires being logged in (max setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "portal. Also settable via MAX_DASHBOARD_PORTAL_URL. Mainly for "
