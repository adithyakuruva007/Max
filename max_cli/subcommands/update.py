...
def build_update_parser(subparsers, *, cmd_update: Callable) -> None:
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Max Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        help=(
            ...
            "kind (git/docker/nix), every running Max service across all "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Force a FULL pre-update backup (quick state snapshot + MAX_HOME zip) for this run, regardless of updates.pre_update_backup",
    )
    update_parser.add_argument(
        "--yes",
        "-y",
        action="store_true",
        default=False,
        help="Run without blocking on prompts: accepts the config-migration and stash-restore prompts, skips the fork-upstream prompt without adding a remote. API-key entry is skipped; run 'max config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--keep-stash",
        action="store_true",
    ...
    update_parser.add_argument(
        ...
        help=(
            ...
            "If the local checkout is on a different branch, max will "
