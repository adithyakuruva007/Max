"""``max sync`` subcommand parser — Skill Sync.

Cloned from ``max_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    max sync status                 show gate/opt-in/head state
    max sync pull                   pull and materialize opted-in skills
    max sync push                   push opted-in skills
    max sync now                    reconcile: pull then push
    max sync enable <skill>         opt a skill into sync
    max sync disable <skill>        opt a skill out of sync
    max sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    max sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  max sync status            what is synced, and from where\n"
            "  max sync enable my-skill   include a skill in your sync\n"
            "  max sync now               pull, then push\n"
            "  max sync propose my-skill  share a skill with your team\n"
        ),
