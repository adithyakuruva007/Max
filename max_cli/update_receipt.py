...
1. **Update receipt** — a machine-readable JSON record of what one
   ``max update`` run discovered, did, skipped (and why), written to
   ``<MAX_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``max_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``max update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``max update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_max_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``max update`` has many early-termination paths (Windows
