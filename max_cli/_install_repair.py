...
- ``max_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``max_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``max_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``max_cli.main`` imports
``managed_uv``, ``max_constants``, and friends only in its late path; the
...
:func:`max_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from max_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (max_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``max acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    # max_constants is stdlib-only, so the canonical layout helpers are safe
...
def ensure_windows_bin_launchers(
    ...
    *,
    ...
    """Re-stage the Windows ``max`` launchers when they vanish.

    On Windows, ``max`` resolves through launchers derived from the venv
    ...
    the managed binary dir — the default Max root's ``bin``
    ...
    a per-machine dir shared by every profile: ``get_max_home()`` would
    point inside ``profiles\\<name>`` under ``max -p``, so the anchor
    here is :func:`max_constants.get_default_hermes_root`.

    Earlier installer versions staged them at ``<checkout>\\bin`` instead —
    inside the git working tree — where ``max update``'s pre-update
    ...
    nothing restored them and ``max`` stopped resolving in every new
    ...
    *windows* and *user_path_entries* are injectable for tests, same pattern
    ...
    # Per-machine anchor: the DEFAULT Max root, not get_max_home() —
    # under ``max -p <name>`` that returns ``profiles\\<name>``, which
    ...
    # every max_cli.main process start (right after the profile
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                "  ✓ Restored max launcher(s): " + ", ".join(restored),
                file=sys.stderr,
            )
    return restored
...
def _read_user_path_raw() -> tuple[list[str], int]:
    ...
    (same discipline as ``max_cli.uninstall``). Only called on Windows.
    """
    import winreg
...
def migrate_windows_bin_path(
    ...
    *,
    ...
    """One-time PATH migration to the ``MAX_HOME\\bin`` launcher layout.

    Runs from the ``max update`` tail (and mirrors what install.ps1's
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    from max_constants import get_default_hermes_root, venv_bin_dir
    ...
    if kept != entries:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                f"  ✓ max launchers now resolve from {home_bin} "
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    ``max acp`` speaks JSON-RPC on stdout.
    """
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    try:
        ...
        # the quarantined shims renamed aside and `max` gone from PATH
