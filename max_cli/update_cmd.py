"""Max update pipeline — extracted from ``max_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``max_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``max_cli.main`` reference — so existing call sites and test monkeypatches
that target ``max_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``max_cli.main``.
...
Imports are one-way: ``max_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``max_cli.main`` reference.

    Lets callers keep patching ``max_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``max_cli.main`` -> ``max_cli.update_cmd`` stays one-way
    at import time.
    """
    from max_cli import main
...
def _purge_stale_hermes_modules() -> None:
    """Evict every cached Max module after the checkout changed in-place.

    ``max update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from max_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``max_cli.gateway`` does
    ``from max_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Max
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Max module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Max modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``max update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``max update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``max_cli.config_defaults``,
    ``max_cli.config``, and ``max_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``max_cli._subprocess_compat`` and
    ``max_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "max_cli.config_defaults",
        "max_cli.config",
        "max_cli.config_migrations",
        "max_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from max_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from max_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``max update`` historically
    ...
    caller): scope config reads/writes via the context-local MAX_HOME
    ...
    try:
        ...
        from max_cli.profiles import _get_profiles_root, _PROFILE_ID_RE
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_max_home_override(entry)
            try:
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'max config migrate' to check manually.")
        return
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'max config migrate' manually to configure."
                )
                response = "n"
    ...
    # context-local MAX_HOME override (never os.environ — other
    ...
    try:
        from max_cli.backup import restore_cron_jobs_all_profiles
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    Skipping is safe because Max pins its editable finder to a *static*
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``max update`` can be driven by a
    ...
    """
    from max_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from max_constants so this can't drift
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `max update`.
    ...
    print("  Run when convenient:  max sessions optimize-storage")
    print(
...
def _reload_process_scan_modules() -> None:
    ...
    ``from max_cli._subprocess_compat import bounded_probe_run``. If the
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``max_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `max update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``max_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``max update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && max update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("MAX_ACTION_ID", "")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Max launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["hermes.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Max")
    print("  Desktop, a gateway, or another max REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `max` command to finish it automatically.")
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.max-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".max-update-staging", ".max-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `max update`.")
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second Max process the updater's drain
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from max_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (max gateway stop), "
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    zip_url = (
        f"https://github.com/NousResearch/max-agent/archive/refs/heads/{branch}.zip"
    )
    ...
    try:
        ...
        try:
            for item in entries:
                ...
                # Max.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `max update` to retry; if the agent won't start, "
            "reinstall from https://max-agent.stardustresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    if not import_ok:
        ...
        print("  Re-run `max update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        from max_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_max_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_max_home())
    ...
    try:
        from max_cli.update_receipt import finalize_update_receipt
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``max update --keep-stash`` (the desktop updater's mode): the
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/max-agent.git",
    "git@github.com:NousResearch/max-agent.git",
    "https://github.com/NousResearch/max-agent",
    "git@github.com:NousResearch/max-agent",
}

OFFICIAL_REPO_URL = "https://github.com/NousResearch/max-agent.git"
...
def _sync_with_upstream_if_needed(
    ...
    *,
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NousResearch/max-agent.")
        print()
        ...
        if assume_yes or (
            ...
            print(
                "  Add it later with: git remote add upstream https://github.com/NousResearch/max-agent.git"
            )
            return False
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/max-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/max-agent.git' to add later."
            )
...
# ``fleet_restart_pending`` lives under MAX_HOME (not next to the venv).
...
def _fleet_restart_pending_marker_path() -> Path:
    """MAX_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    return get_max_home() / _FLEET_RESTART_PENDING_NAME
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `max update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `max update` or `max gateway restart`.",
            file=stream,
        )
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``max update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: max gateway restart")
    sys.exit(1)
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from max_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``max update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a Max Desktop backend child, an unrelated ``max`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``max tools``."""
    try:
        from max_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Max Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``max tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Max Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Max Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `max update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `max update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Max' editable-install extras or
    ...
    try:
        from max_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `max update` is exactly the step
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    max update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `max update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from max_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `max update`.")
            failed = []
            if any(
    ...
    # Max profile using this checkout. Keep one per-checkout cache under the
    # shared Max root rather than rerunning npm once per named profile.
    ...
    # `max update` case. Synchronous and can block ~11s on a true cold
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `max update`.")
        return list(labels)
    ...
    # `max update` comes from the *desktop* build, not this step; that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    from max_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from max_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    # /usr/local/bin/hermes, code at /usr/local/lib/max-agent).
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``max-acp`` launcher next to the ``max`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``max update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``max-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Max as not installed even when it is.

    The shim simply delegates to the sibling ``max`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``max`` / ``max-acp``
    launchers into the managed binary dir ``$MaxHome\bin`` and puts THAT
    ...
    ``max_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``max-acp`` is already present next to the
    ``max`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        ...
        try:
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Max Agent — ACP launcher (written by `max update`).\n"
            ...
            continue
        print(f"  ✓ Installed max-acp launcher → {acp_cmd}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of MAX_HOME under
      ``backups/`` (restorable via ``max import``; the #48200 wrong-path
    ...
    try:
        ...
        # NOTE: this function later does `from max_constants import
        # get_max_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from max_cli.config import get_max_home as _get_home
        ...
        try:
            from max_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from max_cli.update_receipt import record_step
    ...
    # Human-readable size
    from max_cli.sizefmt import format_bytes
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".max-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    for proc in proc_iter:
        ...
        # the `-m max_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``max_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``hermes.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``max`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `max` launch will complete it")
...
def _hermes_holder_subcommand(cmdline: str) -> str | None:
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("max_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``max dashboard`` must not be labeled as the
    ...
    hint_by_subcommand = {
        "serve": "  ← Max backend (if the Desktop app is open, close it)",
        "dashboard": "  ← max dashboard (stop it: max dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``max update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from max_cli._scan_venv_blockers import _is_pausable_gateway
...
def _ledger_manual_serve_holders(
    ...
    Max spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from max_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning max-setup, and the update-in-progress marker
    ...
    the update with "Max is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Max backend (``max_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.max-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "max_cli.main" in argv_low and (
...
def _ledger_reapable_backend_pids(
    ...
    (``max_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Max ``serve``/``dashboard`` backends safe to reap during a
    ...
    - Only Max backends (``max_cli.main`` + ``serve``/``dashboard``)
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.max-runtime`` interpreter child alive and holding the install open
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``max serve`` / ``max dashboard`` argv.
    ...
    """
    if "max_cli.main" not in (cmdline or "").lower():
        return False
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from max_cli.gateway import is_macos
    ...
    print("    max gateway status")
    if any(not name.startswith("ai.max.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.max.") for name in ordered):
...
def _restart_launchd_gateway_after_update(
    *, supervision_verify: bool = True
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: max gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: max gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.max.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``max_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``max_cli.gateway`` in the failing
...
def _recover_gateway_restart_after_abort(
    ...
    ``max update`` normally performs the fleet restart in the interpreter
    ...
    command = [
        sys.executable,
        "-m",
        "max_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
    env["MAX_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_MAX_GATEWAY", "MAX_GATEWAY", "MAX_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``max_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``max update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``max-setup.exe``) executes
    ``$MAX_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_max_home()) / "bootstrap-cache"
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``max update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `max desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "max_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Max-managed Node on PATH: when
    # `max update` runs inside the desktop updater chain (Desktop →
    # max-setup → max update), the shell PATH customizations are lost,
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `max desktop` to retry)")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from max_cli.update_receipt import begin_update_receipt
    ...
    # running Max runtime, its supervisor, and its running code version —
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Max backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Max backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .max-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Max is still
        ...
        if _venv_holders:
            ...
            # `max serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Max backends are leaks,
            ...
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Max backend process(es) "
    ...
    # process by design (a CLI `max update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Max gateways, Desktop backends, or other "
        ...
        # no error, no warning, ``max doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && max update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``max update`` can catch up even when git is already up to date.
        ...
        # marker survives and the next ``max`` launch finishes the install
        ...
        # symbols in max_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `max` again "
        ...
        if not import_ok:
            ...
            print("    Run `max update` again — if it persists, reinstall:")
            print("    https://max-agent.stardustresearch.com")
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if Max re-prompts for permissions you already "
                "granted (toggle shows ON), the stored grant is stale — run "
                "`tccutil reset ScreenCapture com.stardustresearch.max` (repeat "
        ...
        try:
            from max_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_max_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # Self-heal the max-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Max on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("max-acp launcher self-heal failed: %s", e)

        # Migrate the Windows max launchers to the managed binary dir
        # (the default Max root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (max-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `max`
        ...
        # the managed clone under the default Max root).
        try:
            from max_cli._install_repair import migrate_windows_bin_path
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `max update`
                # must stay fast; `max computer-use install --upgrade`
        ...
        #
        # Purge stale cached Max modules FIRST: the import below pulls
        ...
        try:
            ...
            try:
                from max_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Restart EVERY ai.max.gateway* LaunchAgent, not only the
            ...
            try:
                from max_cli.update_receipt import record_gateway_restart
        ...
        try:
            from max_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import max_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``max update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from max_cli.main import main; main()" update --yes'
                    )
            try:
                from max_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``max_cli.gateway`` no longer imports,
