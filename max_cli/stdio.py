...
on Japanese locales, etc.).  Max's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``max_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Max is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``MAX_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("MAX_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Max-managed Git install directories so
    ...
    # already-running shells don't see the change — which means max
    ...
    # install without Max-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Max's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Max's
    ``max config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Max (User env var in Windows Settings, or
    export in a PowerShell profile) and Max picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Max-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\max\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``max`` from right after install) retain their old PATH.

    Any subprocess Max spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Max-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "max", "git", "cmd"),
        os.path.join(local_appdata, "max", "git", "bin"),
        os.path.join(local_appdata, "max", "git", "usr", "bin"),
        # Max venv Scripts directory — host of the max.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes max, but harmless to include.
        os.path.join(local_appdata, "max", "max-agent", "venv", "Scripts"),
