"""Offline, non-destructive recovery for a damaged Max session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``max_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``max_state._backup_db_file``.
    """
    from max_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Max process using this profile and retry. "
                "This includes the interactive `max` CLI session this "
                ...
                "Max process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `max` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from max_cli.session_lost_and_found import (
