"""Cross-process mutual exclusion for in-flight Max updates.

Three different surfaces can start an update of the same install tree:

* ``max update`` from a terminal,
...
* the desktop's Update button, which hands off to the Tauri
  ``max-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``max update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <MAX_HOME>/.max-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``max update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Max is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``max-setup`` binary under ``~/.max`` is only refreshed by a full
  ...
  ancestry check those users get exit 2 ("Max is still running") on every
  GUI update forever, with no Max process actually running.
"""
...
MARKER_NAME = ".max-update-in-progress"

# Set by an orchestrating updater (the Tauri `max-setup --update` flow) to
# its own pid before spawning `max update` as a child stage. The parent
...
HANDOFF_PID_ENV = "MAX_UPDATE_HANDOFF_PID"
...
# to show "Max is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Max home (never the context-local profile override):
    the Rust updater resolves ``$MAX_HOME`` or the platform default, and the
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``max update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``max-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Max update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `max update` as a stage): we run under ITS claim
