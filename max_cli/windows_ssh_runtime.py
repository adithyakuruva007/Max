...
def _root() -> Path:
    ...
    # helper's token, including custom MAX_HOME layouts.
...
def process_state(pid: int, creation_time_ns: int, hermes_path: str, spawn_nonce: str) -> dict[str, Any]:
    ...
    # argv[0] is either the max exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs max_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "max_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "max_cli.main" in argv[2])
    )
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves max_cli's own location and prepends its parent, because the
    ...
    # the max_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the max_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not os.path.isabs(configured_path):
        raise ValueError("Max path must be absolute")
    ...
    if not os.path.isfile(python_entry):
        raise ValueError("Max Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('max_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_hermes(hermes_path: str) -> dict[str, Any]:
    ...
    if not os.path.isabs(hermes_path) or not os.path.isfile(path):
        raise ValueError("Max path is not an executable file")
