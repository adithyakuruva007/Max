"""max webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    max webhook subscribe <name> [options]
    max webhook list
    max webhook remove <name>
    max webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.max/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _max_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: max gateway run
"""
...
def webhook_command(args):
    """Entry point for 'max webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: max webhook {subscribe|list|remove|test}")
        print("Run 'max webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (max gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: max webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from max webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (max gateway run)")
