...
Pure logic: walks a Max config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `max doctor` and a future `max migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Max config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Max config and return retirement issues.
...
def apply_migration(
    ...
    from max_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `max migrate xai` reports nothing to migrate
