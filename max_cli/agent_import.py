"""max import-agent — import Claude Code / Codex CLI setups into Max.

Usage:
    max import-agent                       # auto-detect ~/.claude or ~/.codex
    max import-agent claude-code           # import from ~/.claude
    max import-agent codex                 # import from ~/.codex
    max import-agent claude-code --dry-run # preview only, no changes
    max import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``max claw migrate`` /
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in MAX_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → MAX_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in MAX_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in MAX_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → MAX_HOME/skills/codex-imports/<name>/
...
deliberately via ``max setup`` or config.yaml.
"""
...
# Same entry delimiter as the Max memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`max config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Max command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Max glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Max equivalent — "
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → MAX_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``max import-agent`` (invoked from max_cli.main)."""
    from max_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: max import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: max import-agent claude-code   or   max import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Max — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: max import-agent "
    ...
    print_info("Secrets:     never imported — run 'max setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: max import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'max setup' "
               "to configure providers, or add them to ~/.max/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from max_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'max setup' or ~/.max/.env.")
        print()
