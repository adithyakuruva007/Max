"""
Backup and import commands for max CLI.

`max backup` creates a zip archive of the entire ~/.max/ directory
(excluding the max-agent repo and transient files).

`max import` restores from a backup zip, overlaying onto the current
MAX_HOME root.
"""
...
from max_constants import get_default_hermes_root, get_max_home, display_max_home
from utils import (
    _preserve_file_mode,
...
from max_cli.sizefmt import format_bytes as _format_size
...
# Where ``max backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``max-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/max-agent/``
...
# MAX_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "max-agent",     # the codebase repo — re-clone instead
    ...
    "browser-profile",
    # Python dependency trees (plugin / MCP-server venvs under MAX_HOME) —
...
# File names that ``max import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE MAX_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Max backup slot."""
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Max backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside MAX_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to max root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``max-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/max-agent/`` — must be preserved.
        if part == "max-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside MAX_HOME.
...
# to the user as a hung `max update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # max_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Max home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Max home directory not found at {hermes_root}")
        sys.exit(1)
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    print(f"Scanning {display_max_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``max-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "max-agent" and not is_root)
        ]
    ...
    # outside MAX_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_max_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: max import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Max backup.
    ...
    # Look for telltale files that a max home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Max backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.max/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a max dir name
        if prefix in {".max", "hermes"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``max import``, which is the
    ...
    so without the chown a ``sudo max import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``max import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Max' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_max_home()}")
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Max configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under MAX_HOME.
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_max_home()}."
            )
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # max_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: max profile list  (after installing hermes)")

        # Guidance
        print()
        if not (hermes_root / "max-agent").is_dir():
            print("Note: The max-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: max update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  max -p {pname} gateway install")
        ...
        try:
            from max_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your Max configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and max backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to MAX_HOME).
...
# — `max update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $MAX_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside MAX_HOME and the entry is silently
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: max snapshot list"
        )
        logger.error(
...
# Relative path of the cron job database inside MAX_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``max update``.
    ...
    Args:
        ...
        hermes_home: Override for the Max home directory (tests).
...
def create_pre_update_snapshots_all_profiles(
    ...
    home = invoking_home or get_max_home()
...
def run_quick_backup(args) -> None:
    """CLI entry point for max backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_max_home()}/state-snapshots/")
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `max claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of MAX_HOME under ``backups/`` before a
    ``max claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``max import <archive>``.  Writes to
    ``<MAX_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `max import` and the
