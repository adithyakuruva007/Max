...
from max_constants import get_default_max_root, get_max_home, display_max_home
...
def run_backup(args) -> None:
    ...
    max_root = get_default_max_root()

    if not max_root.is_dir():
        print(f"Error: Max home directory not found at {max_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"max-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"max-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(max_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(max_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(max_root)
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        has_config = (max_root / "config.yaml").exists()
        has_env = (max_root / ".env").exists()
        ...
        for member in members:
            ...
            try:
                target.resolve().relative_to(max_root.resolve())
            except ValueError:
        ...
        profiles_dir = max_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print("  Run: max profile list  (after installing max)")

        # Guidance
        print()
        if not (max_root / "max-agent").is_dir():
...
def _quick_snapshot_root(max_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(max_home)
    if not root.exists():
        return []
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        max_home: Override for the Max home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(max_home), keep=keep)
...
def _write_full_zip_backup(out_path: Path, max_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``max_root`` to ``out_path``.
...
def _pre_update_backup_dir(max_home: Optional[Path] = None) -> Path:
    home = max_home or get_max_home()
...
def create_pre_update_backup(
    ...
    """
    max_root = max_home or get_default_max_root()
    if not max_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(max_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, max_root)
    if result is None:
        return None
