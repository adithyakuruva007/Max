"""Per-terminal session breadcrumbs for ``max -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$MAX_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``max -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_max_home() / "terminal-sessions"
