"""
Max CLI - Unified command-line interface for Max Agent.

Provides subcommands for:
- max chat          - Interactive chat (same as ./hermes)
- max gateway       - Run gateway in foreground
- max gateway start - Start gateway service
...
def _ensure_utf8():
    ...
    can even start — e.g. `max setup` on a fresh Pi.
    ...
    max_cli/stdio.py::configure_windows_stdio() runs later from the entry
