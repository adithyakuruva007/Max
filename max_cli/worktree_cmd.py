"""``max worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``max_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    max worktree list             # audit: verdict + reason per tree
    max worktree prune            # reap safe trees + merged branches
    max worktree prune --dry-run  # show the plan, change nothing
    max worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `max worktree prune`."
        )
