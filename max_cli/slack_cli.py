"""``max slack ...`` CLI subcommands.

Today only ``max slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ max slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Max. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Max deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Max on Slack's older Assistant messaging
    ...
    """
    from max_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Max in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Max in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Max agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``max_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$MAX_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Max")
    ...
    """
    name = getattr(args, "name", None) or "Max"
    description = getattr(args, "description", None) or "Your Max agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "max slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"max slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "max slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "max slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Max app\n"
