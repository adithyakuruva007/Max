...
This module is imported by ``max_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no max_cli.config,
...
``max_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(hermes_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect MAX_HOME?"""
...
def _resolved_home() -> str:
    hermes_home = os.environ.get("MAX_HOME", "").strip()
    if hermes_home:
        return hermes_home
    return os.path.join(os.path.expanduser("~"), ".max")
...
def container_mode_may_be_active() -> bool:
    ...
    default_home = os.path.join(os.path.expanduser("~"), ".max")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect MAX_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``max --version`` output (also used by /version).
    ...
    ``max version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        from max_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``max --version`` before the heavy import wall.
    ...
    container. Termux keeps the MAX_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("MAX_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
