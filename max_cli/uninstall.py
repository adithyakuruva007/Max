"""
Max Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.max/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Max PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing max-agent or max PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Max Agent" comment and following line
                if '# Max Agent' in line or '# max-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('max' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing max
                if 'max' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the max wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "max",
        Path("/usr/local/bin/max"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains max_cli reference)
                content = wrapper.read_text()
                if 'max_cli' in content or 'max-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(max_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``max`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Max home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (max_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``max gateway run`` processes
    ...
    try:
        from max_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from max_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``MAX_HOME`` and ``MAX_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\max\git\cmd``, ``%LOCALAPPDATA%\max\git\bin``,
#      ``%LOCALAPPDATA%\max\git\usr\bin``, ``%LOCALAPPDATA%\max\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\max\git\`` and Node to
#      ``%LOCALAPPDATA%\max\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``max dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\max\gateway-service\`` and sometimes
...
def _max_path_markers(max_home: Path) -> list[str]:
    """Path-entry substrings that identify Max-owned User-PATH entries."""
    root = str(max_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare max-agent install dir.
    markers = [root + "\\max-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if MAX_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "max".  We don't want to catch
    # unrelated entries like "cmax-foo" or "ephermeral", so we look for
    # backslash-max as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(max_home: Path) -> list[str]:
    """Strip Max-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _max_path_markers(max_home)
            kept: list[str] = []
...
def remove_max_env_vars_windows() -> list[str]:
    """Delete MAX_HOME and MAX_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("MAX_HOME", "MAX_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(max_home: Path) -> list[Path]:
    """Delete PortableGit and Node installs the Windows installer created under
    ``%LOCALAPPDATA%\\max\\``.  Only called on full uninstall; they're
...
def _is_default_max_home(max_home: Path) -> bool:
    """Return True when ``max_home`` points at the default (non-profile) root."""
    try:
        from max_constants import get_default_max_root
        return max_home.resolve() == get_default_max_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its MAX_HOME directory.

    We shell out to ``max -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current MAX_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m max_cli.main` so we don't depend on a `max`
    #    wrapper that may be half-removed mid-uninstall.
    max_invocation = [_sys.executable, "-m", "max_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                max_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's MAX_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``max uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$MAX_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from max_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(max_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Max Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Max Chat GUI installation was found.")
        print(f"  Checked: {max_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Max agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(max_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Max agent at {max_home / 'max-agent'}")
        print(f"  • Your config, sessions, and secrets under {max_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Max agent is still installed. Run 'max' to use the CLI,")
    print("or 'max uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.max/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.max/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie MAX_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_max_home(max_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.max data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Max Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {max_home / 'config.yaml'}")
    print(f"  Secrets: {max_home / '.env'}")
    print(f"  Data:    {max_home / 'cron/'}, {max_home / 'sessions/'}, {max_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their MAX_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Max data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Max code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, max_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Max PATH entries from shell configs / Windows User PATH")
    print("  • Max wrapper scripts and Max-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Max config/data: {max_home}")
        if _is_default_max_home(max_home):
        ...
        print(f"  • Keep Max config/data: {max_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``max`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$MAX_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in max_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\max\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(max_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Max-owned PATH entries in User environment")

        log_info("Removing MAX_HOME / MAX_GIT_BASH_PATH User env vars...")
        removed_env = remove_max_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Max-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing max command...")
    ...
    #     (only when they still point into this Max home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Max-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(max_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Max-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(max_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE MAX_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from max_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(max_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.max/, just remove the max-agent subdir
            if max_home in project_root.parents or project_root.parent == max_home:
    ...
    #     under MAX_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(max_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(max_home)
    ...
    # 5. Optionally remove ~/.max/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile MAX_HOME dirs live under
        ...
        try:
            if max_home.exists():
                shutil.rmtree(max_home)
                log_success(f"Removed {max_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {max_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {max_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://max-agent.stardustresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://max-agent.stardustresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Max Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m max_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import max_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``max_constants`` + ``max_cli.colors``
    (and lazily ``max_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m max_cli.uninstall")
    parser.add_argument(
        "--mode",
