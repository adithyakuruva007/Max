    ...
    for config_path in configs:
        try:
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Max-owned file, and
...
def remove_wrapper_script():
    ...
    wrapper_paths = [
        ...
        Path.home() / ".local" / "bin" / "max-acp",
        Path.home() / ".local" / "bin" / "max-agent",
        Path("/usr/local/bin/hermes"),
        Path("/usr/local/bin/max-acp"),
        Path("/usr/local/bin/max-agent"),
    ]
...
def _hermes_path_markers(hermes_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    ...
    holding the max launchers and the managed uv) — only wanted when
...
def remove_path_from_windows_registry(hermes_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    ...
    dir holding the max launchers and the managed uv) to the sweep. Only
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``max`` launchers install.ps1 staged in the managed
    binary dir (the default Max root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``max`` in a new terminal resolves to a launcher whose target is
...
def _uninstall_profile(profile) -> None:
    ...
    hermes_invocation = [_sys.executable, "-m", "max_cli.main", "--profile", name]
...
def run_gui_uninstall(args):
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Max agent at {hermes_home / 'max-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Max agent is still installed. Run 'hermes' to use the CLI,")
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    if full_uninstall:
        print(f"  • Max config/data: {hermes_home}")
        ...
        print(f"  • Keep Max config/data: {hermes_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    #     would dangle — `max` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows max launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows max launchers found")
