"""``max debug`` debug tools for Max Agent.

Currently supports:
    max debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.max/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[max debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.max/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``max debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``max debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `max debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "max-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``max debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.max/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``max debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "max-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from max_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_max_home() / "logs" / filename) if filename else None


# Logs written by a client process rather than by this backend. When the
# desktop app talks to a remote/docker/SSH backend, `max debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Max Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``max dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``max dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``max debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "max debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `max debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  max debug delete <url>")

    print("\nShare these links with the Max team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Max version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``max debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from max_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "max debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `max debug share --local` to print the report instead, "
            "or `max debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        ...
        "  GitHub Issues        https://github.com/NousResearch/max-agent/issues\n"
        "  Nous Portal Support  https://portal.stardustresearch.com/help\n"
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: max debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'max debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``max debug`` call.
    ...
    # best-effort — any failure is swallowed so ``max debug`` stays
