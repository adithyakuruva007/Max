...
entries via ``max mcp catalog`` or the interactive ``max mcp picker``,
and install them with ``max mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into max-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``max mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.max/.env`` (the
...
from max_constants import get_max_home, get_optional_mcps_dir
...
from max_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.max/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Max install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Max understands version {_MANIFEST_VERSION})"
        )
    ...
    if t_type == "http" and a_type == "api_key":
        ...
        # at connect time.
        from max_cli.mcp_config import _env_key_for_server
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Max is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Max
        understands. Update Max to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_max_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.max/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.max/.env via save_env_value()."""
...
def _build_server_config(
    ...
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from max_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from max_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    *,
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``max mcp configure <name>``.
    """
    print()
    ...
    if entry.tools.default_excluded and prior_selection is None:
        if prior_exclude is not None:
            ...
            print(color(
                ...
                "config.yaml or run "
                f"`max mcp configure {entry.name}` to change.",
                Colors.GREEN,
            ))
            return
    ...
    if probed is None:
        ...
        if prior_selection is not None:
            ...
            print(color(
                ...
                f"Run `max mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `max mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `max mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `max mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `max auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`max auth {entry.auth.provider}` if you have not "
    ...
    from max_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new Max session to load its tools.",
        Colors.GREEN,
    ))
