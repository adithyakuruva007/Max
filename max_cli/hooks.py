"""max hooks — inspect and manage shell-script hooks.

Usage::

    max hooks list
    max hooks test <event> [--for-tool X] [--payload-file F]
    max hooks revoke <command>
    max hooks doctor

Consent records live under ``~/.max/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.max/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``max hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: max hooks {list|test|revoke|doctor}")
        print("Run 'max hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.max/config.yaml.")
        print("See `max hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.max/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `max hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `max hooks test` and `max hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Max wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `max hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `max hooks doctor` would execute
