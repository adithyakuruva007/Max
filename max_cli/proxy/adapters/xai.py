...
# xAI's public API is OpenAI-compatible for the endpoints Max commonly
# uses. The Responses endpoint is included because Max' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Max-managed OAuth credentials."""

    auth_hint = "max auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`max auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`max auth reset xai-oauth` or re-authenticate with "
                    "`max auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `max auth add xai-oauth --type oauth`."
            )
