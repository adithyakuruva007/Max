...
Reads the user's Nous OAuth state from ``~/.max/auth.json`` through the
...
# Endpoints inference-api.stardustresearch.com actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `max auth add nous` first."
                )
    ...
    # to max_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
