"""Client for uploading ``max debug share`` bundles to Nous-internal S3.

This is the opt-in (``--nous``) destination for ``max debug share``.
...
NAS_BASE = os.environ.get(
    "MAX_DIAGNOSTICS_BASE_URL", "https://portal.stardustresearch.com"
)
...
_USER_AGENT = "max-agent/debug-share"
