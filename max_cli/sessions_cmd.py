"""``max sessions`` command — extracted from ``max_cli/main.py``.
...
Helpers that stay in ``max_cli.main`` (``get_max_home``,
...
``max_cli.main.<name>`` keep reaching this code path, and so imports stay
...
#: Default age floor for `max sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`max sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from max_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    max sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "import":
        from max_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from max_state import workspace_key as _ws_key
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `max sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `max sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `max sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: max sessions pin <session_id>"
            )
            return
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`max sessions optimize` later to reclaim freed space.)")
