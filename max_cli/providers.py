"""
Single source of truth for provider identity in Max Agent.
...
2. **Max overlays** — transport type, auth patterns, aggregator flags,
...
# -- Max overlay ----------------------------------------------------------
# Max-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class MaxOverlay:
    """Max-specific provider metadata layered on top of models.dev."""
...
MAX_OVERLAYS: Dict[str, MaxOverlay] = {
    "moa": MaxOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": MaxOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nous": MaxOverlay(
        ...
        base_url_override="https://inference-api.stardustresearch.com/v1",
    ),
    "openai-codex": MaxOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": MaxOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": MaxOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": MaxOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="MAX_QWEN_BASE_URL",
    ),
    "lmstudio": MaxOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": MaxOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": MaxOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": MaxOverlay(
    ...
    "alibaba-coding-plan": MaxOverlay(
        transport="openai_chat",
    ...
    "opencode-go": MaxOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": MaxOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "tencent-tokenhub": MaxOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "tencent-tokenplan": MaxOverlay(
        transport="anthropic_messages",
    ...
    "nebius-token-factory": MaxOverlay(
        transport="openai_chat",
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Max overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Max overlay
    ...
    overlay = MAX_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + max extra
    ...
    if overlay is not None:
        # Max-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and MAX_OVERLAYS, so
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Max overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in MAX_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Max provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from max_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
