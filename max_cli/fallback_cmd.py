"""
max fallback — manage the fallback provider chain.
...
Subcommands:
  max fallback [list]   Show the current fallback chain (default when no subcommand)
  max fallback add      Pick provider + model via the same picker as `max model`,
                           then append the selection to the chain
  max fallback remove   Pick an entry to delete from the chain
  max fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.max/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from max_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `max model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  max fallback add")
        print()
        return
    ...
    print("  Docs: https://max-agent.stardustresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `max model`, then append the selection to the chain."""
    from max_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `max model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `max fallback list` to view, or `max fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``max fallback [subcommand]``."""
