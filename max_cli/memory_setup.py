"""max memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``max memory setup`` — if the update-time refresh only reinstalled
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_max_home() / "hindsight" / "config.json"
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as max tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``max update`` heals the active memory provider
    ...
    # data volume (MAX_LAZY_INSTALL_TARGET). install_specs handles the
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'max memory setup' to see available providers.\n")
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.max/plugins/ and try again.\n")
        return
...
def _write_env_vars(
    ...
    Delegates to ``max_cli.config.save_env_value`` so every key flows
    ...
    ``MAX_HOME`` / etc.), CR/LF stripping on the value, and the atomic
    ...
    ``.env`` -> ``os.environ`` chain on the next Max startup, and the
    ...
    profile). It is applied through the context-local Max home override
    ...
    token = set_max_home_override(hermes_home) if hermes_home is not None else None
    try:
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if provider:
            ...
            if provider.is_available():
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        print(line)
                print(
                    "  Note: systemd/gateway services do not inherit ~/.max/.env —"
                )
                print(
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.max/plugins/")
