...
late-binding seam in :mod:`max_cli.web_deps` (``late`` for callables,
...
from max_cli.web_deps import late, LateState
from max_cli.web_models import (
...
_log = logging.getLogger("max_cli.web_server")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from max_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `max tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `max computer-use install`
