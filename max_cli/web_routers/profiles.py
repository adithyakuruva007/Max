...
late-binding seam in :mod:`max_cli.web_deps` so tests that
...
from max_cli.web_deps import late
...
def get_profiles_projects_tree(preview_limit: int = 3, session_limit: int = 2000):
    ...
    the repo-scan policy, the MAX_HOME junk filters — through the
...
async def update_profile_soul(name: str, body: ProfileSoulUpdate):
    ...
    try:
        ...
        # profiles seed SOUL.md at the umask default (max_cli.profiles
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps max_cli.profiles.export_profile /
# import_profile (the same machinery behind `max profile export|import`).
...
async def export_profile_endpoint(name: str, body: ProfileExport):
    ...
    if not output:
        ...
        staging = get_max_home() / "profile-exports"
        try:
