...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    hermes_home = Path(os.environ.get("MAX_HOME", "/opt/data"))
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the max user already — directory is hermes-
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources MAX_HOME (and any extra env) via with-contenv —
             so e.g. ``-e MAX_HOME=/data/hermes`` is honored at run
          ...
          4. Drops to the max user and exec's
             ``max -p <profile> gateway run`` (or just ``hermes
    ...
    def _render_log_run(profile: str) -> str:
        ...
        — so a container started with ``-e MAX_HOME=/data/hermes``
        ...
        return (
            ...
            # Create the leaf and clear a stale s6-log lock as max when
            ...
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid max mkdir -p "$log_dir"\n'
            f'  s6-setuidgid max rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
