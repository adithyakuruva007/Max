...
``cli.py``'s ``MaxCLI`` class. ``MaxCLI`` inherits ``CLICommandsMixin`` so
...
from max_constants import display_max_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        Syntax:
            ...
            /rollback <N> --all       — classic full restore (may overwrite
                                        files you edited after Max did)
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: max --checkpoints")
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Max changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Max hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Max config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_max_home()}/state-snapshots/):\n")
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from max_cli.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `max profile import <archive>`.")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `max journey`).
        ...
        from cli import _cprint
        from max_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from max_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run max chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in max_cli/colors.py
    ...
    def _handle_profile_command(self):
        ...
        from max_cli.slash_exec import CommandContext, execute_command
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        # "Is `max gateway` running?" is the correct diagnosis, and the
        ...
        # CLI printed "Is `max gateway` running?" while the gateway was
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `max gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `max sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from max_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /sessions or `max sessions list` to see available sessions.")
            return
        ...
        # same directory as a startup `max -c`/`--resume`. The startup resume
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        commits, same as ``max -w``).

        ``prune`` is the same attended reclaim as ``max worktree prune``
        (max_cli/worktree_gc.py): never deletes tracked changes, unique
        ...
        if sub in {"new", "add", "create"}:
            ...
            # same way `max -w` and session-resume cwd restore do.
            try:
                os.chdir(wt_info["path"])
            except OSError as e:
                print(f"  ⚠ Created worktree but could not enter it: {e}")
            os.environ["TERMINAL_CWD"] = wt_info["path"]
            # Register for the same keep-if-unpushed cleanup as `max -w`.
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        All resolution/persistence goes through max_cli.personality —
        the single owner of personality state on every surface.
        """
        from max_cli.personality import (
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from max_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from max_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from max_cli.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to max_cli.curator so the CLI and the `max curator`
        ...
        try:
            from max_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from max_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to max_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            from max_cli.write_approval_commands import handle_pending_subcommand
            ...
            if out is not None:
                print(out)
                return
        from max_cli.skills_hub import handle_skills_slash
    ...
    def _handle_plan_command(self, cmd: str):
        ...
        ``.max/plans/`` via ``write_file``. No engine, no model-tool
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``max bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: max bundles create "
        ...
        _cprint(
            ...
            f"Manage with `max bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Max-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `max cron`.
        """
        from cli import _DIM, _RST, _cprint
        from max_cli.heartbeat import parse_interval, format_interval
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from max_cli.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cprint(
            ...
            f"lives only while this Max process runs — use `max cron` for "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Max keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from cli import _DIM, _RST, _cprint
        from max_cli.goals import draft_contract
    ...
    def _handle_loop_command(self, cmd: str) -> None:
        ...
        from max_cli.loops import dispatch_loop_command
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from max_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_max_home()}/skins/\n")
            return
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from max_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from max_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from max_cli.focus_view import format_hidden_line
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Max is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Max is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Max is busy; /stop still cancels it."
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        """
        from cli import _ACCENT, _DIM, _RST, _cprint, save_config_value
        from max_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from max_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Max Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``max update`` so the user sees update output directly and gets
        ...
        when cancelled.
        """
        from max_cli.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update Max Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Max Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Max Agent",
            detail="This will exit the current session and run `max update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Max' hotword listener.
