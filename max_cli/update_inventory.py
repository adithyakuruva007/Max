...
One read-only pass that answers, BEFORE any mutation: what Max runtimes
...
`detect_install_method`) — so `max update --plan` can run on a live
...
Deployment kinds (the concept most fleet-update bugs were missing):
    ...
    git      — source checkout; updatable in place via `max update`
...
Supervisors (how a runtime is restarted after code changes):
    ...
    desktop           — Desktop app supervises `max serve`; it respawns
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Max runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "max update"
...
def _detect_supervisor_for_pid(
    ...
    if pid in service_pids:
        try:
            from max_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"max -p {profile} gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    try:
        ...
        try:
            from max_cli.image_provenance import read_image_provenance
    ...
    try:
        from max_cli.gateway import find_windows_gateway_services
    ...
    try:
        from max_cli.gateway import find_profile_gateway_processes
    ...
    # launched `max serve --host <ip>` for a remote Desktop, or a
    # long-lived `max dashboard`. Every serve/dashboard registers itself
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Max services: none detected — code swap only.")
        return
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    print("      max gateway restart                # active profile")
    print("      max -p <profile> gateway restart   # named profile")
    return True
