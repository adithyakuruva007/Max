"""On-demand worktree + branch reclaim (``max worktree`` / ``/worktree prune``).

The startup pruner in ``cli._prune_stale_worktrees`` is deliberately
conservative and silent: it runs before the banner on every ``max -w``
...
- untracked-only dirt is ARCHIVED to ``~/.max/archive/worktree-prune/``
...
def _archive_untracked(tree: Path, untracked: List[str]) -> Optional[Path]:
    ...
    dest = (
        Path.home() / ".max" / "archive" / "worktree-prune"
        / f"{tree.name}-{stamp}"
    )
    try:
...
def audit_worktrees(repo_root: str, *, with_sizes: bool = True) -> List[TreeRecord]:
    ...
    for entry in sorted(worktrees_dir.iterdir()):
        ...
        if lock_state == "live":
            rec("keep", "in use by a running max session")
            continue
