"""MCP picker — interactive `max mcp picker` (also the default `max mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``max mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `max plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from max_cli.cli_output import prompt_yes_no
from max_cli.curses_ui import curses_single_select
from max_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Max session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from max_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `max mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: max mcp install <name>    Picker: max mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Max is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Max — run `max update` "
...
def show_catalog() -> None:
    """`max mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`max mcp picker` (and default `max mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`max mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `max mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
