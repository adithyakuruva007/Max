"""Dashboard process-hygiene helpers — extracted from ``max_cli/main.py``.
...
helpers that STAY in ``max_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``max_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``max_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``max_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    # miss profiled launches — `max --profile p serve --host <ip>` contains
    # neither "max serve" nor "max_cli.main serve". Every serve/
    ...
    # truncated view.
    try:
        from max_cli.process_identity import ledger_entries
...
def _hermes_home_for_pid(pid: int) -> str | None:
    """Best-effort ``MAX_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("MAX_HOME")
        if home:
            return home
    except Exception:
        pass
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Max Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``max update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    ...
    """Stable owner key: ``MAX_HOME`` when known, else ``--profile`` / ``-p``.

    ``MAX_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.max`` roots) keep a resolved ``home:`` key so
...
def _filter_dashboard_respawn_candidates(
    ...
    *,
    ...
    4. Cap at most one managed backend per profile / ``MAX_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``max update`` respawn detaches with ``start_new_session=True``, so
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``max dashboard`` / ``max serve`` processes.
    ...
    When ``restart_managed`` is true (the ``max update`` path), a detected
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``max update``'s systemd
    ...
    # caller asked for restarts (the `max update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot MAX_HOME before the kill so per-profile caps
        ...
        if already_restarted_units:
            # Already handled directly by the caller (e.g. max update's
...
def _detect_concurrent_hermes_instances(
    ...
    The Max Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    ...
    #      hermes.exe sitting *under* a non-Max parent (e.g. a Max
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        max serve --host 127.0.0.1 --port 0
        max serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "hermes" not in cmd and "max_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (MAX_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("MAX_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``max serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``max serve`` on a Mac
...
def _hermes_home_dir() -> Path:
    """Resolved Max home (MAX_HOME override or ~/.max)."""
    override = os.environ.get("MAX_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    # only check the suffix so a relocated MAX_HOME (different leading path)
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local max serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``max serve`` backends with no parent.
    ...
    running under MAX_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never MAX_DESKTOP_CHILD_PID entries
    ...
    - never a candidate younger than ``_REAP_MIN_AGE_SECONDS`` (or whose age
      ...
      only after the backend reports MAX_BACKEND_READY, so during
    ...
    for pid, cmd in scanned:
        ...
        # MAX_BACKEND_READY, so a sibling spawned seconds ago is not yet
