...
from max_cli.dashboard_auth import list_session_providers
from max_cli.dashboard_auth.audit import AuditEvent, audit_log
from max_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from max_cli.dashboard_auth.cookies import (
    clear_sso_attempt_cookie,
    read_session_cookies,
    read_sso_attempt_cookie,
    set_sso_attempt_cookie,
)
from max_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /max``, the
    ``login_url`` is prefixed (``/max/login?next=...``) so the
    ...
    """
    from max_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["max.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``max_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from max_cli.dashboard_auth.cookies import clear_session_cookies
        from max_cli.dashboard_auth.prefix import prefix_from_request
