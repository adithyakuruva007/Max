...
Profile-aware location: ``$MAX_HOME/logs/dashboard-auth.log``.
...
from ``max_constants`` or other max_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$MAX_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``max_constants.get_max_home`` semantics: env var wins,
    else ``~/.max``. A local copy avoids an import cycle with the
    middleware which lives below ``max_cli``.
    """
    home = os.environ.get("MAX_HOME") or str(Path.home() / ".max")
