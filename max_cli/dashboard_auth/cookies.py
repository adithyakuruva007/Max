...
Three cookies in play:
  ...
  - max_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-max_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "max_session_at"
SESSION_RT_COOKIE = "max_session_rt"
PKCE_COOKIE = "max_session_pkce"
...
SSO_ATTEMPT_COOKIE = "max_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /max`` we want ``Path=/max`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (max #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/max``)
