...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``MAX_DASHBOARD_PUBLIC_URL`` env var or
...
def _prefix(request: Request) -> str:
    ...
    ``max_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
...
async def auth_native_authorize(
    ...
    from max_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from max_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    from max_cli.dashboard_auth.base import RefreshExpiredError
