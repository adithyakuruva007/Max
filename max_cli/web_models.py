"""Pydantic request/response models for the Max dashboard web server.

Extracted verbatim from ``max_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from max_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``max model`` custom flow collects.
    api_key: str = ""
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under MAX_HOME.
    output: str = ""
