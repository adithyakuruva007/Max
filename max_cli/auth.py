...
is persisted in ~/.max/auth.json with cross-process file locking.
...
# httpx is imported lazily: it costs ~30ms at import time and max_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("max_cli.auth.httpx.Client", ...) keeps working in tests.
...
CODEX_OAUTH_USER_AGENT = f"hermes-cli/{_MAX_CLI_VERSION}"
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for Max' Responses transport.
    """
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "anthropic": ProviderConfig(
        ...
        # from it), so removing it would stop Max finding a setup-token
    ...
    "opencode-free": ProviderConfig(
        ...
        # secret to configure. Select via `max model` / `/model free`.
        api_key_env_vars=(),
    ),
...
def _file_lock(
    ...
    # A stress test with 20 concurrent Max processes reproduced this
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # max-agent install tree (#25821, #93050).
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted Max
    ...
    if pconfig is None:
        from max_cli.providers import get_provider
    ...
    # explicit for users who never set Max up for it. Only Max-scoped
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Max persists a Codex 429's ``reset_at`` locally and freezes the
...
# ``get_default_hermes_root()`` returns — ``~/.max`` on Linux/macOS,
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`max_constants.get_default_hermes_root` returns — so
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "Nous refresh tokens are single-use — only Max may call the "
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "Max is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("Max is not logged into Nous Portal.",
...
def get_api_key_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    # the MaxOverlay keyless flag — the same source the provider catalog
    # and GUI contract tests use.
    try:
        from max_cli.providers import MAX_OVERLAYS
        _overlay = MAX_OVERLAYS.get(provider_id)
    except Exception:
        _overlay = None
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`max_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (max_cli.model_selection_guards): the cost
    ...
    try:
        ...
        from max_cli.model_search import model_search_text
    ...
    from max_cli.curses_ui import format_radio_item_ansi
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `max model` again to switch to Nous Portal.")
            return
