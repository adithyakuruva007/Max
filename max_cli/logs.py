"""``max logs`` — view and filter Max log files.
...
Usage examples::
    ...
    max logs                    # last 50 lines of agent.log
    max logs -f                 # follow agent.log in real time
    max logs errors             # last 50 lines of errors.log
    max logs gateway -n 100    # last 100 lines of gateway.log
    max logs gui -f            # follow gui.log (dashboard/pty/ws)
    max logs desktop -f        # follow desktop.log (Electron app boot/backend)
    max logs --level WARNING    # only WARNING+ lines
    max logs --session abc123   # filter by session ID substring
    max logs --component tools  # only tool-related lines
    max logs --since 1h         # lines from the last hour
    max logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_max_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Max runs — try 'max chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_max_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_max_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_max_home()}/logs/")
        return

    print(f"Log files in {display_max_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'max chat' to generate logs)")
