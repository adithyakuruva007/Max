...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    real gateway on the same MAX_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import max_cli.gateway  # noqa: F401
    except Exception:
        pass
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire max_cli.gateway import into a background thread so the event
    # loop is not blocked and MAX_DASHBOARD_READY fires without delay.
    ...
    # since the app has no gateway running the scheduler. Server `max
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("MAX_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``max-0day``
...
class ModelAssignment(BaseModel):
    ...
    # ``max model`` custom flow collects. Honored only on the main slot for
...
def _count_status_active_sessions() -> int:
    ...
    while another Max process is writing to it.
    """
    from max_state import DEFAULT_DB_PATH, SessionDB
...
def _default_max_root_is_opt_data() -> bool:
    ...
    try:
        ...
        root = get_default_max_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    a container — e.g. the max-webui image sharing the Max source tree),
    ...
    """
    if _default_max_root_is_opt_data():
        return True
    try:
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``max:fs:writeText`` hardening: the path is
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            from max_cli.dashboard_auth import list_providers as _list_providers
        ...
        status = {
            ...
            "can_update_max": not _dashboard_local_update_managed_externally(),
...
async def run_curator():
    ...
    try:
        proc = _spawn_max_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def run_prompt_size():
    try:
        proc = _spawn_max_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_max_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_max_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
def _spawn_max_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "MAX_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["max", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    ...
    return _spawn_max_action(subcommand, "gateway-restart"), False
...
async def update_max():
    ...
    if _dashboard_local_update_managed_externally():
        ...
        _record_completed_action("max-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "max-update",
    ...
    try:
        proc = _spawn_max_action(["update"], "max-update")
    except Exception as exc:
...
@app.get("/api/max/update/check")
async def check_max_update(force: bool = False):
    """Report whether a Max update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/max/update`` actually runs ``max update``.
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Max surface. FTS can't find those unless the
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    max_home = get_max_home()
    values: Dict[str, Any] = {}

    # Common native provider stores.
    for path in (
        max_home / f"{name}.json",
        max_home / name / "config.json",
    ):
    ...
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("max-memory-store")
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from max_cli.model_cost_guard import expensive_model_warning
...
def _whatsapp_session_path() -> Path:
    from max_constants import get_max_dir

    return get_max_dir("platforms/whatsapp/session", "whatsapp/session")
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's MAX_HOME.
...
# the canonical ``max auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.max/.anthropic_oauth.json`` — Max-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_max_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_max_oauth_credentials,
            _get_max_oauth_file,
        )
    except ImportError:
        read_max_oauth_credentials = None  # type: ignore
        _get_max_oauth_file = None  # type: ignore

    max_creds = None
    if read_max_oauth_credentials:
        try:
            max_creds = read_max_oauth_credentials()
        except Exception:
            max_creds = None
    if max_creds and max_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "max_pkce",
            "source_label": f"Max PKCE ({_get_max_oauth_file() if _get_max_oauth_file else None})",
            "token_preview": _truncate_token(max_creds.get("accessToken")),
            "expires_at": max_creds.get("expiresAt"),
            "has_refresh_token": bool(max_creds.get("refreshToken")),
        }
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("max_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_max_oauth_file
                oauth_file = _get_max_oauth_file()
...
#          → persists to ~/.max/.anthropic_oauth.json AND credential pool
...
# Guarded so max web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Max file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``max auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_max_oauth_file
    oauth_file = _get_max_oauth_file()
    payload = {
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `max sessions stats`).
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process MAX_HOME at import time. The dashboard is a single
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from max_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from max_cli.mcp_config import _replace_mcp_servers
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the MAX_HOME
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``max mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        ...
        # resolve MAX_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = MaxTokenStorage(name)
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `max mcp catalog` / `max mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds MAX_HOME-derived paths in the child.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_max_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_max_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def run_doctor():
    try:
        proc = _spawn_max_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_max_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _new_dashboard_backup_path() -> Path:
    ...
    return _dashboard_backup_dir() / f"max-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_max_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `max import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_max_action(args, "import")
    except Exception as exc:
...
async def list_checkpoints():
    ...
    # Checkpoints live under <max_home>/checkpoints/.  Surface a count +
...
async def prune_checkpoints():
    try:
        proc = _spawn_max_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_max_action`` tracks one process/log per name, so a shared
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`max -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the MAX_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_max_home()
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's MAX_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``max profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from MAX_HOME).
    """
    from max_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``max profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the MAX_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``max profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from max_cli import profile_describer
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``max tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``max tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `max tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``max tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.max/.env`` via ``save_env_value`` —
    the same store ``max tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``max tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``max -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same MAX_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``max computer-use permissions grant`` as a background action.
...
def _resolve_chat_argv(
    ...
    """
    from max_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="max-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="max-console",
                )
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_max_root_is_opt_data() else "local"
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/max/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /max`` on every request. We
    ...
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (max serve): web UI disabled — use "
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/max``)
    ...
    # under ``/max`` they'd hit ``mission-control.tilos.com/fonts/...``
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_max_home() / "dashboard-themes"
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"max_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    # the max-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `max dashboard register` or "
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Max backend listening on {host}:{actual_port}")
            else:
