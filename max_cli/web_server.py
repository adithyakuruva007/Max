"""
Max Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m max_cli.main web          # Start on http://127.0.0.1:9119
    python -m max_cli.main web --port 8080
"""
...
from max_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
try:
    ...
    # running `max dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["MAX_WEB_DIST"]) if "MAX_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``max gateway run`` — but the
    desktop app spawns a ``max dashboard`` backend, not a gateway, so a cron
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            from max_cli.profiles import profiles_to_serve
...
def _warm_gateway_module() -> None:
    ...
    ``max_cli.gateway``. But the first WS connection and its initial
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from max_cli.gateway import _get_restart_drain_timeout
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `max update` current before the
    ...
    """
    try:
        from max_state import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `max update` kept 500ing every
    ...
    # Import max_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # model picker) can detect when `max update` replaced the code
    ...
    # Desktop-spawned backends (MAX_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hermes
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("MAX_DESKTOP") == "1":
        ...
        try:
            from max_cli.gateway import _reap_unsupervised_gateway_orphans
...
app = FastAPI(title="Max Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from max_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# MAX_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("MAX_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Max-Session-Token"
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".max-ssh-runtime-{nonce}")
...
#
# This list is defined in ``max_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from max_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Max-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    ``public_url`` is already Max' canonical browser-facing URL behind a
    ...
    """
    from max_cli.dashboard_auth.prefix import resolve_public_url
...
async def _dashboard_auth_gate(request: Request, call_next):
    from max_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from max_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `max egress setup` "
            "and `max egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Max (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during max update. "
...
from max_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Max provider slug — fine.
    ...
    1. Vendor-name → Max-provider mapping: when the provider string is
       not a known Max provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from max_cli.config import get_compatible_custom_providers
    from max_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from max_cli.model_normalize import normalize_model_for_provider
    from max_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Max
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from max_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    Max process is using it.
    """
    from max_state import _default_db_path
...
_MANAGED_FILES_ROOT_ENV = "MAX_DASHBOARD_FILES_ROOT"
...
# MAX_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to MAX_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Max credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no MAX_HOME resolution, so it blocks these trees wherever
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("MAX_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``max update``.
    ...
    a container — e.g. the hermes-webui image sharing the Max source tree),
    the dashboard's ``max update`` button is the correct update path and
    ...
    if _default_hermes_root_is_opt_data():
        return True
    try:
        from max_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Max root is actually /opt/data
    # (the container/hosted layout) or when MAX_DASHBOARD_FILES_ROOT is set.
    if _default_hermes_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Max inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    tmp = target.with_name(f".{target.name}.max-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``max_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from max_cli import web_git as _web_git  # noqa: E402
...
from max_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from max_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from max_cli.profiles import _check_gateway_running, profiles_to_serve
...
# install, minted on first read and persisted under the ROOT Max home
# (get_default_hermes_root()) — NOT the profile-scoped MAX_HOME — so every
...
def _read_or_create_install_id() -> Optional[str]:
    """Read (or mint + persist) the install id under the root Max home.
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors max_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_max_home() at call time
    ...
    try:
        ...
        # MAX_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of max_cli.gateway blocks the
        ...
        # and which providers are registered so ``max status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Stardust Research" or
        ...
        try:
            from max_cli.auth import get_nous_session_validity
        ...
        try:
            ...
            storage_check = await run_in_threadpool(_probe_state_db, get_max_home())
        ...
        try:
            ...
            status["memory"] = await run_in_threadpool(
                ...
                profile_dir if profile_dir else get_max_home(),
            )
        except Exception:
            status["memory"] = {"pressure": "unknown"}

        # Disk-usage rollup (NS-656, same lineage as OOF-2/OOF-107 fleet
        # disk-exhaustion incidents). One statvfs call on MAX_HOME's
        ...
        try:
            from max_state import SessionDB as _SDB
            from max_constants import get_max_home as _ghh
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Max Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the max home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_max_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from max_cli.auth import get_nous_auth_status_local
    ...
    return {
        ...
        "subscription_url": "https://portal.stardustresearch.com/manage-subscription",
        "features": features,
    }
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from max_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.max/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_max_home() / "logs"
...
def _dashboard_spawn_executable() -> str:
    ...
    runpy.run_module('max_cli.main', ...)"``) — so ``sys.executable`` is
...
def _spawn_hermes_action(
    ...
    *,
    ...
    """Spawn ``max <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``max_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "max_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _MAX_GATEWAY=1. Inheriting it makes a spawned `max gateway restart`
    ...
    action_env = {**os.environ, "MAX_NONINTERACTIVE": "1"}
    action_env.pop("_MAX_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("MAX_ACTION_ID")
    if action_id:
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== max update started "):
            last_start = index
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``max gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``max gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``max gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``MAX_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_hermes():
    """Kick off ``max update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Max updates are managed outside this dashboard in "
    ...
    try:
        proc = _spawn_hermes_action(
            ["update"],
            "hermes-update",
            env_overrides={"MAX_ACTION_ID": action_id},
        )
    except Exception as exc:
        _log.exception("Failed to spawn max update")
...
async def check_hermes_update(force: bool = False):
    """Report whether a Max update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/hermes/update`` actually runs ``max update``.

    Returns:
        install_method: 'apt' | 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed Max version string
    ...
    if install_method == "apt":
        payload["message"] = (
            "Max is managed by Termux APT; run `pkg upgrade max-agent`."
        )
        return payload
    ...
    try:
        ...
        if force:
            try:
                (get_max_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.max/config.yaml`` under ``tts.``.
    """
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    ...
    """
    try:
        from max_cli.update_receipt import read_latest_receipt
...
async def get_update_receipt():
    ...
    if not receipt:
        raise HTTPException(
            status_code=404,
            detail="No update receipt found (no `max update` run recorded).",
        )
...
from max_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from max_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from max_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from max_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Max supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_max_home() / provider.name / "config.json"
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/max is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (MAX_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    # Holographic stores under plugins.max-memory-store.
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_max_home()))
            return
...
async def get_egress_status():
    ...
    from max_cli.proxy_cli import format_status_text
...
# in max_cli/config.py — listed here for deterministic ordering in the UI.
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    boot.  When ``max update`` (or a manual ``git pull``) replaces the
    ...
    return (
        ...
        f"(systemctl --user restart hermes-dashboard, or max dashboard --port <port>)"
    )
...
async def get_model_options(
    ...
    try:
        ...
        from max_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `max model` does so GUI onboarding lands on a
    ...
    try:
        from max_cli.inventory import build_models_payload, load_picker_context
        from max_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from max_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from max_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.max/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from max_cli.model_selection_guards import combined_selection_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (max_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from max_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``max model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from max_cli.main import _auto_provider_name, _save_custom_provider
...
async def update_config(body: ConfigUpdate, profile: Optional[str] = None):
    def _run():
        ...
        # different MAX_HOME than this process's gateway sessions.
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``max model`` universe). This is what lets the desktop Keys tab
    ...
    try:
        from max_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `max model` provider — would be
        ...
        # Vertex — a `max model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `max model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from max_cli.credential_lifecycle import save_provider_env_credential
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level MAX_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from max_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Max from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Max to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Max from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Max to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Max in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Max through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Max via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Max through an IMAP/SMTP mailbox.",
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Max to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Max inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Max to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Max through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Max to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect Max to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Max.",
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Max from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Max over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Max through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Max over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Max to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Max as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Max via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://max-agent.stardustresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Max Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in max gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in max gateway setup",
...
# max_cli/setup_hidden_env.py. Shared with the `max setup gateway`
# wizard so the surfaces ask for the same things.
from max_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # MAX_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    from max_constants import get_hermes_dir
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.max-agent.stardustresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"MaxDashboard/{__version__}"
@dataclass
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Max Agent").strip() or "Max Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Max appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the MAX_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized MAX_HOME)
...
# the canonical ``max auth add <provider>`` command so the dashboard can
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.max/.anthropic_oauth.json`` — Max-managed terminal PKCE
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Max PKCE ({_get_hermes_oauth_file() if _get_hermes_oauth_file else None})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Max even
    when they also have a separate Max-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Max never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "max auth add nous",
        "docs_url": "https://portal.stardustresearch.com",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "max auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "max auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "max auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "max auth add xai-oauth",
        "docs_url": "https://max-agent.stardustresearch.com/docs/guides/xai-grok-oauth",
    ...
    # server). Max previously reimplemented that subscription-OAuth PKCE
    ...
    # credentials. Login still works via the terminal (`max auth add
    # anthropic`, unaffected by this change) or a plain API key below.
    {
        "id": "anthropic",
        "name": "Anthropic API Key",
        "flow": "external",
        "cli_command": "max auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from max_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Max, so the disconnect
    ...
    user sees exactly what executes, and Max then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # catalog entry) but, unlike other external providers, Max still OWNS
    # the credential it can show here: the Max-managed PKCE file
    # (~/.max/.anthropic_oauth.json) and its credential-pool entry, both
    # written by `max auth add anthropic` in the terminal. Those are ours
    ...
    if provider.get("flow") == "external" and provider.get("id") != "anthropic":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Max — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``max model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `max model` universe (zero-edit for new plugins).
    try:
        from max_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"max auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `max model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the Max-managed PKCE file and auth-store entry.
            ...
            if provider_id == "anthropic":
                ...
                try:
                    from max_cli.auth import clear_provider_auth
            ...
            try:
                from max_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
# points at `max auth add anthropic` (terminal PKCE, unaffected) instead.
#
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from max_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from max_cli.auth import persist_nous_credentials
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `max auth add xai-oauth`: first credential may become
            ...
            # prior ``max auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``max model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Max "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from max_cli.auth import _save_codex_tokens
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `max update` until the first message forced a writable open.
    """
    from max_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from max_state import SessionDB, is_malformed_schema_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``max serve`` — it runs neither the
    ...
    try:
        ...
        from max_cli.config import load_config as _load_full_config
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `max sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_max_home()
...
async def get_logs(
    ...
    from max_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_max_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from max_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    """
    from max_cli import profiles as profiles_mod
    try:
        return [
            {
                "name": name,
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (MAX_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.max``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    MAX_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, MAX_HOME)."""
    from max_cli import profiles as profiles_mod
...
from max_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from max_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/max_cli/
        # test_config_read_guard.py). The MAX_HOME override scopes
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``max gateway stop`` persists ``"stopped"``; start and
...
#
# Wraps the same config data layer the CLI uses (max_cli.mcp_config), so
# servers managed here show up under `max mcp list` and vice versa.  Secrets
...
from max_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from max_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from max_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.max_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from tools.mcp_oauth import MaxTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_max_home_override(flow.max_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.max_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = MaxTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        hermes_home=flow.max_home,
                    )
...
#
# Wraps the same JSON store the CLI uses (max_cli.webhook); the webhook
...
# spawn the real `max gateway <verb>` so behaviour matches the CLI exactly.
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `max auth remove`) can seed again.
        # Mirrors the `max auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``max auth remove`` uses: each source cleans up its
    ...
    from max_cli.auth import suppress_credential_source
...
async def list_hooks():
    ...
    def _run():
        ...
        try:
            from max_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    cp_dir = get_max_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``max`` subprocess, and
...
from max_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from max_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _profile_setup_command(name: str) -> str:
    ...
    return "max setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local MAX_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local MAX_HOME override (same mechanism as
    ...
    """
    from max_constants import set_max_home_override, reset_max_home_override
    from max_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the MAX_HOME override. Returns the
    number of skills newly disabled.
    """
    from max_constants import set_max_home_override, reset_max_home_override
    from max_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_max_home()`` at call
       time — the context-local override from ``set_max_home_override``
    ...
    retargeted to the *current* ``get_max_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a MAX_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_max_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_max_home()`` at call time
...
from max_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from max_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Max,
...
#
# The endpoint spawns the same ``max --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from max_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from max_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from max_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from max_cli.dashboard_auth.audit import AuditEvent, audit_log
...
def _resolve_chat_argv(
    ...
    Default: whatever ``max --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``MAX_TUI_RESUME`` env var —
    matching what ``max_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``MAX_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``MAX_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``MAX_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``MAX_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_max_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``max -p <name>`` performs. The in-process
    ``MAX_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from max_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Max TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope still overrides MAX_HOME before config is
    # bridged into the child environment.
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    if profile_dir is not None:
        env["MAX_HOME"] = str(profile_dir)
    try:
        from max_cli.config import (
    ...
    env.setdefault("MAX_TUI_DISABLE_MOUSE", "1")
    env.setdefault("MAX_TUI_INLINE", "1")
    ...
    if resume:
        ...
        env["MAX_TUI_RESUME"] = resume

    if sidecar_url:
        env["MAX_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile MAX_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["MAX_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``MAX_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("MAX_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from max_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
# ---------------------------------------------------------------------------
# /api/console — safe Max Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Max CLI
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from max_cli.console_engine import MaxConsoleEngine

        engine = MaxConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Max Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Max inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("MAX_TUI_RESUME") or raw_resume
...
# MAX_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`max_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from max_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Max Teal canvas before the SPA flips the configured user
    theme into place.
...
def mount_spa(application: FastAPI):
    ...
    and the SPA's runtime ``__MAX_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `max serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("MAX_SERVE_HEADLESS") == "1"
    if _headless:
        _msg = (
            "Headless backend (hermes serve): web UI disabled — use "
            "`max dashboard` for the browser UI."
        )

        @application.get("/{full_path:path}")
        async def no_frontend(full_path: str):
            # Desktop token handshake (#94227): the Electron shell boots by
            # fetching `/` and extracting ``window.__MAX_SESSION_TOKEN__``
            ...
            # `max update` replaced the backend) had no way to adopt the
            ...
            if full_path == "" and not gated:
                ...
                return HTMLResponse(
                    ...
                    f"window.__MAX_SESSION_TOKEN__={token_js};"
                    "window.__MAX_AUTH_REQUIRED__=false;"
                    "</script></head><body>"
                    "Headless backend (hermes serve): web UI disabled — use "
                    "`max dashboard` for the browser UI."
                    "</body></html>",
    ...
    # (#82614): a long-lived `max dashboard --skip-build` process that
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``__MAX_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__MAX_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__MAX_BASE_PATH__="{prefix}";'
                f"window.__MAX_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__MAX_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__MAX_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__MAX_BASE_PATH__="{prefix}";'
                f"window.__MAX_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``MAX_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Max Teal canvas, then
    ...
    # (the MC Pages app), not the Max backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Max Teal",         "description": "Classic dark teal — the canonical Max look"},
    {"name": "default-large", "label": "Max Teal (Large)", "description": "Max Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.max/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.max/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_max_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``MAX_HOME``.
    """
...
async def get_dashboard_themes():
    ...
    from `~/.max/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as max_cli.plugins):
    1. User plugins:    ~/.max/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.max/plugins/  (only if MAX_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from max_cli.plugins import get_bundled_plugins_dir
    ...
    # MAX_HOME override (e.g. embedded /chat under --open-profile).
    #
    # #87197: when the process itself is profile-scoped (``--profile <name>``
    # sets ``MAX_HOME=<root>/profiles/<name>``), the launch home is the
    # profile directory, which has no ``plugins/`` — user plugins are
    # installed in the max root (``~/.max/plugins``). Scan the default
    # root as well (``get_default_hermes_root()`` unwraps
    # ``<root>/profiles/<name>`` → ``<root>`` and returns a custom
    # ``MAX_HOME`` unchanged when it *is* the root), mirroring how
    # ``max_cli.plugins`` resolves plugin install locations. The
    ...
    # ``max_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("MAX_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".max" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from max_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_max_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from max_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from max_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.max/plugins/``) ship with the CWD and are
...
from max_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``MAX_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("MAX_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``MAX_PARENT_PID`` is the Electron Desktop PID, not necessarily this
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``MAX_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("MAX_PARENT_PID")
    start_marker = os.environ.get("MAX_PARENT_START_MARKER")
    nonce = os.environ.get("MAX_PARENT_NONCE")
    ...
    try:
        poll = max(0.5, float(os.environ.get("MAX_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
# spawn (and any script wrapping ``max serve``) cannot tell "port occupied"
...
# MAX_BACKEND_READY sentinel (which is NOT changed by any of this).
...
def _write_machine_sentinel_line(line: str) -> None:
    ...
    MAX_DESKTOP_READY_FILE channel) for port discovery instead. Never
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'max serve' / 'max dashboard' backend or the Max gateway. "
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``MAX_SERVE_HEADLESS``), so
    ...
    try:
        from max_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing MAX_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Max Agent exposes a
            ...
            if host in _LOOPBACK_HOST_VALUES:
                _public_url_for_msg = ""
                try:
                    from max_cli.dashboard_auth.prefix import (
                ...
                _local_only_hint = (
                    ...
                    "(and unset MAX_DASHBOARD_PUBLIC_URL) to restore the "
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `max dashboard register` (Nous Portal) or "
            ...
            # (#54489).
            try:
                from max_cli.config import load_config as _load_cfg
                from max_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`max plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # MAX_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            try:
                from max_cli.process_identity import reap_orphaned_mcp_helpers
            ...
            # for standalone `max serve` (no MAX_PARENT_PID env).
            ...
            # is what lets `max update` relaunch a manually-started serve
            # on its real endpoint instead of dropping it (#63206).
            try:
                from max_cli.process_identity import (
            ...
            ready_token = "MAX_BACKEND_READY" if headless else "MAX_DASHBOARD_READY"
            ...
            if headless:
                ...
                print(f"  Max backend listening on {host}:{actual_port}", flush=True)
            else:
                print(f"  Max Web UI → http://{host}:{actual_port}")
