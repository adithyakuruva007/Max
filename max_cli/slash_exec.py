...
``CommandDef.execute`` (max_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/max_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``max_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: max bundles create <name> --skill <s1> --skill <s2>\n"
