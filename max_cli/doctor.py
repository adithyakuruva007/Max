...
from max_cli.config import get_project_root, get_max_home, get_env_path
...
load_max_dotenv(max_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-max and dashboard static services are up
    ...
    for static in ("main-max", "dashboard"):
...
def run_doctor(args):
    ...
    if config_path.exists():
        ...
        try:
            import yaml
            from max_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
    ...
    for subdir_name in expected_subdirs:
        subdir_path = max_home / subdir_name
    ...
    memories_dir = max_home / "memories"
    ...
    state_db_path = max_home / "state.db"
    ...
    wal_path = max_home / "state.db-wal"
    if wal_path.exists():
        try:
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "max"
        ...
        _cmd_link = _cmd_link_dir / "max"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(max not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/max → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/max points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/max → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/max — run 'max doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/max exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/max not found",
                    "(max command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/max → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/max symlink — run 'max doctor --fix'")
