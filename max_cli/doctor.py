"""
Doctor command for max CLI.

Diagnoses issues with Max Agent setup.
"""
...
from max_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
MAX_HOME = get_max_home()
_DHH = display_max_home()  # user-facing display path (e.g. ~/.max or ~/.max/profiles/coder)

# Load environment variables from ~/.max/.env so API key checks work
_env_path = get_env_path()
load_hermes_dotenv(hermes_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from max_cli.colors import Colors, color
from max_cli.models import _MAX_USER_AGENT
from max_cli.vercel_auth import describe_vercel_auth
from max_constants import OPENROUTER_MODELS_URL
...
from max_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Max containers"
...
def _hermes_database_paths(hermes_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Max-managed SQLite databases."""
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``max_cli.sqlite_safe_read``).
...
def _report_database_journal_modes(
    ...
    from max_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = hermes_home if hermes_home is not None else MAX_HOME
    try:
        databases = _hermes_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list Max databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.max/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("MAX_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from max_cli.auth import get_minimax_oauth_auth_status
...
from max_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        lines.append((
            ...
            "(stop the listed processes, then run 'max sessions "
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'max sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. MAX_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("MAX_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("MAX_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Max Relay plugin."""
    from max_cli.relay_plugin_cutover import (
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches max_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``max --version`` reporting a stale version while
    ...
    """
    try:
        from max_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != max_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'max update', or set "
            "max_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `max profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `max doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from max_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the MAX_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("MAX_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via MAX_MANAGED_DIR={managed_dir}")
...
def check_macos_tcc_grants() -> None:
    ...
    Post-#73681 builds pin ``designated => identifier "com.stardustresearch.max"``
    ...
    if "cdhash" in dr.lower():
        check_warn(
            ...
            "Run `max update` to get the stable identifier-pinned signing "
            "identity, then re-grant permissions once.",
        )
        return
    if "certificate" in dr.lower():
        # Certificate-anchored DR (max desktop --setup-tcc-identity, or a
        ...
        check_ok(
            ...
            "anchor, see `max desktop --setup-tcc-identity`)",
        )
    check_info(
        "If macOS still re-prompts for permissions (toggle shows ON): the stored "
        "grant is stale — run `tccutil reset ScreenCapture com.stardustresearch.max` "
        "(repeat per affected service), toggle it ON in System Settings, then "
        "fully quit & relaunch Max once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/Max.app``) — the only layout whose
    ...
    ``_desktop_packaged_executable``'s selection. ``/Applications/Max.app``
    is deliberately not probed: it is the separately-signed Max-Setup
    launcher (``com.stardustresearch.max.setup``, certificate-anchored), whose
    grants are stable by construction and unaffected by rebuilds.
    """
    root = Path(__file__).resolve().parents[1]
    release_dir = root / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Max.app") if p.is_dir()]
    if not candidates:
        return None
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    """
    try:
        from max_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    check_info(
        ...
        "app Full Disk Access and Max will never trip per-folder dialogs "
        ...
        "    then enable your terminal (and Max.app if you use Desktop), "
        "and restart them once. With Max' stable signing identities the "
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `max`.
    os.environ.setdefault("MAX_INTERACTIVE", "1")

    # Handle `max doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.max/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Max Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`max doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from max_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Max already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and max_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.max/.env (primary location for user config)
    env_path = MAX_HOME / '.env'
    if env_path.exists():
        ...
        # matches max_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'max setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'max setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'max setup' to create one")
                issues.append("Run 'max setup' to create .env")
    
    # Check ~/.max/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = MAX_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from max_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from max_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'max config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from max_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.max/.env or run 'max setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'max setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'max config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from max_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from max_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'max setup' to migrate config")
                else:
                    issues.append("Run 'max doctor --fix' or 'max setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'max doctor --fix'")
        except Exception:
            pass

        # Detect stale MAX_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives MAX_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from max_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("MAX_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"MAX_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `max setup` run)",
                )
                if should_fix:
                    if remove_env_value("MAX_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale MAX_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove MAX_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the MAX_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale MAX_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'max doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from max_cli.config import load_env as _load_env_depr
            from max_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Max' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Max a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Max Agent Persona\n\n"
                "<!-- Edit this file to customize how Max communicates. -->\n\n"
    ...
    if state_db_path.exists():
        try:
            ...
            from max_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'max doctor --fix' "
                        "(or 'max sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from max_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'max doctor --fix' "
                        "(or 'max sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Max
        ...
        try:
            from max_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'max sessions optimize-storage' "
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'max doctor --fix' to checkpoint")
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    if should_fix:
                        ...
                        issues.append(f"Broken symlink at {_cmd_link_display}/hermes — run 'max doctor --fix'")
                ...
                if should_fix:
                    ...
                    issues.append(f"Missing {_cmd_link_display}/hermes symlink — run 'max doctor --fix'")
    ...
    try:
        from max_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'max-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # — it resolves lazily via npx (or a global/Max-managed install)
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `max update` does, so a session's first browser call
            ...
            # `max update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `max doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable MAX_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Max one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'max config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``max doctor``.
    ...
    # never the legitimate source for `max doctor`.
    ...
    try:
        ...
        if api_disabled or web_not_ready:
            issues.append("Run 'max setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = MAX_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: max skills list)")
    ...
    try:
        from max_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = MAX_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: max memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: max memory setup",
                    "No Honcho API key — run 'max memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run max memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: max memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: max memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from max_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "max -p" in content:
                            _m = _re.search(r"max -p (\S+)", content)
    ...
    # `max doctor --live` (real network calls; bounded + read-only).
    try:
        from max_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'max doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
