...
# load_max_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_max_dotenv().
...
# process.  ``load_max_dotenv()`` is called at module-import time from
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_max_dotenv()`` call.  Returns
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``max_cli.config._sanitize_env_lines`` which
    already knows all valid Max env-var names and can split
...
def load_max_dotenv(
    *,
    max_home: str | os.PathLike | None = None,
    ...
    home_path = Path(max_home or os.getenv("MAX_HOME", Path.home() / ".max"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    ``home_path`` are no-ops.  ``load_max_dotenv()`` runs at import
