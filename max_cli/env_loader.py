"""Helpers for loading Max .env files consistently across entrypoints."""
...
# Used by setup / `max model` flows to label detected credentials so
...
# MAX_HOME paths we've already pulled external secrets for during this
# process.  ``load_hermes_dotenv()`` is called at module-import time from
# several hot modules (cli.py, max_cli/main.py, run_agent.py,
...
def _known_hermes_env_keys() -> set[str]:
    """Return the combined set of known Max env-var keys.
    ...
    """
    from max_cli.config import _EXTRA_ENV_KEYS
    from max_cli.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent Max process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Max keys absent from ``.env``.
    ...
    parent Max process injects and that silently change *which provider
    ...
    ``tests/max_cli/test_dump_env_visibility.py``), and a startup scrub
...
def reset_secret_source_cache() -> None:
    """Forget which MAX_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `max setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``max_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from max_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    """Load Max environment files with user config taking precedence.

    Behavior:
    - `~/.max/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("MAX_HOME", Path.home() / ".max"))
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Max keys
    ...
    #   EnvironmentFile=-/path/to/.max/.op.env
    ...
    # 2. A fresh ``max update`` retry just completed a deferred dependency
    ...
    # TERMINAL_ENV=docker left in ~/.max/.env (e.g. written by an older
    # `max setup` before the user switched terminal.backend in config.yaml)
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``max_cli.config.apply_terminal_config_to_env`` — the
    ...
    Scoped to the process MAX_HOME: the shared bridge reads the
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of MAX_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Max reads
    ...
    owns the once-per-MAX_HOME guard, the post-apply ASCII
    ...
    time from several hot modules (cli.py, max_cli/main.py,
    ...
    # a generic dict entry must not force crypto load on every max launch.
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `max model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `max` startup, so populating the
    # shared cache here lets main.py's early bridge and max_logging reuse
...
def _process_hermes_home() -> Path:
    """The MAX_HOME the shared config cache is keyed to."""
    try:
