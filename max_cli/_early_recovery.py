"""Dependency-light venv recovery that runs BEFORE max_cli.main's imports.

The ``max`` console entry point is ``max_cli.main:main``.  Importing
``max_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``max_cli.env_loader``, ``yaml`` via ``max_cli.config``, ...).  In
...
a corrupted venv.  ``max_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``max_cli.main`` to become
...
#
# ``max update`` renames the live ``hermes*.exe`` shims aside
...
# no ``max`` on PATH, and the command that would repair it IS ``hermes
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    invocation, and ``max acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Max already vendors
    (``~/.max/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".max" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``max acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``max_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``max_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``max update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `max update`: CLI and Desktop retries preserve
...
# every `max` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``max update`` (or the self-lock preflight, #83569) left the
    ...
    ``max_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``max acp`` included).
    ...
    try:
        ...
        try:
            ...
            print(
                "  The next launch will retry; max will keep working from "
