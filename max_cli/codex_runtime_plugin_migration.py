"""Migrate Max' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Max' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Max mcp_servers.<n>.command/args/env  → codex stdio transport
  Max mcp_servers.<n>.url/headers       → codex streamable_http transport
  Max mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Max mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Max-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Max' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Max config.")
...
# Max keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Max' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, max_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Max MCP server config to the codex inline-table dict
    ...
    has_command = bool(max_cfg.get("command"))
    has_url = bool(max_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(max_cfg["command"])
        args = max_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = max_cfg.get("env") or {}
        if env:
        ...
        out["url"] = str(max_cfg["url"])
        headers = max_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Max' transport: sse hint is informational; codex auto-negotiates
        if max_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in max_cfg:
        try:
            out["tool_timeout_sec"] = float(max_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in max_cfg:
        try:
            out["startup_timeout_sec"] = float(max_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if max_cfg.get("enabled") is False:
        out["enabled"] = False
    ...
    for key in max_cfg:
        ...
        elif key not in _KNOWN_MAX_KEYS:
            skipped.append(f"{key} (unknown Max key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (MAX_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Max)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Max' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Max' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Max
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="max-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a MAX_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed max-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.max`` symlink that happens to live under ``/private/var/folders``
...
def _build_max_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Max' own
    ...
    so a max installed under /opt/, /usr/local/, or a venv all work.
    MAX_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # MAX_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_max_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever MAX_HOME its launcher sets
    ...
    # — that would override the launcher's MAX_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("MAX_HOME", tmp_path) would
    ...
    max_home = os.environ.get("MAX_HOME") or ""
    if max_home and _looks_like_test_tempdir(max_home):
        max_home = ""
    if max_home:
        env["MAX_HOME"] = max_home
    # PYTHONPATH passes through so a worktree-launched max finds the
    ...
    env["MAX_QUIET"] = "1"
    env["MAX_REDACT_SECRETS"] = env.get("MAX_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_max_tools: bool = True,
) -> MigrationReport:
    """Translate Max mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        max_config: full ~/.max/config.yaml dict
    ...
    max_servers = (max_config or {}).get("mcp_servers") or {}
    if not isinstance(max_servers, dict):
        report.errors.append(
            "mcp_servers in Max config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in max_servers.items():
    ...
    # Inject Max' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Max for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/max_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_max_tools:
        translated["max-tools"] = _build_max_tools_mcp_entry()
        if "max-tools" not in report.migrated:
            report.migrated.append("max-tools")
