"""``max approvals suggest`` — mine approval history into allowlist proposals.

Max has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.max/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, max self-management, …) are eligible.
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  max approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.max/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``max approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``max approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from max_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: max approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `max approvals <subcommand> -h` for details."
    )
    return 1
