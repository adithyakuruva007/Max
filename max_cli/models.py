...
_MAX_USER_AGENT = f"max-cli/{_MAX_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from max_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $MAX_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Max refreshes the cache from
...
# path. The *interactive* default (GUI onboarding / ``max model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in max_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.stardustresearch.com/v1
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``max model`` shows.
