"""Runtime-backed validation behind ``max plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary MAX_HOME
    ...
    stack.enter_context(
        patch.dict(
            ...
            {
                "MAX_HOME": str(home),
                "MAX_BUNDLED_PLUGINS": str(bundled),
                "MAX_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Max discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_max_home() / "plugins"
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".max" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from max_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Max supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Max never auto-installs plugin dependencies; "
    ...
    if schema:
        from max_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Max' real scanner and registration path."""
    try:
