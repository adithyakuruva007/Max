...
configured for "cli" in `max tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (MAX_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `max chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"max -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "max -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"max -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "max -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "max -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to MAX_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("MAX_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "max -z: --provider requires --model (or MAX_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"max -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("max -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``max -z`` / oneshot mode.

    Oneshot bypasses ``MaxCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when max is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from max_cli.config import load_config
    from max_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("MAX_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from max_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("MAX_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # MaxCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    from max_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                MAX_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via MAX_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via MAX_YOLO_MODE=1
