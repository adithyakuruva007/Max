...
from max_cli.archive_safe import (
    archive_root_dirs,
    make_targz,
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Max home
    (:func:`max_constants.get_max_home`).  Used by single-profile
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    try:
        ...
        source = get_max_home() / "config.yaml"
        ...
        # Creation must not fail over this; `max model` still sets it later.
        pass
...
def profiles_to_serve(
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own MAX_HOME. When
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    only the essential skills (e.g. ``max-agent``), reporting
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    hermes_markers = ("max_cli.main", "hermes-gateway", "tui_gateway")
    ...
    # interpreter.
    _MAX_CONSOLE_SCRIPT_NAMES = frozenset({"hermes", "max-agent", "max-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Max process: either an entrypoint marker in argv, a
            # resolved executable named `max`, or a python interpreter
            # directly exec'ing a `max`-named console-script shim (argv[0]
            ...
            if not is_hermes and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                ...
                # (pyproject.toml [project.scripts]: hermes, max-agent,
                # max-acp) rather than a bare `startswith("hermes")` --
                ...
                is_hermes = script_stem in _MAX_CONSOLE_SCRIPT_NAMES
            if not is_hermes:
                continue
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``max sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``MAX_REDACT_SECRETS`` — share archives
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    When MAX_HOME is already set, the configured spelling IS the
