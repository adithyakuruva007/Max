...
Usage:
    ...
    max --version           Show version and update status
...
from max_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python max_cli/main.py`` (script
# mode — sys.path[0] is max_cli/, not the repo root) can import the
# canonical helpers; everything else lives in max_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from max_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``max update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from max_cli.env_loader import ...`` / ``from max_cli.config import
...
# package directory, so if IT can't import, nothing else in max_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from max_cli import _early_recovery as _early_recovery_mod
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``MAX_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``max … chat -q`` with stdio on a pipe. This
...
def _try_ultrafast_version() -> bool:
    """Handle ``max --version`` before config/logging imports."""
...
from max_cli.subcommands.sync import build_sync_parser
...
from max_cli.subcommands.pause import build_pause_parser
...
from max_cli.subcommands.verify import build_verify_parser
...
from max_cli.subcommands.import_agent import build_import_agent_parser
from max_cli.subcommands.config import build_config_parser
from max_cli.subcommands.skin import build_skin_parser
...
from max_cli.subcommands.monitoring import build_monitoring_parser
...
def _apply_profile_override() -> None:
    ...
    from max_cli._parser import top_level_value_flag_sets
    ...
    # See issue #22502.
    hermes_home_env = os.environ.get("MAX_HOME", "")
    ...
    if profile_name is not None:
        try:
            ...
            return
        os.environ["MAX_HOME"] = hermes_home
...
# Windows launcher self-heal — the ``max`` command users run is a COPY of
# the venv console script, staged into the managed binary dir (the default
# Max root's ``bin``, next to the managed uv) by install.ps1. That dir
# lives OUTSIDE the git checkout precisely because an earlier layout staged
# the copies at ``<checkout>\bin``, where ``max update``'s autostash
# (``git stash push --include-untracked``) swept them off disk; with the
# desktop updater's ``--keep-stash`` nothing restored them and ``max``
...
# ``python -m max_cli.main``. Costs a few stat calls when healthy; gates
# fail toward inaction so source checkouts are untouched. Sits AFTER the
# profile override on purpose — no max module may be imported before
# profiles resolve. The launcher dir itself is per-machine (the helper
# anchors on the DEFAULT root, not MAX_HOME), so profile sessions heal
# the same shared dir.
if sys.platform == "win32":
    try:
        from max_cli import _install_repair as _install_repair_mod
...
def _relative_time(ts) -> str:
    ...
    in :mod:`max_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from max_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_default, dict):
            from max_cli.config import split_model_config_default
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from max_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``max -c`` from repo A continues repo A's last session rather than the
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'max sessions list' to see available sessions, or "
            ...
            try:
                from max_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _ensure_tui_node() -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "MAX_HOME": hermes_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        if bin == "node":
            ...
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $MAX_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Max installed,
    ...
    # max_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from max_constants import with_hermes_node_path
        ...
        if result.returncode != 0:
            ...
            # here before doing any work; repair once (upgrade a Max-managed
            ...
            from max_cli.npm_engine import maybe_repair_npm_engine
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("MAX_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["MAX_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("MAX_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["MAX_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("MAX_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["MAX_CWD"]) / python_path
...
def _launch_tui(
    ...
    # TUI child is a max process: propagate the profile-home contract via
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``max … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    # Code / Codex CLI) and resume the newly created Max session.
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        print(f"  (later: max --resume {_imported_id})")
    ...
    if _termux_should_prefetch_update_check():
        try:
            from max_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    #
    # FIRST RUN is the exception: with an empty ~/.max/skills the banner
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from max_cli.config import get_max_home
            skills_dir = Path(get_max_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import max_cli.banner as _banner_mod
...
def cmd_whatsapp(args):
    ...
    from max_constants import find_node_executable, with_hermes_node_path
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            ...
            pass
    from max_cli.setup import run_setup_action_with_navigation
...
def select_provider_and_model(args=None):
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``max model``
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``max model`` flow, then route aux tasks
    to them here.
    """
    from max_cli.config import load_config
    from max_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
# surface so argparse wiring and test monkeypatches on max_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `max` invocation, including fast paths like `max --version`
...
_LAZY_COMMAND_EXPORTS = {
    "max_cli.sessions_cmd": (
        "cmd_sessions",
    ),
    "max_cli.dashboard_procs": (
...
def _self():
    ...
    keeps monkeypatches on max_cli.main.<name> working, exactly like a
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.max/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `max sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set MAX_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "MAX_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="max sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="max sync now")
...
def cmd_verify(args):
    ...
    from max_cli.verify_cmd import run_verify_command
...
def cmd_skin(args):
    ...
    from max_cli.skin_cmd import skin_command
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # `max --version` pre-import fast path (the `version` subcommand
...
# Update pipeline lives in max_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on max_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in MAX_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``max update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``max update``
    ...
    ``max`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``max update``
    ...
    ``max_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $MAX_HOME."""
    from max_constants import get_max_home
    return get_max_home() / "web-ui-build-stamp.json"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    # of a Max-managed install, or a freshly provisioned managed npm when
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "max_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # as `max update`'s _update_node_dependencies() (ui-tui + web +
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Max.app/Contents/MacOS/Max → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``max desktop`` skips the rebuild that would fix
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Max.exe
...
# Max.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Max.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Max.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 max-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Max.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Max.exe from backup.")
        print("    Your existing version was kept and still works. Run `max desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `max desktop --force-build` to rebuild, or re-run the Max")
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Max.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Max is damaged and can't be opened" — and macOS TCC forgets every
...
def _desktop_macos_setup_tcc_identity(identity: str = "Max Local Signing") -> bool:
    """Create/import a self-signed code-signing cert and configure Max to use it.

    One-shot setup for ``max desktop --setup-tcc-identity``. Creates a
    ...
    # Point Max at the identity (config.yaml, not .env — it's not a secret).
    try:
        from max_cli.config import set_config_value
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.stardustresearch.max"
    )
    return True
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `max desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the MAX_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Max Desktop (Linux only, best-effort).
    ...
    shell. ``max uninstall --gui`` removes it.
    """
    try:
        from max_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    # `MAX_DESKTOP_DISABLE_GPU=... max desktop` and
    # `ELECTRON_OZONE_PLATFORM_HINT=... max desktop` keep working.
    ...
    # `MAX_DESKTOP_PASSWORD_STORE=... max desktop` keeps working.
    if sys.platform == "linux" and "MAX_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["MAX_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Max Local Signing"
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Max-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → max-setup →
            # max update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `max update` path).
            ...
            if not source_mode:
                ...
                # success on a Max.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Max shows up
...
# Dashboard process-hygiene helpers live in max_cli/dashboard_procs.py
...
# max_cli.main.<name> keep resolving unchanged.
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Max installs Linux services in the
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but max launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``max update``.
    ...
    log_path = get_max_home() / "logs" / "dashboard-restart.log"
    try:
...
# Install-scoped breadcrumbs live next to the venv (not under $MAX_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``max update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        from max_cli import _install_repair as _ir
        ...
        if self_locked:
            print(
                "  Max is still running from the launcher that needs "
                "replacing. Close other Max windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``max ...`` command holds its own shim, and an editable install run
...
_UPDATE_REEXEC_ENV = "MAX_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    The child re-runs ``max update``, so the whole remaining flow — the
    ...
    cmd = [str(python_exe), "-m", "max_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from max_constants import project_venv_dir
...
def _venv_scripts_dir() -> Path | None:
    ...
    from max_constants import project_venv_dir, venv_bin_dir
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    names = set(_load_console_script_names()) or {"hermes", "max-agent", "max-acp"}
...
def _quarantine_running_hermes_exe(
    ...
    when ``max update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Max Desktop / gateway / REPL).
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Max versions queued for our shims.

    Max used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    update; a failed *restore* leaves the install with no ``max`` on PATH,
...
def _run_quarantined_install(
    ...
    *,
    ...
    try:
        ...
        # quarantined aside and `max` missing from PATH after a green
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    Called early on every max invocation. Two cases the old unconditional
    ``unlink()`` got wrong, both ending with ``max`` gone from PATH:
    ...
    2. **Concurrency.** A fresh quarantine file may belong to an update in
       ...
       ``max update`` does exactly this). Leave anything inside the grace
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `max update`:")
    print(
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    """
    from max_constants import venv_bin_dir
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    ``hermes.exe`` shim is locked during ``max update``, or when uv/distlib
    skips a launcher write. The symptom is ``max-agent.exe`` and
    ``max-acp.exe`` present but ``hermes.exe`` missing, so ``max`` drops
...
def cmd_update(args):
    ...
    if getattr(args, "plan", False):
        ...
        # Max runtime across profiles, its supervisor, and its running
    ...
    # exit 1 errors).
    from max_cli.update_contract import (
    ...
    from max_cli.update_lock import (
    ...
    try:
        ...
        try:
            from max_cli.update_receipt import finalize_pending_update_receipt
...
def cmd_profile(args):
    ...
    if action is None:
        ...
        from max_cli.profiles import format_profile_label
    ...
    if action == "list":
        ...
        from max_cli.profiles import normalize_profile_name, rename_profile
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Max {data['hermes_requires']}")
        if data.get("source"):
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No max dashboard or serve processes running.")
        return 0

    print(f"{len(live)} max dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    provider on the spot — or point them at ``max dashboard register`` for
    OAuth.
    ...
    print("    [2] OAuth via Nous Portal (run `max dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    max dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "MAX_DASHBOARD_OAUTH_CLIENT_ID into ~/.max/.env for you.\n"
            "  Docs: https://max-agent.stardustresearch.com/docs/"
    ...
    try:
        from max_cli.config import load_config, save_config
        from max_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from max_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.max/desktop-ssh: a
    # literal "~/.max/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of MAX_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_max_home(): a non-default sticky profile (or any MAX_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_max_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".max" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``MAX_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``max dashboard`` that
...
def cmd_dashboard(args):
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with max serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with MAX_DESKTOP=1 plus
    # MAX_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # MAX_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `max dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited MAX_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # MAX_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (MAX_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets MAX_SERVE_HEADLESS below.
    if os.environ.get("MAX_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("MAX_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("MAX_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("MAX_SERVE_HEADLESS", None)
    ...
    if (
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: MAX_HOME is explicitly pinned to the
        ...
        try:
            from max_constants import get_default_hermes_root
            env["MAX_HOME"] = str(get_default_hermes_root())
        except Exception:
    ...
    from max_cli.resource_limits import apply_nofile_soft_limit
    ...
    if _headless_backend:
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom MAX_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "MAX_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # MAX_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["MAX_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ MAX_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset MAX_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads MAX_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["MAX_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from MAX_WEB_DIST: {_dist_root}")
...
def cmd_dashboard_register(args):
    ...
    from max_cli.dashboard_register import cmd_dashboard_register as _impl
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``max <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``max photon``
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that MAX_YOLO_MODE is set before ANY
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from max_cli.plugins import start_background_plugin_discovery
...
def _try_fast_chat_launch() -> bool:
    ...
    ``max`` / ``max -w -s foo --yolo`` / ``max chat`` don't need the
    ...
    """
    if os.environ.get("MAX_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: max-agent[otlp])")
...
def _cmd_skills_trust(args):
    """``max skills trust [path]`` / ``max skills untrust [path]``.
...
def _advertise_agent_env() -> None:
    ...
    (``max-agent`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``MAX_AGENT`` is the Max-specific marker. setdefault: never
    clobber an outer harness (e.g. Max running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "max-agent")
    os.environ.setdefault("MAX_AGENT", "true")
...
def main():
    ...
    # If the checkout changed since the last launch (max update, manual
    ...
    worktree_parser = subparsers.add_parser(
        ...
        description=(
            "Attended reclaim for the .worktrees/ directory max -w sessions "
            "accumulate. Never deletes uncommitted tracked changes, unique "
            "unpushed commits, or in-use trees; untracked-only scratch is "
            "archived to ~/.max/archive/worktree-prune/ before removal. See: "
            "https://max-agent.stardustresearch.com/docs/user-guide/cli#worktree-cleanup"
        ),
    )
    ...
    def _dispatch_worktree(_args):
        from max_cli.worktree_cmd import cmd_worktree
    ...
    browser_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "default profile so Max can copy it — DESTRUCTIVE (unsaved tabs "
    ...
    def _dispatch_browser(_args):
        from max_cli.browser_connect import (
            UNSUPPORTED_CHANNEL,
    ...
    # `max proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://max-agent.stardustresearch.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from max_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``max proxy`` subparser (dest='proxy_command').
    ...
    # =========================================================================
    # pause / resume commands  (parser built in max_cli/subcommands/pause.py)
    # =========================================================================
    ...
    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Max gateways)
    # =========================================================================
    from max_cli.subcommands.peer import build_peer_parser
    ...
    # =========================================================================
    from max_cli.portal_cli import add_parser as _add_portal_parser
    ...
    # =========================================================================
    # verify command  (parser built in max_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # approvals command  (parser built in max_cli/subcommands/approvals.py)
    # =========================================================================
    ...
    # =========================================================================
    # import-agent command  (parser: max_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from max_cli.agent_import import import_agent_command
    ...
    # =========================================================================
    # skin command  (parser built in max_cli/subcommands/skin.py)
    # =========================================================================
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import os as _os
            import subprocess
            from max_cli.tools_config import _cua_driver_contract_status
            ...
            override = _os.environ.get("MAX_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by MAX_CUA_DRIVER_CMD, or unset "
                            "the override and run: max computer-use install --upgrade"
                        )
                    else:
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Max",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Max session store "
            "so it can be resumed with 'max --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in max_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on max_cli.main.cmd_sessions keep working.
    ...
    # NOTE: the `max version` subcommand was removed — `max --version`
    ...
    # --yolo: set MAX_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # ``max egress start`` refusing when credential_source=bitwarden
