#!/usr/bin/env python3
"""
Max CLI - Main entry point.

Usage:
    max                     # Interactive chat (default)
    max chat                # Interactive chat
    max gateway             # Run gateway in foreground
    max gateway start       # Start gateway as service
    max gateway stop        # Stop gateway service
    max gateway status      # Show gateway status
    max gateway install     # Install gateway service
    max gateway uninstall   # Uninstall gateway service
    max setup               # Interactive setup wizard
    max logout              # Clear stored authentication
    max status              # Show status of all components
    max cron                # Manage cron jobs
    max cron list           # List cron jobs
    max cron status         # Check if cron scheduler is running
    max doctor              # Check configuration and dependencies
    max honcho setup                    # Configure Honcho AI memory integration
    max honcho status                   # Show Honcho config and connection status
    max honcho sessions                 # List directory → session name mappings
    max honcho map <name>               # Map current directory to a session name
    max honcho peer                     # Show peer names and dialectic settings
    max honcho peer --user NAME         # Set user peer name
    max honcho peer --ai NAME           # Set AI peer name
    max honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    max honcho mode                     # Show current memory mode
    max honcho mode [hybrid|honcho|local]  # Set memory mode
    max honcho tokens                   # Show token budget settings
    max honcho tokens --context N       # Set session.context() token cap
    max honcho tokens --dialectic N     # Set dialectic result char cap
    max honcho identity                 # Show AI peer identity representation
    max honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    max honcho migrate                  # Step-by-step migration guide: OpenClaw native → Max + Honcho
    max version             Show version
    max update              Update to latest version
    max uninstall           Uninstall Max Agent
    max acp                 Run as an ACP server for editor integration
    max sessions browse     Interactive session picker with search

    max claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: max_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``max_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``max update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``max_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, max crashes on import and the user can't run
# ``max update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import max_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'max' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``max tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``max.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("max")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"max", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"max")
        # Windows: the .exe name is already ``max.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* max_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("MAX_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".max", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``MAX_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("MAX_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``MAX_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("MAX_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`max --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from max_cli import __release_date__, __version__

    print(f"Max Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``max --version`` before config/logging imports on Termux."""
    if os.environ.get("MAX_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from max_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from max_cli.subcommands.cron import build_cron_parser
from max_cli.subcommands.gateway import build_gateway_parser
from max_cli.subcommands.profile import build_profile_parser
from max_cli.subcommands.model import build_model_parser
from max_cli.subcommands.setup import build_setup_parser
from max_cli.subcommands.postinstall import build_postinstall_parser
from max_cli.subcommands.whatsapp import build_whatsapp_parser
from max_cli.subcommands.slack import build_slack_parser
from max_cli.subcommands.login import build_login_parser
from max_cli.subcommands.logout import build_logout_parser
from max_cli.subcommands.auth import build_auth_parser
from max_cli.subcommands.status import build_status_parser
from max_cli.subcommands.webhook import build_webhook_parser
from max_cli.subcommands.hooks import build_hooks_parser
from max_cli.subcommands.doctor import build_doctor_parser
from max_cli.subcommands.security import build_security_parser
from max_cli.subcommands.dump import build_dump_parser
from max_cli.subcommands.debug import build_debug_parser
from max_cli.subcommands.backup import build_backup_parser
from max_cli.subcommands.import_cmd import build_import_cmd_parser
from max_cli.subcommands.config import build_config_parser
from max_cli.subcommands.console import build_console_parser
from max_cli.subcommands.version import build_version_parser
...
from max_cli.subcommands.dashboard import build_dashboard_parser
from max_cli.subcommands.gui import build_gui_parser
from max_cli.subcommands.logs import build_logs_parser
from max_cli.subcommands.prompt_size import build_prompt_size_parser
from max_cli.subcommands.memory import build_memory_parser
from max_cli.subcommands.acp import build_acp_parser
from max_cli.subcommands.tools import build_tools_parser
from max_cli.subcommands.insights import build_insights_parser
from max_cli.subcommands.skills import build_skills_parser
from max_cli.subcommands.pairing import build_pairing_parser
from max_cli.subcommands.plugins import build_plugins_parser
from max_cli.subcommands.mcp import build_mcp_parser
from max_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (max tools, max setup, max model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'max {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any max module import.
#
# Many modules cache MAX_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("MAX_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.max/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set MAX_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `max mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Max' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo max -p <name>` against the invoking user's home.
        ...
        candidate = home / ".max" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`max chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors max_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If MAX_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.max/profiles/coder or
    # /opt/data/profiles/coder).  If MAX_HOME points to the max root
    # instead (e.g. systemd hardcodes MAX_HOME=/root/.max), we must
    # still read active_profile — the user may have switched profiles via
    # `max profile use` and the gateway should honour that choice.
    # See issue #22502.
    max_home_env = os.environ.get("MAX_HOME", "")
    if profile_name is None and max_home_env:
        if Path(max_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the max root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as MAX_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``max gateway run`` to mean
    # "the root MAX_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("MAX_S6_SUPERVISED_CHILD"):
        try:
            from max_constants import get_default_max_root

            active_path = get_default_max_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set MAX_HOME
    if profile_name is not None:
        try:
            from max_cli.profiles import resolve_profile_env

            max_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            max_home = _resolve_sudo_user_profile_env(profile_name)
            if not max_home:
            ...
            # A bug in profiles.py must NEVER prevent max from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["MAX_HOME"] = max_home
...
# Load .env from ~/.max/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from max_cli.config import get_max_home
from max_cli.env_loader import load_max_dotenv

load_max_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → MAX_REDACT_SECRETS env
# var BEFORE max_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_max_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "MAX_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["MAX_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `max` subcommands
...
try:
    from max_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from max_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if max_constants not importable yet
...
from max_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to max_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (max_cli.main._model_flow_*) keep resolving unchanged.
from max_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_max_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("MAX_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("MAX_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from max_cli.config import get_env_path, get_max_home, load_config
    from max_cli.auth import get_auth_status

    # Determine whether Max itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_max_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_max_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Max has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Max to use their tokens.
    if _has_max_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, max_bin
        cli_args: the original CLI arguments (everything after 'max')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    max_bin = container_info["max_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo max {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo max {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  max --tui --resume {target}")
    if title:
        print(f'  max --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @max/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@max" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/max-ink/package.json",
    "packages/max-ink/index.js",
    "packages/max-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``MAX_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("MAX_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``MAX_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("MAX_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    max_home = os.environ.get("MAX_HOME") or str(Path.home() / ".max")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "MAX_HOME": max_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(max_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(max_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if max_cli_dir is None:
        max_cli_dir = Path(__file__).parent
    bundled = max_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``max update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("MAX_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Max checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `max update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Max checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `max --tui`\n"
        "If the checkout is still inconsistent, run `max update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (MAX_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("MAX_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from max_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("MAX_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with MAX_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset MAX_TUI_DIR (e.g. `unset MAX_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @max/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @max/ink resolves through
        # packages/max-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "max-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from max_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from max_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="max-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["MAX_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["MAX_PYTHON_SRC_ROOT"] = os.environ.get(
        "MAX_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("MAX_PYTHON", sys.executable)
    env.setdefault("MAX_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["MAX_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["MAX_MODEL"] = model
        env["MAX_INFERENCE_MODEL"] = model
    if provider:
        env["MAX_TUI_PROVIDER"] = provider
        env["MAX_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["MAX_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["MAX_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["MAX_TUI_SKILLS"] = value
    if query:
        env["MAX_TUI_QUERY"] = query
    if image:
        env["MAX_TUI_IMAGE"] = image
    if checkpoints:
        env["MAX_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["MAX_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["MAX_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["MAX_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["MAX_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["MAX_ACCEPT_HOOKS"] = "1"
    ...
    # MAX_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `max --tui` try to
    ...
    # still set MAX_TUI_RESUME themselves.
    env.pop("MAX_TUI_RESUME", None)
    if resume_session_id:
        env["MAX_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `max update` so
    ...
    if code == 42:
        from max_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``MAX_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``max kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``max kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("MAX_KANBAN_BOARD"):
        return
    try:
        from max_cli.kanban_db import get_current_board

        os.environ["MAX_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.max/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Max — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Max still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``MAX_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("MAX_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'max sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        ...
        from max_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'max doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Max isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  max setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'max setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from max_cli.banner import prefetch_update_check
    ...
    # user's ~/.max/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["MAX_SESSION_SOURCE"] = args.source
...
def cmd_proxy(args):
    ...
    from max_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from max_cli.config import get_env_value, save_env_value
    from max_constants import find_node_executable, with_max_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Max?")
        print()
    ...
    # has no creds.json.  Every subsequent `max gateway` then paid a 30s
    ...
    session_dir = get_max_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: max gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `max gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  max gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Max Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  max gateway")
        ...
        print()
        print("  Or install as a service: max gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'max whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``max whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``max_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from max_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_postinstall(args):
    ...
    from max_cli.config import stamp_install_method
    from max_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Max post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from max_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``max model``) and the setup wizard
    ...
    from max_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("MAX_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from max_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'max model' for "
                "available providers, or run 'max doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in max_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.max/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.max/.env if the active provider is not 'custom'.
    ...
    """
    from max_cli.config import get_env_value, save_env_value, load_config
...
#
# Max uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `max model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `max model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`max_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from max_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Max only falls back to a lightweight backend (OpenRouter)")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``max model`` flow, then route aux tasks to them here.
    """
    from max_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from max_cli.auth import _prompt_model_selection
    from max_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from max_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from max_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from max_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Max URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# downstream call sites read `max_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `max` invocation — including
# fast paths like `max --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from max_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``max setup`` / ``max model``.
    ...
    recover from a malformed paste without editing ``~/.max/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from max_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from max_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `max setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Max will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Max CLI with a provider."""
    from max_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from max_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from max_cli.auth_commands import auth_command
...
def cmd_cron(args):
    """Cron job management."""
    from max_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from max_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``max slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: max slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `max slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from max_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from max_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from max_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from max_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from max_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `max security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from max_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from max_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from max_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from max_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Max home directory to a zip file."""
    if getattr(args, "quick", False):
        from max_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from max_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Max backup from a zip file."""
    from max_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from max_cli.config import detect_install_method
    from max_cli.banner import format_banner_version_label
    ...
    try:
        from max_cli.banner import check_for_updates
        from max_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Max Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from max_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from max_cli.uninstall import run_gui_uninstall
    ...
    from max_cli.uninstall import run_uninstall
...
# Critical files that Max must be able to import immediately after an
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``max`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="max-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``max update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``max_cli/web_dist/`` (per vite.config.ts
    outDir: "../max_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "max_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``max`` launcher present but ``max_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the max venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — max venv python3, no nix-shell overhead
    ...
    # a max venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``max update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``max web``).
    ...
    # build path usable on a stock `py -m max_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (max web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``max update`` can unconditionally call ``max desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``max desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $MAX_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $MAX_HOME."""
    from max_constants import get_max_home
    return get_max_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``max update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Max.app/Contents/MacOS/Max"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Max.exe",
            release_dir / "win-ia32-unpacked" / "Max.exe",
            release_dir / "win-arm64-unpacked" / "Max.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "max",
            release_dir / "linux-unpacked" / "Max",
            release_dir / "linux-arm64-unpacked" / "max",
            release_dir / "linux-arm64-unpacked" / "Max",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Max'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Max.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Max.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Max.exe: Access is
    ...
    an unrelated "Max" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Max is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Max.app/Contents/MacOS/Max  ->  app bundle = .../Max.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``max desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Max Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Max Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the MAX_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from max_logging import setup_logging as _setup_logging_gui
    ...
    # with_max_node_path() copies os.environ when called with no arg.
    env = with_max_node_path()
    if getattr(args, "fake_boot", False):
        env["MAX_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["MAX_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "max_root", None):
        env["MAX_DESKTOP_MAX_ROOT"] = str(Path(args.max_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["MAX_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["MAX_DESKTOP_CWD"] = os.getcwd()
    ...
    # `MAX_DESKTOP_DISABLE_GPU=... hermes desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "MAX_DESKTOP_DISABLE_GPU" not in os.environ:
        env["MAX_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  max gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Max.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Max.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Max.exe, close any")
                    print("  running Max desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> max desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Max is
    ...
    if source_mode:
        print("→ Launching Max Desktop from source build...")
    ...
    print(f"→ Launching packaged Max Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``max dashboard`` processes other than ourselves.

    ``max dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``max update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Max Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``max serve`` as
    ...
    environment variable ``MAX_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "max dashboard",
        "max_cli.main dashboard",
        "max_cli/main.py dashboard",
        # The headless backend (`max serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "max serve",
        "max_cli.main serve",
        "max_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "max.*dashboard"`) keeps us consistent
            # with `max_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `max update`.
    ...
    print("  Preview now:  max curator run --dry-run")
    print("  Pause it:     max curator pause")
    print(
        "  Docs:         https://max-agent.stardustresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``max update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``max update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: max curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``max dashboard`` processes.

    Called at the end of ``max update`` (default ``reason``) and also
    from ``max dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Max Desktop Electron app spawns this dashboard as a
    # backend child, it sets MAX_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("MAX_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.max-update-staging"
    backup = f"{dst}.max-update-old"
...
def _update_via_zip(args):
    """Update Max Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `max update "
            f"--branch {branch}`, or update against main with `max update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/StardustResearch/max-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="max-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"max-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for max-agent itself
        ...
        # GitHub ZIPs extract to max-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"max-agent-{branch}")
    ...
    from max_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: max skills list-modified  "
    ...
    try:
        from max_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "max-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Max behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes max completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Max couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Max couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Max behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Max couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Max couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `max update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/StardustResearch/max-agent.git",
    "git@github.com:StardustResearch/max-agent.git",
    "https://github.com/StardustResearch/max-agent",
    "git@github.com:StardustResearch/max-agent",
}
OFFICIAL_REPO_URL = "https://github.com/StardustResearch/max-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_max_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_max_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Max repository.")
        print("  This means you may miss updates from StardustResearch/max-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/StardustResearch/max-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/StardustResearch/max-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``max update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from max_constants import get_default_max_root

    default_home = get_default_max_root()
...
# Install-scoped breadcrumb dropped right before ``max update`` mutates the
...
# survives and the next ``max`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $MAX_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``max update`` left half-done.
    ...
    protocol stream (``max acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if max.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _max_exe_shims(scripts_dir)
            if shims:
    ...
    try:
        ...
        print(
            "⚠ A previous `max update` was interrupted mid-install — "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``max update`` is still progressing even if pip/uv itself is silent.
    """
...
def _max_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_max_instances(
    ...
    The Max Desktop Electron app spawns ``max.EXE`` as a backend child,
    so during ``max update`` the user-invoked process and the desktop's
    ...
    shims (``max.exe`` / ``max-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``max update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``max.exe``) is a separate process from the Python
    ...
    for shim in _max_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated max.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``max update``
    ...
    #      max.exe sitting *under* a non-Max parent (e.g. a Max
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "max.exe"
    lines = ["✗ Another max.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Max Desktop, exit any open `max` REPLs, and")
    lines.append("  stop the gateway (`max gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `max update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_max_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``max.exe``.
    ...
    when ``max update`` runs, ``max.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``max.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next max invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Max Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Max Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _max_exe_shims(scripts_dir):
        ...
        print(
            "    Close Max Desktop, exit other `max` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `max update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_max_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``max.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``max.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``max`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_max_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``max.exe.old.*`` left by prior updates.

    Called early on every max invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``max update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `max update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``max.exe`` / ``max-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_max_exe`` for the rationale.
    """
    ...
    # hours later inside ``max update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``max.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``max.exe`` shim is locked during ``max update``, or when uv/distlib
    skips a launcher write. The symptom is ``max-agent.exe`` and
    ``max-acp.exe`` present but ``max.exe`` missing, so ``max`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `max update --force` after "
            "closing other max processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m max_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``max update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``max.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``max`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `max update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `max update --force` after closing other max processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from max_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$MAX_HOME/bin/uv``, but on Termux the official
    ...
    """
    from max_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `max update`, so we install root-only first
    ...
    nixos_env = with_max_node_path(_nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `max update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``max update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.max/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``max update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``max update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.max/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``max update --gateway``) the update is already
    ...
    try:
        ...
        # max_cli.config.get_max_home to simulate setup failure.
        from max_cli.config import get_max_home as _get_max_home

        logs_dir = _get_max_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.max/logs/update.log`` only, never the terminal.

    During ``max update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``max update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``max update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from max_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from max_cli.config import recommended_update_command
        from max_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``max update`` without requiring a reinstall.  The
    ...
    ``max`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/max.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v max'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/max, code at /usr/local/lib/max-agent).
    fhs_link = Path("/usr/local/bin/max")
    ...
    path_comment = (
        "# Max Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of MAX_HOME before running the update.
    ...
    MAX_HOME directories.  The ``--backup`` flag on ``max update``
    ...
    # ``max_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # MAX_HOME can add minutes to every update. Users who want the
    ...
    try:
        from max_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_max_home so the user sees ~/.max/...
    try:
        from max_constants import get_max_home, display_max_home

        home = get_max_home()
        try:
            display_path = f"{display_max_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  max import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``max update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``max update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run max from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.max-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".max-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The max.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m max_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``max
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Max processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Max Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Max desktop app / other Max terminals, then re-run:"
    )
    lines.append("    max update")
    lines.append("  (or use `max update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    max.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Max...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m max_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: max gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``max gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from max_cli import gateway_windows
        from max_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``max update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Max Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Max Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `max update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Max via pip (for PyPI installs)."""
    from max_cli import __version__
    from max_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        ...
        cmd = [pipx, "upgrade", "max-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "max-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "max-agent"]
    ...
    print("✓ Update complete! Restart max to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Max Agent...")
    print()

    # On Windows, abort early if another max.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_max_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `max serve` backend) will keep .pyd
    ...
    # the max.exe shim guard above, but its lock probe only checks the shim
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://max-agent.stardustresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Max windows/gateways and re-run: max update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        # though `git pull` can't touch $MAX_HOME, this is cheap
        ...
        try:
            from max_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in max_cli/config.py bricked
        # every user who ran ``max update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``max
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``max update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_max_home added to max_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``max`` launch finishes the
        ...
        # build.  ``max desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``max desktop`` shouldn't be forced into a full
        # Electron build by ``max update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from max_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "max_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `max desktop` to retry)")
        ...
        # current — so copy it straight over ~/.max/cache/model_catalog.json
        ...
        # modules in this process.  Reload max_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_max_home() added since the last release.
        try:
            import importlib
            import max_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit MAX_HOME so
        # it is not affected by sync_skills()'s module-level MAX_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's MAX_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from max_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'max config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: max config migrate")
            else:
                print()
                print("Skipped. Run 'max config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from max_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `max curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``max update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from max_cli.tools_config import install_cua_driver
        ...
        # When running as ``max update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_max_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... max-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... max-gateway*)
            ...
            # Discover all max-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. max-gateway.service or max-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `max update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: max gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: max gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: max -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Max gateway unit files are still installed.
        # When both max.service (from a pre-rename install) and the
        # current max-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `max update` surfaces the issue until the user migrates.
        try:
            from max_cli.gateway import (
                has_legacy_max_units,
                _find_legacy_max_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_max_units():
                print()
                print("⚠ Legacy Max gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_max_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (max.service) fight the current")
                print("  max-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    max gateway migrate-legacy")
                print()
        ...
        print("  max model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``max -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from max_constants import display_max_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `max profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_max_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → max -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  max profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     max -p {name} chat")
                else:
        ...
        from max_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from max_constants import get_max_home as _hh
        ...
        from max_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `max profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → max -p {name}  ({wrapper})")
        print()
        ...
        from max_cli.profiles import profile_exists, validate_alias_name
        ...
        from max_cli.profiles import rename_profile
        ...
        from max_cli.profiles import export_profile
        ...
        from max_cli.profiles import import_profile
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="max_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  max -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      max -p {plan.manifest.name} chat")
        ...
        from max_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `max profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"max -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from max_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("max_requires"):
            print(f"Requires:     Max {data['max_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from max_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.max_requires:
        print(f"  Requires: Max {mf.max_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``max dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``max dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No max dashboard processes running.")
        return 0

    print(f"{len(pids)} max dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``max dashboard register`` for OAuth.
    ...
    try:
        from max_cli.dashboard_auth import list_providers
        if list_providers():
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No max dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `max update`.
    ...
    # launches the dashboard (`worker dashboard` → MAX_HOME points into
    ...
    if (
        ...
        and os.environ.get("MAX_DESKTOP") != "1"
    ):
        ...
        # MAX_HOME.  We must resolve the root explicitly instead of just
        # dropping MAX_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV MAX_HOME=/opt/data`), so an unset
        # MAX_HOME falls back to $HOME/.max = /opt/data/.max — an
        ...
        # update-button guard also misfires).  get_default_max_root()
        # returns the root for both layouts: ~/.max for a standard install
        ...
        try:
            from max_constants import get_default_max_root
            env["MAX_HOME"] = str(get_default_max_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop MAX_HOME) rather than block the reroute.
    ...
    # the same logs directory as every other Max surface.
    try:
        from max_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["MAX_SERVE_HEADLESS"] = "1"
    elif "MAX_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["MAX_WEB_DIST"])
            if "MAX_WEB_DIST" in os.environ
            else PROJECT_ROOT / "max_cli" / "web_dist"
        )
    ...
    try:
        from max_cli.mcp_startup import start_background_mcp_discovery
    ...
    from max_cli.web_server import start_server
...
def cmd_gateway_enroll(args):
    ...
    from max_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from max_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from max_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Max log files."""
    from max_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Max command console."""
    from max_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from max_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``max -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``max_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``max -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``max`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("MAX_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if getattr(args, "oneshot", None):
        ...
        from max_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["MAX_DEFER_AGENT_STARTUP"] = "1"
            os.environ["MAX_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["MAX_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `max --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_max_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_max_home()}/memories/\n")
    else:
        from max_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Max Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from max_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from max_cli.tools_config import run_post_setup_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from max_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from max_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from max_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from max_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from max_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from max_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for max CLI."""
    # Cosmetic: make the process show up as 'max' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from max_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``max.exe.old.*`` quarantine files left by previous
    # ``max update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_max_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``max update``
    ...
    # ``max skills install update``) merely defers recovery one launch;
    # under-matching (missing ``max -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in max_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from max_cli.moa_cmd import cmd_moa
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://max-agent.stardustresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `max model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.max/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://max-agent.stardustresearch.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from max_cli import secrets_cli as _secrets_cli
    from max_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from max_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in max_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in max_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in max_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in max_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `max whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in max_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from max_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in max_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in max_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in max_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in max_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in max_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in max_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from max_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from max_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in max_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in max_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in max_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in max_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in max_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in max_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.max/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo max uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from max_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in max_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in max_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in max_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in max_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in max_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from max_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in max_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``max --help``, ``max version``, ``max logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from max_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from max_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Max shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from max_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from max_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in max_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in max_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `max computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `max tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `max computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Max app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from max_cli.tools_config import _cua_driver_cmd
            # Honor MAX_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from max_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: max computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: max computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: max computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: max computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: max computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: max computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in max_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <max home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            from max_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from max_cli.session_export_md import redact_session_data
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_max_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from max_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_max_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_max_home() / "sessions"
            ...
            # `max sessions prune`: no time window and no filters at all
            ...
            # Launch max --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in max_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in max_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in max_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in max_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in max_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in max_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in max_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in max_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Max-Setup.exe success screen tells users to
    # run `max desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in max_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in max_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in max_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``max -c Pokemon Agent Dev`` → ``max -c 'Pokemon Agent Dev'``
    ...
    from max_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'max -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (max hooks list, cron
