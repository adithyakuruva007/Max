"""``max gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.max/auth.json``) — the same path ``max dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.max/.env``. The per-gateway secret authenticates the WS upgrade;
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `max auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `max gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.max/.env (0600 dir) and never logs.
    to_write = {
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in max_cli/gateway.py. Best-effort: any failure to determine the
    ...
    try:
        ...
        home = Path(get_max_home()).resolve()
        try:
