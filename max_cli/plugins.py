...
4. **Pip plugins**     – packages that expose the ``max_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "max_agent.plugins"

_NS_PARENT = "max_plugins"
...
class PluginContext:
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_max_dotenv()`` startup — after
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to max_cli/. Two layouts are
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `max plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``max_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``max_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
