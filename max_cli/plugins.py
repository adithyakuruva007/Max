...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with max-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.max/plugins/<name>/``
3. **Project plugins** – ``./.max/plugins/<name>/`` (opt-in via
...
from max_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from max_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from max_cli.plugin_capabilities import (
...
# ---------------------------------------------------------------------------
#
# Set ``MAX_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.max/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("MAX_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When MAX_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Max loggers.
    """
    ...
    logger.debug(
        "MAX_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Max' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by max_cli.kanban_db when a task
    ...
    # separate `max -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `max kanban dispatch`),
    ...
    # ``max kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: max_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Max home."""
...
# Highest manifest schema version this Max understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Max "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `max plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    #              in ~/.max/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``max plugins list``. For a flat plugin at
    ...
    # ``max_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Max never
    ...
    # (``max plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Max paths, traversal syntax, and the security-
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_max_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        # threads. Other Max config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from max_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Return the active Max profile name (e.g. ``"default"``).

        Derived from ``MAX_HOME`` via
        :func:`max_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.max/profiles/<name>``, or ``"custom"`` when
        ``MAX_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Max core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``max honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``max <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`max_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from max_cli.dashboard_auth import DashboardAuthProvider
        from max_cli.dashboard_auth.registry import (
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        ``~/.max/.env`` loads, before Max reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Max' Slack adapter wires registered handlers into its
    ...
    def register_telegram_handler(self, factory: Callable) -> None:
        ...
        Max' Telegram adapter invokes registered factories at ``connect()``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``max model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: max_cli.main imports plugins indirectly
        from max_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.max/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # by resolved max home). The ledger is therefore keyed per manager
        ...
        # contributed, so `max plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                ...
                self.unload()
            if env_var_enabled("MAX_SAFE_MODE"):
                logger.info("MAX_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from max_cli.env_loader import load_hermes_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Max plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Max core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Max core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Max Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # Built-in backends auto-load — they ship with max and must
            ...
            # all ~20 of them added several seconds to every `max`
            # invocation — including plain `max chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Max ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `max plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from max_cli.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_max_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_max_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.max/plugins/)
        user_dir = get_max_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.max/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("MAX_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".max" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set MAX_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from max_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from max_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Max process (e.g. a pip
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``max plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`max plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``max tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `max plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Max validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Max does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `max plugins list` would
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from max_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_max_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        # from a different Max home (profile switch reusing a slug
...
# Keyed cache: resolved Max home -> PluginManager. Max supports
# multiple profiles via different MAX_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_max_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# max_constants.set_max_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_max_home() / "plugins"`` and some
    ...
    can temporarily switch Max home (env var *or* the context-local
    ``set_max_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Max home instead of
    being one process-wide singleton.
    """
    try:
        return get_max_home().expanduser().resolve()
    except Exception:
        return get_max_home().expanduser()
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Max profile/home.
    ...
    profile switch — via ``MAX_HOME`` or the context-local
    ``set_max_home_override()`` — gets its own manager with its own
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_max_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from max_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``max tools`` TUI so plugin-provided toolsets appear
