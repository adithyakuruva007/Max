"""
Dump command for max CLI.

Outputs a compact, plain-text summary of the user's Max setup
...
from max_cli.config import get_max_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.max/.env.
    ...
    exports. ``max debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.max_build_sha`` by the Dockerfile's
    ``MAX_GIT_SHA`` build-arg (see ``max_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``max dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from max_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(max_home: Path) -> int:
...
def _cron_summary(max_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = max_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"max_home:      {display_max_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.max/.env: a managed
        ...
        # A credential added via `max auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `max auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["max-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(max_home)}")
    lines.append(f"  skills:             {_count_skills(max_home)}")
