...
The Max docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.max/cache/model_catalog.json``.
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/NousResearch/max-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_max_home() / "cache" / "model_catalog.json"
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Max silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`max update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``max update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.max/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``max model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
