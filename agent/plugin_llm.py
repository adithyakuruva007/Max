...
Plugins built on Max Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Max
...
:class:`~max_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Max' main config
...
handles every provider, fallback chain, and per-task override Max
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`max_cli.plugins.PluginContext`
