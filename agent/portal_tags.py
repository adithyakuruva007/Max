...
Every Max request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Max Agent and bucket it by client release.
...
The version is sourced live from ``max_cli.__version__`` so it auto-aligns
...
``max_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    """Return the current Max release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``max_cli`` cannot be imported (should
...
def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Max session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Max conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.max/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Max
    ...
    """
    tags = ["product=max-agent", max_client_tag()]
