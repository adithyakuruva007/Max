...
from max_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.max/.env``. See NousResearch/max-agent#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'max memory status'. Note: "
        "systemd/gateway services do not inherit ~/.max/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from max_cli.providers import MAX_OVERLAYS, get_provider

        overlay = MAX_OVERLAYS.get(provider)
    ...
    try:
        ...
        from max_cli.models import normalize_provider as normalize_model_provider
        from max_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: max config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$MAX_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_max_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$MAX_HOME`` just means the notice
...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — Max never even initiates the compaction), so no
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .max.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / MAX_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.max/SOUL.md as the primary
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server"}:
    ...
    elif agent.provider in {"nous", "nous-portal", "stardustresearch"}:
        ...
        try:
            from max_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
    ...
    agent._cache_ttl = "5m"
    try:
        from max_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind MAX_DEV_CREDITS.
    ...
    # both live under ~/.max/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from max_logging import setup_logging, setup_verbose_logging
    ...
    if agent.verbose_logging:
        ...
        # on the fact that max_logging.setup_logging() does not
        ...
        # handler level inside max_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from max_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from max_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `max model` to "
                        "select a provider, or run `max setup` for first-time "
                        "configuration."
                    )
    ...
    # A multiplexed gateway may enter a different MAX_HOME after
    ...
    # worker (kanban_show tool is present iff MAX_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["MAX_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.max/sessions/ alongside gateway sessions
    hermes_home = get_max_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.max/sessions/session_{sid}.json)
    ...
    try:
        from max_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `max --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from max_cli.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical Max behavior) or left to LM Studio's
    ...
    # Lets an enterprise admin append to or replace Max' built-in
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from max_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from max_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from max_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Max Agent.  Choose a model with at least "
    ...
    # Nous Max 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from max_cli.model_switch import _check_hermes_model_warning

            _hermes_warn = _check_hermes_model_warning(agent.model or "")
            if _hermes_warn:
                _user_msg = (
                    "⚠ Stardust Research Max 3 & 4 models are NOT agentic — they "
