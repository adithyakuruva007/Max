...
def init_agent(
    ...
    agent._cache_ttl = "5m"
    try:
        from max_cli.config import load_config as _load_pc_cfg
    ...
    setup_logging(max_home=_ra()._max_home)
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from max_cli.config import load_config as _load_br_cfg
    ...
    agent.logs_dir = max_home / "sessions"
    ...
    try:
        from max_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from max_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "max"
                    except Exception:
                        pass
    ...
    if codex_app_server_auto_compaction not in {"native", "max", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, max, off.",
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from max_cli.model_switch import _check_max_model_warning

            _max_warn = _check_max_model_warning(agent.model or "")
            if _max_warn:
                _user_msg = (
                    "⚠ Nous Research Max 3 & 4 models are NOT agentic — they "
