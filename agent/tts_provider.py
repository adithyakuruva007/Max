...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Max with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.max/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Max
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``max tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``max setup`` for
