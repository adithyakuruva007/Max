...
def _hermes_root_path() -> Path:
    ...
    try:
        from max_constants import get_default_hermes_root  # local import to avoid cycles
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside MAX_WRITE_SAFE_ROOT "
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    for hd in hermes_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the Max real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the Max real-profile browser "
...
# edited skills under both ``~/.max/profiles/hermes-security/skills/``
...
# ``…/sandboxes/<backend>/<task>/home/.max/…`` segment and warns
...
# (the agent's view inside the container is plain ``/root/.max/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.max`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.max/…`` and returns the
    index where the inner Max-state portion starts. Returns ``None`` for
    ...
    for i, part in enumerate(parts):
        ...
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".max":
            return i + 4
    return None
...
# that prefix: the agent sees plain ``/root/.max/…``.  The root:root
