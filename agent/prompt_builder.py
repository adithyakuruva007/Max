...
from max_constants import get_max_home, get_skills_dir, is_wsl
...
_MAX_MD_NAMES = (".max.md", "MAX.md")


def _find_max_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.max.md`` or ``MAX.md``.
...
MAX_AGENT_HELP_GUIDANCE = (
    ...
    "information. Load the `max-agent` skill with skill_view(name='max-agent') "
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `max computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Max "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Max terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the Max WebUI, a browser-based chat interface. "
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Max Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `max-agent` skill "
            "first. It has the actual commands (e.g. `max config set …`, `max tools`, "
            "`max setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    ...
    """
    try:
        from max_cli.config import ensure_max_home
        ensure_max_home()
    except Exception as e:
        logger.debug("Could not ensure MAX_HOME before loading SOUL.md: %s", e)

    soul_path = get_max_home() / "SOUL.md"
...
def _load_max_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.max.md / MAX.md — walk to git root."""
    max_md_path = _find_max_md(cwd_path)
    if not max_md_path:
        return ""
    try:
        content = max_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = max_md_path.name
        try:
            rel = str(max_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        ...
        return _truncate_content(
            ...
            read_path=str(max_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", max_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .max.md / MAX.md  (walk to git root)
    ...
    project_context = (
        _load_max_md(cwd_path, context_length)
