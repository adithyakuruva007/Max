...
configured under ``monitoring.export.otlp``. Lets an operator stream Max
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install max-agent[otlp]``), imported lazily so the
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'max-agent[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
