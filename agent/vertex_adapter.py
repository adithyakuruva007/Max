"""Vertex AI (Google Cloud) adapter for Max Agent.

Provides authentication and configuration for Vertex AI's OpenAI-compatible
endpoint. This allows Max to use Gemini models via Google Cloud with
...
# who installed plain `max-agent` and only later selected a Gemini model.
try:
...
def has_explicit_vertex_config() -> bool:
    """True only when the user deliberately pointed Max at Vertex.
    ...
    Max up for Vertex would suddenly see it (and could spend against those
    credentials). So this checks only Max-scoped signals:
      ...
      * a resolvable ``VERTEX_CREDENTIALS_PATH`` service-account file
        (the Max-specific path var — NOT ``GOOGLE_APPLICATION_CREDENTIALS``).
    """
    if _resolve_project_override():
        return True
