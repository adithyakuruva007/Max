...
from max_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.max/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    MAX_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``MAX_KANBAN_TASK`` / ``MAX_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("MAX_KANBAN_TASK") or os.getenv("MAX_KANBAN_BOARD"):
        ...
        # The Max Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``max_cli.config`` on the
...
# `max-agent` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting Max itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with Max, so disable requests for these names are ignored
# everywhere the disabled list is consulted.
ESSENTIAL_SKILLS: frozenset = frozenset({"max-agent"})
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``MAX_PLATFORM`` or
            ``MAX_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("MAX_PLATFORM")
        or get_session_env("MAX_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``max config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``max`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.max/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``max`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against MAX_HOME, not cwd
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.max/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.max/skills`` at the git root) are
...
#   <root>/.max/skills/   — Max-native location
...
# a one-line "run `max skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when MAX_HOME itself lives inside a git checkout:
    ``<root>/.max/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``max skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`max skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under MAX_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_max_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.max/skills/`` (including via symlinks) or configured
    ...
    # ~/.max/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Max can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    m = metadata.get("max") or metadata.get("hermes") or {}
    if not isinstance(m, dict):
        m = {}
    return {
        "fallback_for_toolsets": m.get("fallback_for_toolsets", []),
        "requires_toolsets": m.get("requires_toolsets", []),
        "fallback_for_tools": m.get("fallback_for_tools", []),
        "requires_tools": m.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    m = metadata.get("max") or metadata.get("hermes")
    if not isinstance(m, dict):
        return []
    raw = m.get("config")
    if not raw:
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Max metadata, VCS, virtualenv/dependency, cache, and skill
