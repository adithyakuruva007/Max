...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(max_meta, dict):
        max_meta = {}
    return {
        "fallback_for_toolsets": max_meta.get("fallback_for_toolsets", []),
        "requires_toolsets": max_meta.get("requires_toolsets", []),
        "fallback_for_tools": max_meta.get("fallback_for_tools", []),
        "requires_tools": max_meta.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    max_meta = metadata.get("max")
    if not isinstance(max_meta, dict):
        return []
    raw = max_meta.get("config")
    if not raw:
        return []
