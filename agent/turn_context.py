...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``max logs``.
    ...
    if (
        ...
        # Max only initiates compaction in "hermes" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Max preflight compression for codex app-server "
                "(mode=%s); Max will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Max (via _emit_status), so it always
