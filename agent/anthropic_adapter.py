"""Anthropic Messages API adapter for Max Agent.

Translates between Max's internal OpenAI-style message format and
...
# Max effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to max's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to max's outer conversation
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      4. Anthropic credential_pool OAuth entry (~/.max/auth.json)
    ...
    # 4. Max credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Max configs.
...
def run_max_oauth_login_pure() -> Optional[Dict[str, Any]]:
...
def read_max_oauth_credentials() -> Optional[Dict[str, Any]]:
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See max-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See max-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See max-agent#13848 (Kimi) and
    max-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Max Agent", "Claude Code")
                text = text.replace("Max agent", "Claude Code")
                text = text.replace("max-agent", "claude-code")
        ...
        #      a) bare Max-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Max surfaces in its CLI. We
