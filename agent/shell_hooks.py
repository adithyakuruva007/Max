...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`max_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.max/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``MAX_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``max_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Max-canonical

    # Inject context for pre_llm_call:
    {"context": "Today is Friday"}

    # Modify tool input for pre_tool_call (Max-canonical):
...
from max_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, windows_hide_flags
...
# max_cli.plugins.get_pre_tool_call_block_message / _get_pre_tool_call_
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``max_cli.config.load_config``
    ...
    setting.  ``MAX_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / MAX_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("MAX_SAFE_MODE"):
        logger.info("MAX_SAFE_MODE=1 — shell-hook registration skipped")
        return []
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / MAX_ACCEPT_HOOKS=1 / "
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``max hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from max_cli.plugins import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
def _evaluate_result(
    ...
    * non-JSON / unparseable stdout on a ``fail_closed`` blocking hook —
    ...
    test helper (:func:`run_once`) so ``max hooks test`` reflects
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Max wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Max
    ``{"action": "block", "message": "..."}`` shape expected by
    :func:`max_cli.plugins.get_pre_tool_call_block_message`.  This is
    ...
    if event == "pre_verify":
        # "continue" (Max) / "block" (Claude-Code Stop: block the stop) both
...
def allowlist_path() -> Path:
    ...
    return get_max_home() / ALLOWLIST_FILENAME
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / MAX_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Max is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``MAX_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("MAX_ACCEPT_HOOKS", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Introspection (used by `max hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``max hooks test`` and ``max hooks doctor``.

    ``kwargs`` is the same dict that :func:`max_cli.plugins.invoke_hook`
    ...
    produce — otherwise scripts tested via ``max hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical Max-wire-shape response — including exit-code-2
    blocking and ``fail_closed`` semantics, so what ``max hooks test``
