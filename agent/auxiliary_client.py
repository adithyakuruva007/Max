...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.max/auth.json active provider)
...
from max_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``MAX_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `max update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Max owns auxiliary retry + provider/model fallback policy (the
    ...
    # Max sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Max control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Max triggers summarization.  Higher values delay
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        ...
        from max_cli.models import fetch_models_with_pricing
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``MAX_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``MAX_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("MAX_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("MAX_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "MaxAgent",
}
...
_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://max-agent.stardustresearch.com",
    "X-Title": "Max Agent",
    "User-Agent": f"MaxAgent/{_MAX_VERSION}",
}
...
# in lockstep with max_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``max_cli.__version__`` is
...
_NOUS_DEFAULT_BASE_URL = "https://inference-api.stardustresearch.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_max_home() / "auth.json"
...
def _codex_cloudflare_headers(
    ...
    the official endpoint always send Max' originator and version. Custom
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Max Agent)",
    ...
    if _is_official_codex_base_url(base_url):
        ...
        headers.update({
            "User-Agent": f"MaxAgent/{__version__}",
            "originator": "max-agent",
        })
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("MAX_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
    except (TypeError, ValueError):
        return 1800
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the Max-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Max plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``max_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.max/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from max_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from max_cli.auth import _agent_key_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        ...
        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("MAX_NOUS_TIMEOUT_SECONDS", 15),
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``max auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``max_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from max_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Max auth store.
    ...
    try:
        from max_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from max_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from max_cli.models import copilot_default_headers
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from max_cli.config import load_config
        from max_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `max setup` or `max model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`max_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from max_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``max config aux reset``)."""
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.stardustresearch.com"):
        return "nous"
...
def _auth_refresh_provider_for_route(
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "inference-api.stardustresearch.com"):
        return "nous"
    return normalized
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``max_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Max' built-in discovery
...
def _resolve_auto_route(
    ...
    #    scenario where a user switches providers via `max model` but the
    #    old OPENAI_BASE_URL lingers in ~/.max/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: max model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.max/.env",
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from max_cli.runtime_provider import _get_named_custom_provider
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so Max owns
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: max auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: max model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: max model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: max doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("max_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from max_cli.models import opencode_zen_free_runtime as _oc_free_rt
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from max_cli.models import _should_use_copilot_responses_api
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`max_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one Max will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if _provider_norm in {"nous", "nous-portal", "stardustresearch"}:
            from max_cli.providers import nous_api_mode
    ...
    if _provider_for_portal in {"nous", "nous-portal", "stardustresearch"}:
    ...
    # Anthropic Messages adapters translate Max reasoning into native
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        effective_base = base_url or ""
        _nous_on_messages = False
        if provider_norm in {"nous", "nous-portal", "stardustresearch"}:
            from max_cli.providers import nous_api_mode
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Max reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `max model`."
                    )
    ...
    try:
        ...
        _heal_is_nous = (
            ...
            or base_url_host_matches(_base_info, "inference-api.stardustresearch.com")
        )
...
async def _async_call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        _heal_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "inference-api.stardustresearch.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            healed_model = _refresh_nous_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
