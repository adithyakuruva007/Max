...
(``gateway/platforms/webhook.py``): inbound wakes Max when the world
changes; outbound tells the world when Max does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Max-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``MAX_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.max/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/hermes-events
          ...
          secret_env: MAX_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `max hooks list`
...
Headers::
    ...
    User-Agent:              Max-Agent-Outbound-Webhook
    X-Max-Event:          <hook event name>
    X-Max-Delivery:       <delivery_id>
    X-Max-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("MAX_SAFE_MODE"):
        logger.info("MAX_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Max-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Max-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
