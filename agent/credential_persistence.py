...
``auth.json``.  They intentionally have no dependency on ``max_cli.auth`` so
...
# Sources Max owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Max-owned OAuth/device-code state)
