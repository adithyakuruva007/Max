"""Upload a Max session transcript to Hugging Face as an agent trace.

Max stores sessions in its own SQLite store (``max_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``max trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Max' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "max-traces"
_MAX_VERSION = "max-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Max OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Max' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Max conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.max/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `max trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/max-traces`` dataset. Returns a
