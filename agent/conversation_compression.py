...
def compress_context(
    ...
    *,
    ...
    # long-lived ``max_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `max update`) to resync.",
    ...
    try:
        ...
        if agent._session_db:
            try:
                ...
                if in_place:
                    ...
                    # from ``max_logging._session_context`` (set once per turn in
                    ...
                    try:
                        agent._session_db.create_session(
                            ...
                            source=agent.platform or os.environ.get("MAX_SESSION_SOURCE", "cli"),
                            model=agent.model,
        ...
        # engines (e.g. max-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See max-lcm#68. Built-in ContextCompressor
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    if auto_mode not in {"native", "max", "off"}:
        auto_mode = "native"
    if not force and auto_mode != "max":
        logger.info(
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="max_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
