...
# Mirror max_cli.config.DEFAULT_CONFIG["compression"] keys of the same name.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    imported ``max_state.SessionDB`` class (and its live instances) is old.
...
def _adopt_live_compression_child(
    ...
    try:
        ...
        os.environ["MAX_SESSION_ID"] = child_session_id
    try:
...
def _direct_messages_for_pre_compress_memory(messages: Any) -> list[dict[str, Any]]:
    ...
    receive one normalized host contract instead of having to infer Max
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `max setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Max "
        ...
        if aux_context < threshold:
            ...
            if threshold_suggestion_viable:
                ...
                msg += (
                    ...
                    f"Max's small-context floor and output reservation "
...
def compress_context(
    ...
    *,
    ...
    # Max' summarizer would only rewrite a local mirror without shrinking
    ...
    # The memory-provider context handoff below is intentionally Max-only:
        ...
        if agent._session_db:
            ...
            try:
                ...
                if in_place:
                    ...
                    # whose thread lacks the MAX_HOME context.
                    try:
                    ...
                    agent._session_db.publish_compression_child(
                        ...
                        or os.environ.get("MAX_SESSION_SOURCE", "cli"),
                        model=agent.model,
                    ...
                    try:
                        from max_cli.goals import migrate_goal_to_session
                        migrate_goal_to_session(old_session_id, agent.session_id, reason="compression")
                    except Exception as _goal_err:
                        logger.debug("Could not migrate goal on compression: %s", _goal_err)
                    # Same boundary hazard for /heartbeat state — carry it too.
                    try:
                        from max_cli.heartbeat import migrate_heartbeat_to_session
                    ...
                    # survive compression.
                    try:
                        from max_cli.loops import migrate_loop_to_session
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    Max' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep Max' transcript
    unchanged.
    """
    auto_mode = str(
    ...
    # way the Max path below does. An active cooldown means a recent
