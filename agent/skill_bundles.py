...
Storage
-------
Bundles live in ``~/.max/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``MAX_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``max bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under MAX_HOME.

    Honors ``MAX_BUNDLES_DIR`` for tests; falls back to
    ``<MAX_HOME>/skill-bundles``.
    """
    override = os.environ.get("MAX_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_max_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `max bundles` CLI subcommand.
# ---------------------------------------------------------------------------
