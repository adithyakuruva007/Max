...
Max' support is deliberately narrow (live verification, Aug 2026):
...
Ownership model: Max' local compression stays fully armed as the
...
def _warn_native_compaction_suppressed_by_checkpoint_gate() -> None:
    ...
    logger.warning(
        ...
        "checkpoint-aware Max compressor stays authoritative."
    )
...
def native_compaction_context_management(
    ...
    *,
    ...
    # the server replaces older context. Keep the checkpoint-aware Max
...
def _is_summary_item(item: Any) -> bool:
    """True when *item* is a canonical Max compression-summary message.
...
def prune_pre_checkpoint_items(
    ...
    - Compression summary messages (``_is_summary_item``, the canonical
      ...
      byte/character-sliced: Max summaries carry structural framing
