...
as ``kind: backend``) or ``~/.max/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        backend can't honor are not advertised), and by ``max tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$MAX_HOME/cache/videos/``, creating parents as needed."""
    from max_constants import get_max_home

    path = get_max_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$MAX_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$MAX_HOME/cache/videos/``.
