...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `max backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - max_home (str): The active MAX_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.max``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "max").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'max memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], max_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'max memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``max_home`` is the active MAX_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE MAX_HOME.

        ``max backup`` only walks MAX_HOME, so any provider state kept
        ...
        of the archive, and ``max import`` restores them to their original
