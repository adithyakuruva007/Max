...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the MAX_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Max runtime / deployment
    "MAX_HOME", "MAX_PROFILE", "MAX_GATEWAY_LOCK_DIR",
    "MAX_MAX_ITERATIONS", "MAX_MAX_TOKENS", "MAX_API_TIMEOUT",
    "MAX_REDACT_SECRETS", "MAX_NOUS_TIMEOUT_SECONDS",
    "_MAX_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "MAX_KANBAN_DB", "MAX_KANBAN_WORKSPACES_ROOT", "MAX_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "MAX_KANBAN_",
    "MAX_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Max writes
    ...
    ``max_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Max parser: save_env_value
    ...
    from max_cli.config import _parse_env_value
...
def build_profile_secret_scope(hermes_home: Path) -> Dict[str, str]:
    ...
    try:
        from max_cli.env_loader import get_secret_source_values
