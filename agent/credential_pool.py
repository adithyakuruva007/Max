...
# un-throttled log fires on *every* selection. On Windows several Max
...
# readiness handshake ("Timed out connecting to Max backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``max_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_anthropic_entry_from_pool_store(
        ...
        (``~/.max/auth.json`` / profile equivalent), so it works for
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``max model`` / ``max auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Max process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``max`` invocation that defaults to the active provider
        (e.g. setup wizard, ``max auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Max
        ...
        # Previously "anthropic" was excluded from this tuple: two Max
        ...
        if self.provider in ("openai-codex", "xai-oauth", "anthropic"):
            ...
            with _auth_store_lock(
                ...
                if self.provider == "anthropic" and synced.source == "claude_code":
                    ...
                    # (or macOS Keychain) that every Max profile's pool
    ...
    def _claude_code_credentials_lock(self):
        ...
        serializes every profile (and every Max process) that might
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``MAX_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "MAX_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "MAX_XAI_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "xai-oauth"
            else "MAX_ANTHROPIC_REFRESH_TIMEOUT_SECONDS"
        )
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                ...
                # _seed_from_singletons() reads ~/.max/.anthropic_oauth.json
                ...
                # a second authority for the same refresh-token family.
                    ...
                    try:
                        ...
                        return self._fail_closed_unpersisted_rotation(
                            entry, wexc, store="~/.max/.anthropic_oauth.json"
                        )
            elif self.provider == "openai-codex":
                # Adopt fresher tokens from auth.json before spending the
                # refresh_token — single-use tokens consumed by another Max
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Max process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Max profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `max model` / `max auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `max model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``max auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `max auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `max auth remove <provider> <N>` is stable across all source types.
    try:
        from max_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Max PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `max setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.max/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `max setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `max auth add nous --label <name>`).  Fall back to the
        ...
        # Max auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from max_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.max/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `max auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `max auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Max auth store.  Without this gate the removal is instantly
        ...
        # Max owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``max model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``max auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from max_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.max/.env over os.environ — the user's config file is the
# authoritative source for Max credentials. Stale env vars from parent
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `max auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.max/.env.
    ...
    try:
        from max_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from max_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.max/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `max auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Max
