...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Pointer to the max-agent skill + docs for user questions about Max itself.
    stable_parts.append(MAX_AGENT_HELP_GUIDANCE)
    ...
    if active_profile == "default":
        stable_parts.append(
            ...
            "under ~/.max/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            ...
            f"and writes ~/.max/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.max/skills/, ~/.max/plugins/, "
            f"~/.max/cron/, ~/.max/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Max never rebuilds or reinjects parts of it
