...
upstream prefix cache warm.  See ``max-agent-dev``'s
...
from max_constants import get_default_hermes_root, get_max_home
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``MAX_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("MAX_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the MAX_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from max_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from max_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound MAX_HOME ContextVar override wins. Surfaces that multiplex
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses :func:`get_default_hermes_root` (NOT ``get_max_home()``): on a
    correctly bound profile session the ambient home IS the profile dir, so
    ``get_max_home()/profiles`` would never contain ``home`` and every
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Max never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want MAX_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the MAX_HOME ContextVar
    ...
    # Pointer to the docs (and, when it exists, the max-agent skill) for
    # user questions about Max itself. The skill_view() pointer is a
    # dangling reference in two cases — no skill tools in the toolset
    # (Blank Slate) OR the max-agent skill not installed — so the
    ...
    stable_parts.append(MAX_AGENT_HELP_GUIDANCE_NO_SKILLS)
    ...
    # MAX_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # the max-agent skill actually present in the index (gating on the
    # rendered index line keeps this a pure string check — no second
    # filesystem scan, and it inherits the index cache's stability).
    if _has_skill_view and "- max-agent:" in skills_prompt:
        stable_parts[_help_guidance_slot] = MAX_AGENT_HELP_GUIDANCE
    ...
    # Coding posture (base Max, any interactive coding surface in a code
    ...
    # `max -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    ...
    # Active-profile hint — names the Max profile the agent is running
    # under so it doesn't conflate ~/.max/skills/ (default profile) with
    # ~/.max/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient MAX_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_hermes_root(): get_max_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_max_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_hermes_root())
    else:
        _home_str = _root_str = str(get_max_home())
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Max profile: default. Other profiles (if any) live "
        ...
        # profile mode is NOT get_max_home().
        profile_home = _home_str
        default_root = get_default_hermes_root()
        post_workspace_parts.append(
            f"Active Max profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Max). Every other surface (desktop chat panel,
    ...
    from max_time import get_timezone as _hermes_tz, now as _hermes_now
    now = _hermes_now()
