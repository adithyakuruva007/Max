...
row taxonomy is inferred from the atlas shape so Max can render both legacy
...
# rows (1536x1872), while older Max/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Max' activity state names. They are not always identical to the
...
# Legacy Max/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Max activity names -> accepted row-name aliases in descending
