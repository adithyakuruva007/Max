...
def _max_meta(fm: dict[str, Any]) -> dict[str, Any]:
    """``metadata.max`` as a dict, tolerant of the string-valued frontmatter
    that ``parse_frontmatter``'s malformed-YAML fallback produces."""
    meta = fm.get("metadata")
    max_section = meta.get("max") if isinstance(meta, dict) else None
    return max_section if isinstance(max_section, dict) else {}


def _related(fm: dict[str, Any]) -> list[str]:
    raw = fm.get("related_skills") or _max_meta(fm).get("related_skills")
...
def _category(fm: dict[str, Any], skill_md: Path) -> str:
    cat = fm.get("category") or _max_meta(fm).get("category")
    if cat:
        return str(cat)
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        path = get_max_home() / "skills" / ".usage.json"
        try:
...
def _skill_roots() -> list[tuple[str, Path]]:
    ...
    home_skills = get_max_home() / "skills"
