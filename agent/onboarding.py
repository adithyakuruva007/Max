...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Max starts and finds ``~/.openclaw/``.

    Points users at ``max claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``max claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Max, run "
        "`max claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `max claw cleanup` (renames it to ~/.openclaw.pre-migration — "
