...
# Wire-name used when Max keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Max's configured provider
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs Max web providers.
        #
        ...
        #    keep Max dispatch so ``web.backend`` / ``web.search_backend``
        ...
        kwargs = {
            # ``-900k`` large-context picker variants are Max-side aliases
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Max dispatches
