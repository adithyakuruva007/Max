...
projection into Max' display, approval bridging, transcript projection into
...
"codex_app_server"`. Max' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Max secret — gateway
        ...
        spawn_env = max_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("MAX_KANBAN_TASK"):
            kanban_db = spawn_env.get("MAX_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "MAX_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("MAX_HOME", os.path.expanduser("~/.max")),
                        "kanban",
                    ),
                )
            )
