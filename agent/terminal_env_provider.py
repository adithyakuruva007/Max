...
Providers live in ``~/.max/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.max/cache`` files land
  inside the backend (e.g. ``"~/.max"`` for home-synced backends,
  ``"/root/.max"`` for root-homed containers), or ``None`` when host
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Max cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``max setup`` after this backend is selected.
