...
* The requested key is passed to the child ONLY via the ``MAX_SECRET_KEY``
...
* The startup/apply path runs the helper exactly ONCE (with an empty
  ``MAX_SECRET_KEY``) — it is never called per-key in a loop, so a
...
# max_cli.env_loader can consume both providers identically.
...
def _run_helper(
    ...
    The key is passed as DATA via ``MAX_SECRET_KEY`` — never interpolated
...
def get_command_secret(
    *,
    ...
    ``MAX_SECRET_KEY``.  Returns None on any failure — never raises."""
...
def apply_command_secrets(
    *,
    ...
    # MAX_SECRET_KEY and parse its stdout as a dotenv blob.
    stdout = _run_helper(command, "", timeout_seconds, max_output_bytes)
    if stdout is None:
    ...
    def remediation(self, kind, cfg: dict) -> str:
        ...
        if kind == ErrorKind.INTERNAL:
            return (
                ...
                "Max discards helper stderr so diagnostics can't leak "
                "secret material."
            )
