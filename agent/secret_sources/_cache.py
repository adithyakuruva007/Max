...
cache problem must never block Max startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Max home used for cache paths.

    ``home_path`` is whatever ``load_max_dotenv()`` already resolved;
    falling back to ``$MAX_HOME`` / ``~/.max`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("MAX_HOME", Path.home() / ".max"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<max_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if MAX_HOME is ever made traversable.
            try:
