...
Max pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.max/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<max_home>/bin/bws`` on
  first use.  Max pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.max/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  hammer the API.
* Failures NEVER block Max startup.  Missing binary, no network,
...
# In-process cache so repeated load_max_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `max chat -q ...`
...
# <max_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.max/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under max_home/cache/.

    Thin wrapper over the shared DiskCache, kept for tests and any direct
    callers; falls back to `$MAX_HOME` / `~/.max` when home is None.
    """
...
def _max_bin_dir() -> Path:
    """Where Max stores its managed binaries.  Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<max_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _max_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``max secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _max_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="max-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<max_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from max_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_max_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `max secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `max secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `max secrets bitwarden setup` to install."
        )
        return result
