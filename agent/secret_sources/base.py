...
``~/.max/.env`` has loaded and BEFORE the rest of Max reads
``os.environ``.
...
* **Startup-time, synchronous.**  ``fetch()`` is called once per process
  (per MAX_HOME) by the orchestrator in
...
class ErrorKind(str, Enum):
    ...
    A fixed vocabulary keeps startup warnings and ``max secrets status``
...
class SecretSource(ABC):
    ...
    Attributes:
        ...
        label: Human-readable name used in startup messages and
            ``max secrets status`` (e.g. ``"Bitwarden Secrets Manager"``).
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        may be malformed.  ``home_path`` is the resolved MAX_HOME.
        """
    ...
    def remediation(self, kind: Optional["ErrorKind"], cfg: dict) -> str:
        ...
        Called by the startup status printer (and ``max secrets ...
        ...
        *what to run* next to fix it — not just what broke.  Sources
        ...
        ``max secrets bitwarden token`` for AUTH_FAILED).  Return an
        ...
        generic = {
            ErrorKind.NOT_CONFIGURED: (
                f"Run `max secrets {self.name} setup` to finish configuration."
            ),
            ErrorKind.BINARY_MISSING: (
                f"Run `max secrets {self.name} setup` to install the helper CLI."
            ),
            ErrorKind.AUTH_FAILED: (
                f"Credentials rejected — run `max secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.AUTH_EXPIRED: (
                f"Credentials expired — run `max secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.NETWORK: (
...
# codes that must not reach Max' own startup output.
...
def run_secret_cli(
    ...
    * The child gets ``PATH``/``HOME``/locale basics plus only the env
      ...
      this point holds every credential Max knows about.
    * ``NO_COLOR=1`` is set and stderr/stdout are ANSI-scrubbed so
      helper diagnostics can't smuggle escape sequences into Max
      output.
