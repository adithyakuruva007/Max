...
The parent Max process may itself be a Kanban dispatcher worker with
MAX_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's MAX_KANBAN_* vars are legitimately in os.environ (cron
...
DELEGATED_CHILD_ENV_MARKER = "MAX_DELEGATED_CHILD_CONTEXT"
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``MAX_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$MAX_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``MAX_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``MAX_KANBAN_*`` vars while losing the ContextVar in
