...
Data resolution order:
  ...
  2. Disk cache (~/.max/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Max ↔ models.dev
# ---------------------------------------------------------------------------

# Max provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Max' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Max ids (built lazily; many-to-one,
...
def _models_dev_to_hermes_ids(mdev_id: str) -> List[str]:
    """Return the Max provider ids that map to *mdev_id* (may be [])."""
...
def _get_cache_path() -> Path:
    ...
    return get_max_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from max_constants import get_max_home
    return get_max_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``max config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Max provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Max id.
...
def _get_provider_models(
    ...
    """Resolve a Max provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``max model``), which is an interactive user-facing flow
    where a fresh catalog is worth a short network wait.
    """
    from max_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Max agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Max provider ID (e.g. "kilocode") or a models.dev
...
def get_model_info(
    ...
    Accepts Max or models.dev provider ID.  Tries exact match then
